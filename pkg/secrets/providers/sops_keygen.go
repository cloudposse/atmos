...
var _ KeyGenerator = (*sopsProvider)(nil)
...
func (p *sopsProvider) GenerateKey(basePath string) (*KeygenResult, error) {
	defer perf.Track(nil, "providers.sopsProvider.GenerateKey")()

	if !isAgeSopsKind(p.kind) {
		return nil, fmt.Errorf("%w: vault %q is %q (only age SOPS vaults generate keys)", ErrKeygenNotSupported, p.name, p.kind)
	}
	if p.ageKey != "" {
