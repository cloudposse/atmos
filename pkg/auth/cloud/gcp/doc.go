...
// GCP credentials in ~/.config/atmos/gcp/ to avoid conflicts with the user's
...
//	err := gcp.Setup(ctx, atmosConfig, "my-identity", creds)
//
//	// Or step by step:
//	err := gcp.PrepareEnvironment(ctx, atmosConfig)
//	paths, err := gcp.SetupFiles(ctx, atmosConfig, "my-identity", creds)
//	err = gcp.SetAuthContext(authContext, "my-identity", creds)
//	err = gcp.SetEnvironmentVariables(ctx, atmosConfig, "my-identity")
package gcp
