...
func normalizeComponentIdentityDefaultMarkers(
	...
	for identityName, rawIdentity := range identities {
		value, keep, err := resolveComponentIdentityMarker(globalAuth, identityName, rawIdentity)
		if err != nil {
			return nil, err
		}
		if keep {
			normalizedIdentities[identityName] = value
		}
	}
...
func resolveComponentIdentityMarker(
	...
	return nil, false, errUtils.Build(errUtils.ErrInvalidIdentityConfig).
