...
func TestParseTerraformFlags(t *testing.T) {
	tests := []struct {
		name            string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Parse flags.
			interpreter := ParseTerraformFlags(cmd, v, tt.positionalArgs, tt.passThroughArgs)
			...
			assert.Equal(t, tt.passThroughArgs, interpreter.GetPassThroughArgs(), "Pass-through args mismatch")
		})
	}
}

func TestTerraformInterpreter_IdentityFlag(t *testing.T) {
	tests := []struct {
		name                     string
		viperValue               string
		flagChanged              bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Parse flags.
			interpreter := ParseTerraformFlags(cmd, v, nil, nil)
...
func TestTerraformInterpreter_Embedding(t *testing.T) {
	// Test that TerraformInterpreter properly embeds GlobalFlags and implements CommandInterpreter.
	v := viper.New()
	v.Set("stack", "dev")
	...
	interpreter := ParseTerraformFlags(cmd, v, []string{"plan", "vpc"}, []string{"-var", "foo=bar"})

	// Verify CommandInterpreter interface implementation.
...
func TestTerraformInterpreter_ZeroValues(t *testing.T) {
	// Test that zero-value TerraformInterpreter is safe to use.
	...
	assert.NotPanics(t, func() {
		...
		_ = interpreter.GetPassThroughArgs()
		_ = interpreter.Stack
	...
	assert.Nil(t, interpreter.GetPassThroughArgs())
}

func TestTerraformFlagsRegistry(t *testing.T) {
	registry := TerraformFlagsRegistry()
...
func TestTerraformFlagsRegistry_IdentityFlag(t *testing.T) {
	registry := TerraformFlagsRegistry()
...
func TestTerraformInterpreter_RealWorldScenarios(t *testing.T) {
	tests := []struct {
		name            string
		scenario        string
		viperValues     map[string]interface{}
		positionalArgs  []string
		passThroughArgs []string
		verify          func(t *testing.T, interpreter TerraformInterpreter)
	}{
		{
		...
		},
		{
			...
			verify: func(t *testing.T, interpreter TerraformInterpreter) {
				...
				assert.Equal(t, []string{"-auto-approve"}, interpreter.GetPassThroughArgs())
			},
		},
		{
