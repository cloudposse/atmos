...
func prepareYAMLForProvenance(yamlData any, ctx *m.MergeContext, atmosConfig *schema.AtmosConfiguration) (string, error) {
	...
	// Fix duplicate indentation in highlighted YAML folded scalar continuation lines.
	// The Chroma highlighter adds color codes in the middle of indentation, causing
	// the whitespace to be duplicated. We need to strip this duplicate indentation.
	highlighted = fixHighlightedIndentation(yamlBytes, highlighted)

	return highlighted, nil
}

// fixHighlightedIndentation fixes duplicate indentation in syntax-highlighted YAML.
// The Chroma highlighter tokenizes leading whitespace and adds ANSI color codes,
// which can cause indentation to appear doubled in the output. This function
// compares the original YAML with the highlighted version and strips duplicate spaces.
func fixHighlightedIndentation(original string, highlighted string) string {
	defer perf.Track(nil, "provenance.fixHighlightedIndentation")()

	originalLines := strings.Split(original, "\n")
	highlightedLines := strings.Split(highlighted, newlineChar)

	if len(originalLines) != len(highlightedLines) {
		// Line count mismatch, return highlighted as-is
		return highlighted
	}
	...
	for i := 0; i < len(originalLines); i++ {
		...
		origSpaces := countLeadingSpaces(origLine)
		hlSpaces := countLeadingSpacesWithANSI(hlLine)

		// If highlighted has more than 2x the original spaces, strip the extras
		if hlSpaces > origSpaces*2 {
			spacesToRemove := hlSpaces - origSpaces
			result.WriteString(stripLeadingSpaces(hlLine, spacesToRemove))
		} else {
			result.WriteString(hlLine)
		}
...
// countLeadingSpacesWithANSI counts leading spaces in a string that may contain ANSI codes.
func countLeadingSpacesWithANSI(s string) int {
	count := 0
	inAnsi := false
...
// stripLeadingSpaces removes the first N spaces from a string that may contain ANSI codes.
