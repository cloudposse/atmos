...
func TestStacksMatrixEntriesForComponent(t *testing.T) {
	stacksMap := map[string]any{
		"ue1-dev": map[string]any{
			"components": map[string]any{
				"terraform": map[string]any{
	...
	t.Run("filters to specific component", func(t *testing.T) {
		entries := StacksMatrixEntriesForComponent("vpc", stacksMap)
		require.Len(t, entries, 2)
		assert.Equal(t, "vpc", entries[0].Component)
		assert.Equal(t, "vpc", entries[1].Component)
		// Stacks should be sorted.
		assert.Equal(t, "ue1-dev", entries[0].Stack)
		assert.Equal(t, "ue1-staging", entries[1].Stack)
	})

	t.Run("component not found returns nil", func(t *testing.T) {
		entries := StacksMatrixEntriesForComponent("nonexistent", stacksMap)
		assert.Nil(t, entries)
	})

	t.Run("nil stacksMap returns nil", func(t *testing.T) {
		entries := StacksMatrixEntriesForComponent("vpc", nil)
		assert.Nil(t, entries)
	})
}
