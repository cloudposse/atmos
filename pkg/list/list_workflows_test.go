...
`
	err = os.WriteFile(filepath.Join(tmpDir, "atmos.yaml"), []byte(atmosConfig), 0o600)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	err = os.WriteFile(emptyWorkflowFile, emptyWorkflowBytes, 0o600)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	err = os.WriteFile(networkingWorkflowFile, networkingWorkflowBytes, 0o600)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	err = os.WriteFile(validationWorkflowFile, validationWorkflowBytes, 0o600)
	require.NoError(t, err)
...
func TestListWorkflowsWithFile(t *testing.T) {
	...
	require.NoError(t, err)
	err = os.WriteFile(testWorkflowFile, testWorkflowBytes, 0o600)
	require.NoError(t, err)
