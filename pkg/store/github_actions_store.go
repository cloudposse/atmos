...
// GitHubActionsStore implements the Store interface backed by GitHub Actions secrets. It is a
...
var (
	_ Store          = (*GitHubActionsStore)(nil)
	_ StatusStore    = (*GitHubActionsStore)(nil)
	_ DeletableStore = (*GitHubActionsStore)(nil)
)
...
func (s *GitHubActionsStore) Set(_ string, _ string, key string, value any) error {
	...
	if value == nil {
		return fmt.Errorf("%w for key %s", ErrNilValue, key)
	}
...
func (s *GitHubActionsStore) getByKey(key string) (any, error) {
	...
	if !s.readAllowed() {
		return nil, fmt.Errorf("%w: %q is only readable inside a GitHub Actions runner — %s, or set options.ci.enabled to override",
			ErrGitHubSecretValueCIOnly, name, s.envHint(name))
	}
	s.alignOnce.Do(s.verifyAlignment)
	raw, ok := lookupGitHubSecretEnv(name)
	if !ok {
		return nil, fmt.Errorf("%w: %q — %s", ErrGitHubSecretNotInEnv, name, s.envHint(name))
	}
...
func githubSecretName(prefix, key string) (string, error) {
	...
	if strings.HasPrefix(name, "GITHUB_") {
		return "", fmt.Errorf("%w: %q (names must not start with the GITHUB_ prefix)", ErrGitHubInvalidSecretName, name)
	}
	if !githubSecretNameRe.MatchString(name) {
		return "", fmt.Errorf("%w: %q (must match [A-Z_][A-Z0-9_]*)", ErrGitHubInvalidSecretName, name)
	}
	return name, nil
}
...
func githubSecretStringValue(value any) (string, error) {
	...
	default:
		...
		if err != nil {
			return "", fmt.Errorf(errFormat, ErrSerializeJSON, err)
		}
		return string(b), nil
	}
}
