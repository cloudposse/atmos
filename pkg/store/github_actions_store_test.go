...
func TestGitHubSecretName(t *testing.T) {
	tests := []struct {
		...
		{name: "empty key", key: "", wantErr: ErrEmptyKey},
		{name: "github prefix rejected", prefix: "github", key: "token", wantErr: ErrGitHubInvalidSecretName},
		{name: "leading digit rejected", key: "1password", wantErr: ErrGitHubInvalidSecretName},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestGitHubActionsStore_Set_WritesEncodedValue(t *testing.T) {
	...
	require.ErrorIs(t, store.Set("dev", "vpc", "db_password", nil), ErrNilValue)
}
...
func TestSealSecret_Errors(t *testing.T) {
	t.Run("invalid base64", func(t *testing.T) {
		...
		require.ErrorIs(t, err, ErrGitHubSealSecret)
	})
	t.Run("wrong key size", func(t *testing.T) {
		_, err := sealSecret(base64.StdEncoding.EncodeToString([]byte("tooshort")), "x")
		require.ErrorIs(t, err, ErrGitHubPublicKeySize)
	})
}

func TestGitHubActionsStore_RegistryWiring(t *testing.T) {
	// Canonical kind and legacy type both resolve to the GitHub Actions kind.
	assert.Equal(t, KindGitHubActions, resolveKind(StoreConfig{Kind: "github/actions"}))
	assert.Equal(t, KindGitHubActions, resolveKind(StoreConfig{Type: "github-actions"}))
