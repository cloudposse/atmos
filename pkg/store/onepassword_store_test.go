...
func TestOnePasswordStore_Get_BadTemplate(t *testing.T) {
	...
	assert.ErrorIs(t, err, ErrOnePasswordReferenceTemplate)
}
...
func TestOnePasswordStore_Set_NilValue(t *testing.T) {
	...
	assert.ErrorIs(t, s.Set("prod", "api", "op://V/i/f", nil), ErrNilValue)
}
...
func TestOnePasswordStore_ImplementsInterfaces(t *testing.T) {
	var s Store = newTestOPStore(newFakeOPClient(nil), "")
	_, ok := s.(StatusStore)
	assert.True(t, ok)
