...
func (c *githubActionsAPIClient) repoIDFor(ctx context.Context) (int, error) {
	c.idOnce.Do(func() {
		...
		if err != nil {
			c.idErr = fmt.Errorf(errWrapFormat, store.ErrGitHubResolveRepoID, err)
			return
		}
...
func (c *githubActionsAPIClient) publicKey(ctx context.Context) (*github.PublicKey, error) {
	...
	if err != nil {
		return nil, fmt.Errorf(errWrapFormat, store.ErrGitHubGetPublicKey, err)
	}
	return pk, nil
}
...
func (c *githubActionsAPIClient) PutSecret(ctx context.Context, name, value string) error {
	...
	if err != nil {
		return fmt.Errorf(errWrapFormatWithID, store.ErrGitHubPutSecret, name, err)
	}
	return nil
}
...
func (c *githubActionsAPIClient) HasSecret(ctx context.Context, name string) (bool, error) {
	...
	if err != nil {
		...
		return false, fmt.Errorf(errWrapFormatWithID, store.ErrGitHubGetSecret, name, err)
	}
	return true, nil
}
...
func sealSecret(publicKeyB64, plaintext string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf(errWrapFormat, store.ErrGitHubSealSecret, err)
	}
	if len(pkBytes) != githubPublicKeySize {
		return "", fmt.Errorf("%w: got %d bytes, want %d", store.ErrGitHubPublicKeySize, len(pkBytes), githubPublicKeySize)
	}
