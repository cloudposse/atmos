...
func newTestKeychainStore(t *testing.T) store.Store {
	t.Helper()
...
func TestKeychainStore_ValidatesEmptyArgs(t *testing.T) {
	...
	assert.ErrorIs(t, s.Set("dev", "vpc", "k", nil), store.ErrNilValue)
...
func TestKeychainStore_RegistryBuildsAndIsSecretByDefault(t *testing.T) {
	...
	cfg := store.StoresConfig{
		"local": {Kind: store.KindKeychain, Options: map[string]any{"backend": keyring.TypeMemory}},
	}
...
func TestKeychainStore_LegacyTypeAlias(t *testing.T) {
	// Both "keychain" and "keyring" legacy type strings resolve to the keychain kind and build a
	// working keychain store. mapLegacyType is unexported in package store, so the alias contract is
	// verified through the public NewStoreRegistry path instead.
	for _, legacyType := range []string{"keychain", "keyring"} {
		t.Run(legacyType, func(t *testing.T) {
			cfg := store.StoresConfig{
				"local": {Type: legacyType, Options: map[string]any{"backend": keyring.TypeMemory}},
			}
