...
func init() {
	store.Register(store.KindKeychain, buildKeychainStore)
}

// buildKeychainStore is the store.StoreFactory for Keychain stores.
func buildKeychainStore(key string, storeConfig store.StoreConfig) (store.Store, error) {
	var opts KeychainStoreOptions
	if err := parseOptions(storeConfig.Options, &opts); err != nil {
		return nil, fmt.Errorf(errParseFmt, store.ErrParseKeychainOptions, err)
	}
	store.WarnIdentityIgnored(key, storeConfig, "Keychain")
...
func NewKeychainStore(options *KeychainStoreOptions) (store.Store, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("%w: %w", store.ErrKeychainInit, err)
	}
...
func (s *KeychainStore) Set(stack string, component string, key string, value any) error {
	...
	if err := s.kr.Set(composed, encoded); err != nil {
		return fmt.Errorf(errWrapFormatWithID, store.ErrKeychainWrite, composed, err)
	}
	return nil
}
...
func (s *KeychainStore) get(composed string) (any, error) {
	...
	if err != nil {
		if errors.Is(err, keyring.ErrNotFound) {
			return nil, fmt.Errorf(errWrapFormatWithID, store.ErrKeychainNotFound, composed, err)
		}
		return nil, fmt.Errorf(errWrapFormatWithID, store.ErrKeychainRead, composed, err)
	}
...
func (s *KeychainStore) Delete(stack string, component string, key string) error {
	...
	if err := s.kr.Delete(composed); err != nil {
		return fmt.Errorf(errWrapFormatWithID, store.ErrKeychainDelete, composed, err)
	}
	return nil
}
...
func keychainEncodeValue(value any) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("%w: %w", store.ErrKeychainWrite, err)
	}
