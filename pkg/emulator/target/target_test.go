...
func TestAWSProfile(t *testing.T) {
	...
	p := AWSProfile(ep)
	assert.Equal(t, "http://localhost:54321", p.Env["AWS_ENDPOINT_URL"])
	assert.Equal(t, "http://localhost:54321", p.Env["AWS_ENDPOINT_URL_S3"], "per-service S3 endpoint for the state backend")
...
func TestAzureProfile(t *testing.T) {
	...
	assert.True(t, strings.Contains(p.Env["AZURE_STORAGE_CONNECTION_STRING"], "BlobEndpoint=http://localhost:30002/devstoreaccount1"))
}

func TestVaultProfile(t *testing.T) {
	ep := &emu.Endpoint{Target: emu.TargetVault, Host: "localhost", Ports: map[int]int{8200: 8200}}
	p := VaultProfile(ep)
	assert.Equal(t, "http://localhost:8200", p.Env["VAULT_ADDR"])
	assert.Equal(t, "http://localhost:8200", p.Env["BAO_ADDR"])
