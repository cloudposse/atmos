...
func TestStackProcessor(t *testing.T) {
	...
	assert.Equal(t, "catalog/terraform/spacelift/defaults", imports[14])
	assert.Equal(t, "catalog/terraform/spacelift/infrastructure-tenant1", imports[15])
	assert.Equal(t, "catalog/terraform/tenant1-ue2-dev", imports[16])
	assert.Equal(t, "catalog/terraform/test-component", imports[17])
	assert.Equal(t, "catalog/terraform/test-component-override", imports[18])
	assert.Equal(t, "catalog/terraform/test-component-override-2", imports[19])
	assert.Equal(t, "catalog/terraform/test-component-override-3", imports[20])
	assert.Equal(t, "catalog/terraform/top-level-component1", imports[21])
	assert.Equal(t, "catalog/terraform/vpc", imports[22])
	assert.Equal(t, "mixins/region/us-east-2", imports[23])
	assert.Equal(t, "mixins/stage/dev", imports[24])
	assert.Equal(t, "orgs/cp/_defaults", imports[25])
	assert.Equal(t, "orgs/cp/tenant1/_defaults", imports[26])
	assert.Equal(t, "orgs/cp/tenant1/dev/_defaults", imports[27])
