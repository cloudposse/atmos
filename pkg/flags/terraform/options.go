...
// Options provides strongly-typed access to parsed Terraform command flags.
//
// Example usage:
//
//	parser := flags.NewPassThroughFlagParser(
//	    flags.Withflags.TerraformFlags(),
//	)
//	opts := terraform.ParseFlags(cmd, viper.GetViper(), positionalArgs, separatedArgs)
//
...
//   - separatedArgs: Arguments after -- separator to pass to terraform.
//
// Example:
//
...
func ParseFlags(cmd *cobra.Command, v *viper.Viper, positionalArgs, separatedArgs []string) Options {
	defer perf.Track(nil, "terraform.ParseFlags")()

	// Extract component from positional args.
	// Terraform commands: atmos terraform <subcommand> <component>
	// Cobra handles the subcommand, so positionalArgs only contains the component.
	// positionalArgs[0] = component name (vpc, rds, etc.)
	component := ""
...
func (t *Options) GetPositionalArgs() []string {
...
func (t *Options) GetSeparatedArgs() []string {
	defer perf.Track(nil, "flagparser.TerraformOptions.GetSeparatedArgs")()

	return t.separatedArgs
}
...
func FlagsRegistry() *flags.FlagRegistry {
	defer perf.Track(nil, "terraform.FlagsRegistry")()
