...
// Example:
//
//	parser := workflow.NewWorkflowOptionsBuilder().
...
func (b *WorkflowOptionsBuilder) WithFile(required bool) *WorkflowOptionsBuilder {
	...
	if required {
		b.options = append(b.options, flags.WithRequiredStringFlag("file", "f", "Specify the workflow file to run"))
	} else {
		b.options = append(b.options, flags.WithStringFlag("file", "f", "", "Specify the workflow file to run"))
	}
	b.options = append(b.options, flags.WithEnvVars("file", "ATMOS_WORKFLOW_FILE"))
	return b
}
...
func (b *WorkflowOptionsBuilder) WithDryRun() *WorkflowOptionsBuilder {
	...
	b.options = append(b.options, flags.WithBoolFlag("dry-run", "", false, "Simulate the workflow without making any changes"))
	b.options = append(b.options, flags.WithEnvVars("dry-run", "ATMOS_WORKFLOW_DRY_RUN"))
	return b
}
...
func (b *WorkflowOptionsBuilder) WithFromStep() *WorkflowOptionsBuilder {
	...
	b.options = append(b.options, flags.WithStringFlag("from-step", "", "", "Resume the workflow from the specified step"))
	b.options = append(b.options, flags.WithEnvVars("from-step", "ATMOS_WORKFLOW_FROM_STEP"))
	return b
}
...
func (b *WorkflowOptionsBuilder) WithStack(required bool) *WorkflowOptionsBuilder {
	...
	if required {
		b.options = append(b.options, flags.WithRequiredStringFlag("stack", "s", "Atmos stack"))
	} else {
...
func (b *WorkflowOptionsBuilder) WithIdentity() *WorkflowOptionsBuilder {
	...
	b.options = append(b.options, flags.WithIdentityFlag())
	return b
}
