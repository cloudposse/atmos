...
// HelmfileOptions provides strongly-typed access to parsed Helmfile command flags.
//
// Example usage:
//
...
func (h *HelmfileOptions) GetPositionalArgs() []string {
	defer perf.Track(nil, "flagparser.HelmfileOptions.GetPositionalArgs")()

	return h.positionalArgs
}

// GetPassThroughArgs returns pass-through arguments (e.g., ["--args", "foo"]).
func (h *HelmfileOptions) GetPassThroughArgs() []string {
	defer perf.Track(nil, "flagparser.HelmfileOptions.GetPassThroughArgs")()
