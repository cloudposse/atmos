...
// result per failing control entry.
func buildComplianceRulesAndResults(report *ComplianceReport) ([]Rule, []Result) {
	rules := make([]Rule, 0, len(report.FailingDetails))
	results := make([]Result, 0, len(report.FailingDetails))
	index := make(map[string]int)

	for i := range report.FailingDetails {
		ctrl := &report.FailingDetails[i]
		id := ctrl.ControlID
		if id == "" {
