...
func TestSave_And_Load(t *testing.T) {
	...
	// The lockfile preamble identifies the file, recommends committing it, and tells
	// users how to regenerate. See `lockFileHeader` in lockfile.go.
	assert.Contains(t, string(content), "# toolchain.lock.yaml — programmatically managed by Atmos.")
	assert.Contains(t, string(content), "RECOMMENDATION: commit this file to version control")
	assert.Contains(t, string(content), "atmos toolchain lock")
