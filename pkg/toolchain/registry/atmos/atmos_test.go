...
func TestNewBuiltinRegistry_LoadsKustomizeOverride(t *testing.T) {
	reg, err := NewBuiltinRegistry()
	require.NoError(t, err)
	require.NotNil(t, reg)

	tool, err := reg.GetTool("kubernetes-sigs", "kustomize")
	require.NoError(t, err)
	require.NotNil(t, tool)

	assert.Equal(t, "kustomize", tool.Name)
	assert.Equal(t, "github_release", tool.Type)
	assert.Equal(t, "kustomize/v", tool.VersionPrefix)
	assert.Equal(t, "kustomize_v{{.SemVer}}_{{.OS}}_{{.Arch}}.tar.gz", tool.Asset)
	assert.Equal(t, "tar.gz", tool.Format)
	assert.Equal(t, "kustomize", tool.BinaryName)
	assert.Equal(t, "atmos-inline", tool.Registry)
}
