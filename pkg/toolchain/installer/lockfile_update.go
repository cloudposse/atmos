...
func loadInstallerLockFile(filePath string) (*installerLockFile, error) {
	data, err := os.ReadFile(filePath) // #nosec G703 -- filePath is the resolved toolchain lockfile path from installer configuration.
	if err != nil {
...
func saveInstallerLockFile(filePath string, lf *installerLockFile) error {
	...
	if err := os.MkdirAll(filepath.Dir(filePath), defaultMkdirPermissions); err != nil { // #nosec G703 -- directory is derived from the resolved lockfile path.
	...
	if err := os.WriteFile(filePath, []byte(header+string(data)), defaultFileWritePermissions); err != nil { // #nosec G703 -- filePath is the resolved toolchain lockfile path from installer configuration.
