...
// exactly once. The yqlib.InitExpressionParser helper guards
// yqlib.ExpressionParser with a plain nil check and every Evaluate call
// runs it, so concurrent first evaluations otherwise write and read that
