...
func TestIsUNCPath(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "valid_unc_path_with_subdirs",
			path:     `\\server\share\folder\subfolder`,
			expected: true,
		},
		{
			name:     "not_unc_path_regular_windows",
			path:     `C:\Users\test`,
			expected: false,
		},
		{
			name:     "not_unc_path_unix",
			path:     "/home/user",
			expected: false,
		},
		{
			name:     "not_unc_path_single_backslash",
			path:     `\server\share`,
			expected: false,
		},
		{
			name:     "not_unc_path_double_backslash_only",
			path:     `\\`,
			expected: false,
		},
		{
			name:     "not_unc_path_insufficient_parts",
			path:     `\\server`,
			expected: false,
		},
		{
...
func TestEnsureAbsolutePath(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "relative_path_made_absolute",
			path:        "relative/path",
			expectError: false,
			checkResult: func(t *testing.T, result string) {
				assert.True(t, filepath.IsAbs(result), "Path should be made absolute")
		...
		},
		{
			...
			expectError: false,
			checkResult: func(t *testing.T, result string) {
				assert.True(t, filepath.IsAbs(result), "Current directory should be absolute")
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result, err := ensureAbsolutePath(tt.path)
...
func TestJoinUNCPath(t *testing.T) {
	tests := []struct {
		name      string
		basePath  string
		component string
		...
		{
			name:      "join_with_component",
			basePath:  `\\server\share`,
			component: "folder",
			expected:  `\\server\share\folder`,
		},
		{
			name:      "join_with_forward_slashes",
			basePath:  `\\server\share`,
			component: "folder/subfolder",
			expected:  `\\server\share\folder\subfolder`,
		},
		{
			name:      "join_with_trailing_backslash",
			basePath:  `\\server\share\`,
			component: "folder",
			expected:  `\\server\share\folder`,
		},
		...
		{
			name:      "component_with_mixed_slashes",
			basePath:  `\\server\share`,
			component: "folder\\subfolder/deep",
			expected:  `\\server\share\folder\subfolder\deep`,
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := joinUNCPath(tt.basePath, tt.component)
			assert.Equal(t, tt.expected, result)
		})
	}
}

func TestCleanUNCPath(t *testing.T) {
	tests := []struct {
		name     string
		path     string
		expected string
		...
		{
			name:     "multiple_triple_backslashes",
			path:     `\\server\\\share\\\folder\\\subfolder`,
			expected: `\\server\share\folder\subfolder`,
		},
		{
			name:     "preserve_unc_prefix",
			path:     `\\server\share`,
			expected: `\\server\share`,
		},
		{
			name:     "complex_redundant_separators",
			path:     `\\server\\\\\share\\folder`,
			expected: `\\server\share\folder`,
		},
	}
