...
func TestShellRunner_ExitCodePreservation(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expectedExit:   1,
			expectError:    true,
			errorSubstring: "workflow step execution failed with exit code 1",
		},
		{
			...
			expectedExit:   2,
			expectError:    true,
			errorSubstring: "workflow step execution failed with exit code 2",
		},
		{
			...
			expectedExit:   42,
			expectError:    true,
			errorSubstring: "workflow step execution failed with exit code 42",
		},
		{
