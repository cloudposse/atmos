...
func TestAtmosBasePath(t *testing.T) {
	...
	assert.Equal(t, "", atmosBasePath(&scanners.Context{}), "nil config")

	abs := &scanners.Context{AtmosConfig: &schema.AtmosConfiguration{BasePathAbsolute: "/abs", BasePath: "/rel"}}
	assert.Equal(t, "/abs", atmosBasePath(abs), "absolute base path preferred")

	rel := &scanners.Context{AtmosConfig: &schema.AtmosConfiguration{BasePath: "/rel"}}
...
func TestSourceComponentPath(t *testing.T) {
	...
	assert.Equal(t, "", sourceComponentPath(&scanners.Context{AtmosConfig: &schema.AtmosConfiguration{}}), "nil info")

	// No base path anywhere → "".
	noBase := &scanners.Context{
		AtmosConfig: &schema.AtmosConfiguration{},
		Info:        &schema.ConfigAndStacksInfo{ComponentFromArg: "bucket"},
	}
