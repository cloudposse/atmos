...
func normalizeArtifactURIs(data []byte, ctx *scanners.Context) []byte {
	if len(data) == 0 {
		return data
	}
...
func newURIMapper(ctx *scanners.Context) uriMapper {
	return uriMapper{
...
func scanComponentPath(ctx *scanners.Context) string {
...
func sourceComponentPath(ctx *scanners.Context) string {
	if ctx == nil || ctx.AtmosConfig == nil || ctx.Info == nil {
		return ""
	}
