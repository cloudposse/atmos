...
func (e *Executor) handleStepError(params *WorkflowParams, stepName string, cmdParams *runCommandParams, err error) stepResultInternal {
	log.Debug("Workflow step failed", logFieldStep, stepName, "error", err)
...
// The baseEnv parameter contains workflow/step env vars that should be preserved alongside auth vars.
func (e *Executor) prepareAuthenticatedEnvironment(ctx context.Context, identity, stepName string, baseEnv []string) ([]string, error) {
	...
	log.Debug("Prepared environment with identity", "identity", identity, logFieldStep, stepName)
	return stepEnv, nil
}
