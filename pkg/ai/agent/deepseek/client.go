...
const (
	...
	// DefaultModel is the default DeepSeek model. Use "deepseek-reasoner" for the reasoning model.
	...
	// DefaultBaseURL is the default DeepSeek API endpoint (OpenAI-compatible).
	DefaultBaseURL = "https://api.deepseek.com"
)

// Client provides a simplified interface to the DeepSeek API for Atmos.
// DeepSeek's API is OpenAI-compatible, so we use the OpenAI SDK with a custom base URL.
type Client struct {
	client *openai.Client
	config *base.Config
}

// NewClient creates a new DeepSeek client from Atmos configuration.
func NewClient(atmosConfig *schema.AtmosConfiguration) (*Client, error) {
	defer perf.Track(atmosConfig, "deepseek.NewClient")()
	...
	// Create OpenAI client with DeepSeek's base URL and timeout.
...
func (c *Client) SendMessage(ctx context.Context, message string) (string, error) {
	defer perf.Track(nil, "deepseek.Client.SendMessage")()
...
func (c *Client) SendMessageWithTools(ctx context.Context, message string, availableTools []tools.Tool) (*types.Response, error) {
	defer perf.Track(nil, "deepseek.Client.SendMessageWithTools")()

	// Convert our tools to OpenAI's format using shared utility.
	deepseekTools := openaicompat.ConvertToolsToOpenAIFormat(availableTools)
...
func (c *Client) SendMessageWithHistory(ctx context.Context, messages []types.Message) (string, error) {
	defer perf.Track(nil, "deepseek.Client.SendMessageWithHistory")()

	// Convert messages to OpenAI format using shared utility.
	deepseekMessages := openaicompat.ConvertMessagesToOpenAIFormat(messages)
...
func (c *Client) SendMessageWithToolsAndHistory(ctx context.Context, messages []types.Message, availableTools []tools.Tool) (*types.Response, error) {
	defer perf.Track(nil, "deepseek.Client.SendMessageWithToolsAndHistory")()
...
func (c *Client) SendMessageWithSystemPromptAndTools(
	...
	defer perf.Track(nil, "deepseek.Client.SendMessageWithSystemPromptAndTools")()
