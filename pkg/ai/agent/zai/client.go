...
const (
	...
	// DefaultModel is the default Z.AI (Zhipu GLM) model.
	...
	// DefaultBaseURL is the default Z.AI API endpoint (OpenAI-compatible).
	DefaultBaseURL = "https://api.z.ai/api/paas/v4"
)

// Client provides a simplified interface to the Z.AI (Zhipu GLM) API for Atmos.
// Z.AI's API is OpenAI-compatible, so we use the OpenAI SDK with a custom base URL.
type Client struct {
	client *openai.Client
	config *base.Config
}

// NewClient creates a new Z.AI client from Atmos configuration.
func NewClient(atmosConfig *schema.AtmosConfiguration) (*Client, error) {
	defer perf.Track(atmosConfig, "zai.NewClient")()
	...
	// Create OpenAI client with Z.AI's base URL and timeout.
...
func (c *Client) SendMessage(ctx context.Context, message string) (string, error) {
	defer perf.Track(nil, "zai.Client.SendMessage")()
...
func (c *Client) SendMessageWithTools(ctx context.Context, message string, availableTools []tools.Tool) (*types.Response, error) {
	defer perf.Track(nil, "zai.Client.SendMessageWithTools")()

	// Convert our tools to OpenAI's format using shared utility.
	zaiTools := openaicompat.ConvertToolsToOpenAIFormat(availableTools)
...
func (c *Client) SendMessageWithHistory(ctx context.Context, messages []types.Message) (string, error) {
	defer perf.Track(nil, "zai.Client.SendMessageWithHistory")()

	// Convert messages to OpenAI format using shared utility.
	zaiMessages := openaicompat.ConvertMessagesToOpenAIFormat(messages)
...
func (c *Client) SendMessageWithToolsAndHistory(ctx context.Context, messages []types.Message, availableTools []tools.Tool) (*types.Response, error) {
	defer perf.Track(nil, "zai.Client.SendMessageWithToolsAndHistory")()
...
func (c *Client) SendMessageWithSystemPromptAndTools(
	...
	defer perf.Track(nil, "zai.Client.SendMessageWithSystemPromptAndTools")()
