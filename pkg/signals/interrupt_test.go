...
func TestExitCleanups_RunInReverseOrder(t *testing.T) {
	var order []string
	d1 := RegisterExitCleanup(func() { order = append(order, "first") })
	d2 := RegisterExitCleanup(func() { order = append(order, "second") })
	defer d1()
	defer d2()

	RunExitCleanups()
	assert.Equal(t, []string{"second", "first"}, order, "cleanups must run most-recent-first, like defers")
}

func TestExitCleanups_DeregisterRemovesAndIsIdempotent(t *testing.T) {
	ran := false
	deregister := RegisterExitCleanup(func() { ran = true })
	deregister()
	deregister()

	RunExitCleanups()
	assert.False(t, ran, "deregistered cleanups must not run")
}
