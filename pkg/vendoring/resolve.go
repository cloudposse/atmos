...
func FilterComponentsByDeclaredTags(vendorFile string, components []string, tags []string) ([]string, error) {
	...
	if ok {
		...
		for i := range sources {
			name := sources[i].Component
			if name == "" {
				continue
			}
			// Reject a component declared more than once across the manifest and its imports --
			// silently keeping the last declaration (last-one-wins) would let a duplicate
			// silently pick which declaration's tags govern --tags filtering, unlike normal
			// vendor.yaml processing (internal/exec's mergeVendorConfigFiles), which already
			// rejects duplicates outright.
			if seen[name] {
				return nil, fmt.Errorf("%w: %q", errUtils.ErrDuplicateVendorComponent, name)
			}
			seen[name] = true
			declaredTags[name] = sources[i].Tags
		}
	}
