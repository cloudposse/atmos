...
func TestStartExistingContainer(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			status: "running",
			setupMock: func(m *MockRuntime) {
				// Should not call Start when container is already running.
			},
			expectStart: false,
			expectError: false,
		},
		{
			name:   "container stopped - starts",
			status: "exited",
			setupMock: func(m *MockRuntime) {
				m.EXPECT().
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := startExistingContainer(context.Background(), mockRuntime, containerInfo, "test-container")
...
// TestStopAndRemoveContainer validates the stopAndRemoveContainer operation.
