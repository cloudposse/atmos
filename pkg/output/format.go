...
func FlattenMap(m map[string]any, prefix, separator string) map[string]any {
	defer perf.Track(nil, "output.FlattenMap")()
...
// singleValueFormatter is a format function for a single key/value pair.
type singleValueFormatter func(key string, value any) (string, error)

// singleValueFormatters maps formats to their single-value formatter functions.
var singleValueFormatters = map[Format]singleValueFormatter{
	FormatJSON:   func(_ string, value any) (string, error) { return formatSingleJSON(value) },
	FormatYAML:   func(_ string, value any) (string, error) { return formatSingleYAML(value) },
	FormatHCL:    formatSingleHCL,
	FormatEnv:    formatSingleEnv,
	FormatDotenv: formatSingleDotenv,
	FormatBash:   formatSingleBash,
...
func dispatchSingleValueFormat(key string, value any, format Format) (string, error) {
	formatter, ok := singleValueFormatters[format]
	if !ok {
		return "", errUtils.Build(errUtils.ErrInvalidArgumentError).
			WithExplanationf("Unsupported format %q.", format).
			WithHintf("Supported formats: %s.", strings.Join(SupportedFormats, ", ")).
