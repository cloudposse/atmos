...
func TestDescribeComponent(t *testing.T) {
	...
	componentSection, err := e.ExecuteDescribeComponent(component, stack, true, true)
	assert.Nil(t, err)
...
func TestDescribeTemplatedComponent(t *testing.T) {
	componentSection, err := e.ExecuteDescribeComponent("eks-blue/cluster", "tenant1-uw2-test-1", true, true)
	assert.Nil(t, err)
	assert.Equal(t, "eks-blue", componentSection["vars"].(map[string]any)["name"])

	componentSection, err = e.ExecuteDescribeComponent("eks-green/cluster", "tenant1-uw2-test-1", true, true)
	assert.Nil(t, err)
