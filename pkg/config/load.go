...
func processConfigImportsAndReapply(path string, tempViper *viper.Viper, content []byte) error {
	...
	if err := mainViper.ReadConfig(bytes.NewReader(content)); err != nil {
		return fmt.Errorf(errUtils.ErrWrappingFormat, errUtils.ErrProfilerParseMainConfig, err)
	}
	...
	if err := tempViper.MergeConfig(bytes.NewReader(content)); err != nil {
		return fmt.Errorf(errUtils.ErrWrappingFormat, errUtils.ErrProfilerMergeMainConfig, err)
	}
	...
	if err := tempViper.MergeConfig(bytes.NewReader(content)); err != nil {
		return fmt.Errorf(errUtils.ErrWrappingFormat, errUtils.ErrProfilerReapplyMainConfig, err)
	}
