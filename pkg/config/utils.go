...
func processEnvVars() error {
	...
	if len(basePath) > 0 {
		u.PrintInfo(fmt.Sprintf("Found ENV var ATMOS_BASE_PATH=%s", basePath))
		Config.BasePath = basePath
	}

	stacksBasePath := os.Getenv("ATMOS_STACKS_BASE_PATH")
	if len(stacksBasePath) > 0 {
		u.PrintInfo(fmt.Sprintf("Found ENV var ATMOS_STACKS_BASE_PATH=%s", stacksBasePath))
	...
	if len(stacksIncludedPaths) > 0 {
		u.PrintInfo(fmt.Sprintf("Found ENV var ATMOS_STACKS_INCLUDED_PATHS=%s", stacksIncludedPaths))
	...
	if len(stacksExcludedPaths) > 0 {
		u.PrintInfo(fmt.Sprintf("Found ENV var ATMOS_STACKS_EXCLUDED_PATHS=%s", stacksExcludedPaths))
	...
	if len(stacksNamePattern) > 0 {
		u.PrintInfo(fmt.Sprintf("Found ENV var ATMOS_STACKS_NAME_PATTERN=%s", stacksNamePattern))
	...
	if len(componentsTerraformBasePath) > 0 {
		u.PrintInfo(fmt.Sprintf("Found ENV var ATMOS_COMPONENTS_TERRAFORM_BASE_PATH=%s", componentsTerraformBasePath))
	...
	if len(componentsTerraformApplyAutoApprove) > 0 {
		u.PrintInfo(fmt.Sprintf("Found ENV var ATMOS_COMPONENTS_TERRAFORM_APPLY_AUTO_APPROVE=%s", componentsTerraformApplyAutoApprove))
	...
	if len(componentsTerraformDeployRunInit) > 0 {
		u.PrintInfo(fmt.Sprintf("Found ENV var ATMOS_COMPONENTS_TERRAFORM_DEPLOY_RUN_INIT=%s", componentsTerraformDeployRunInit))
	...
	if len(componentsInitRunReconfigure) > 0 {
		u.PrintInfo(fmt.Sprintf("Found ENV var ATMOS_COMPONENTS_TERRAFORM_INIT_RUN_RECONFIGURE=%s", componentsInitRunReconfigure))
	...
	if len(componentsTerraformAutoGenerateBackendFile) > 0 {
		u.PrintInfo(fmt.Sprintf("Found ENV var ATMOS_COMPONENTS_TERRAFORM_AUTO_GENERATE_BACKEND_FILE=%s", componentsTerraformAutoGenerateBackendFile))
	...
	if len(componentsHelmfileBasePath) > 0 {
		u.PrintInfo(fmt.Sprintf("Found ENV var ATMOS_COMPONENTS_HELMFILE_BASE_PATH=%s", componentsHelmfileBasePath))
	...
	if len(componentsHelmfileKubeconfigPath) > 0 {
		u.PrintInfo(fmt.Sprintf("Found ENV var ATMOS_COMPONENTS_HELMFILE_KUBECONFIG_PATH=%s", componentsHelmfileKubeconfigPath))
	...
	if len(componentsHelmfileHelmAwsProfilePattern) > 0 {
		u.PrintInfo(fmt.Sprintf("Found ENV var ATMOS_COMPONENTS_HELMFILE_HELM_AWS_PROFILE_PATTERN=%s", componentsHelmfileHelmAwsProfilePattern))
	...
	if len(componentsHelmfileClusterNamePattern) > 0 {
		u.PrintInfo(fmt.Sprintf("Found ENV var ATMOS_COMPONENTS_HELMFILE_CLUSTER_NAME_PATTERN=%s", componentsHelmfileClusterNamePattern))
	...
	if len(workflowsBasePath) > 0 {
		u.PrintInfo(fmt.Sprintf("Found ENV var ATMOS_WORKFLOWS_BASE_PATH=%s", workflowsBasePath))
...
func processCommandLineArgs(configAndStacksInfo ConfigAndStacksInfo) error {
	if len(configAndStacksInfo.BasePath) > 0 {
		...
		u.PrintInfo(fmt.Sprintf("Using command line argument '%s' as base path for stacks and components", configAndStacksInfo.BasePath))
	}
	if len(configAndStacksInfo.TerraformDir) > 0 {
		Config.Components.Terraform.BasePath = configAndStacksInfo.TerraformDir
		u.PrintInfo(fmt.Sprintf("Using command line argument '%s' as terraform directory", configAndStacksInfo.TerraformDir))
	}
	if len(configAndStacksInfo.HelmfileDir) > 0 {
		Config.Components.Helmfile.BasePath = configAndStacksInfo.HelmfileDir
		u.PrintInfo(fmt.Sprintf("Using command line argument '%s' as helmfile directory", configAndStacksInfo.HelmfileDir))
	}
	if len(configAndStacksInfo.ConfigDir) > 0 {
		Config.Stacks.BasePath = configAndStacksInfo.ConfigDir
		u.PrintInfo(fmt.Sprintf("Using command line argument '%s' as stacks directory", configAndStacksInfo.ConfigDir))
	}
	if len(configAndStacksInfo.StacksDir) > 0 {
		Config.Stacks.BasePath = configAndStacksInfo.StacksDir
		u.PrintInfo(fmt.Sprintf("Using command line argument '%s' as stacks directory", configAndStacksInfo.StacksDir))
	}
	if len(configAndStacksInfo.DeployRunInit) > 0 {
		...
		Config.Components.Terraform.DeployRunInit = deployRunInitBool
		u.PrintInfo(fmt.Sprintf("Using command line argument '%s=%s'", g.DeployRunInitFlag, configAndStacksInfo.DeployRunInit))
	}
	if len(configAndStacksInfo.AutoGenerateBackendFile) > 0 {
		...
		u.PrintInfo(fmt.Sprintf("Using command line argument '%s=%s'", g.AutoGenerateBackendFileFlag, configAndStacksInfo.AutoGenerateBackendFile))
	}
	if len(configAndStacksInfo.WorkflowsDir) > 0 {
		Config.Workflows.BasePath = configAndStacksInfo.WorkflowsDir
		u.PrintInfo(fmt.Sprintf("Using command line argument '%s' as workflows directory", configAndStacksInfo.WorkflowsDir))
	}
	if len(configAndStacksInfo.InitRunReconfigure) > 0 {
		...
		Config.Components.Terraform.InitRunReconfigure = initRunReconfigureBool
		u.PrintInfo(fmt.Sprintf("Using command line argument '%s=%s'", g.InitRunReconfigure, configAndStacksInfo.InitRunReconfigure))
	}
	return nil
}
