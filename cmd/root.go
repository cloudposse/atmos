...
// configCommandToken is the "config" command/argument token recognized by the
// built-in configuration-validation command forms.
...
func isBuiltinConfigValidationArgs(args []string) bool {
	...
	return matchesConfigValidationCommand(rest)
}

// skipLeadingRootFlags consumes leading root flags (and their values) from args
// and returns the remaining arguments. The ok result is false when a "--"
// terminator or a value-consuming flag with no value ends the argument list
// before any command token can follow.
...
// matchesConfigValidationCommand reports whether the flag-stripped arguments
// spell one of the built-in configuration-validation command forms:
// "config validate", "validate config", or "validate schema config".
func matchesConfigValidationCommand(args []string) bool {
	if len(args) < 2 {
		return false
	}
	if args[0] == configCommandToken {
		return args[1] == "validate"
	}
	if args[0] != "validate" {
		return false
	}
	if args[1] == configCommandToken {
		return true
	}
	return args[1] == "schema" && len(args) >= 3 && args[2] == configCommandToken
}
