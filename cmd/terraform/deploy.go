...
func init() {
	...
	deployParser = flags.NewStandardParser(
		...
		flags.WithBoolFlag("verify-plan", "", false, "Force planfile drift verification before applying, overriding config; requires planfile storage configured under components.terraform.planfiles (use --verify-plan=false to disable)"),
