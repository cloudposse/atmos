...
func init() {
	...
	planParser = flags.NewStandardParser(
		...
		flags.WithBoolFlag("hide-no-changes", "", false, "Hide Terraform plan logs for components with no changes. Implies --log-order=grouped"),
		...
		flags.WithEnvVars("hide-no-changes", "ATMOS_TERRAFORM_PLAN_HIDE_NO_CHANGES"),
