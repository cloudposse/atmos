...
func TestParseTerraformRunOptionsRejectsInvalidValues(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantErr: `invalid value for flag: --failure-mode "eventually": supported values are "fail-fast", "keep-going"`,
		},
		{
			...
			wantErr: `invalid value for flag: --log-order "interleaved": supported values are "stream", "grouped"`,
		},
	}
