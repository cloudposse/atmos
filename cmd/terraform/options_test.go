...
func TestVerifyPlanCLIOverride(t *testing.T) {
	tests := []struct {
		name       string
		verifyPlan *bool
		want       schema.PlanfileVerifyMode
	}{
		{"unset defers to config/CI default", nil, ""},
		{"--verify-plan forces fail", boolPtr(true), schema.PlanfileVerifyFail},
		{"--verify-plan=false forces off", boolPtr(false), schema.PlanfileVerifyOff},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			opts := &TerraformRunOptions{VerifyPlan: tt.verifyPlan}
			assert.Equal(t, tt.want, opts.VerifyPlanCLIOverride())

			// Confirm the resolved override flows through to info.
			info := &schema.ConfigAndStacksInfo{}
			applyOptionsToInfo(info, opts)
			assert.Equal(t, tt.want, info.VerifyPlanMode)
		})
	}
}

func TestParseTerraformRunOptions_VerifyPlanTriState(t *testing.T) {
	tests := []struct {
		name  string
		setup func(*viper.Viper)
		want  *bool
	}{
		{"unset leaves VerifyPlan nil", func(v *viper.Viper) {}, nil},
		{"--verify-plan true", func(v *viper.Viper) { v.Set("verify-plan", true) }, boolPtr(true)},
		{"--verify-plan=false", func(v *viper.Viper) { v.Set("verify-plan", false) }, boolPtr(false)},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			v := viper.New()
			...
			assert.NoError(t, err)
			if tt.want == nil {
				assert.Nil(t, opts.VerifyPlan)
				return
			}
			if assert.NotNil(t, opts.VerifyPlan) {
				assert.Equal(t, *tt.want, *opts.VerifyPlan)
			}
		})
	}
}
