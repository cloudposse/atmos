...
func validateTerraformRunOptions(opts *TerraformRunOptions) error {
	...
	if mode := strings.ToLower(strings.TrimSpace(opts.FailureMode)); mode != "" {
		...
		default:
			return fmt.Errorf("%w %q: supported values are %q, %q", errUtils.ErrInvalidTerraformFailureMode, opts.FailureMode, terraformFailureModeFailFast, terraformFailureModeKeepGoing)
		}
	}
	...
	if logOrder := strings.ToLower(strings.TrimSpace(opts.LogOrder)); logOrder != "" {
		...
		default:
			return fmt.Errorf("%w %q: supported values are %q, %q", errUtils.ErrInvalidTerraformLogOrder, opts.LogOrder, terraformLogOrderStream, terraformLogOrderGrouped)
		}
	}
	return nil
}
