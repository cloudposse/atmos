...
func validateTerraformRunOptions(opts *TerraformRunOptions) error {
	...
	if mode := strings.ToLower(strings.TrimSpace(opts.FailureMode)); mode != "" {
		...
		default:
			return fmt.Errorf("%w: --failure-mode %q: supported values are %q, %q", errUtils.ErrInvalidFlagValue, opts.FailureMode, terraformFailureModeFailFast, terraformFailureModeKeepGoing)
		}
	}
	...
	if logOrder := strings.ToLower(strings.TrimSpace(opts.PlanLogOrder)); logOrder != "" {
		...
		default:
			return fmt.Errorf("%w: --log-order %q: supported values are %q, %q", errUtils.ErrInvalidFlagValue, opts.PlanLogOrder, terraformPlanLogOrderStream, terraformPlanLogOrderGrouped)
		}
	}
	return nil
}
