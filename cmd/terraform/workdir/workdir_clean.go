...
Workdirs are ephemeral and can be regenerated by running 'atmos terraform init'.`,
  ...
  atmos terraform workdir clean --expired --ttl=24h --dry-run`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if all {
			return cleanAllWorkdirs(&atmosConfig, dryRun)
		}

		component := args[0]
		componentConfig := resolveComponentConfig(&configInfo, component, stack)
		return cleanSpecificWorkdir(&atmosConfig, component, stack, componentConfig)
	},
}
...
func cleanAllWorkdirs(atmosConfig *schema.AtmosConfiguration, dryRun bool) error {
	...
	if err := workdirManager.CleanAllWorkdirs(atmosConfig, dryRun); err != nil {
		return err
	}

	// A dry run already reported what it would clean (see provWorkdir.CleanAllWorkdirs); saying
	// "cleaned" here too would falsely imply something was actually deleted.
	if !dryRun {
...
func cleanSpecificWorkdir(atmosConfig *schema.AtmosConfiguration, component, stack string, componentConfig map[string]any) error {
