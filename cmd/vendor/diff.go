...
func resolveDiffComponents(cmd *cobra.Command, v *viper.Viper) ([]string, error) {
	...
	// flags.BuildConfigAndStacksInfo carries --base-path/--config/--config-path/--profile into the
	// config load, matching clean/verify's use of ProcessCommandLineArgs for the same purpose -- an
	// empty schema.ConfigAndStacksInfo{} silently ignored those global flags.
	//
	...
	// directly and don't.
	info := flags.BuildConfigAndStacksInfo(cmd, v)
	atmosConfig, cfgErr := cfg.InitCliConfig(info, stack != "" || len(labels) > 0)
	if cfgErr != nil {
		return nil, cfgErr
	}
	...
	return resolveVendorSelectorComponents(&VendorSelectorOptions{
		...
		TypeChanged:   typeSelectorChanged(cmd, v),
	})
}
