...
`), 0o644))
	...
	t.Run("a tag-matched source with no component field is rejected, not passed through as \"\"", func(t *testing.T) {
		anonFile := filepath.Join(t.TempDir(), "vendor.yaml")
		require.NoError(t, os.WriteFile(anonFile, []byte(`apiVersion: atmos/v1
kind: AtmosVendorConfig
spec:
  sources:
    - source: github.com/cloudposse/terraform-aws-vpc
...
`), 0o644))
		...
		got, err := resolveVendorTagsSelector(anonFile, []string{"networking"})
		require.Error(t, err)
		assert.ErrorIs(t, err, errUtils.ErrComponentRequired)
		assert.Nil(t, got)
	})
}
...
// TestResolveVendorSelectorComponents_TagsOnlyPropagatesBrokenVendorYamlError proves a broken
// vendor.yaml surfaces as an error through BOTH resolveVendorTagsSelector's own
// vendoring.ListDeclaredSources call AND the resolveVendorSelectorComponents switch that invokes it
// (the --tags-only, no-base-selector branch) -- rather than being masked as "matched nothing".
func TestResolveVendorSelectorComponents_TagsOnlyPropagatesBrokenVendorYamlError(t *testing.T) {
	brokenFile := filepath.Join(t.TempDir(), "vendor.yaml")
	require.NoError(t, os.WriteFile(brokenFile, []byte("spec: ["), 0o644))

	got, err := resolveVendorSelectorComponents(&VendorSelectorOptions{
		AtmosConfig: &schema.AtmosConfiguration{},
		Tags:        []string{"networking"},
		VendorFile:  brokenFile,
	})
...
// TestResolveVendorSelectorComponents_ComponentAndTagsPropagatesBrokenVendorYamlError proves that
// when --tags composes with --component (an already-resolved base selector, not the --tags-only
// path above), a broken vendor.yaml surfacing through the FilterComponentsByDeclaredTags narrowing
// step is also propagated as an error rather than silently dropping the component.
func TestResolveVendorSelectorComponents_ComponentAndTagsPropagatesBrokenVendorYamlError(t *testing.T) {
	brokenFile := filepath.Join(t.TempDir(), "vendor.yaml")
	require.NoError(t, os.WriteFile(brokenFile, []byte("spec: ["), 0o644))
