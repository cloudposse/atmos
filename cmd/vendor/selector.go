...
// typeSelectorChanged reports whether --type was effectively set to a non-default value, either via
// the flag itself (cmd.Flags().Changed) or -- since --type is bound to v via BindFlagsToViper -- an
// environment variable (e.g. ATMOS_TYPE) with no --type flag on the command line. Checking only
// Changed("type") misses that second case: the effective type v.GetString("type") returns DOES
// change, but Changed("type") stays false, so callers narrowing by TypeChanged would silently ignore
// an env-set type, violating flag precedence (CLI flags > env vars > config > defaults). Compares
// against the flag's own registered DefValue rather than a hardcoded literal so this can't drift out
// of sync if the flag's default is ever changed.
func typeSelectorChanged(cmd *cobra.Command, v *viper.Viper) bool {
	...
	return v.GetString("type") != f.DefValue
}
...
// returns an empty result, not an error, from THIS function -- exactly like `vendor pull --tags`
// finds nothing to filter in a component.yaml-only repo. That said, resolveVendorSelectorComponents
// (this function's own caller, for the "--tags alone" branch) applies its own zero-match check
// afterwards and turns an empty result into errUtils.ErrInvalidArgumentError -- so a --tags-only
// selector against a component.yaml-only repo is not a silent end-to-end no-op, only within this
...
func resolveVendorTagsSelector(vendorFile string, filterTags []string) ([]string, error) {
	...
	for i := range sources {
		...
		if source.Component == "" {
			return nil, errUtils.Build(errUtils.ErrComponentRequired).
				WithExplanation(fmt.Sprintf("A vendor.yaml source declares tags %v but no 'component' field.", source.Tags)).
				WithHint("Add 'component: <name>' to every vendor.yaml source entry.").
				Err()
		}
