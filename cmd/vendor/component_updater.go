...
//
// Components is the already-normalized (normalizeComponentSelectors) --component selector, not the
// raw cmd.Flags().Changed("component") state: "--component=" (an explicit but empty value) sets
// Changed("component") true, yet normalizeComponentSelectors reduces it to an empty slice, so
// checking Changed() alone would incorrectly reject "--component= --stack=dev" as a conflict even
// though there's no actual --component selector left to conflict with --stack.
//
...
func validateUpdateSelectorFlags(components []string, stack string, labels map[string]string) error {
