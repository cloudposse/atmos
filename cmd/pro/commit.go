...
import (
	...
	propkg "github.com/cloudposse/atmos/pkg/pro"
	"github.com/cloudposse/atmos/pkg/schema"
)

// commitCmd executes 'pro commit' CLI command.
var commitCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		return propkg.ExecuteCommit(&atmosConfig, message, comment, addPattern, stageAll)
	},
}

func init() {
	commitCmd.Flags().StringP("message", "m", "", "Commit message (required, max 500 chars)")
	commitCmd.Flags().String("comment", "", "Optional PR comment to post alongside the commit (max 2000 chars)")
	commitCmd.Flags().String("add", "", "File pattern to stage before committing (e.g. \"*.tf\")")
	commitCmd.Flags().BoolP("all", "A", false, "Stage all changes before committing (git add -A)")
