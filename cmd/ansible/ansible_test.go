...
func TestPlaybookCommandStructure(t *testing.T) {
	t.Run("playbook command has correct properties", func(t *testing.T) {
		// Usage is generated by the positional args builder in init().
		assert.Equal(t, "playbook <component>", playbookCmd.Use)
...
func TestInitConfigAndStacksInfoDoubleDash(t *testing.T) {
	...
	// from the "--" separator (not from a positional offset). Uses the real
	// builder-generated validator wired onto playbookCmd in init().
...
func TestPlaybookCmdArgsValidationWithDoubleDash(t *testing.T) {
	...
	// `--stack`/`-s` and `--dry-run` flags plus the same builder-generated
	// Args validator and FParseErrWhitelist. Parsing a real argv populates
	// ArgsLenAtDash, which the separator-aware validator relies on to ignore
	// ansible-playbook passthrough args.
