...
var commandMaps = map[string]func(cmd *cobra.Command){
	"plan": func(cmd *cobra.Command) {
		...
		cmd.PersistentFlags().Bool("plan-skip-planfile", false, "Skip writing the plan to a file by not passing the `-out` flag to Terraform when executing the command. Set it to `true` when using Terraform Cloud since the `-out` flag is not supported. Terraform Cloud automatically stores plans in its backend")
	},
