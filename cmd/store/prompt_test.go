...
func TestConfirmAction(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := confirmAction("Delete this value?")
...
func TestPromptForStoreValue_FormError(t *testing.T) {
	...
	_, err := promptForStoreValue()
	require.Error(t, err)
	assert.ErrorIs(t, err, boom)
	assert.Contains(t, err.Error(), "store prompt failed")
}

// TestPromptForStoreValue_PTY exercises the real masked-input body. The huh accessible password
...
func TestPromptForStoreValue_PTY(t *testing.T) {
