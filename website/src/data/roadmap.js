  ...
  initiatives: [
    ...
    },
    {
      ...
      milestones: [
        ...
        { label: '`atmos version --short` for scripting', status: 'shipped', quarter: 'q1-2026', pr: 2174, changelog: 'version-short-flag', docs: '/cli/commands/version/usage', description: 'New `--short` / `-s` flag on `atmos version` that prints only the version number—no banner, no update check. Also adds `--format plain` as an explicit format value.', benefits: 'Capture the version in shell scripts with a single clean command: `ATMOS_VER=$(atmos version --short)`. No more fragile grep-based parsing.' },
      ],
      issues: [],
      prs: [
