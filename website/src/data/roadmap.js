  ...
  initiatives: [
    {
      ...
      status: 'in-progress',
      milestones: [
        { label: 'Added `atmos auth` command framework', status: 'shipped', quarter: 'q2-2025', version: 'v1.196.0', docs: '/cli/commands/auth/usage', changelog: 'introducing-atmos-auth', description: 'Unified command for managing authentication across cloud providers and CI systems.', benefits: 'One command replaces aws-vault, saml2aws, gcloud auth, and azure login. Credentials are managed consistently across all providers.' },
        { label: 'AWS IAM Identity Center (SSO)', status: 'shipped', quarter: 'q2-2025', version: 'v1.196.0', docs: '/cli/configuration/auth/providers', changelog: 'introducing-atmos-auth', description: 'Native support for AWS SSO login with automatic credential caching and session refresh.', benefits: 'Log in once and work across all AWS accounts. Sessions refresh automatically without interrupting your workflow.' },
        { label: 'AWS IAM Users', status: 'shipped', quarter: 'q4-2025', version: 'v1.201.0', docs: '/cli/configuration/auth/providers', description: 'Support for traditional IAM user credentials with secure credential management.', benefits: 'Teams with IAM users can adopt Atmos auth without changing their identity provider.' },
        { label: 'Assume Root capability', status: 'shipped', quarter: 'q4-2025', version: 'v1.201.0', docs: '/cli/configuration/auth/providers', changelog: 'aws-assume-root-identity', description: 'Centralized root access management for organizations that need controlled root-level operations.', benefits: 'Auditable root access for break-glass scenarios. No shared root credentials or password managers.' },
        { label: 'GitHub OIDC', status: 'shipped', quarter: 'q3-2025', version: 'v1.196.0', docs: '/cli/configuration/auth/providers', changelog: 'introducing-atmos-auth', description: 'Native GitHub Actions OIDC integration for secure, secretless CI/CD authentication to AWS.', benefits: 'No AWS access keys in GitHub secrets. Credentials are scoped to the repository and workflow.' },
        { label: 'Azure AD / Workload Identity Federation', status: 'shipped', quarter: 'q3-2025', version: 'v1.199.0', docs: '/cli/configuration/auth/providers', changelog: 'azure-authentication-support', description: 'Authenticate to Azure using Entra ID (Azure AD) with support for Workload Identity Federation for CI/CD.', benefits: 'Multi-cloud teams use the same auth patterns for AWS and Azure without separate tooling.' },
        { label: 'SAML Provider', status: 'shipped', quarter: 'q3-2025', version: 'v1.196.0', docs: '/cli/configuration/auth/providers', changelog: 'introducing-atmos-auth', description: 'Enterprise SAML-based authentication for organizations using identity providers like Okta or OneLogin.', benefits: 'Enterprises can enforce their existing IdP policies without custom scripts or third-party tools.' },
        { label: 'Keyring backends (system, file, memory)', status: 'shipped', quarter: 'q3-2025', version: 'v1.196.0', docs: '/cli/configuration/auth/keyring', changelog: 'flexible-keyring-backends', description: 'Flexible credential storage with system keychain integration, encrypted file storage, or in-memory sessions.', benefits: 'Credentials are stored securely based on your environment—macOS Keychain, Linux secret-tool, or encrypted files.' },
        { label: 'Component-level auth identities', status: 'shipped', quarter: 'q3-2025', version: 'v1.197.0', docs: '/cli/configuration/auth/identities', changelog: 'authentication-for-workflows-and-custom-commands', description: 'Define different AWS identities per component, enabling multi-account deployments from a single workflow.', benefits: 'Deploy to prod, staging, and dev accounts in a single workflow without switching credentials.' },
        { label: 'Per-step workflow authentication', status: 'shipped', quarter: 'q3-2025', version: 'v1.197.0', docs: '/cli/configuration/auth/identities', changelog: 'authentication-for-workflows-and-custom-commands', description: 'Each workflow step can assume a different identity, enabling cross-account orchestration.', benefits: 'Complex multi-account deployments are expressed declaratively. No credential juggling in scripts.' },
        { label: 'Isolated shell sessions (`atmos auth shell`)', status: 'shipped', quarter: 'q4-2025', version: 'v1.196.0', docs: '/cli/commands/auth/auth-shell', changelog: 'atmos-auth-shell', description: 'Launch isolated shell sessions with specific identity credentials pre-loaded for secure multi-identity workflows.', benefits: 'Work in multiple AWS accounts simultaneously with isolated shells. No credential conflicts or accidental cross-account operations.' },
        { label: 'Zero-config AWS SSO identity management', status: 'shipped', quarter: 'q4-2025', version: 'v1.200.0', changelog: 'aws-sso-identity-auto-provisioning', description: 'Automatic SSO identity provisioning without manual configuration—Atmos detects and configures SSO settings.', benefits: 'Get started with AWS SSO instantly. No manual identity configuration required.' },
        { label: 'Identity flag for describe commands', status: 'shipped', quarter: 'q4-2025', version: 'v1.197.0', changelog: 'describe-commands-identity-flag', description: 'Use --identity flag with describe commands to see configuration as it would appear under a specific identity.', benefits: 'Debug identity-specific configurations without switching credentials.' },
        { label: 'Seamless first login with provider fallback', status: 'shipped', quarter: 'q4-2025', version: 'v1.203.0', pr: 1918, changelog: 'auth-login-provider-fallback', description: 'Automatic provider fallback when no identities are configured, enabling seamless first-time login with auto_provision_identities.', benefits: 'Just run atmos auth login on first use. No need to know about --provider flag.' },
    ...
    },
    {
      ...
      status: 'in-progress',
      milestones: [
        { label: 'Zero-config terminal output (auto TTY/color)', status: 'shipped', quarter: 'q3-2025', version: 'v1.198.0', docs: '/cli/configuration/settings/terminal', changelog: 'zero-config-terminal-output', description: 'Terminal output adapts automatically to your environment—colors when interactive, clean text when piping. Works identically in CI and locally without configuration.', benefits: 'No terminal configuration needed. Output is readable regardless of where commands run—locally, in CI, or piped to files.' },
        { label: 'Force flags (`--force-tty`, `--force-color`)', status: 'shipped', quarter: 'q3-2025', version: 'v1.198.0', docs: '/cli/global-flags', changelog: 'zero-config-terminal-output', description: 'Override auto-detection for CI environments or screenshot generation with explicit TTY and color control.', benefits: 'Generate consistent screenshots for documentation or force colors in CI environments that support them.' },
        { label: 'Auto-degradation (TrueColor→256→16→None)', status: 'shipped', quarter: 'q3-2025', version: 'v1.198.0', docs: '/cli/configuration/settings/terminal', changelog: 'zero-config-terminal-output', description: 'Graceful color fallback from 16 million colors to 256 to 16 to plain text based on terminal support.', benefits: 'Output looks good on any terminal—from modern iTerm2 to legacy xterm to CI logs.' },
        { label: 'Secret masking (built-in patterns + custom expressions)', status: 'shipped', quarter: 'q3-2025', version: 'v1.198.0', docs: '/cli/configuration/settings/mask', changelog: 'zero-config-terminal-output', description: 'Automatic detection and masking of 120+ secret patterns (AWS keys, tokens, passwords) in terminal output.', benefits: 'Secrets are protected automatically. Safe to share terminal output or stream logs without audit risk.' },
        { label: 'Added `--chdir` flag for multi-repo workflows', status: 'shipped', quarter: 'q3-2025', version: 'v1.195.0', docs: '/cli/global-flags', changelog: 'introducing-chdir-flag', description: 'Run Atmos commands from any directory by specifying the project root with --chdir.', benefits: 'Scripts and CI jobs can target any repo without changing directories or managing paths.' },
        { label: 'Parent directory search & git root discovery', status: 'shipped', quarter: 'q3-2025', version: 'v1.198.0', changelog: 'parent-directory-search-and-git-root-discovery', description: 'Run Atmos from any directory within your project—automatic discovery of atmos.yaml by searching parent directories up to the git repository root.', category: 'featured', priority: 'high', benefits: 'Run atmos from anywhere in your project. No need to cd to the root directory first.' },
        { label: 'Native Dev Containers', status: 'shipped', quarter: 'q4-2025', version: 'v1.201.0', docs: '/cli/configuration/devcontainer', changelog: 'native-devcontainer-support', description: 'Native Dev Container support with automatic container lifecycle management—start, stop, attach, and exec.', category: 'featured', priority: 'high', benefits: 'Consistent development environments across the team. New contributors are productive immediately.' },
        { label: 'Interactive prompts for missing flags', status: 'shipped', quarter: 'q4-2025', version: 'v1.199.0', changelog: 'interactive-flag-prompts', description: 'Prompts for missing required flags with tab completion and validation.', category: 'featured', priority: 'high', benefits: 'Discover available options interactively instead of reading docs. Commands guide you through required inputs.' },
        { label: 'Atmos Profiles', status: 'shipped', quarter: 'q4-2025', version: 'v1.199.0', docs: '/cli/configuration/profiles', changelog: 'atmos-profiles', description: 'Named configuration profiles for different environments or projects, switchable on the fly.', benefits: 'Switch between projects or environments without editing config files or setting environment variables.' },
        { label: 'Automated Terraform backend provisioning', status: 'shipped', quarter: 'q4-2025', version: 'v1.200.0', docs: '/components/terraform/backend-provisioning', changelog: 'automatic-backend-provisioning', description: 'Automatic backend.tf generation from stack configuration. Backends are defined once and inherited.', category: 'featured', priority: 'high', benefits: 'No more copy-pasting backend blocks. Backend configuration is defined once and applied consistently.' },
        ...
        { label: 'Component-aware tab completion for all commands', status: 'shipped', quarter: 'q4-2025', version: 'v1.198.0', docs: '/cli/commands/completion', changelog: 'component-aware-stack-completion', description: 'Shell completion that understands your stacks and components for faster command entry.', category: 'featured', priority: 'high', benefits: 'Tab completion shows only valid stacks and components. Discover what exists without searching config files.' },
        { label: 'Filesystem paths for components', status: 'shipped', quarter: 'q4-2025', version: 'v1.201.0', changelog: 'path-based-component-resolution', description: 'Reference components by filesystem path instead of component name. Run commands from component directories.', codeExample: 'atmos terraform plan ./components/vpc -s plat-ue1-prod', category: 'featured', priority: 'high', benefits: 'Work on components naturally by navigating to their directory. Component names are inferred from the path.' },
        { label: 'Automatic toolchain installation and version management', status: 'shipped', quarter: 'q4-2025', version: 'v1.200.0', description: 'Install and manage tool versions (Terraform, OpenTofu, Helmfile) based on component requirements.', benefits: 'The correct tool versions are installed automatically. No tfenv, tgswitch, or manual version management.' },
        { label: 'Terminal themes', status: 'shipped', quarter: 'q4-2025', version: 'v1.198.0', docs: '/cli/configuration/settings/terminal', changelog: 'terminal-themes', description: 'Customizable terminal color themes with built-in presets and user-defined themes.', benefits: 'Match your terminal output to your preferences or brand. Switch themes without code changes.' },
        { label: 'Theme-aware help text', status: 'shipped', quarter: 'q4-2025', version: 'v1.200.0', changelog: 'theme-aware-help', description: 'Help text respects your terminal theme settings for consistent styling.', benefits: 'Help output matches your theme. Consistent look across all Atmos commands.' },
        { label: 'Helpful error messages with hints', status: 'shipped', quarter: 'q4-2025', version: 'v1.199.0', changelog: 'helpful-errors', description: 'Error messages include actionable hints, rich context, and optional Sentry integration for enterprise error tracking.', benefits: 'Understand what went wrong and how to fix it. Enterprise teams get centralized error tracking.' },
        { label: 'Interactive Terraform prompts', status: 'shipped', quarter: 'q4-2025', version: 'v1.202.0', changelog: 'interactive-terraform-prompts', description: 'Interactive component and stack selection when running Terraform commands without arguments.', benefits: 'Discover and select components interactively. No need to remember exact names.' },
        { label: 'Seamless first login with provider fallback', status: 'shipped', quarter: 'q4-2025', version: 'v1.203.0', pr: 1918, changelog: 'auth-login-provider-fallback', description: 'Auth login automatically falls back to provider when no identities exist, enabling zero-friction first-time login.', benefits: 'New users just run atmos auth login. No need to understand --provider flag on first use.' },
        { label: 'Configuration provenance tracking', status: 'shipped', quarter: 'q4-2025', version: 'v1.195.0', changelog: 'provenance-tracking', description: 'Track where every configuration value comes from—which file, which import, which override.', benefits: 'Debug complex inheritance chains. Understand exactly how configuration was assembled.' },
        { label: 'Global environment variables', status: 'shipped', quarter: 'q4-2025', version: 'v1.201.0', changelog: 'global-env-section', description: 'Define environment variables globally in atmos.yaml that apply to all commands.', benefits: 'Set common environment variables once. No repetition across components.' },
        { label: 'Metadata inheritance', status: 'shipped', quarter: 'q4-2025', version: 'v1.200.0', changelog: 'metadata-inheritance', description: 'Metadata section now supports full inheritance from base stacks.', benefits: 'Define metadata patterns once and inherit everywhere. Consistent naming across stacks.' },
      ],
      issues: [],
      prs: [
    ...
    },
    {
      ...
      status: 'shipped',
      milestones: [
        { label: '`atmos list stacks` to browse all configured stacks', status: 'shipped', quarter: 'q1-2025', version: 'v1.110.0', docs: '/cli/commands/list/stacks', description: 'List all stacks in your infrastructure with filtering and formatting options.', codeExample: 'atmos list stacks --format json', benefits: 'See all your stacks at a glance. New team members can understand infrastructure scope immediately.' },
        { label: '`atmos list components` to discover available components', status: 'shipped', quarter: 'q1-2025', version: 'v1.110.0', docs: '/cli/commands/list/components', description: 'Discover all available components across your infrastructure catalog.', codeExample: 'atmos list components', benefits: 'Find available components without digging through directories. See what\'s ready to deploy.' },
        { label: '`atmos list workflows` to explore automation patterns', status: 'shipped', quarter: 'q3-2025', version: 'v1.155.0', docs: '/cli/commands/list/workflows', description: 'Browse available workflows with their descriptions and step counts.', codeExample: 'atmos list workflows', benefits: 'Discover automation patterns your team has defined. Understand what workflows are available before running them.' },
        { label: '`atmos list affected` to identify changes for targeted CI/CD', status: 'shipped', quarter: 'q4-2025', version: 'v1.203.0', docs: '/cli/commands/list/affected', changelog: 'list-affected-command', description: 'Identify which stacks and components are affected by changes, enabling targeted CI/CD pipelines.', category: 'featured', priority: 'high', benefits: 'CI only plans/applies what changed. Large monorepos deploy in minutes instead of hours.' },
        { label: 'Customizable list columns for tailored views', status: 'shipped', quarter: 'q4-2025', version: 'v1.200.0', changelog: 'customizable-list-command-output', description: 'Configure which columns appear in list output to show the data relevant to your workflow.', category: 'featured', priority: 'high', benefits: 'Show exactly the information you need—environment, region, component status—without noise.' },
        { label: 'Easy filter output using jq-like queries (`--query`)', status: 'shipped', quarter: 'q4-2025', version: 'v1.200.0', docs: '/cli/commands/list/stacks', description: 'Filter and transform list output with familiar jq-style expressions (powered by JMESPath) for precise data extraction.', codeExample: 'atmos list stacks --query "[?components.terraform.vpc]"', category: 'featured', priority: 'high', benefits: 'Query your infrastructure like a database. Find stacks with specific components or configurations instantly.' },
      ],
      issues: [],
      prs: [
    ...
    },
    {
      ...
      status: 'shipped',
      milestones: [
        { label: 'New workflow step types (show, sleep, stage, alert, etc.)', status: 'shipped', quarter: 'q3-2025', version: 'v1.190.0', docs: '/cli/configuration/workflows', description: 'Rich step types including message display, timed delays, user prompts, and alert notifications.', category: 'featured', priority: 'high', benefits: 'Build interactive deployment workflows with user confirmations and status updates.' },
        { label: 'New interactive input types for workflow steps', status: 'shipped', quarter: 'q3-2025', version: 'v1.190.0', docs: '/cli/configuration/workflows', description: 'Typed inputs for workflow steps with validation—strings, numbers, booleans, and selections.', category: 'featured', priority: 'high', benefits: 'Inputs are validated before execution starts. Bad values are caught early, not mid-deployment.' },
        { label: 'Run any workflow without specifying the workflow file', status: 'shipped', quarter: 'q3-2025', version: 'v1.199.0', docs: '/cli/configuration/workflows', changelog: 'workflow-file-auto-discovery', description: 'Workflows are automatically discovered without needing to specify the file path.', category: 'featured', priority: 'high', benefits: 'Just run atmos workflow <name>. No need to remember which file contains which workflow.' },
        { label: 'Workflows can now change directories before executing commands', status: 'shipped', quarter: 'q3-2025', version: 'v1.201.0', docs: '/cli/configuration/workflows', changelog: 'working-directory-support', description: 'Execute workflow steps in specific directories, enabling multi-repo orchestration.', category: 'featured', priority: 'high', benefits: 'Orchestrate across multiple repositories in a single workflow.' },
        { label: 'Integration with `atmos auth` identities for per-step authentication', status: 'shipped', quarter: 'q3-2025', version: 'v1.197.0', docs: '/cli/configuration/auth/identities', changelog: 'authentication-for-workflows-and-custom-commands', description: 'Each workflow step can use different credentials for cross-account deployments.', category: 'featured', priority: 'high', benefits: 'Deploy to multiple accounts in sequence without credential juggling.' },
        { label: 'Unified task execution (`pkg/runner`)', status: 'shipped', quarter: 'q4-2025', version: 'v1.201.0', pr: 1901, description: 'A single execution engine for all task types—Terraform, Helmfile, shell, and custom commands.', benefits: 'Consistent behavior across all command types. Same output handling, error reporting, and logging.' },
      ],
      issues: [],
      prs: [
    ...
    },
    {
      ...
      milestones: [
        ...
        { label: 'Added `!terraform.output` YAML function', status: 'shipped', quarter: 'q3-2025', version: 'v1.185.0', docs: '/functions/yaml/terraform.output', description: 'Reference outputs from other components without data sources.', codeExample: 'subnet_ids: !terraform.output vpc.private_subnet_ids', benefits: 'Wire components together declaratively. Dependencies are explicit in configuration.' },
        { label: 'Added `!store` YAML function', status: 'shipped', quarter: 'q3-2025', version: 'v1.185.0', docs: '/functions/yaml/store', description: 'Access secrets from configured stores (SSM, Secrets Manager, etc.).', codeExample: 'api_key: !store ssm:/myapp/api-key', benefits: 'Reference secrets at deploy time. No hardcoded values or environment variables needed.' },
        { label: 'Added `!literal` YAML function', status: 'shipped', quarter: 'q4-2025', version: 'v1.202.0', docs: '/functions/yaml/literal', changelog: 'literal-yaml-function', description: 'Pass raw HCL/JSON values without YAML interpretation for complex Terraform expressions.', codeExample: 'policy: !literal \'jsonencode({...})\'', benefits: 'Use complex HCL expressions without YAML escaping headaches.' },
        { label: 'Custom component types with registry', status: 'shipped', quarter: 'q4-2025', version: 'v1.195.0', docs: '/cli/configuration/commands', changelog: 'introducing-command-registry-pattern', description: 'Register custom component types that integrate seamlessly with Atmos commands and workflows.', benefits: 'Your custom commands get the same tab completion, list commands, and workflow integration as built-ins.' },
        { label: 'Semantic type completion', status: 'shipped', quarter: 'q4-2025', version: 'v1.198.0', docs: '/cli/commands/completion', description: 'Context-aware shell completion that understands your component types and suggests valid options.', benefits: 'Tab completion suggests values that make sense for each argument, not just filenames.' },
        { label: 'Auth support for custom commands', status: 'shipped', quarter: 'q4-2025', version: 'v1.197.0', docs: '/cli/configuration/auth/identities', changelog: 'authentication-for-workflows-and-custom-commands', description: 'Custom commands can leverage Atmos authentication for cloud provider access.', benefits: 'Custom scripts use the same identity management as Terraform. One auth system for everything.' },
        { label: 'Added `!random` YAML function', status: 'shipped', quarter: 'q4-2025', version: 'v1.199.0', docs: '/functions/yaml/random', changelog: 'random-yaml-function', description: 'Generate random values (strings, numbers, UUIDs) directly in stack configuration.', codeExample: 'suffix: !random string:8', benefits: 'Generate unique identifiers without external tools. Useful for resource naming and testing.' },
        { label: 'Added `!env` function with stack manifest support', status: 'shipped', quarter: 'q4-2025', version: 'v1.199.0', changelog: 'env-function-stack-manifest-support', description: 'The !env function now reads from stack manifest env sections, not just system environment.', benefits: 'Reference environment variables defined in stack configuration. Consistent behavior across local and CI.' },
        { label: 'Circular dependency detection for YAML functions', status: 'shipped', quarter: 'q4-2025', version: 'v1.196.0', changelog: 'yaml-function-circular-dependency-detection', description: 'Automatic detection of circular dependencies between YAML functions with clear error messages.', benefits: 'Catch configuration errors before deployment. Clear error messages show the dependency chain.' },
        { label: 'Deferred YAML function evaluation in merge', status: 'shipped', quarter: 'q4-2025', version: 'v1.201.0', changelog: 'deferred-yaml-functions-evaluation-in-merge', description: 'YAML functions are now evaluated after merge, allowing functions to reference inherited values.', benefits: 'Functions work correctly with inheritance. Reference values from base stacks in function arguments.' },
        { label: 'Boolean flags with defaults for custom commands', status: 'shipped', quarter: 'q4-2025', version: 'v1.201.0', changelog: 'custom-command-boolean-flags', description: 'Custom commands now support boolean flags with configurable default values.', benefits: 'Create intuitive CLI flags for custom commands. Defaults reduce repetitive typing.' },
    ...
    },
    {
      ...
      status: 'in-progress',
      milestones: [
        { label: 'Native GitHub OIDC enables automatic role assumptions', status: 'shipped', quarter: 'q3-2025', version: 'v1.196.0', docs: '/cli/configuration/auth/providers', changelog: 'introducing-atmos-auth', description: 'Secretless CI/CD with native OIDC—no AWS access keys stored in GitHub secrets.', category: 'featured', priority: 'high', benefits: 'No long-lived credentials to rotate. Security posture improves and audit burden decreases.' },
    ...
    },
    {
      ...
      status: 'in-progress',
      milestones: [
        { label: 'File-scoped locals', status: 'shipped', quarter: 'q4-2025', version: 'v1.202.0', docs: '/stacks/locals', changelog: 'file-scoped-locals', description: 'Define local variables at the file level for DRY configuration—familiar to Terragrunt users.', category: 'featured', priority: 'high', benefits: 'Avoid repetition with local variables. Terragrunt users can apply the same patterns they know.' },
        { label: 'Imperative stack names', status: 'shipped', quarter: 'q4-2025', version: 'v1.201.0', docs: '/stacks/name', changelog: 'stack-manifest-name-override', description: 'Simple stack naming with a direct name field instead of complex name templates.', category: 'featured', priority: 'high', benefits: 'Name stacks explicitly when you need control. No wrestling with name template edge cases.' },
        { label: 'File generation (`generate` blocks)', status: 'shipped', quarter: 'q4-2025', version: 'v1.200.0', description: 'Generate files like backend.tf and provider.tf from stack configuration with inheritance support.', category: 'featured', priority: 'high', benefits: 'Components are pure Terraform. Generated files keep configuration DRY without duplication.' },
        { label: 'Automatic backend provisioning', status: 'shipped', quarter: 'q4-2025', version: 'v1.200.0', docs: '/components/terraform/backend-provisioning', description: 'Provision the backend itself (S3 bucket with native state locking) for cold-start scenarios and one-shot Terraform deployments.', category: 'featured', priority: 'high', benefits: 'Bootstrap new environments without a chicken-and-egg problem. State backends are provisioned automatically.' },
        { label: 'AWS context YAML functions', status: 'shipped', quarter: 'q4-2025', version: 'v1.201.0', docs: '/functions/yaml/aws.account-id', changelog: 'aws-yaml-functions', description: 'Access AWS caller identity in stack configuration.', codeExample: 'account_id: !aws.account-id', category: 'featured', priority: 'high', benefits: 'Reference the current account, region, or identity in configuration dynamically.' },
        { label: '`plan --all` and `apply --all`', status: 'shipped', quarter: 'q4-2025', version: 'v1.200.0', docs: '/cli/commands/terraform/terraform-apply', description: 'Plan or apply all affected components in a single command with dependency ordering.', benefits: 'Deploy entire environments with one command. Dependencies are respected automatically.' },
        { label: 'Automatic source provisioning', status: 'shipped', quarter: 'q4-2025', version: 'v1.203.0', pr: 1877, changelog: 'terraform-source-provisioner', docs: '/cli/commands/terraform/source/source', description: 'Automatically fetch component sources without explicit vendoring—just reference and deploy.', category: 'featured', priority: 'high', benefits: 'Components are fetched on demand like Terraform modules. No vendor step to remember.' },
    ...
    },
    {
      ...
      status: 'in-progress',
      milestones: [
        { label: 'Test coverage from <20% to 74%', status: 'shipped', quarter: 'q1-2025', version: 'v1.150.0', description: 'Test coverage improved from less than 20% to 74% over the course of 2025.', category: 'featured', priority: 'high', benefits: 'Fewer regressions reach users. Changes can be made confidently knowing tests catch issues.' },
        { label: 'Weekly release cadence', status: 'shipped', quarter: 'q2-2025', version: 'v1.170.0', description: 'Predictable weekly releases every Tuesday with semantic versioning.', benefits: 'Plan upgrades around a known schedule. Features ship faster with smaller, incremental changes.' },
        { label: 'Claude Code + Claude skills', status: 'shipped', quarter: 'q3-2025', version: 'v1.195.0', description: 'AI-assisted development with custom Claude Code skills for Atmos-specific patterns.', category: 'featured', priority: 'high', benefits: 'Contributors get AI assistance that understands Atmos patterns and conventions.' },
        { label: 'CodeRabbit review integration', status: 'shipped', quarter: 'q3-2025', version: 'v1.190.0', description: 'Automated code reviews with AI-powered suggestions and security analysis.', benefits: 'PRs get immediate feedback. Security issues are caught before human review.' },
        { label: 'Nightly releases (RC candidates)', status: 'shipped', quarter: 'q4-2025', version: 'v1.200.0', description: 'Automated nightly builds for testing upcoming features before stable release.', category: 'featured', priority: 'high', benefits: 'Test new features before they ship. Report issues while fixes are still easy.' },
        { label: 'PR feature releases', status: 'shipped', quarter: 'q4-2025', version: 'v1.200.0', description: 'Every PR gets a pre-release version for easy testing before merge.', benefits: 'Test the exact changes in a PR without building locally. Validate fixes before they merge.' },
    ...
    },
    {
      ...
      status: 'shipped',
      milestones: [
        { label: 'Changelog introduction', status: 'shipped', quarter: 'q1-2025', version: 'v1.100.0', docs: '/changelog', description: 'Detailed changelogs for every release with feature announcements and migration guides.', category: 'featured', priority: 'high', benefits: 'Know exactly what changed in each release. Plan upgrades with full visibility into breaking changes.' },
        { label: 'Every `atmos.yaml` section documented', status: 'shipped', quarter: 'q3-2025', version: 'v1.190.0', docs: '/cli/configuration', description: 'Complete reference for every configuration option in atmos.yaml with examples and defaults.', category: 'featured', priority: 'high', benefits: 'Find the exact option you need without reading source code. Every setting has examples.' },
        { label: 'Every CLI command documented', status: 'shipped', quarter: 'q3-2025', version: 'v1.190.0', docs: '/cli/commands', description: 'Comprehensive documentation for every CLI command with usage examples and screenshots.', category: 'featured', priority: 'high', benefits: 'Learn any command from its documentation. Screenshots show exactly what to expect.' },
        { label: 'Cross-linking commands to configs', status: 'shipped', quarter: 'q3-2025', version: 'v1.190.0', docs: '/cli/configuration', description: 'Navigate from any command to its related configuration and vice versa.', category: 'featured', priority: 'high', benefits: 'Jump between commands and their configuration options. Understand how settings affect behavior.' },
        { label: 'Design patterns refresh', status: 'shipped', quarter: 'q3-2025', version: 'v1.190.0', docs: '/design-patterns', description: 'Updated design patterns with real-world examples for common infrastructure scenarios.', category: 'featured', priority: 'high', benefits: 'Apply proven patterns instead of inventing solutions. Learn from real-world examples.' },
        { label: 'Versioning strategy docs', status: 'shipped', quarter: 'q3-2025', version: 'v1.199.0', docs: '/design-patterns/version-management', changelog: 'comprehensive-version-management-documentation', description: 'Complete guide to version management including component versioning and upgrade strategies.', category: 'featured', priority: 'high', benefits: 'Upgrade components safely with a clear strategy. Understand version compatibility.' },
        { label: 'New learning section', status: 'shipped', quarter: 'q4-2025', version: 'v1.201.0', docs: '/learn/concepts-overview', changelog: 'documentation-reorganization', description: 'Step-by-step tutorials for getting started with Atmos from scratch.', category: 'featured', priority: 'high', benefits: 'New users have a clear path from zero to productive. Concepts build on each other logically.' },
        { label: 'Added migration guides', status: 'shipped', quarter: 'q4-2025', version: 'v1.201.0', docs: '/migration/terragrunt', changelog: 'migration-guides', description: 'Detailed guides for migrating from Terragrunt, Terraform workspaces, and native Terraform to Atmos.', category: 'featured', priority: 'high', benefits: 'Migrate from your current tool with step-by-step guidance. Understand the mapping between concepts.' },
        { label: 'Roadmap', status: 'shipped', quarter: 'q4-2025', version: 'v1.202.1', docs: '/roadmap', changelog: 'product-roadmap', description: 'Public roadmap showing past accomplishments and future plans.', category: 'featured', priority: 'high', benefits: 'See what\'s coming and plan accordingly. Understand the direction of the project.' },
      ],
      issues: [],
      prs: [],
    },
