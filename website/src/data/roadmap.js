  ...
  initiatives: [
    ...
    },
    {
      ...
      milestones: [
        ...
        { label: 'Custom hooks with built-in kinds for cost and security scanners', status: 'in-progress', quarter: 'q2-2026', pr: 2482, changelog: 'custom-hooks', description: 'Hook system extended with a `kind` discriminator and built-in kinds for infracost, checkov, trivy, and kics. Generic `kind: command` lets users wire any toolchain-pinned binary. Tool output streams through pkg/io and a single markdown summary renders in the terminal.', codeExample: 'hooks:\n  cost: { events: [after-terraform-plan], kind: infracost }\n  security: { events: [before-terraform-plan], kind: trivy }', benefits: 'Catch cost spikes and security misconfigurations before deploy. Zero-config use of common scanners; escape hatch for any custom tool. Same hook works locally and in CI.' },
        { label: 'Hook lifecycle events for `atmos terraform generate planfile`', status: 'shipped', quarter: 'q2-2026', pr: 2524, changelog: 'generate-planfile-hooks', description: '`atmos terraform generate planfile` now fires `before.terraform.generate.planfile` and `after.terraform.generate.planfile` hook events and accepts `--ci` (with `ATMOS_CI`/`CI` auto-detection), matching the lifecycle already available on plan/apply/deploy. User-defined hooks configured in stack manifests now fire for `generate planfile` runs.', codeExample: 'hooks:\n  notify: { events: [after-terraform-generate-planfile], kind: command, command: notify.sh }', benefits: 'Policy-engine pipelines that bridge `generate planfile` to Checkov/OPA/conftest get the same notification, store-write, and custom-command extension points that `plan` has had all along.' },
