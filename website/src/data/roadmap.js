  ...
  initiatives: [
    ...
    },
    {
      ...
      milestones: [
        ...
        { label: 'Hook lifecycle events for `atmos terraform generate planfile`', status: 'shipped', quarter: 'q2-2026', changelog: 'generate-planfile-hooks', description: '`atmos terraform generate planfile` now fires `before.terraform.generate.planfile` and `after.terraform.generate.planfile` hook events and accepts `--ci` (with `ATMOS_CI`/`CI` auto-detection), matching the lifecycle already available on plan/apply/deploy. User-defined hooks configured in stack manifests now fire for `generate planfile` runs.', codeExample: 'hooks:\n  notify: { events: [after-terraform-generate-planfile], kind: command, command: notify.sh }', benefits: 'Policy-engine pipelines that bridge `generate planfile` to Checkov/OPA/conftest get the same notification, store-write, and custom-command extension points that `plan` has had all along.' },
