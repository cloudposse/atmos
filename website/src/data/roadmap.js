...
export const roadmapConfig = {
  ...
  // HARD CAP: 6 items. Edit only when the user explicitly asks.
  // See .claude/agents/roadmap.md → "Featured Items: Curated, Max 6, Do Not Modify".
  featured: [
    {
      id: 'atmos-ai',
      icon: 'RiRobot2Line',
      ...
      quarter: 'q1-2026',
      docs: '/ai',
      changelog: 'introducing-atmos-ai',
      experimental: true,
    },
    {
      id: 'cloud-auth',
  ...
  initiatives: [
    {
      ...
      milestones: [
        ...
        { label: 'Zero-config AWS SSO identity management', status: 'shipped', quarter: 'q4-2025', changelog: 'aws-sso-identity-auto-provisioning', version: 'v1.200.0', description: 'Automatic SSO identity provisioning without manual configuration—Atmos detects and configures SSO settings.', benefits: 'Get started with AWS SSO instantly. No manual identity configuration required.', experimental: true, category: 'featured', priority: 'high' },
        ...
        { label: 'Automatic EKS kubeconfig tied to identities', status: 'shipped', quarter: 'q1-2026', pr: 2149, changelog: 'eks-kubeconfig-authentication', docs: '/tutorials/eks-kubeconfig-authentication', description: 'Automatic kubeconfig generation for EKS clusters using Atmos-managed AWS credentials. Integrations link clusters to identities for auto-provisioning on login.', benefits: 'No aws eks update-kubeconfig commands. Kubectl works immediately after Atmos auth login.', category: 'featured', priority: 'high' },
        { label: 'EKS exec credential plugin (atmos aws eks token)', status: 'shipped', quarter: 'q1-2026', pr: 2149, changelog: 'eks-kubeconfig-authentication', docs: '/cli/commands/aws/eks-token', description: 'Short-lived EKS token generation via STS pre-signed URLs without the AWS CLI. Automatically called by kubectl.', benefits: 'kubectl authentication is seamless. Tokens refresh automatically without manual intervention.' },
        ...
        { label: 'ECR Public registry authentication', status: 'shipped', quarter: 'q2-2026', pr: 2231, changelog: 'ecr-public-authentication', description: 'Authenticated pulls from public.ecr.aws to eliminate rate limits on public ECR images.', benefits: 'CI builds pulling BuildKit, binfmt, or other public ECR images no longer hit unauthenticated rate limits.' },
        ...
        { label: 'Required identities for multi-account components', status: 'shipped', quarter: 'q1-2026', changelog: 'auth-required-concurrent-identities', description: 'Identities marked required: true are automatically authenticated without prompting. The default identity stays primary; required non-default identities are authenticated as secondary and written to the shared credentials file.', benefits: 'Multi-account Terraform patterns with provider aliases work in CI with OIDC. No manual credential setup for hub-spoke networking or cross-account deployments.' },
        ...
        { label: 'Ambient credential support (IRSA/IMDS/ECS)', status: 'shipped', quarter: 'q1-2026', pr: 2254, changelog: 'ambient-credential-support', prd: 'ambient-identity', description: 'Two new identity kinds: generic `ambient` passthrough and AWS-specific `aws/ambient` that resolves credentials from IRSA, EC2 instance profiles, and ECS task roles via the default credential chain.', benefits: 'Run Atmos natively in EKS, EC2, and ECS without additional auth configuration. Chain with assume-role for cross-account access.', category: 'featured', priority: 'high' },
        { label: 'Interactive profile suggestion for missing identities', status: 'shipped', quarter: 'q2-2026', pr: 2333, changelog: 'interactive-profile-suggestion', prd: 'interactive-profile-suggestion', description: 'When an identity is not in the loaded config, Atmos now checks whether another profile defines it and either prompts to switch (TTY) or enriches the error with hints (non-TTY). A generic fallback extends the behavior to `auth login`, `auth exec`, `auth shell`, `auth env`, `auth console`, and `auth whoami` when the base config has no usable auth config at all. Also adds `profiles.default` in atmos.yaml to pin a default profile.', benefits: 'No more grepping for which profile owns which identity — or which profile holds the only auth config. The first error names the candidate profile(s) and the exact re-run command.' },
        { label: 'Atmos Pro STS — GitHub token broker for CI', status: 'shipped', quarter: 'q2-2026', changelog: 'atmos-pro-github-sts', description: 'Two new `atmos auth` kinds: `kind: atmos/pro` federates GitHub Actions OIDC into an Atmos Pro session, and `kind: github/sts` mints short-lived, least-privilege, scoped GitHub App installation tokens. Tokens are injected as per-owner GIT_CONFIG_* git URL rewrites so `atmos vendor pull`, Atmos `source:` components, and `terraform init` of private `git::https://…` modules work with zero .tf changes. Tokens are revoked at command-end and on logout. Also adds ATMOS_PRO_GITHUB_TOKEN for Atmos-native go-getter operations.', benefits: 'Zero-secret CI access to private Terraform modules and components. Just-in-time GitHub tokens are scoped to the repos your pipeline actually needs, revoked automatically, and never stored in CI secrets.' },
        { label: 'Shared OIDC session across aws/iam-identity-center providers (SSO session sharing)', status: 'shipped', quarter: 'q2-2026', pr: 2553, changelog: 'aws-sso-session-sharing', prd: 'aws-sso-session-support', description: 'Multiple aws/iam-identity-center providers pointing at the same SSO portal (same start_url + region) now share a single OIDC token. One browser flow unlocks every provider. Refresh-token support added for silent renewal within the portal session window — no re-prompt every hour. Zero atmos.yaml config changes required.', benefits: 'Teams with multi-account SSO configurations log in once per portal instead of once per provider. Long-running sessions refresh silently without interruption.' },
      ],
      issues: [],
      prs: [
    ...
    },
    {
      ...
      milestones: [
        ...
        { label: 'Git repository metadata YAML functions (`!git.repository`, `!git.owner`, `!git.name`, `!git.host`, `!git.url`)', status: 'shipped', quarter: 'q2-2026', pr: 2558, changelog: 'git-repository-yaml-functions', docs: '/functions/yaml/git.repository', description: 'New YAML functions expose Git repository metadata derived from the `origin` remote, working across GitHub, GitLab, Bitbucket, and Azure DevOps. `!git.repository` returns the `<owner>/<name>` slug (matching GitHub\'s `GITHUB_REPOSITORY`), with `!git.owner`, `!git.name`, `!git.host`, and `!git.url` for the individual parts. All support a fallback value.', benefits: 'Tag resources or build paths with the repository slug without shelling out through `!exec` and `sed`. Portable across Git hosts and CI environments.' },
        { label: '`atmos.Resolve` template function for composing YAML functions', status: 'shipped', quarter: 'q2-2026', pr: 2558, changelog: 'atmos-resolve-template-function', docs: '/functions/template/atmos.Resolve', description: 'New template function evaluates any Atmos YAML function (e.g. `!git.repository`, `!exec`, `!store`) at template-render time, so its result can be composed with other strings and template variables in a single value — for example prefixing a `workspace_key_prefix` with the repository slug.', benefits: 'Compose YAML-function results with paths and templates in one value, which a bare YAML tag cannot do. Removes the need for `!exec` shell concatenation.' },
      ],
      issues: [],
      prs: [
    ...
    },
    {
      ...
      milestones: [
        ...
        { label: '`--process-templates` / `--process-functions` flags across every `atmos list` command', status: 'shipped', quarter: 'q2-2026', pr: 2363, changelog: 'list-process-flags', description: 'Five list subcommands (instances, components, metadata, sources, stacks) now accept --process-templates and --process-functions, matching the flag surface of describe affected / describe stacks. Go template functions like atmos.Component(...) and YAML functions like !terraform.state are finally controlled by separate, correctly-named dials.', codeExample: 'atmos list instances --upload --process-functions=false', benefits: 'Run list commands with or without tofu/terraform on $PATH. Same naming, defaults, and env-var bindings (ATMOS_PROCESS_TEMPLATES / ATMOS_PROCESS_FUNCTIONS) across the list and describe command families. Includes a fix that makes atmos list instances --upload work end-to-end in CI against stacks that use atmos.Component(...) in templates.' },
        { label: '`--skip` flag across every `atmos list` command for selective YAML function bypass', status: 'shipped', quarter: 'q2-2026', pr: 2413, changelog: 'list-skip-flag', description: 'Five list subcommands (instances, components, metadata, sources, stacks) now accept --skip <yaml-function>, matching the flag already exposed by describe affected / describe component / describe stacks. Skip a single function (e.g. terraform.state) while leaving the rest of YAML function processing — including !template — enabled.', codeExample: 'atmos list instances --upload --skip terraform.state', benefits: 'Atmos Pro uploads work end-to-end in CI against stacks that mix backend-resolving functions (!terraform.state) with computed booleans (!template). --process-functions=false is no longer the only escape hatch, so settings.pro.enabled stays a real boolean. ATMOS_SKIP env var works across the family; ATMOS_AFFECTED_SKIP remains as a back-compat alias for list affected.' },
        { label: '`atmos list instances` `--stack`, `--filter`, `--query` now work as documented', status: 'shipped', quarter: 'q2-2026', pr: 2413, changelog: 'list-instances-stack-filter-query', description: 'Three documented `atmos list instances` flags were silently ignored: `--stack` returned every component in every stack, `--filter` was a TODO stub, and `--query` was set on the options struct but never read. They now do what the docs say — `--stack` is a path.Match glob, `--filter` is a YQ predicate (rows where the expression is truthy are kept), and `--query` projects each row via YQ (scalars land in a `value` column; maps become row keys).', codeExample: "atmos list instances --stack 'tenant1-*' --filter '.component == \"vpc\"'", benefits: 'CI matrices and Atmos Pro uploads targeting a single stack pattern stop scanning every stack. Documented behavior matches actual behavior. Also closes a latent ENV-precedence gap: ATMOS_LIST_FORMAT and ATMOS_UPLOAD are now honored.' },
      ],
      issues: [],
      prs: [
    ...
    },
    {
      ...
      milestones: [
        ...
        { label: 'New interactive input types for workflow steps (input, confirm, choose, filter, file, write)', status: 'shipped', quarter: 'q4-2025', docs: '/cli/configuration/workflows', pr: 1899, changelog: 'custom-commands-step-types', description: 'Interactive step types for user input (input, confirm), selection (choose, filter, file), and output (write). Typed inputs with validation—strings, numbers, booleans, and selections.', category: 'featured', priority: 'high', benefits: 'Inputs are validated before execution starts. Bad values are caught early, not mid-deployment.' },
    ...
    },
    {
      ...
      milestones: [
        ...
        { label: 'SSE-C encryption support for `!terraform.state`', status: 'shipped', quarter: 'q1-2026', pr: 2060, changelog: 'sse-c-support', docs: '/functions/yaml/terraform.state', description: 'Read Terraform state from S3 buckets encrypted with customer-provided keys (SSE-C).', benefits: 'Access SSE-C encrypted state files without workarounds. Compatible with OpenTofu and Terraform SSE-C configuration.' },
        ...
        { label: 'Dotenv files with `!include`', status: 'shipped', quarter: 'q2-2026', pr: 1930, docs: '/cli/configuration/env', changelog: 'dotenv-include-support', description: 'Dotenv filenames (`.env`, `.env.*`, and exact `*.env`) included with `!include` now parse as environment maps for direct and YAML merge-key usage in CLI and stack env sections. `ATMOS_`-prefixed keys in the CLI `env` section are promoted into Atmos\' own environment before profile resolution, so a project can pin `ATMOS_PROFILE` (and other `ATMOS_*` settings) in a committed `.env`.', codeExample: 'env: !include .env  # .env: ATMOS_PROFILE=dev', benefits: 'Keep local and environment-specific settings in dotenv files without implicit loading. Pin a profile per project so the whole team and CI share the same defaults — an exported env var or `--profile` still wins.' },
        ...
        { label: 'Custom hooks with built-in kinds for cost and security scanners', status: 'shipped', quarter: 'q2-2026', pr: 2482, changelog: 'custom-hooks', description: 'Hook system extended with a `kind` discriminator and built-in kinds for infracost, checkov, trivy, and kics. Generic `kind: command` lets users wire any toolchain-pinned binary. Tool output streams through pkg/io and a single markdown summary renders in the terminal.', codeExample: 'hooks:\n  cost: { events: [after-terraform-plan], kind: infracost }\n  security: { events: [before-terraform-plan], kind: trivy }', benefits: 'Catch cost spikes and security misconfigurations before deploy. Zero-config use of common scanners; escape hatch for any custom tool. Same hook works locally and in CI.' },
        { label: '`before-terraform-init` and `after-terraform-init` lifecycle hook events; `--skip-hooks` fix for before-* hooks', status: 'shipped', quarter: 'q2-2026', pr: 2574, changelog: 'terraform-init-hooks-and-skip-hooks-fix', description: '`after-terraform-init` is a new hook event; `before-terraform-init` was documented but never fired for user hooks — now it does. Also fixes `--skip-hooks` being ignored for before-* events (plan/apply/deploy), so the flag now suppresses all hook phases consistently.', benefits: 'Hook authors can run setup or validation steps around `terraform init`. `--skip-hooks` now reliably bypasses every hook phase, not just after-* events.' },
        ...
        { label: 'Terraform Cloud backend support for !terraform.output', status: 'shipped', quarter: 'q1-2026', changelog: 'terraform-cloud-output-function', pr: 2058, description: 'Fixed backend.tf.json generation for Terraform Cloud/Enterprise to use proper terraform.cloud structure instead of terraform.backend.cloud', benefits: 'Enable !terraform.output YAML function to work seamlessly with Terraform Cloud and Terraform Enterprise backends' },
        ...
        { label: 'Custom component types with registry', status: 'shipped', quarter: 'q4-2025', pr: 1904, docs: '/cli/configuration/commands', changelog: 'custom-component-types', description: 'Register custom component types that integrate seamlessly with Atmos commands and workflows.', benefits: 'Your custom commands get the same tab completion, list commands, and workflow integration as built-ins.' },
        { label: 'Native Ansible component support', status: 'shipped', quarter: 'q1-2026', pr: 2042, docs: '/cli/commands/ansible/usage', changelog: 'ansible-component-support', description: 'Ansible as a first-class component type with full stack processor support, variable inheritance, and playbook execution.', benefits: 'Unified orchestration of infrastructure provisioning (Terraform) and configuration management (Ansible) from the same stack manifests.' },
        { label: 'Semantic type completion', status: 'shipped', quarter: 'q4-2025', pr: 1904, docs: '/cli/commands/completion', changelog: 'custom-component-types', description: 'Context-aware shell completion that understands your component types and suggests valid options.', benefits: 'Tab completion suggests values that make sense for each argument, not just filenames.' },
        ...
        { label: 'Git context YAML functions', status: 'shipped', quarter: 'q2-2026', pr: 2537, changelog: 'git-yaml-functions', description: 'Added !git.root, !git.sha, !git.branch, and !git.ref for resolving repository metadata during config and stack processing.', codeExample: 'version: !git.ref', benefits: 'Pin dev source versions to the current commit while keeping prod refs explicit. !git.root aliases !repo-root, and all functions support fallbacks for non-Git contexts.' },
        ...
        { label: 'Remote stack imports', status: 'shipped', quarter: 'q1-2026', pr: 2037, changelog: 'remote-stack-imports', description: 'Import stack configurations from remote URLs (HTTP, Git, S3, GCS) using go-getter. Share configurations across repositories without vendoring.', benefits: 'Maintain organization-wide defaults in a central repository. Reference shared configs directly from their source with version pinning.' },
        { label: 'Templated import paths', status: 'shipped', quarter: 'q2-2026', pr: 2554, docs: '/stacks/imports#referencing-earlier-imports-in-import-paths', changelog: 'templated-import-paths', description: 'Import paths (including a remote import\'s Git ?ref=) can reference settings/vars defined by earlier imports in the same manifest.', codeExample: 'import:\n  - ./_defaults\n  - "github.com/org/repo//stacks/catalog/foo?ref={{ .settings.context.deployment_repo_version }}"', benefits: 'Pin a versioned prod catalog and component source from a single variable—isolate prod from dev changes while keeping both in one repo.' },
        { label: 'Remote import source caching (clone once + TTL)', status: 'shipped', quarter: 'q2-2026', pr: 2571, changelog: 'faster-remote-stack-imports', docs: '/stacks/imports#caching-remote-imports', description: 'Each unique remote Git repository is cloned at most once per Atmos invocation; all subdir imports from the same repo are resolved from that shared clone. An opt-in per-import ttl: (or global imports.ttl default in atmos.yaml) reuses the cloned source across invocations until it expires, enabling a warm CI cache (e.g. actions/cache of ~/.cache/atmos/stack-imports) to skip re-cloning entirely. Mutable refs like ?ref=main stay fresh by default—re-cloned once per run unless a ttl is set. Builds on the existing source-provisioning TTL/duration mechanism.', benefits: 'Large stack catalogs with many remote imports see dramatically faster run times—one clone per repo instead of one per import. CI pipelines with a cache layer can skip the network entirely on warm runs. Mutable refs stay correct by default, and ttl is opt-in so there are no surprises.' },
        ...
        { label: 'Added `!unset` YAML function', status: 'shipped', quarter: 'q2-2026', pr: 1521, changelog: 'unset-yaml-function', docs: '/functions/yaml/unset', description: 'Delete keys from inherited configuration for clean overrides.', codeExample: 'deprecated_setting: !unset', benefits: 'Remove inherited values without workarounds. Clean up configuration by exception.' },
        { label: 'Added `!append` YAML function', status: 'shipped', quarter: 'q2-2026', pr: 1513, changelog: 'append-yaml-function', docs: '/functions/yaml/append', description: 'Append items to inherited lists instead of replacing them.', codeExample: 'extra_tags: !append [new-tag]', benefits: 'Add to inherited lists without copying the full list. Simpler inheritance for array values.' },
    ...
    },
    {
      ...
      milestones: [
        ...
        { label: 'Simplified GitHub Actions with native CI mode', status: 'in-progress', quarter: 'q1-2026', pr: 1891, docs: '/ci', description: 'CLI auto-detects CI environments and generates rich job summaries with resource badges, collapsible diffs, and status checks. Replaces separate actions like github-action-atmos-terraform-plan.', category: 'featured', priority: 'high', benefits: 'No wrapper scripts needed. Same command works locally and in CI. Ships with GitHub Actions support; provider architecture enables future GitLab and Azure DevOps support.' },
        ...
        { label: 'Matrix output auto-routes to $GITHUB_OUTPUT in CI', status: 'shipped', quarter: 'q2-2026', docs: '/ci#deploy-affected', changelog: 'describe-affected-matrix-auto-output', description: '`atmos describe affected --format=matrix` now writes to $GITHUB_OUTPUT automatically when ci.enabled is true, matching the existing behavior of `atmos list instances --format=matrix`. The explicit --output-file=$GITHUB_OUTPUT flag is no longer required in workflow YAML.', benefits: 'Simpler workflow YAML. Both matrix-producing commands now behave the same way, making copy-pasted patterns work consistently.' },
        { label: 'describe affected evaluates all provisioned sections', status: 'shipped', quarter: 'q2-2026', docs: '/cli/commands/describe/affected#evaluated-sections', changelog: 'describe-affected-all-sections', description: 'atmos describe affected now compares every provisioned component section between refs — providers, required_providers (provider versions), hooks, generate, backend, auth, command, and more — including scalar sections. A new describe.affected.sections setting lets you override the evaluated set.', benefits: 'No more missed components in CI when only providers, hooks, or provider versions change. Affected detection now matches what Atmos actually provisions.' },
        ...
        { label: 'Component-level retry for transient Terraform errors', status: 'shipped', quarter: 'q2-2026', pr: 2431, docs: '/stacks/components/terraform/retry', changelog: 'terraform-component-retry', description: 'Per-component `retry:` block with regex-driven `conditions:` matched against captured terraform output. Wraps init, workspace, and main subprocess invocations independently with exponential/linear/constant backoff. Inherits across abstract components.', benefits: 'Unattended pipelines recover from 502 Bad Gateways, registry rate limits, and TLS handshake timeouts without manual re-runs. Real Terraform failures still fail fast because retry is opt-in per-pattern.' },
        ...
        { label: 'Atmos Git — foundational capability for GitOps enablement', status: 'shipped', quarter: 'q2-2026', changelog: 'atmos-gitops', pr: 2597, description: 'Top-level git.repositories config for managed repos with auth identities, signing, and automatic XDG workdirs. New atmos git command group (init/clone/pull/status/diff/commit/push/list --all, with -- native git arg passthrough) replaces native CI checkout via GitHub STS tokens. A git hook kind publishes generated artifacts on lifecycle events (after.terraform.apply) to the current or a managed deployment repo. Local .git/hooks shims via atmos git hooks install/run. Enables GitOps pipelines where generated artifacts are committed to a source-of-truth repository consumed by Argo CD, Flux, or CI.', benefits: 'Git becomes a first-class Atmos primitive. CI pipelines replace fragile git checkout steps with atmos git clone. Generated artifacts are committed automatically after apply without write tokens in workflow YAML.' },
        { label: 'CI build cache via GitHub Actions Cache', status: 'shipped', quarter: 'q2-2026', pr: 2579, changelog: 'ci-build-cache', description: 'New `ci.cache` block restores the Atmos cache root (~/.cache/atmos — toolchain binaries, vendored components, remote import clones, provider/plugin caches) at startup and saves it at exit using the same store actions/cache uses (GitHub Actions Cache Service v2). Runs automatically (auto: restore|save|both) or via explicit `atmos ci cache restore|save|list|delete` subcommands; key is derived from toolchain.lock.yaml + OS/arch.', benefits: 'Hundreds-to-thousands of Terraform jobs per hour stop re-downloading tools, providers, and modules from upstream on every run — faster executions, fewer transient and rate-limit failures, and a smaller supply-chain attack surface. No hand-wired actions/cache key/path logic to maintain.' },
      ],
      issues: [],
      prs: [
        { number: 1891, title: 'Native CI Integration with Summary Templates and Terraform Command Registry' },
    ...
    },
    {
      ...
      milestones: [
        ...
        { label: 'Automatic backend provisioning', status: 'shipped', quarter: 'q4-2025', docs: '/stacks/components/provision/backend', description: 'Provision the backend itself (S3 bucket with native state locking) for cold-start scenarios and one-shot Terraform deployments.', category: 'featured', priority: 'high', benefits: 'Bootstrap new environments without a chicken-and-egg problem. State backends are provisioned automatically.', experimental: true },
        ...
        { label: '`plan --all` and `apply --all`', status: 'shipped', quarter: 'q4-2025', docs: '/cli/commands/terraform/terraform-apply', changelog: 'terraform-all-dependency-order', description: 'Plan or apply all components in a stack with automatic dependency ordering using topological sort. Destroy runs in reverse order. Circular dependencies are detected.', category: 'featured', priority: 'high', benefits: 'Deploy entire environments with one command. Dependencies are respected automatically.', demoId: 'terraform-plan' },
        ...
        { label: 'Graph-backed Terraform bulk execution', status: 'shipped', quarter: 'q2-2026', pr: 2466, docs: '/cli/commands/terraform/plan', changelog: 'terraform-dag-concurrency', description: 'Route Terraform --all, --components, and --query runs through a scheduler-backed dependency graph built from dependencies.components, with settings.depends_on fallback and reverse ordering for destroy.', category: 'featured', priority: 'high', benefits: 'Bulk Terraform commands run in predictable dependency order, and independent plan nodes can opt into bounded concurrency for faster feedback.' },
    ...
    },
    {
      ...
      milestones: [
        ...
        { label: 'Branch and tag installation via --use-version (ref:)', status: 'shipped', quarter: 'q2-2026', pr: 2569, changelog: 'use-version-ref', description: 'Install the latest build of any branch or tag with --use-version=ref:<name> (e.g., ref:main, ref:v1.199.0, ref:heads/main). The ref is resolved to its current commit on every run, so a mutable branch like main always tracks the latest build, while the SHA-keyed cache avoids reinstalling when the ref has not moved.', benefits: 'Pin a moving target once (ref:main) instead of chasing a new sha: after every merge. Test unreleased fixes on a branch with one command.' },
    ...
    },
    {
      ...
      milestones: [
        ...
        { label: 'Raw Markdown pages and "Copy Markdown" button', status: 'shipped', quarter: 'q2-2026', changelog: 'copy-markdown-button', pr: 2503, description: 'Every doc page is now served as raw Markdown at <url>.md. A "Copy Markdown" / "View Markdown" button appears on each page. A <link rel="alternate" type="text/markdown"> tag enables crawler discovery. The MDX→Markdown normalizer also improves llms-full.txt.', benefits: 'LLMs, agents, and tools that consume Markdown documentation get clean, normalized content. Users can copy page Markdown in one click for use in AI prompts or issue reports.' },
      ],
      issues: [],
      prs: [],
    },
    {
      ...
      milestones: [
        ...
        { label: 'Session management with SQLite storage', status: 'shipped', quarter: 'q1-2026', docs: '/ai/sessions', changelog: 'introducing-atmos-ai', description: 'SQLite-backed persistent sessions with full CRUD operations, auto-compact, and retention policies.', benefits: 'Resume conversations days or weeks later with full context. Sessions are provider-aware.' },
        { label: 'Tool execution with permission controls', status: 'shipped', quarter: 'q1-2026', docs: '/ai/tools', changelog: 'introducing-atmos-ai', description: 'AI can execute read-only Atmos commands with three-tier permission system (allowed/restricted/blocked).', benefits: 'AI can inspect your infrastructure automatically. Granular control over what tools AI can use.' },
        { label: 'Persistent permission cache', status: 'shipped', quarter: 'q1-2026', docs: '/ai/tools', changelog: 'introducing-atmos-ai', description: 'Permission decisions persist across sessions in .atmos/ai.settings.local.json.', benefits: '80%+ reduction in permission prompts. Decisions are remembered across all AI sessions.' },
        { label: 'Project instructions (ATMOS.md)', status: 'shipped', quarter: 'q1-2026', docs: '/ai/instructions', changelog: 'introducing-atmos-ai', description: 'Persistent project instructions via ATMOS.md file that AI references across all sessions.', benefits: 'AI knows your project conventions. Reduce repetitive context loading.' },
        { label: 'Marketplace skills (21+ skills)', status: 'shipped', quarter: 'q1-2026', docs: '/ai/skills', changelog: 'introducing-atmos-ai', description: '21+ specialized marketplace skills including atmos-terraform, atmos-stacks, atmos-components, atmos-validation, atmos-config, and more, following the Agent Skills open standard.', benefits: 'Task-specific AI expertise. Each skill has focused tools and system prompts.' },
        { label: 'MCP server integration', status: 'shipped', quarter: 'q1-2026', docs: '/ai/mcp-server', changelog: 'introducing-atmos-ai', description: 'Model Context Protocol server for Claude Desktop, VSCode, and other MCP clients.', benefits: 'Use Atmos tools from any MCP-compatible client. Standardized AI-to-tool communication.' },
        { label: 'LSP client for YAML/Terraform validation', status: 'shipped', quarter: 'q1-2026', docs: '/lsp/lsp-client', changelog: 'introducing-atmos-ai', description: 'Language Server Protocol client integrating yaml-language-server and terraform-ls with AI.', benefits: 'IDE-quality validation in AI chat. Precise error locations with suggested fixes.' },
        { label: 'LSP server for Atmos-specific features', status: 'shipped', quarter: 'q1-2026', docs: '/lsp/lsp-server', changelog: 'introducing-atmos-ai', description: 'Native LSP server with autocomplete, hover documentation, and validation for Atmos configurations.', benefits: 'IDE integration for any LSP-compatible editor. 13+ editors supported.' },
        { label: 'Token caching for cost savings', status: 'shipped', quarter: 'q1-2026', docs: '/ai/providers', changelog: 'introducing-atmos-ai', description: 'Prompt caching saves up to 90% on API costs by reusing system prompts and project instructions.', benefits: 'Significant cost reduction for heavy AI usage. Automatic caching for most providers.' },
        { label: 'Non-interactive execution (`atmos ai exec`)', status: 'shipped', quarter: 'q1-2026', docs: '/cli/commands/ai/exec', changelog: 'introducing-atmos-ai', description: 'Execute AI prompts non-interactively with JSON output for CI/CD and scripting.', benefits: 'Automate AI-powered analysis in pipelines. Standard exit codes for error handling.' },
        { label: 'Session export/import', status: 'shipped', quarter: 'q1-2026', docs: '/cli/commands/ai/sessions', changelog: 'introducing-atmos-ai', description: 'Export sessions to JSON/YAML/Markdown for team collaboration and backup.', benefits: 'Share troubleshooting sessions with your team. Archive incident resolutions.' },
    ...
    },
    {
      ...
      tagline: 'Connect Atmos AI to the MCP ecosystem',
      description:
        'Model Context Protocol (MCP) servers expose cloud provider APIs, documentation, and tooling as AI-callable tools. This initiative connects Atmos to external MCP servers—AWS, GCP, Azure, and community-built servers—making their capabilities available inside atmos ai chat, ask, and exec without custom code.',
      progress: 100,
      status: 'shipped',
      milestones: [
        { label: 'Core MCP client infrastructure', status: 'shipped', quarter: 'q1-2026', pr: 2267, changelog: 'mcp-server-integrations', description: 'Session lifecycle management, multi-session manager, and config parsing with validation. Session.go, manager.go, and config.go in pkg/mcp/client/ handle server startup, stdio transport, and graceful shutdown.', benefits: 'Connect to any stdio-based MCP server from atmos.yaml configuration. Sessions are managed automatically.' },
        { label: 'MCP tool bridge and AI integration', status: 'shipped', quarter: 'q1-2026', pr: 2267, changelog: 'mcp-server-integrations', description: 'BridgedTool implements the tools.Tool interface, enabling MCP server tools to appear alongside native Atmos AI tools. Includes tool name sanitization for AI provider compatibility and registration in the AI tool registry.', benefits: 'MCP tools are indistinguishable from native tools in AI conversations. No special syntax needed to use them.' },
        { label: 'MCP management commands', status: 'shipped', quarter: 'q1-2026', pr: 2267, changelog: 'mcp-server-integrations', description: 'Five new CLI commands: `atmos mcp list`, `atmos mcp tools`, `atmos mcp test`, `atmos mcp status`, and `atmos mcp restart`. All commands use themed table output consistent with other Atmos commands.', benefits: 'Inspect, test, and manage MCP server connections from the CLI. Diagnose connectivity issues without editing config files.' },
        { label: 'Auth and toolchain integration', status: 'shipped', quarter: 'q1-2026', pr: 2267, changelog: 'mcp-server-integrations', description: 'The `auth_identity` field injects Atmos-managed credentials into MCP server environment. Toolchain resolution handles `uvx` and `npx` prerequisites. YAML functions are evaluated in env values for dynamic configuration.', benefits: 'MCP servers receive the correct credentials automatically. No manual credential setup for AWS or GCP MCP servers.' },
        { label: 'IDE integration via export', status: 'shipped', quarter: 'q1-2026', pr: 2267, changelog: 'mcp-server-integrations', description: '`atmos mcp export` produces a `.mcp.json` file compatible with Claude Code, Cursor, and other MCP-aware IDEs. Auth-wrapped servers use `atmos auth exec` to inject credentials into IDE-launched server processes.', benefits: 'IDEs get the same MCP servers configured in atmos.yaml without duplicate configuration. One source of truth for all MCP tooling.' },
        { label: 'Smart tool routing with --mcp flag', status: 'shipped', quarter: 'q1-2026', pr: 2267, changelog: 'mcp-server-integrations', description: 'Two-pass server selection uses the configured AI provider to pick the best MCP server for each tool call. The `--mcp` flag on ask/chat/exec and `ATMOS_AI_MCP` env var control server selection. Graceful fallback ensures commands work even when MCP servers are unavailable.', benefits: 'AI automatically picks the right MCP server for each task. Override server selection when needed.' },
        { label: 'Human-readable tool names in AI output', status: 'shipped', quarter: 'q1-2026', pr: 2267, changelog: 'mcp-server-integrations', description: 'BridgedToolInfo interface exposes display names for MCP tools. MarkdownFormatter shows tool executions in `atmos ai ask` output using clean names instead of raw MCP identifiers. cleanToolName normalizes namespace separators.', benefits: 'AI output clearly shows which MCP tool was called with readable names. Easier to understand what actions AI took.' },
        { label: 'MCP example with AWS servers', status: 'shipped', quarter: 'q1-2026', pr: 2267, changelog: 'mcp-server-integrations', description: 'Complete example in examples/mcp/ demonstrating 8 AWS MCP servers: billing, pricing, security, IAM, CloudTrail, documentation, knowledge base, and AWS API. Includes auth, toolchain, and AI configuration.', benefits: 'Start using MCP with AWS in minutes. Example covers the most common AWS use cases with working configuration.' },
      ],
      issues: [],
      prs: [],
    },
    ...
    },
    {
      ...
      milestones: [
        ...
        { label: 'SARIF and OCSF output for security findings', status: 'shipped', quarter: 'q2-2026', pr: 2483, changelog: 'aws-security-sarif', description: 'New `--format=sarif` emits SARIF 2.1.0 documents for code scanning, while `--format=ocsf` emits OCSF 1.4.0 Detection Finding events for SIEM and security data lake ingestion. Findings keep their Atmos context including stack, component, and remediation metadata.', benefits: 'Atmos security findings plug directly into code-scanning and security analytics platforms — no translation layer required.' },
