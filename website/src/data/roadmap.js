  ...
  initiatives: [
    ...
    },
    {
      ...
      milestones: [
        ...
        { label: 'Per-step retry with backoff for custom command steps', status: 'shipped', quarter: 'q2-2026', changelog: 'custom-command-step-retries', pr: 2583, description: 'Custom command steps in atmos.yaml now support a per-step retry: block (max_attempts, backoff_strategy, initial_delay, max_delay, multiplier, random_jitter, max_elapsed_time) using the same RetryConfig as workflow steps. A shell or atmos step that fails with a transient error is automatically retried with configurable backoff. Steps without a retry block are unchanged.', benefits: 'Unattended pipelines survive transient registry 5xx errors, network blips, and rate limits without manual re-runs. Retry is opt-in per-step, so real failures still fail fast.' },
