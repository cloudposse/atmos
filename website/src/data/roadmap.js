  ...
  initiatives: [
    ...
    },
    {
      ...
      milestones: [
        ...
        { label: 'Toolchain support for Aqua `github_archive` package type', status: 'shipped', quarter: 'q2-2026', pr: 2416, changelog: 'toolchain-aqua-archive-and-content', description: 'The Aqua-compatible toolchain registry now resolves `github_archive` package definitions, downloading the GitHub-generated source tarball at `archive/refs/tags/{version}.tar.gz`. Matches upstream aquaproj/aqua semantics: `tar.gz` is hardcoded, and `asset`/`url`/`format` fields are intentionally ignored.', benefits: 'Tools that ship as repository archives (adr-tools, tfenv, tgswitch, and many single-script projects) now install through `atmos toolchain install` using their existing Aqua registry definitions. No registry workarounds required.' },
        { label: 'Toolchain support for Aqua `github_content` package type', status: 'shipped', quarter: 'q2-2026', pr: 2416, changelog: 'toolchain-aqua-archive-and-content', description: 'The Aqua-compatible toolchain registry now resolves `github_content` package definitions, downloading a single file from a repo at a tag via `raw.githubusercontent.com/{owner}/{repo}/{version}/{path}`. Matches upstream aquaproj/aqua semantics: required `path` field, with `asset`/`url`/`format` fields ignored.', benefits: 'Tools that ship as a single raw file in a repo (kubens, kubectx, and many one-file scripts) now install through `atmos toolchain install` using their existing Aqua registry definitions. No registry workarounds required.' },
      ],
      issues: [],
      prs: [
