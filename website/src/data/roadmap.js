...
export const roadmapConfig = {
  ...
  initiatives: [
    ...
    },
    {
      ...
      status: 'in-progress',
      milestones: [
        { label: 'File-scoped locals', status: 'shipped', quarter: 'q4-2025', docs: '/stacks/locals', changelog: 'file-scoped-locals', description: 'Define local variables at the file level for DRY configuration—familiar to Terragrunt users.', benefits: 'Avoid repetition with local variables. Terragrunt users can apply the same patterns they know.' },
        { label: 'Imperative stack names', status: 'shipped', quarter: 'q4-2025', docs: '/stacks/name', changelog: 'stack-manifest-name-override', description: 'Simple stack naming with a direct name field instead of complex name templates.', benefits: 'Name stacks explicitly when you need control. No wrestling with name template edge cases.' },
        { label: 'File generation (`generate` blocks)', status: 'shipped', quarter: 'q4-2025', description: 'Generate files like backend.tf and provider.tf from stack configuration with inheritance support.', benefits: 'Components are pure Terraform. Generated files keep configuration DRY without duplication.' },
        { label: 'Automatic backend provisioning', status: 'shipped', quarter: 'q4-2025', docs: '/components/terraform/backend-provisioning', description: 'Provision the backend itself (S3 bucket with native state locking) for cold-start scenarios and one-shot Terraform deployments.', benefits: 'Bootstrap new environments without a chicken-and-egg problem. State backends are provisioned automatically.' },
        { label: 'AWS context YAML functions', status: 'shipped', quarter: 'q4-2025', docs: '/functions/yaml/aws.account-id', description: 'Access AWS caller identity in stack configuration.', codeExample: 'account_id: !aws.account-id', benefits: 'Reference the current account, region, or identity in configuration dynamically.' },
        ...
        { label: 'Automatic source provisioning', status: 'planned', quarter: 'q1-2026', pr: 1877, description: 'Automatically fetch component sources without explicit vendoring—just reference and deploy.', benefits: 'Components are fetched on demand like Terraform modules. No vendor step to remember.' },
        { label: 'Concurrent component provisioning', status: 'planned', quarter: 'q1-2026', description: 'Deploy multiple components in parallel with dependency-aware orchestration.', benefits: 'Large deployments complete faster. Independent components run in parallel.' },
        { label: 'Automatic provider caching', status: 'planned', quarter: 'q1-2026', pr: 1882, description: 'Cache Terraform providers across components to speed up init and reduce bandwidth.', benefits: 'Faster terraform init. Providers are downloaded once and shared across components.' },
        { label: 'Provider auto-generation', status: 'shipped', quarter: 'q3-2025', docs: '/components/terraform/providers', description: 'Generate provider.tf with proper credentials and region configuration from stack metadata.', benefits: 'Providers are configured automatically from stack context. No manual provider blocks.' },
        { label: 'Multi-stack formats', status: 'planned', quarter: 'q2-2026', pr: 1842, description: 'Support for alternative stack formats including single-file stacks and Terragrunt-style layouts.', benefits: 'Migrate from Terragrunt incrementally. Use the stack format that fits your project.' },
      ],
      issues: [],
      prs: [
    ...
    },
    {
      ...
      milestones: [
        ...
        { label: 'Every `atmos.yaml` section documented', status: 'shipped', quarter: 'q3-2025', docs: '/cli/configuration/configuration', description: 'Complete reference for every configuration option in atmos.yaml with examples and defaults.', category: 'featured', priority: 'high', benefits: 'Find the exact option you need without reading source code. Every setting has examples.' },
        { label: 'Every CLI command documented', status: 'shipped', quarter: 'q3-2025', docs: '/cli/commands/commands', description: 'Comprehensive documentation for every CLI command with usage examples and screenshots.', category: 'featured', priority: 'high', benefits: 'Learn any command from its documentation. Screenshots show exactly what to expect.' },
        { label: 'Cross-linking commands to configs', status: 'shipped', quarter: 'q3-2025', docs: '/cli/commands/commands', description: 'Navigate from any command to its related configuration and vice versa.', category: 'featured', priority: 'high', benefits: 'Jump between commands and their configuration options. Understand how settings affect behavior.' },
        ...
        { label: 'Roadmap', status: 'shipped', quarter: 'q4-2025', docs: '/roadmap', description: 'Public roadmap showing past accomplishments and future plans.', category: 'featured', priority: 'high', benefits: 'See what\'s coming and plan accordingly. Understand the direction of the project.' },
      ],
      issues: [],
      prs: [],
    },
