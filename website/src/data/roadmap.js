  ...
  initiatives: [
    ...
    },
    {
      ...
      milestones: [
        ...
        { label: 'Per-command default args in atmos.yaml', status: 'shipped', quarter: 'q2-2026', changelog: 'per-command-default-args', docs: '/cli/configuration/default-args', description: 'Set default command-line arguments for any command in atmos.yaml using a path-derived args: list (top-level args: for global, describe.args, terraform.args, describe.component.args, etc.). Default args are raw, pass-through tokens spliced onto the command line before parsing, so any flag, repeated flag, or positional works with no per-command wiring. Overridable by profiles, environment variables, and the command line (CLI > ENV > config).', benefits: 'Stop retyping the same flags. Disable identity or YAML-function processing by default, per command, and flip those defaults per environment with profiles — no code changes as new flags are added.' },
      ],
      issues: [],
      prs: [
