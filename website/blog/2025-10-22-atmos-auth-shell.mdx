...
We're excited to introduce [`atmos auth shell`](/cli/commands/auth/shell), a new command that makes working with multiple cloud identities more secure.
...
The command supports multiple identities, custom shells ([`--shell`](/cli/commands/auth/shell#flags)), custom arguments (`--`), shell nesting tracking (`ATMOS_SHLVL`), and works cross-platform.
...
**Use [`atmos auth env`](/cli/commands/auth/env) when:**
