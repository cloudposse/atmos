...
Before profiles, you had environment variables and config scattered everywhere:

- `.bashrc` for developers with `ATMOS_IDENTITY`, `ATMOS_LOGS_LEVEL`, etc.
- GitHub Actions secrets for CI with different values
- GitLab CI variables for other pipelines
- Different dotfiles for different team members
- `atmos.yaml` overrides that vary by machine
...
- **Profile Commands**: Use [`atmos profile list`](/cli/commands/profile/profile-list) and [`atmos profile show <name>`](/cli/commands/profile/profile-show) to discover and inspect profiles
