...
Vendoring pulls a copy of somebody else's module into your own repository, so the code you run is code you can read. It stops working cleanly the moment two environments need two different versions of the same module: the copy on disk can only be one of them, and the file recording where it came from sits apart from the configuration that uses it. A top-level `source` field on a component addresses both. Declare where the component comes from alongside the rest of its configuration, for **Terraform**, **Helmfile**, and **Packer** components, and Atmos downloads it the first time you use it.
...
We've added automatic just-in-time (JIT) source provisioning and new `source` command groups for terraform, helmfile, and packer. When you run any command, Atmos automatically provisions the component source if the directory doesn't exist:
...
The `source` provisioner works with the [workdir provisioner](/changelog/component-workdir-isolation) for concurrent operations across all component types (terraform, helmfile, and packer). When both are enabled:
...
Tell us how source-based versioning holds up against your own vendoring workflow, or ask for a source type we do not cover yet, by [opening an issue](https://github.com/cloudposse/atmos/issues).
