...
Atmos workflows already supported [`--stack`](/cli/commands/workflow#flags). They now also support [`--tags`](/cli/commands/workflow#flags) and [`--labels`](/cli/commands/workflow#flags), forwarding those selectors to every nested `type: atmos` step.
