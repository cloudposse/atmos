...
We're thrilled to announce native Azure authentication support in Atmos! You can now authenticate to Azure using [`atmos auth login`](/cli/commands/auth/login) with device code flow, OIDC, and service principals - working identically to `az login` with full Terraform provider compatibility.
...
- Check status anytime: [`atmos auth whoami`](/cli/commands/auth/whoami)
