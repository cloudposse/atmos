...
Now, Atmos defaults the template to `.` (current directory) when not specified, allowing Packer to load all `*.pkr.hcl` files from the component directory automatically:
...
- [Packer Build Command](/cli/commands/packer/packer-build)
