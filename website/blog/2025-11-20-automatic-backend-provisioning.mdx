...
The `provision.backend` configuration supports Atmos's inheritance system, allowing you to set defaults at any level in your stack hierarchy.

**Enable for all development environments:**
...
Components automatically inherit these settings through Atmos's deep-merge system. This gives you maximum flexibility:
- Set defaults at organization or environment level
- Override for specific components when needed
- Use catalog inheritance for reusable patterns
- Different policies for dev/staging (auto-provision) vs prod (pre-provisioned)
