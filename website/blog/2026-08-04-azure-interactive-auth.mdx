...
Atmos now supports the `azure/interactive` provider — the same interactive browser login `az login` uses (authorization code + PKCE on a localhost redirect). One command, [`atmos auth login`](/cli/commands/auth/login), opens your browser, signs you in, and sets up everything Terraform and the `az` CLI need.
