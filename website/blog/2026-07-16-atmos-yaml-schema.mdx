...
- **YAML functions are first-class.** Where a section expects a map, the schema also accepts [`!include shared.yaml`](/functions/yaml/include), [`!env`](/functions/yaml/env), [`!exec`](/functions/yaml/exec), and every other function `atmos.yaml` supports — so dynamic configs validate cleanly.
...
Validation now happens out of the box. With no configuration at all, [`atmos validate schema`](/cli/commands/validate/schema) checks `atmos.yaml`, `atmos.d/` fragments, and project-local profiles:
