...
Atmos now supports using dotenv files directly with the [`!include`](/functions/yaml/include) YAML function.
...
Use [`!include.raw`](/functions/yaml/include.raw) when you want the raw file contents instead.
...
No `--profile` flag, no exported variable — just [`atmos terraform plan vpc -s dev`](/cli/commands/terraform/plan). Atmos promotes the `ATMOS_*` values into its own environment before it resolves the active profile. An exported `ATMOS_PROFILE` (or `--profile`) still wins, so ad-hoc overrides like `ATMOS_PROFILE=ci atmos …` keep working. This is a great way to make sure everyone on a team — and CI — runs with the same defaults for a given repository.
