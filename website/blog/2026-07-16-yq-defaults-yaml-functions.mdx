...
We've fixed an issue where YQ default values (using the `//` fallback operator) in [`!terraform.state`](/functions/yaml/terraform.state) and [`!terraform.output`](/functions/yaml/terraform.output) YAML functions were not being evaluated when components weren't provisioned or outputs didn't exist.
