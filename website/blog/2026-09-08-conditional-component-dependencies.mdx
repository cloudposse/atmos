...
Required dependencies remain the default, preserving existing behavior. Optional dependencies are included normally when their target is present and enabled. Missing or disabled targets simply omit that edge. Invalid dependency configuration still fails instead of being hidden by optionality.
...
Omit `required` when the target must be available. Optionality only suppresses genuine target absence or disabled state; malformed entries remain errors.
