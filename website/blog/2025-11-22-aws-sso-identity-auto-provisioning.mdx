...
Now, you authenticate once with [`atmos auth login --provider {provider}`](/cli/commands/auth/login), and Atmos automatically discovers and provisions all available permission sets as identities. You can immediately use any role without touching configuration files.
...
1. User runs any Atmos command: [`atmos terraform plan`](/cli/commands/terraform/plan)
...
Use [`atmos auth list`](/cli/commands/auth/list) to see all identities, including provisioned ones:
...
We'd love to hear how auto-provisioning works for your use case. Share your experience in [GitHub Discussions](https://github.com/orgs/cloudposse/discussions) or report issues in [GitHub Issues](https://github.com/cloudposse/atmos/issues).
