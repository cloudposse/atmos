...
The [`atmos auth console`](/cli/commands/auth/console) command now supports isolated browser sessions, allowing you to have multiple cloud provider consoles open simultaneously — one per identity — without logout conflicts.
...
Or per-invocation with the [`--isolated`](/cli/commands/auth/console#flags) flag:
...
![AWS requires you to log out before signing into a different account](/img/changelog/aws-logout-conflict.png)
