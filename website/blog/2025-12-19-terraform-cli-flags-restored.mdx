...
Flags are now registered only on the commands that use them:
...
code but not in the new flag registry. The fix registers these flags on the specific commands that use them,
following the pattern established by other command-specific flags like `--from-plan` and `--affected`.
