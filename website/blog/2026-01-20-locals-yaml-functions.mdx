...
Locals now support YAML functions like [`!env`](/functions/yaml/env), [`!exec`](/functions/yaml/exec), [`!store`](/functions/yaml/store), [`!terraform.state`](/functions/yaml/terraform.state), and [`!terraform.output`](/functions/yaml/terraform.output).
