...
When you run [`atmos terraform plan vpc -s dev`](/cli/commands/terraform/plan), Atmos:
...
For manual provisioning or CI/CD pipelines, use the [`atmos terraform backend`](/cli/commands/terraform/terraform-backend) command:
...
- [CLI Documentation](/cli/commands/terraform/terraform-backend)
