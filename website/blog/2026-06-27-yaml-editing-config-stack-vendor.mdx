...
- **`atmos vendor update`** checks each Git source for a newer version (honoring per-source
  semver `constraints`) and updates the `version` field in place — preserving comments,
  anchors, and `{{.Version}}` templates. Use `--check` for a dry run and `--pull` to fetch
  afterward.
- **`atmos vendor diff`** shows the Git diff between two versions of a component, without a
  local checkout.
