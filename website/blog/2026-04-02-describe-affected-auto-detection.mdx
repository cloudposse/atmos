...
The [`atmos describe affected`](/cli/commands/describe/affected) command now auto-detects the base commit in CI environments, eliminating the need for verbose flag wiring in your workflows.

<!--truncate-->

## What Changed

We introduced the [`--base`](/cli/commands/describe/affected#flags) flag as a unified replacement for [`--ref`](/cli/commands/describe/affected#flags) and [`--sha`](/cli/commands/describe/affected#flags), and added automatic base resolution when running in CI with `ci.enabled: true`.
