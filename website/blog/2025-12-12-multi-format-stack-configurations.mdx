...
<Tabs>
  <TabItem value="with-name" label="With Explicit Name" default>
...
The block label `"production"` becomes the explicit stack name, overriding any `name_template` or `name_pattern`.

  </TabItem>
  <TabItem value="without-name" label="Without Explicit Name">
...
Without a label, the stack name is derived from `name_template`, `name_pattern`, or the filename.
