...
We've shipped a feature that developers working with complex infrastructure configurations have been asking for: **[provenance tracking](/cli/commands/describe/component#flags)**. With the new [`--provenance`](/cli/commands/describe/component#flags) flag in `atmos describe component`, you can now see exactly where every configuration value originated—down to the file, line number, and column.
...
:::note
The [`--query`](/cli/commands/describe/component#flags) flag is not supported with `--provenance`. To filter provenance output, pipe it through tools like `grep`, `yq`, or `jq`.
:::
