...
ECR credentials are written to `~/.docker/config.json`, the standard Docker config location. This means Docker commands work immediately after login—no additional environment variables or configuration required.
...
- [ECR Authentication Tutorial](/tutorials/ecr-authentication)
- [Auth Login Command](/cli/commands/auth/login)
- [ECR Login Command](/cli/commands/auth/ecr-login)
