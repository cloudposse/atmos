...
in [`atmos ai chat`](/cli/commands/ai/chat), [`atmos ai ask`](/cli/commands/ai/ask), and [`atmos ai exec`](/cli/commands/ai/exec) — no custom integration code needed.
...
Gemini CLI. Atmos adds `description`, `identity`, and `timeout` as extensions:
...
Servers with `identity` are automatically wrapped with [`atmos auth exec`](/cli/commands/auth/exec) for
