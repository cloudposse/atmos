...
Running a command from a subdirectory could change where Atmos looked for toolchain files. Automatically installing a missing dependency could also modify the project's `.tool-versions`. This update fixes both: [toolchain paths](/cli/configuration/toolchain#configuration-options) now resolve from the configured project base, and automatic installs leave declared versions unchanged.
...
For CI that must prohibit lockfile writes, [frozen mode](/cli/configuration/toolchain#frozen-installs-in-ci) requires complete entries for the current platform, including on binary cache hits.
