...
For example, running [`atmos theme show`](/cli/commands/theme/show) without arguments now displays an interactive menu:
...
- [`atmos auth`](/cli/commands/auth/usage) commands - Identity selection.

**Coming soon:** We're actively working to add interactive prompts to core functionality including [`atmos terraform`](/cli/commands/terraform/usage), [`atmos helmfile`](/cli/commands/helmfile/usage), and [`atmos packer`](/cli/commands/packer/usage). These commands will prompt for component and stack selection when values are missing, making it even easier to work with infrastructure components.
