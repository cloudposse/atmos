...
Atmos now correctly detects component file changes when running [`atmos describe affected`](/cli/commands/describe/affected).
...
- Vendored components configured with `source` (even without an explicit `component` field)
