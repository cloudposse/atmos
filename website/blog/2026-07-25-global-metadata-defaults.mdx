...
Stack manifests can now declare stack-wide [metadata](/stacks/components/component-metadata#global-stack-wide-metadata) defaults that every component inherits, with the component's own `metadata:` still free to override them.
...
Precedence runs lowest to highest: stack-wide default → the component's [`metadata.inherits`](/stacks/components/component-metadata#inherits) base-component chain → the component's own local `metadata:` block, which always wins:
...
Combine a stack-wide default with the [`!labels`](/functions/yaml/labels)/[`!tags`](/functions/yaml/tags) YAML functions (see [Tags and Labels](/changelog/tags-and-labels)) to feed a shared label set into every component's Terraform variables without repeating it:
