...
The [`!terraform.state`](/functions/yaml/terraform.state) YAML function now supports reading from S3 buckets encrypted with customer-provided keys (SSE-C).
...
Organizations that use SSE-C for S3 state encryption previously couldn't use `!terraform.state` to reference outputs from those state files. This meant either switching encryption strategies or falling back to [`!terraform.output`](/functions/yaml/terraform.output) (which runs `terraform output` and requires the full Terraform/OpenTofu binary). With SSE-C support, `!terraform.state` reads the state file directly from S3 with the correct encryption headers, keeping lookups fast and dependency-free.
