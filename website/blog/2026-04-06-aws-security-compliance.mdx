...
With [`atmos aws security analyze`](/cli/commands/aws/security/analyze), one command replaces that entire workflow:
...
which code manages each affected resource. Add [`--ai`](/cli/commands/aws/security/analyze#flags) for AI-powered remediation with specific
...
- **Compliance reports** — [`atmos aws compliance report --framework cis-aws`](/cli/commands/aws/compliance/report) for framework-specific posture
