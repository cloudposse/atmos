...
The example above uses the default `GITHUB_TOKEN`, which is fine for opening the PR — but GitHub
deliberately excludes its own default Actions token from re-triggering `on: pull_request`/`on: push`
workflows, so a plan/validate workflow that's supposed to run against the new PR won't fire.
...
[Atmos Pro STS](/changelog/atmos-pro-github-sts) and exported as `ATMOS_PRO_GITHUB_TOKEN`, which
`--pull-request` already prefers over `GITHUB_TOKEN`:
...
the wrapped command's environment, and the Component Updater's GitHub client picks it up
automatically.
