...
The [`atmos vendor update`](/cli/commands/vendor/vendor-update) command already finds and writes newer versions locally. Turning that
...
The [`--pull-request`](/cli/commands/vendor/vendor-update#flags) flag does the whole cycle natively: discover available updates, write them
...
No other flags or config changes are required. [`atmos auth exec`](/cli/commands/auth/exec) mints and exports the token for
