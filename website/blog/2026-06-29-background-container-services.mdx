...
Atmos's built-in emulator drivers cover AWS (`floci/aws`, `ministack/aws`, `localstack/aws`),
GCP (`floci/gcp`), Azure (`floci/az`), Kubernetes (`k3s`), Vault/OpenBao (`openbao`,
`vault`), OCI/Terraform registries (`registry`), and a Mockoon-backed 1Password Connect server
(`mockoon/1password-connect`). See the [emulator component reference](/stacks/components/emulator#supported-drivers--targets)
for the current driver and target list.

For usage and configuration, see [Using Containers](/components/container).
