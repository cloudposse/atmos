...
Then configure Atmos to build from your Dockerfile:
...
Launch it just like any other devcontainer:
...
Atmos will automatically build the image from your Dockerfile, tag it as `atmos-devcontainer-geodesic`, and create the container.

:::tip Example Available
Check out the complete example in [`examples/devcontainer-build`](https://github.com/cloudposse/atmos/tree/main/examples/devcontainer-build) which includes a working Dockerfile, devcontainer.json, and atmos.yaml for building custom Geodesic containers.
:::
...
For usage and configuration, see [Development Containers](/cli/configuration/devcontainer).
