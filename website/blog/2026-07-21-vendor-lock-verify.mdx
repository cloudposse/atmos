...
The [`atmos vendor pull`](/cli/commands/vendor/pull) command always re-fetches everything, every time, whether or not anything
...
[`atmos vendor verify`](/cli/commands/vendor/vendor-verify) checks against, with zero network access:
...
- The `silent` mode re-fetches with no reporting — the behavior every `vendor pull` had before this
  existed.
- The `warn` mode (the default) re-fetches and prints one line naming what drifted and why.
- The `strict` mode refuses to run at all until you pass `--refresh-lock`, so an unreviewed local
  edit can never get silently overwritten — or silently kept — without someone noticing.
...
[`atmos vendor update`](/cli/commands/vendor/vendor-update) or `--refresh-lock` re-resolves it. An exact pin like `version: v1.5.0`
