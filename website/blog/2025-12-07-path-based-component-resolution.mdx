...
title: "Use a Filesystem Path Where a Component Name Goes"
authors: [osterman]
tags: [feature, dx]
date: 2025-12-07T12:00:00.000Z
---

Almost every tool in your terminal acts on the directory you are standing in. You change into a Terraform module, run a plan, and it plans what is in front of you. Atmos was the exception: it wanted the component's name as written in your stack configuration, which is not always what the folder in front of you is called. Component commands now accept a filesystem path anywhere they accept a name.

<!--truncate-->

## The Problem

A component's name lives in the stack configuration, and it does not have to resemble where the component sits on disk. The directory `components/terraform/infra/vpc` is the component `infra/vpc`. Two components named `station/1` and `station/2` can both point at `components/terraform/weather`, in which case neither name appears in the path at all. If you navigate the repository by directory, and most people do, every command began with translating a path you were already looking at into a name you had to recall or go look up.
...
- **Non-interactive environments** - When a path cannot be resolved, or Atmos cannot prompt you to disambiguate it, the command fails with the matching component names instead of guessing
...
If a path you expected to work does not resolve, or the disambiguation prompt gets in your way, [open an issue](https://github.com/cloudposse/atmos/issues).
