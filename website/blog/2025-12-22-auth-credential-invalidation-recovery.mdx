...
title: 'Automatic Recovery from AWS Credential Invalidation'
authors:
  - aknysh
tags:
  - bugfix
...
Atmos now automatically detects when your AWS credentials have been rotated or revoked and prompts you for new credentials inline.
...
When `atmos auth login` encounters an `InvalidClientTokenId` error from AWS, it now:
...
Atmos now detects three common AWS STS errors:
...
- **Single command recovery**: `atmos auth login` now handles everything inline
