...
We fixed a regression in [`atmos auth shell`](/cli/commands/auth/shell) where it wasn't setting the required environment variables for credential file resolution. Previously, when you ran:
...
- [Workflow Authentication](/workflows/steps/identity)
- [Custom Command Authentication](/cli/configuration/commands/identity#using-authentication-with-custom-commands)
