...
- **5-level inheritance**: The `generate` section follows Atmos's standard inheritance model with merge from lowest to highest priority:
  ...
  3. **Base component level** (via [`metadata.inherits`](/stacks/components/component-metadata#inherits))
...
- **CLI integration**: New [`atmos terraform generate files`](/cli/commands/terraform/generate/files) command with `--all`, `--dry-run`, and `--clean` flags
- **Auto-generation**: Enable `auto_generate_files: true` in `atmos.yaml` to automatically generate files during terraform commands
- **Clean integration**: Generated files are automatically cleaned up by [`atmos terraform clean`](/cli/commands/terraform/clean)
