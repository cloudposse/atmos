...
- **Wrong field names**: Examples showed `backend` and `config` instead of the correct `kind` and `options`
- **Wrong store selectors**: Examples mixed old `type` names with the newer slash-style `kind` names
- **Non-existent backends**: Vault was documented before it was implemented
