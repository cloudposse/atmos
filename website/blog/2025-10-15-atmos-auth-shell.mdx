...
## Why This Matters: Identity-Based Authentication

The key feature of `atmos auth shell` is **seamless identity-based authentication**. Easily switch between different cloud identities, roles, and accounts without manually managing credentials:
...
The command also supports custom shells (`--shell`), custom arguments (`--`), shell nesting tracking (`ATMOS_SHLVL`), and works cross-platform.
