...
Prefer to wire it yourself? [`atmos ci cache paths --format=github`](/cli/commands/ci/cache/paths) emits the `key`, `path`, and `restore-keys` as step outputs for a plain `actions/cache` step.
...
- **Atmos-managed:** `cloudposse/atmos/actions/github-runtime@v1` + [`atmos ci cache restore`](/cli/commands/ci/cache/restore)/`save` (or `auto: both`).
