...
We've improved how Atmos merges YAML functions across configuration layers.
Atmos now defers merging YAML functions (which have different types than concrete values) until after the standard
merge completes, eliminating the type conflicts that previously occurred when templates or other functions were used in
layered stack configurations.
