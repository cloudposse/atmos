...
[`atmos list instances`](/cli/commands/list/list-instances) now supports `--format=matrix`, producing GitHub Actions-compatible JSON for driving parallel CI/CD jobs — the same format already available in [`atmos describe affected`](/cli/commands/describe/affected).
