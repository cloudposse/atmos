...
- **Local Git hook shims** — [`atmos git hooks install`](/cli/commands/git/hooks/install) wires `.git/hooks/*` (pre-commit, commit-msg, ...) to Atmos workflows and custom commands, replacing Husky-style glue.
- **Native CI checkout** — in GitHub Actions with `ci.enabled: true`, a bare [`atmos git clone`](/cli/commands/git/clone) infers the current repository from CI metadata and clones it into the workspace, replacing `actions/checkout`. GitHub STS credentials flow to every Git subprocess automatically via `GIT_CONFIG_*`.
