...
`atmos auth logout` now preserves keychain credentials by default for faster re-authentication. Only session data is cleared. Use [`--keychain`](/cli/commands/auth/logout#flags) to permanently delete credentials.
...
Interactive confirmation prevents accidents. Use [`--force`](/cli/commands/auth/logout#flags) for CI/CD:
