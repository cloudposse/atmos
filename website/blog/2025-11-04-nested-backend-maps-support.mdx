...
When generating backend configurations with [`atmos terraform generate backends --format hcl`](/cli/commands/terraform/generate/backends), nested maps were missing from the output. For example:
...
For usage and configuration, see [State Backends](/components/terraform/backends).
