...
- **Lifecycle hooks.** Templates can run declared work before or after generation—formatting,
  ...
  can use [`--skip-hooks`](/cli/commands/scaffold/generate) as an explicit per-invocation escape hatch.
- **Day-two updates.** The [`atmos scaffold generate --update`](/cli/commands/scaffold/generate) command re-runs a template against
  ...
  The [`--merge-strategy`](/cli/commands/scaffold/generate) option selects manual, ours, or theirs conflict handling.
