...
The [`atmos auth env`](/cli/commands/auth/env) command now supports `--format=github` for direct output to `$GITHUB_ENV`, eliminating shell pipelines in CI workflows.
...
- **Explicit output:** Use [`--output-file`](/cli/commands/auth/env#flags) (or `-o`) to specify a different file
