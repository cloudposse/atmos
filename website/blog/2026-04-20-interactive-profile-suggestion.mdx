...
The identity-specific suggestion above needs a name to search on. That misses a more common case: you run [`atmos auth login`](/cli/commands/auth/login) (no `--identity`, no `--profile`) in a repo whose entire auth config lives in profiles. The base `atmos.yaml` has neither `auth.identities` nor `auth.providers`, and the old behavior was a flat `no providers available` — with no hint that the answer lived one flag away.
