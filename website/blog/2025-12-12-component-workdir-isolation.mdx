...
For local components, enable workdir isolation with a simple metadata flag:
...
components:
  terraform:
    vpc:
      metadata:
        workdir: true  # Enable isolated workdir
      vars:
        cidr_block: "10.0.0.0/16"
```

When `metadata.workdir: true` is set, Atmos:

1. Creates `.workdir/terraform/<component>/` before `terraform init`
...
When we introduce JIT (Just-In-Time) vendoring support, workdir isolation will be automatic for any component with a `metadata.source`:
...
# Clean the source cache (for remote sources)
...
Workdir isolation is available now. Simply add `metadata.workdir: true` to any component that needs isolation:
...
For more details, see the [Component Workdir documentation](/docs/cli/configuration/components#workdir-isolation).
