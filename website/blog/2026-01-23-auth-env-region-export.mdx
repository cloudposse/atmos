...
The [`atmos auth env`](/cli/commands/auth/env) command now exports `AWS_REGION` and `AWS_DEFAULT_REGION`
...
For atmos commands ([`atmos terraform plan`](/cli/commands/terraform/plan), [`atmos terraform apply`](/cli/commands/terraform/apply), etc.),
region is **automatically injected** - no sourcing required. The [`!env AWS_REGION`](/functions/yaml/env)
