...
Here's `render` turning a recording into a rendered artifact, `play` replaying a
recording in the terminal, and `--cast` capturing a command's own session as it runs:
