...
# Production-scale (10 inputs, 24 top-level sections, nested maps+lists)
...
(10 inheritance layers, 24 top-level sections including nested maps, tags, providers, backend,
lists, and scalars) shows ~44 µs per full stack merge on a typical CI/CD server — well under
any practical latency budget even for large configurations with many stacks.
...
### Edge case: `sliceDeepCopy` result length (defined contract)
...
`len(dst)`).  This is an **intentional divergence** from mergo — the native implementation
adopts this as a defined contract that it owns independently.  The behavior is verified in
[`merge_native.go`](https://github.com/cloudposse/atmos/blob/main/pkg/merge/merge_native.go)
and cross-validated against mergo in
...
Migration of these remaining sites is tracked as a follow-up; the dependency will be
removed once those two call sites are ported.  Until then, a future CVE in mergo could
still affect atmos.  If you have concerns, please open or comment on an issue in the
[cloudposse/atmos](https://github.com/cloudposse/atmos/issues) repository.
