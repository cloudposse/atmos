...
`len(dst)`).  This is the **defined contract** for this implementation — it was also the
observed mergo behavior, but the native implementation owns this contract independently.
...
Migration of these remaining sites is tracked as a follow-up work item (see
[PR #2201 description](https://github.com/cloudposse/atmos/pull/2201) for status).
Once the follow-up issue is opened, this sentence will be updated with its `#number`.
Until then, a future CVE in mergo still affects atmos — and the dependency will be
removed as part of the tracked follow-up.
