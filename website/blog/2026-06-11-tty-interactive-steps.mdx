...
title: "Hand the Terminal to Your Commands with tty and interactive Steps"
authors: [osterman]
tags: [feature]
---

Custom command and workflow shell steps now support docker-style `tty` and `interactive` fields, so commands like `aws ssm start-session`, `ssh`, `psql`, and `vim` can take over the terminal properly — with Ctrl-C going to the session instead of killing Atmos.
...
You can also use `interactive: true` on its own for prompt-style commands that read stdin but don't need a full TTY — output stays masked and captured as usual, while Ctrl-C is handled by the command.
...
See the [Interactive and TTY Steps](/cli/configuration/commands#interactive-and-tty-steps) documentation for the full reference, including semantics of each flag and platform notes.
