...
```yaml
# base.yaml -> eks.dependencies.components: [{ name: vpc }, { name: iam-role }]
...
```yaml
import:
  - base   # eks.dependencies.components: [{ name: vpc }, { name: iam-role }]
...
- **Dependencies** — add `dependencies.components` entries without restating the base ones.
