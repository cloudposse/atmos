---
slug: deferred-yaml-function-merge
title: "Deferred YAML Function Merge: Solving Type Conflicts in Configuration Processing"
authors: [aknysh]
tags: [feature, atmos-core, configuration, yaml-functions]
date: 2025-11-29
---

We've implemented a new **deferred merge infrastructure** that fundamentally changes how Atmos handles YAML functions during configuration merging. This solves a long-standing challenge with type conflicts when templates and other YAML functions appear in hierarchical stack configurations.
...
**Key files:**
- `pkg/merge/deferred.go` - Context and data structures (55 lines)
- `pkg/merge/merge_yaml_functions.go` - Deferral logic (366 lines)
- `pkg/merge/merge.go` - Core merge functions (551 lines)
...
2. Merges multiple values per path using `list_merge_strategy`
3. Applies final values to result map
4. TODO: Process YAML functions (next phase)
