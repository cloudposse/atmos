...
In air-gapped or offline environments, set `ATMOS_GITHUB_ARCHIVED_CHECK_TIMEOUT=0s` to disable the check entirely, or reduce it (e.g. `ATMOS_GITHUB_ARCHIVED_CHECK_TIMEOUT=2s`) to shorten the per-repo timeout.
...
Atmos calls the GitHub REST API (`GET /repos/{owner}/{repo}`) and inspects the `archived` field. The check runs **once per unique repository URL** across all sources and components in a single run — even if multiple vendor entries reference different subdirectories of the same repository. Results are cached in memory so no duplicate API calls are made.
