...
The check is **best-effort and non-blocking**: if the GitHub API is unavailable (network error, rate limit, missing token), the check is silently skipped at debug level and vendoring proceeds normally. Vendoring is never blocked by this check.
