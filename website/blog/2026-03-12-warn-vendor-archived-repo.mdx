...
The check is **best-effort and non-blocking**: if the GitHub API is unavailable (network error, rate limit, missing token), the check is skipped and a `DEBUG`-level message is logged—vendoring proceeds normally. Vendoring is never blocked by this check.
