...
Defaults can come from the environment with the [`!env`](/functions/yaml/env) function (`!env VAR fallback`) in a workflow step's `default`, `prompt`, `options`, and `placeholder`. [`!exec`](/functions/yaml/exec) is supported too. Custom commands defined in `atmos.yaml` already resolved `!env`; workflows now do as well.
