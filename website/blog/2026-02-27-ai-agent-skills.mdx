...
Atmos now ships 16 agent skills that give AI coding assistants deep knowledge of Atmos conventions, stack configuration, Terraform orchestration, authentication, validation, and more. Skills build on two open standards -- [AGENTS.md](https://agents.md/) and [Agent Skills](https://agentskills.io/specification) -- and work across Claude Code, OpenAI Codex, Gemini CLI, Cursor, Windsurf, GitHub Copilot, and other AI tools.

<!--truncate-->

## What Changed

Atmos includes a new `agent-skills/` directory at the repository root containing 16 domain-specific skills:

- **Core Configuration**: `atmos-stacks`, `atmos-components`, `atmos-vendoring`, `atmos-schemas`
- **Orchestration**: `atmos-terraform`, `atmos-helmfile`, `atmos-packer`, `atmos-ansible`
- **Platform**: `atmos-auth`, `atmos-stores`, `atmos-workflows`, `atmos-custom-commands`
- **Integrations**: `atmos-gitops`, `atmos-validation`, `atmos-templates`
...
Symlink the skills into each tool's configuration directory:
...
# Windsurf -- auto-discovers AGENTS.md in workspace, or:
