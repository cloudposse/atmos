...
This also **fixes a latent bug**: the original code used `strings.HasPrefix(arg+"=", flag)` which could
produce false positives for flags sharing a common prefix (e.g., `--terraform-command-extra` matching
...
The new test suite adds 11 focused test functions covering:
...
- Error paths: missing values, multiple `=` signs, invalid options
