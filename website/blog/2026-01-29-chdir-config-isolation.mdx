...
The [`--chdir`](/cli/global-flags#directory-change-examples) flag now correctly isolates configuration loading when changing to a directory with its own Atmos configuration.
...
8. CLI argument ([`--config-path`](/cli/global-flags#core-global-flags))
