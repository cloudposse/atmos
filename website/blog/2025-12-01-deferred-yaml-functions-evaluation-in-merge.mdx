...
YAML functions (like [`!template`](/functions/yaml/template), [`!terraform.output`](/functions/yaml/terraform.output), [`!store.get`](/functions/yaml/store.get), and others) are represented as strings during
...
- [`!terraform.state`](/functions/yaml/terraform.state) - State file queries
- `!store.get` / [`!store`](/functions/yaml/store) - Store lookups
- [`!exec`](/functions/yaml/exec) - Command execution
- [`!env`](/functions/yaml/env) - Environment variable expansion
...
For usage and configuration, see [Type-Aware Merging of YAML Functions](/reference/yaml-function-merging).
