...
Only [`atmos terraform plan vpc -s my-legacy-prod-stack`](/cli/commands/terraform/plan) works. Using `-s legacy-prod` correctly returns an error because that's not the canonical name.
