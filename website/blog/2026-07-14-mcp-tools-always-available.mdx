...
You set `mcp.enabled: true`, ran [`atmos mcp start`](/cli/commands/mcp/start), and got: `failed to initialize AI components:
...
`ai.tools.enabled` was designed for [`atmos ai chat`](/cli/commands/ai/chat)/`ask`/`exec` — Atmos's own assistant deciding
