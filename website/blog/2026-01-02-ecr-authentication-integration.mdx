...
Integration failures during [`atmos auth login`](/cli/commands/auth/login) are logged but don't block authentication. Your identity credentials succeed even if ECR login fails - you can retry the integration separately.
...
- [Auth Command Overview](/cli/commands/auth/usage)
- [Authentication Configuration](/cli/configuration/auth)

For usage and configuration, see [ECR Authentication](/tutorials/ecr-authentication).
