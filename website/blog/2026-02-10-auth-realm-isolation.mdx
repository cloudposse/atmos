...
The [`atmos auth whoami`](/cli/commands/auth/whoami) and [`atmos auth login`](/cli/commands/auth/login) commands now display the active realm:
...
For usage and configuration, see [Authentication](/cli/configuration/auth).
