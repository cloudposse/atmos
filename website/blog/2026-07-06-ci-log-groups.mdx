...
- **`auto`** (default) — the finest grouping per command. A **workflow** renders one group per
  step; a direct **[`atmos terraform plan`](/cli/commands/terraform/plan)** renders one group per phase (`init`, then `plan`):
...
A step that runs [`atmos terraform apply`](/cli/commands/terraform/apply) is one group, not three — the outermost group always wins.
