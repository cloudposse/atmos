...
When [`atmos auth login`](/cli/commands/auth/login) encounters an `InvalidClientTokenId` error from AWS STS, it now:
...
A bug was fixed where **session duration configured via [`atmos auth user configure`](/cli/commands/auth/user/configure) was not being used**,
...
3. [`atmos auth logout`](/cli/commands/auth/logout) + `atmos auth login` wouldn't fix it
