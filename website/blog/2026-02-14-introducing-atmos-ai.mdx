...
Different tasks need different expertise. Atmos AI provides **21+ specialized skills** you can install from the marketplace:
...
**Switch skills with Ctrl+A** during conversations!
...
The AI uses **project-specific context** across all sessions via `ATMOS.md`:
...
Permission decisions now persist across sessions! When you approve or deny a tool execution, Atmos AI can remember your decision.
...
Choice (a/y/n/d): a
✅ Permission saved to .atmos/ai.settings.local.json
```

**Key Benefits:**
- **Reduced Prompt Fatigue** - 80%+ fewer permission prompts for repeat operations
- **Session Persistence** - Decisions remembered across all AI sessions
- **Manual Control** - Edit `.atmos/ai.settings.local.json` directly for batch updates
- **Secure by Default** - Cache checked after blocked tools, before allowed tools
...
**NEW:** Atmos AI now supports **prompt caching** (token caching) to dramatically reduce API costs by reusing frequently-sent content like system prompts and project instructions.
...
- ATMOS.md instructions: 8,000 tokens
...
AI: [Switches to atmos-validation skill]
...
```bash
atmos ai chat
# Press Ctrl+A → select "atmos-config"

You: My stack validation is failing with "undefined variable"

AI: [Switches to atmos-config]
...
```bash
# Edit ATMOS.md with your preferred editor
vim ATMOS.md
```
...
Atmos AI can automatically discover and include relevant project files in conversations using intelligent pattern matching, eliminating the need to manually specify files.

**Why Context Discovery?**
- Better AI responses with access to relevant project files
- No need to copy-paste file contents
- Intelligent filtering respects `.gitignore` for security
- Configurable limits prevent context overflow
- Cached discovery for fast performance
    ...
    settings:
      ai:
        context:
          ...
          max_size_mb: 10 # Limit total size
...
<Terminal title="atmos ai">
    ```bash
    # Add specific patterns for this query
    atmos ai ask "Review my Go code" --include "**/*.go" --exclude "**/*_test.go"

    # Analyze Terraform with custom patterns
    atmos ai exec "Check configurations" --include "**/*.tf" --exclude "**/.terraform/**"

    # Disable auto-discovery when not needed
    atmos ai exec "General question" --no-auto-context
    ```
</Terminal>

**How It Works:**
1. **Pattern Matching** - Glob patterns like `**/*.yaml` find all matching files recursively
2. **Gitignore Filtering** - Files in `.gitignore` are automatically excluded
3. **Size Limits** - Discovery stops at `max_files` or `max_size_mb` to prevent overwhelming context
4. **Caching** - Discovered files are cached with configurable TTL for faster subsequent queries
5. **Context Injection** - Matched files are automatically included in AI prompts for better responses
...
- File list visibility control with `show_files` setting

Learn more: [Context Discovery Documentation](/ai#automatic-context-discovery)

### Skill Marketplace

Install specialized skills from the marketplace following the [Agent Skills open standard](https://agentskills.io):
...
# Use in TUI - press Ctrl+A to switch skills
atmos ai chat
```

**Features:**
- Multi-skill packages (one install command for all skills)
- Version pinning (`cloudposse/atmos@v1.200.0`)
- Force reinstall (`--force`) and skip confirmation (`--yes`)
- Reference file loading for rich skill prompts
...
- ✅ **Automatic Context Discovery** - Intelligent file discovery with .gitignore support and caching

### Short-term (Next 3 Months)
- ✅ **Skill Marketplace** - Install community skills from the Agent Skills registry *(shipped!)*
...
- **Enhanced Instructions** - Automatic instructions updates from conversations
- **IDE Plugins** - Native VSCode/JetBrains integration
- **Team Collaboration** - Shared sessions across teams

### Long-term (6-12 Months)
- **Private Skill Registries** - Company-specific skills
- **Advanced Security** - Skill sandboxing, GPG signatures
...
- [AI Skills](/ai/skills) - Marketplace-installed skills following the Agent Skills open standard
