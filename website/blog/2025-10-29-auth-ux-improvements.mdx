...
Terraform commands now support interactive identity selection when you use [`--identity`](/cli/commands/auth/usage#flags) without specifying a name:
...
- **[`atmos auth logout <identity>`](/cli/commands/auth/logout)** - Clear one identity's cached credentials (keyring + files)
...
Each identity in [`atmos auth list`](/cli/commands/auth/list) now shows authentication status indicators:
