...
```bash
# Remove only dev-admin credentials
...
- **`atmos auth logout <identity>`** - Remove one identity's credentials
- **`atmos auth logout --provider <name>`** - Remove all identities using a provider
- **`atmos auth logout --all`** - Remove all authenticated identities
