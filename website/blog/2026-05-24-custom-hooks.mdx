...
wrapper, no GitHub Actions glue — [`atmos terraform plan vpc -s prod`](/cli/commands/terraform/plan)
...
A **pre-flight check** runs once per [`atmos terraform …`](/cli/commands/terraform/usage) invocation —
