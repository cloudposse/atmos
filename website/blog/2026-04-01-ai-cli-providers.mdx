...
When `mcp.servers` is configured, Atmos automatically passes MCP servers to the CLI tool.
...
- **Gemini CLI** — `.gemini/settings.json` in cwd
...
<ActionCard title="Example: AI with Claude Code">
    Complete example with Claude Code CLI provider, 8 AWS MCP servers, and automatic auth.
    <div>
