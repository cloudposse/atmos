...
<Intro>
Atmos natively supports opinionated workflows for [Terraform](https://www.terraform.io/) and [OpenTofu](/projects/configuration/opentofu).
It's compatible with every version of terraform and designed to work with multiple different versions of Terraform
concurrently.
<img width="170" align="right" src={useBaseUrl('/img/hashicorp-terraform.svg')} alt="Terraform"/>
</Intro>

Keep in mind that Atmos does not handle the downloading or installation of Terraform; it assumes that any
required commands are already installed on your system. To automate this, consider creating a [Custom Command](/custom-commands/custom-commands) to install Terraform.
