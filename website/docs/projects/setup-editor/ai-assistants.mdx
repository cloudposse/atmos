...
# Step 2: Install the Atmos skills plugin (all 23+ skills)
/plugin install atmos@cloudposse
```

Skills are available immediately after installation and persist across sessions. One plugin installs all 23+ skills -- the AI loads only the relevant skill on demand, so there is no performance overhead.
