...
2. **CI/CD Integrations**: Atmos supports several common methods for CI/CD, with [native CI integration in GitHub Actions](/ci) being the recommended method. We maintain and invest the most time and effort into GitHub Actions. However, we also support integrations with [Spacelift](/cli/configuration/integrations/spacelift) and [Atlantis](/cli/configuration/integrations/atlantis).
...
Make sure to [configure dependencies](/stacks/dependencies/components) between components using the `dependencies.components` section.
...
- [GitHub Actions](/ci): Use `atmos terraform deploy --affected` to deploy components in dependency order. For independent components, the matrix examples in the [native CI guide](/ci) fan out in parallel.
