...
The `!terraform.output` YAML function allows reading the outputs ([remote state](/connections/remote-state))
...
The [`!terraform.state`](/functions/yaml/terraform.state) YAML function reads outputs **directly from the configured Terraform or OpenTofu backend**, bypassing the `terraform output` or `tofu output` pipeline — it's **very fast**, doesn't require provider initialization, and supports [all Terraform/OpenTofu backends](/components/terraform/backends) (local, S3, azurerm, gcs, remote, consul, cos, http, kubernetes, oss, pg, cloud) for accessing [remote state](/connections/remote-state).
...
<ActionCard>
  ...
  <PrimaryCTA to="/connections/remote-state">Learn How</PrimaryCTA>
</ActionCard>
