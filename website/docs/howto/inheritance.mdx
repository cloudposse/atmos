...
In the overall [variable merge order](/learn/merge-order), base component vars (from `metadata.inherits`) sit between global-level vars and component-level vars — the current component's values always override inherited values.
...
|---------|-----------|-------|
| `vars` | Yes | Variables passed to Terraform |
| `settings` | Yes | Component settings |
| `env` | Yes | Environment variables |
| `backend` | Yes | Backend configuration |
| `providers` | Yes | Provider overrides |
| `metadata` | Yes | All fields except `metadata.inherits` and `metadata.type` |

:::info Metadata Inheritance
The `metadata` section is inherited from base components, with two exceptions:

1. **`metadata.inherits`** - Not inherited because it defines the inheritance relationship itself
2. **`metadata.type`** - Not inherited because component type (`abstract`, `real`, or custom) is per-component and should be explicitly defined by each component.

This enables patterns like defining `metadata.name` once in a base component for stable [workspace key prefixes](/components/terraform/backends).

To disable metadata inheritance (for backwards compatibility with older configurations):
...
configurations (`vars`, `settings`, `env`, `metadata`, and other sections) from `ComponentA`, and can override the default values from `ComponentA`.
...
configurations (`vars`, `settings`, `env`, `metadata`, and other sections) from `ComponentA` and `ComponentB`, and can override the default values
...
After this Multilevel Inheritance chain gets processed by Atmos, `ComponentC` will inherit all the configurations (`vars`, `settings`, `env`, `metadata`, and other
