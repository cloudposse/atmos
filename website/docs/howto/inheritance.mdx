...
| `metadata` | Yes | All fields except `metadata.inherits` and `metadata.type: abstract` |
...
2. **`metadata.type: abstract`** - Not inherited because `abstract` is a base component designation indicating the component is not deployable. Child components that inherit from abstract components are concrete implementations and should not inherit the abstract type. Other `type` values (e.g., `real`, custom types) ARE inherited normally.

This enables patterns like defining `metadata.name` once in a base component for stable [workspace key prefixes](/core-concepts/components/terraform/backends).
