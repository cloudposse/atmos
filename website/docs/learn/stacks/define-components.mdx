...
<Intro>
Components are configured in the `components` section of stack files. This reference covers the schema for Terraform components, including `vars`, `metadata`, `settings`, and `env`. For Helmfile components, see [Using Helmfiles](/components/helmfile).
</Intro>
