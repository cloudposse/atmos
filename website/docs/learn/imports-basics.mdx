...
:::tip
Your values always override imported values at the same level. But if an imported file sets a var at the **component** level, setting it at the **global** level won't override it — you need to match the level.
:::
...
## Scope Levels Beat Import Order

Import order controls which file wins **at the same level**. But component-level `vars` always beat global-level `vars`, no matter which file they came from.
...
components:
  terraform:
    vpc:
      vars:
        cidr_block: "10.0.0.0/16"   # Component level
```
</File>
...
vars:
  cidr_block: "172.16.0.0/16"       # Global level — loses!
```
</File>

The fix: override at the same level.
...
components:
  terraform:
    vpc:
      vars:
        cidr_block: "172.16.0.0/16"  # Same level — wins
```
</File>

:::tip Debugging
Use `atmos describe component vpc -s prod-us-east-1` to see final merged values, or add `--provenance` to see exactly which file each value came from.
:::
