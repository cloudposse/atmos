...
:::tip After Imports: Scope Levels Matter
Import resolution is only the first stage. After all files merge into one tree, Atmos applies a second merge
at the **component level** where scope matters: component-level `vars` override global-level `vars`, regardless
of which file they came from. See [Variable Merge Order](/learn/merge-order) for the complete picture.
:::
