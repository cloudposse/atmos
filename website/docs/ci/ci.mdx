...
before any get canceled, but it cannot be combined with `cancel-in-progress: true`.

Use remote state locking for safety. GitHub environments add approval gates and merge queues
order merges, but neither guarantees deployment *execution* order — reach for an explicit
promotion workflow or deployment controller when you need that guarantee.
:::
