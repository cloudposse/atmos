...
Run Atmos directly in GitHub Actions to plan and deploy infrastructure.

The examples run in the Atmos container, which supplies the CLI. Set the
[`ATMOS_VERSION` repository variable](https://docs.github.com/en/actions/learn-github-actions/variables)
to a specific Atmos release; there is no `latest` tag. Replace `vpc` and `prod` with your
component and stack names.

Use the CI configuration in [Quick Start](#quick-start) and configure
[Authentication](#authentication) for your infrastructure. The plan and apply workflows below
include the [permissions](#permissions) and GitHub token needed for CI reporting.
...
| SBOM workflow artifact upload | `contents: read`; GitHub Actions runtime credentials (see [SBOM Artifacts](#sbom-artifacts)) |
...
<dl>
  ...
  <dt>[`atmos vendor update --pull-request`](/cli/commands/vendor/vendor-update)</dt>
  <dd>Open (or update) a pull request with available vendored-component updates directly from CI — replaces the deprecated Component Updater action.</dd>
</dl>
