...
The selected store handles both upload and download, so `plan` and `deploy` agree on where the
planfile lives. A failure *while* uploading or downloading does not switch stores.
