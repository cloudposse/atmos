...
A companion setting, `components.terraform.planfiles.on_missing` (`fail` | `warn` | `off`), governs the
case where **no stored plan was found** to verify against. It defaults to the resolved `verify` mode, so a
fail-by-default CI deploy fails loudly instead of silently applying an unverified fresh plan. See
[Missing stored plan](/components/terraform/planfiles#missing-stored-plan).
