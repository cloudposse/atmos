...
matches what was reviewed, so the deploy uses the apply-time role and state while still guaranteeing no
material drift. This avoids the most common saved-plan failure — the identity that created the plan
differing from the one applying it. (To replay the stored binary directly, use `--from-plan` /
