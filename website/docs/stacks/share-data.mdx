...
The [`!terraform.state`](/functions/yaml/terraform.state) YAML function reads outputs **directly from the configured Terraform or OpenTofu backend**, bypassing the `terraform output` or `tofu output` pipeline — it's **very fast**, doesn't require provider initialization, and supports [all Terraform/OpenTofu backends](/components/terraform/backends) (local, S3, azurerm, gcs, remote, consul, cos, http, kubernetes, oss, pg, cloud) for accessing [remote state](/stacks/remote-state).
...
When you combine data sources with [vendoring](/vendoring/vendor), [terraform backends](/components/terraform/backends) and [provider](/components/terraform/providers) generation, you can leverage any Terraform module as a "root module" and provision it as a component with Atmos.
:::
...
<ActionCard>
  ...
  <PrimaryCTA to="/templates/datasources">Learn More</PrimaryCTA>
</ActionCard>
...
<ActionCard>
  ...
  <PrimaryCTA to="/stacks/remote-state">Learn How</PrimaryCTA>
</ActionCard>
