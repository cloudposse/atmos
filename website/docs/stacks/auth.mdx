...
<dl>
  <dt>`kind`</dt>
  <dd>Provider type (e.g., `aws/iam-identity-center`, `azure/interactive`, `azure/device-code`, `azure/oidc`, `azure/cli`, `gcp/adc`, `gcp/workload-identity-federation`).</dd>
...
<dl>
  <dt>`kind`</dt>
  <dd>Identity type (e.g., `aws/permission-set`, `aws/assume-role`, `aws/ambient`, `ambient`, `azure/subscription`, `gcp/service-account`, `gcp/project`).</dd>
