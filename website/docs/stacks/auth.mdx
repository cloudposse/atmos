...
<Intro>
The `auth` section configures how Atmos obtains and manages credentials for cloud providers and external services. Auth configuration is primarily defined in `atmos.yaml` but can be extended at the component level via `settings.auth`.
</Intro>
...
Components can reference or extend auth configuration via `settings.auth`:
