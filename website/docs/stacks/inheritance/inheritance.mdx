...
<Intro>
Inheritance provides a template-free way to customize Stack configurations. When combined with [imports](/stacks/imports), it provides the ability to combine multiple configurations through ordered deep-merging of configurations. Inheritance is how you manage configuration variations, without resorting to [templating](/stacks/templates).
</Intro>

Atmos supports the following concepts and principles of **Component-Oriented Programming (COP)**:

- [Single Inheritance](/stacks/inheritance#single-inheritance) - when an Atmos component inherits the configuration properties from
  another Atmos component

- [Multiple Inheritance](/stacks/inheritance#multiple-inheritance) - when an Atmos component inherits the configuration from more than one Atmos
  component

These concepts and principles are implemented and used in Atmos by combining two features: [`import`](/stacks/imports)
and [`metadata`](/stacks/define-components) component's configuration section.

:::info
The mechanics of mixins and inheritance apply only to the [Stack](/stacks) configurations. Atmos knows nothing about the underlying
...
In **Component-Oriented Programming (COP)** implemented in Atmos, a [mixin](/stacks/inheritance/mixins) is an abstract base component that is never
