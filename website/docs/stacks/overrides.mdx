...
<Intro>
The `overrides` section provides **file-scoped variable overrides**. Unlike regular `vars` which apply to all components in the entire top-level stack after import resolution, `overrides` apply only to components defined in the current manifest file and its imports.
</Intro>
...
Overrides also have the **highest merge priority** in the [variable merge order](/learn/merge-order), making them useful when you need a value to win regardless of where else it's defined.

:::tip When You Don't Need Overrides
If each top-level stack imports a single blueprint (no competing manifests), you don't need `overrides`. Plain `vars` at the same scope level is sufficient — the importing file's values override the imported file's values. See the [Blueprint Configuration](/design-patterns/stack-organization/blueprint-configuration) pattern.
:::
