...
For file-scoped changes to `command`, `env`, `hooks`, `providers`, `settings`, or Terraform generated files, keep using `overrides`.
:::
...
For Terraform components, `terraform.overrides` also supports:

- [`generate`](/stacks/generate) - Override generated file definitions for Terraform components
...
`terraform.overrides.generate` is file-scoped like the other override sections and has the highest priority when Atmos merges generated file definitions. Use it when a generated file should apply to the Terraform components visible from the current manifest and its imports, without affecting sibling manifests imported into the same top-level stack.
