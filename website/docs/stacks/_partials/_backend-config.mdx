...
For state files encrypted with [SSE-C (Server-Side Encryption with Customer-Provided Keys)](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ServerSideEncryptionCustomerKeys.html),
provide the customer key so that [`!terraform.state`](/functions/yaml/terraform.state) can decrypt state when reading directly from S3:
...
The [`sse_customer_key`](https://developer.hashicorp.com/terraform/language/backend/s3#sse_customer_key) must be a base64-encoded 256-bit (32-byte) key. It can also be provided via the `AWS_SSE_CUSTOMER_KEY`
environment variable. The backend attribute takes precedence over the environment variable.

:::note
The `sse_customer_key` attribute is used by Atmos when reading state directly via [`!terraform.state`](/functions/yaml/terraform.state).
...
above, the resolved `CI_JOB_TOKEN` value. The same plaintext values also persist in
`.terraform/terraform.tfstate` and in any saved plan file (see
[Planfile Backend Limitations](/components/terraform/planfiles#backend-limitations)).
Never commit these files, log their contents, or upload them as CI artifacts.

Prefer the `TF_HTTP_USERNAME` and `TF_HTTP_PASSWORD` environment variables and omit
`username`/`password` from the backend configuration entirely — Terraform/OpenTofu reads
them at runtime without persisting them to disk. `backend.tf.json` should still be
excluded via `.gitignore` (Atmos regenerates it on demand when
`auto_generate_backend_file` is enabled).
:::
...
See [Backend Provisioning](/stacks/components/provision/backend) for complete documentation.
