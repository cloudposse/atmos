...
:::tip
You can use [Atmos Stack Manifest Templating](/stacks/templates) in `depends_on`.
...
As you can see in the examples above, we can use [Atmos Stack Manifest Templating](/stacks/templates) in the `stack` attribute to dynamically specify the stack.
