...
:::tip Don't confuse `backend:` with `provision.backend:`
The `backend:` block on this page describes *where* state is stored (bucket, region, key).
The separate `provision.backend:` block tells Atmos to *automatically create that location*
if it doesn't exist. See [Backend Provisioning](/stacks/components/provision/backend) for the
stack-config schema, or [Automatic Backend Provisioning](/components/terraform/backend-provisioning)
