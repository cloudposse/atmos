...
This type of error can fail many components at the same time. A drift-detection run, a bulk `apply`, or a CI job can query the registry for many components in a short time window. One registry connection problem can then fail the whole batch. Configure `retry` on each affected component. Or configure `retry` on one shared abstract base component — see [Inheritance](#inheritance). The whole batch then recovers on its own. You do not need to start a manual re-run.
