...
<dl>
  ...
  <dd>File-scoped temporary variables for DRY configuration within a single manifest.</dd>
...
Rain components use the standard sections listed above, plus `template`, `name`, `config`, `params`, `tags`, and `settings.rain` for CloudFormation configuration.

See [Rain Components](/stacks/components/rain) for details.
...
# Defaults for all Rain components
rain:
  env:
