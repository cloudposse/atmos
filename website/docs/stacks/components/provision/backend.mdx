...
- **Entra ID hardening**: When the backend sets `use_azuread_auth: true`, a **newly created**
  ...
  creation time — auto-provisioning skips backends that already exist, so an existing storage
  account is not modified. Harden a pre-existing account separately (e.g. set
