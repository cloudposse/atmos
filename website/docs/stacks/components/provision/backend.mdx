...
<dl>
  ...
  <dd>
    ...
    Supports S3 backends (with optional native S3 locking, Terraform 1.10+) and
    `azurerm` backends (resource group, storage account, and blob container).
...
- **Public access**: Blocked (`AllowBlobPublicAccess=false`); the state container is private
- **Entra ID hardening**: When the backend sets `use_azuread_auth: true`, a **newly created**
  storage account gets shared-key access disabled (no account keys to leak). This applies only at
  creation time — an existing storage account retains its shared-key setting. If its backend is
  incomplete, Atmos can still apply blob data protection (versioning + soft delete) and create the
  missing container. Harden a pre-existing account separately (e.g. set
  `allowSharedKeyAccess: false` via the module or `az storage account update`).
- **Tags**: Automatic resource tags (`Name`, `ManagedBy=Atmos`)
...
backend:
  resource_group_name: rg-tfstate-cus     # Required
  storage_account_name: sttfstateplatcus  # Required (globally unique, 3-24 lowercase alphanumeric)
  container_name: tfstate                  # Required
  key: vpc.terraform.tfstate
  use_azuread_auth: true                   # Recommended — disables shared-key access
...
Backend provisioning is opted in per component or per toolchain because it has stronger side effects (creating cloud resources). Like the [workdir default](/stacks/components/provision/workdir#global-defaults), a stack-wide default belongs in the toolchain section (`terraform.provision`) of a shared stack manifest, not in `atmos.yaml` settings.
