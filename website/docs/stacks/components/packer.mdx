...
<Intro>
  Packer components build machine images (AMIs, VM images, container images)
  using HashiCorp Packer. They allow you to manage image builds with the same
  stack-based configuration approach used for Terraform and Helmfile.
</Intro>
...
Values in the `env` section are injected only into the Packer subprocess environment and are
...
[`!secret`](/functions/yaml/secret) in `env`, and read them in the Packer template with the
...
<File title="components/packer/bastion/build.pkr.hcl">
```hcl
...
6. **Use Dependencies:** Define `dependencies.components` when images need to be built in a specific order.
