...
Rejected with a hard error on any command that processes the manifest:
...
- Template calls to `atmos.Component`, `atmos.Store`, `atmos.GomplateDatasource`, and gomplate datasources (`ds`/`datasource`)

If a value needs one of these, move it into `vars` or `settings` and reference it from there — the value stays available to the component while the selectors remain pure.
