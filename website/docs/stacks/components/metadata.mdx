...
<Intro>
The `metadata` section configures how Atmos interprets and manages a component. Metadata can be set at the global level (under `terraform:`, `helmfile:`, or `packer:`) to provide defaults for all components of that type, or within component definitions (`components.terraform.<name>.metadata`) for component-specific settings.
</Intro>
...
The `metadata` section can be used in two places:

**1. Global Level (Defaults for All Components):**
...
**2. Component Level (Component-Specific Settings):**
...
When metadata is defined at multiple levels, the following precedence applies (lowest to highest priority):

1. **Global metadata** (from `terraform.metadata`, `helmfile.metadata`, or `packer.metadata`)
2. **Base component metadata** (from `metadata.inherits`)
3. **Component metadata** (highest priority)
...
## Global Metadata Examples

### Setting Default Workspace Pattern

Set a default `terraform_workspace_pattern` for all Terraform components in a stack:
...
components:
  terraform:
    vpc:
      # Inherits terraform_workspace_pattern: "{component}"
      vars:
        cidr: "10.0.0.0/16"

    eks:
      # Also inherits terraform_workspace_pattern: "{component}"
      vars:
        cluster_name: "prod"
```
</File>

This eliminates the need to set `terraform_workspace_pattern` in every component's metadata.

### Combining Global and Component Metadata

Global metadata provides defaults, which can be overridden at the component level:

<File title="stacks/mixed.yaml">
```yaml
terraform:
  metadata:
    terraform_workspace_pattern: "{component}"
...
components:
  terraform:
    vpc:
      # Uses global workspace pattern and locked setting
      vars:
        cidr: "10.0.0.0/16"
    
    critical-db:
      metadata:
        locked: true  # Overrides global locked: false
      # Still uses global workspace pattern
      vars:
        instance_class: "db.r5.xlarge"
```
</File>

### Using Global Metadata with Inheritance

Global metadata has the lowest priority, so base components and component-specific settings override it:

<File title="stacks/catalog/vpc-defaults.yaml">
```yaml
terraform:
  metadata:
    terraform_workspace_pattern: "{component}"  # Stack-wide default
...
components:
  terraform:
    vpc/defaults:
      metadata:
        type: abstract
        terraform_workspace_pattern: "{tenant}-{stage}"  # Overrides global
        component: vpc/v2
```
</File>
  ...
  terraform:
    vpc-prod:
      metadata:
        inherits:
          - vpc/defaults
        # Inherits "{tenant}-{stage}" pattern from vpc/defaults, not global
      vars:
        cidr: "10.0.0.0/16"
```
</File>
