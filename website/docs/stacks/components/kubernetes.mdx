...
By default `path` is a directory (one generated file per rendered object, same
as `split: true`). Set `split: false` to write `path` as a single exact file
instead — required whenever the consumer expects a specific filename, such as
