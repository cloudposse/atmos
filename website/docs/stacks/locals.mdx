...
Components can reference merged locals (global + component-type) in their `vars`:
...
:::note
Component-level locals (defined inside individual component definitions) are **not supported**. Only global and component-type scopes are implemented. Use global or section-level locals instead.
:::
