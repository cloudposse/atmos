...
<dl>
  ...
  <dd>
    In the `settings.templates.settings` section in [Atmos stack manifests](/stacks). The `settings.templates.settings` section can be defined globally per organization, tenant, account, or per component. Atmos deep-merges the configurations from all scopes into the final result using [inheritance](/stacks/inheritance).
  </dd>
...
Atmos deep-merges the configurations from the `settings.templates.settings` section in [Atmos stack manifests](/stacks)
with the `templates.settings` section in `atmos.yaml` [CLI config file](/cli/configuration) using [inheritance](/stacks/inheritance).

The `settings.templates.settings` section in [Atmos stack manifests](/stacks) takes precedence over
...
3. **Use Terraform Remote State Directly**: Instead of relying on template functions to retrieve remote state, [use Terraform's native ability to retrieve the remote state](/connections/share-data/#using-terraform-remote-state) of other components.
...
- In [Atmos Component Vendoring](/vendor/vendor-manifest)
- In [Imports](/stacks/imports)
- In [Stack Manifests](/stacks)
...
- `Go` templates in [Atmos Vendoring](/vendor) and [Atmos Component Vendoring](/vendor/vendor-manifest)
...
The [Using Templates in the URLs of Datasources](/stacks/templates/datasources#using-templates-in-the-urls-of-datasources)
