---
title: Configure Dependencies Between Components (Legacy)
sidebar_position: 2
...
`settings.depends_on` is deprecated. Use [`dependencies.components`](/stacks/dependencies/components) instead.

The new format uses a simpler list syntax with append merge behavior:
...
See [Component Dependencies](/stacks/dependencies/components) for the recommended approach.
...
Support for dependencies is reliant on the [integration](/cli/configuration/integrations) used and not all integrations support dependencies.
