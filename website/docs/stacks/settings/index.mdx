...
- **Dependencies:** Define component dependencies with [`dependencies.components`](/stacks/dependencies/components).
...
### Legacy Spacelift Settings

:::warning Deprecated
For existing Spacelift installations only. New projects should use [native CI](/ci); see the [legacy Spacelift reference](/deprecated/spacelift) when maintaining an existing installation.
:::
...
Declare ordering relationships outside `settings` with [`dependencies.components`](/stacks/dependencies/components). The legacy [`settings.depends_on`](/stacks/settings/depends_on) form is retained only for migration reference.
...
The resulting component configuration for `vpc` would include:
...
4. **Leverage Dependencies:** Use `dependencies.components` to ensure components are applied in the correct order.
