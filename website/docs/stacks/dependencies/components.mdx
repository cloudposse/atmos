...
<dl>
  ...
  <dt>required <em>(boolean or template, default: true)</em></dt>
  <dd>Whether the target must be concrete and enabled. Set to `false` when the dependency should provide ordering only when that target is available. A template may render this value to `true` or `false`; any other rendered value returns an error.</dd>
</dl>
...
Component dependencies are required by default. A required target that is missing, disabled, or abstract returns an error when Atmos processes the declaring component.

Set `required: false` when an integration is available in only some environments:
...
When `observability` is available, Atmos includes it in dependency ordering. When it is absent, disabled, or abstract, Atmos omits only that optional edge. Invalid dependency entries still return errors.

Use a template when optionality depends on stack values. The template must render
to a boolean:
...
dependencies:
  components:
    - name: observability
      required: "{{ .vars.observability_required }}"
```

For commands bounded by `--stack`, `--labels`, `--components`, or `--tags` together with `--include-dependencies` or `--include-dependents`, Atmos evaluates only the selected components and their reachable dependency closure. An unrelated component cannot cause template, YAML-function, or required-target errors for that operation.
