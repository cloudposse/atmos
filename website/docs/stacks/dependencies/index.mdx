...
<Intro>
The `dependencies` section declares two kinds of relationships: required tool versions (`dependencies.tools`) and component dependencies (`dependencies.components`). Tool dependencies pin CLI versions like Terraform, kubectl, or helm; component dependencies define ordering and watch-paths used by `atmos describe affected` and CI/CD integrations.
</Intro>
...
The `dependencies.components` section declares relationships between Atmos components, files, and folders. These dependencies are used by `atmos describe affected` to determine which components are impacted by a change, by `atmos describe dependents` to find downstream consumers, and by CI/CD integrations (such as Spacelift) to apply components in the correct order.

For full reference — cross-stack dependencies, file/folder watching, cross-type (`kind`) dependencies, merge behavior, and migration from `settings.depends_on` — see the dedicated [Component Dependencies](/stacks/dependencies/components) page.
