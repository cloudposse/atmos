...
description: Use the env section to set environment variables for your Terraform, Helmfile, Packer, Ansible, or Rain components.
id: env
---
...
Environment variables defined under `terraform`, `helmfile`, `packer`, `ansible`, or `rain` apply to all components of that type:
...
Use [`!include`](/functions/yaml/include) to load a dotenv file directly into a stack `env` section:
...
2. **Component-type `terraform.env:` / `helmfile.env:` / `packer.env:` / `ansible.env:` / `rain.env:`** (applies to all components of that type)
