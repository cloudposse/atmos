...
description: Use the env section to set environment variables for your Terraform, Helmfile, Packer, or Ansible components.
id: env
---
...
Use [`!include`](/functions/yaml/include) with YAML merge keys to load dotenv files into stack `env` sections. The `<<` key is YAML's merge-key syntax, the same YAML mechanism commonly used with anchors and aliases:
