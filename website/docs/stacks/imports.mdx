...
:::warning Remote imports bring configuration, not component code
...
To pull the component code from the same (or another) remote repo, give the component a [`source`](/cli/commands/terraform/source) so Atmos vendors the root module just-in-time at plan/apply time:
...
Alternatively, [vendor](/cli/commands/vendor/pull) the component into the repo ahead of time with `atmos vendor pull`. In short: **remote imports + remote components are two separate mechanisms** — importing the config does not bring the code; you must provision the source (or vendor it) explicitly.
...
- `local` - resolve nested imports from the consuming repository's `stacks.base_path` (default)
- `remote` - resolve nested imports from the remote source's stack base path
...
<dl>
  ...
  <dt>`nested_imports` - (string)</dt>
  <dd>Controls how imports inside the imported file are resolved. Use `local` to resolve nested imports from the consuming repository's stack base path, or `remote` to resolve them from the remote source's stack base path. Defaults to `local`.</dd>

  <dt>`ttl` - (string)</dt>
  <dd>Cache duration for the cloned source repository of a remote (Git) import. When set, the clone is reused across Atmos invocations until it expires (e.g. with a warm CI cache), skipping the re-clone. Within a single invocation a source repo is always cloned at most once regardless of `ttl`. When unset, the source is re-cloned once per invocation. Accepts durations like `0s`, `5m`, `1h`, `7d`, or keywords like `daily`. Set a default for all imports with the top-level `imports.ttl` setting in `atmos.yaml`. See [Caching Remote Imports](#caching-remote-imports).</dd>
...
Because imports are evaluated **in order**, the value must be defined by an import that
appears **before** the import that references it.

<dl>
  <dt>Requires templating enabled</dt>
  <dd>Import-path templating only runs when [`templates.settings.enabled`](/cli/configuration/templates) is `true`. When disabled, `{{ ... }}` in an import path is left literal.</dd>

  <dt>`skip_templates_processing`</dt>
  <dd>Leaves `{{ ... }}` in the path literal (the import is not rendered).</dd>

  <dt>Missing values</dt>
  <dd>If a referenced value is not defined by an earlier import (or the import's `context`), Atmos fails with an error. Set `ignore_missing_template_values: true` on the import (or [`templates.settings.ignore_missing_template_values`](/cli/configuration/templates) globally) to leave unresolved values as-is instead.</dd>

  <dt>Nested templates are resolved</dt>
  <dd>A referenced value may itself contain template syntax. Import paths are rendered to a **fixed point** — Atmos keeps rendering until the result stops changing — so a value like `settings.context.deployment_repo` that embeds `{{ atmos.Resolve "!git.repository" }}` is fully resolved when used in a later import path. This lets you define a composite value (such as a repository URL) **once** and reuse it across imports.</dd>
</dl>

### Composing a value once and reusing it

Because referenced values are themselves rendered, you can assemble a repository
URL from [YAML functions](/functions/yaml) in a single place and
reference it from every import (and from component `source` blocks):

```yaml title="stacks/deploy/prod/_defaults.yaml"
settings:
  context:
    deployment_repo_version: main
    # `deployment_repo` itself contains templates; Atmos resolves them when the
    # value is used in an import path.
...
:::note Component `source` blocks
The same `{{ .settings.context.deployment_repo }}` reference works in a component's
`source.uri`/`version`, but those are rendered during **component processing**, which
runs a single template pass by default. To resolve a value that itself contains
templates there, set [`templates.settings.evaluations: 2`](/cli/configuration/templates).
Note that increasing `evaluations` also re-processes any
[escaped templates](/templates#excluding-templates-in-stack-manifest-from-processing-by-atmos)
on each extra pass.
:::

:::note Import paths vs. file content
...
- [Configure the Project](/quick-start/advanced/configure-project)
