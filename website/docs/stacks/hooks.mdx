...
<dl>
  <dt>`migration`</dt>
  <dd>Path to a single `tfmigrate` migration file. Omit this field to use `tfmigrate` history mode with `config`.</dd>

  <dt>`config`</dt>
  <dd>Path to a `tfmigrate` config file. Use this to configure `migration_dir` and durable `history.storage`.</dd>
...
History mode is enabled by omitting `migration` and supplying `config`:
...
Atmos exposes helper variables so `.tfmigrate.hcl` can reuse the component's
