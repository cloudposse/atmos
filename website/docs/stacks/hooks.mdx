...
<dl>
  ...
  <dd>Which engine runs the hook. One of `store`, `command`, `git`, `step`, `steps`, `infracost`, `checkov`, `trivy`, `kics`. Aliased from legacy `command:` field.</dd>

  <dt>CI hook kinds</dt>
  <dd>`ci.check`, `ci.output`, `ci.summary`, `ci.upload`, and `ci.download` are reserved for Atmos CI integrations.</dd>
