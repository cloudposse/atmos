...
<dl>
  ...
  <dt>`ci.results`</dt>
  <dd>Uploads the raw SARIF to GitHub **Code Scanning** (the Security tab) as tracked alerts. Requires GitHub Advanced Security on private repos. Default off (opt-in). The analysis category is derived automatically from the scan target (`atmos/&lt;component&gt;` for shared component source, `atmos/&lt;stack&gt;/&lt;component&gt;` for a per-stack workdir) so per-component uploads don't overwrite each other.</dd>
</dl>
...
`ci.summary` and `ci.annotations` need **no special permissions** — the step
summary is a file write and annotations are log output. `ci.results` uploads to
Code Scanning, which requires the `security-events: write` permission (and GitHub
Advanced Security on private repositories). Note: pull requests **from forks** do
not receive `security-events: write` on the default token, so `ci.results` is
skipped there.
