...
<Intro>
Components are the building blocks of your infrastructure. They are opinionated, reusable units of infrastructure-as-code that solve specific problems—Terraform or OpenTofu root modules, Helm releases, Helmfiles, Kubernetes manifests, Packer templates, Ansible playbooks, containers, or emulators. You can also define [custom component types](/components/custom).
</Intro>
...
1. **Implementation** — The infrastructure or application definition, such as a root module, chart, manifest, image, or playbook
...
Atmos natively supports these component types:
...
| **[Container](/components/container)** | Container images & services | Build and run persistent, stack-scoped containers |
| **[Emulator](/components/emulator)** | Stack config via `components.emulator.*.driver` (no `components/emulator/` directory) | Run cloud-API, Kubernetes, and backing-service emulators offline |
| **[CloudFormation](/stacks/components/aws-cloudformation)** *(experimental)* | CloudFormation templates | Deploy AWS CloudFormation stacks directly via the AWS SDK |
...
│   └── webserver/
└── container/           # Container build contexts
    └── api/
```
...
- [Custom Component Types](/components/custom) — Bring any tool under stack-based config
- [Container Components](/components/container) — Build and run persistent, stack-scoped containers
- [Emulator Components](/components/emulator) — Run cloud-API, Kubernetes, and backing-service emulators offline
