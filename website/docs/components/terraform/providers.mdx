...
With Atmos, you don't need a separate tagging tool. The AWS provider's `default_tags` argument tags every resource a
provider manages without touching a single resource block, and since Atmos already generates the provider
configuration for you, `default_tags` is the most convenient way to guarantee consistent tagging across every
component and stack. Combine it with Atmos's built-in [YAML functions](/functions/yaml) — such as
