...
our [best practices](/best-practices/components) to get all the benefits of Atmos. For example, when using our [Terraform components](https://github.com/cloudposse-terraform-components), we frequently use [Terraform Remote State](/stacks/remote-state) to retrieve the outputs from other components.
