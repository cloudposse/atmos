...
our [best practices](/best-practices/components) to get all the benefits of Atmos. For example, when using our [Terraform components](https://github.com/cloudposse/terraform-aws-components), we frequently use [Terraform Remote State](/connections/remote-state) to retrieve the outputs from other components.
...
[Terraform Component Remote State](/connections/remote-state) and configure the `static` remote state for
