...
```yaml title="stacks/orgs/acme/_defaults.yaml"
terraform:
  backend_type: s3  # s3, remote, vault, azurerm, gcs, cloud, consul, cos, http, kubernetes, oss, pg
  backend:
    s3:
      acl: "bucket-owner-full-control"
      encrypt: true
  ...
  remote_state_backend_type: s3 # s3, remote, vault, azurerm, gcs, cloud, static, consul, cos, http, kubernetes, oss, pg
