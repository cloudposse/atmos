...
`verify` governs the comparison when a stored plan **exists**. A separate setting,
`components.terraform.planfiles.on_missing`, governs what happens when **no stored plan was found** to
verify against (for example, the `plan` job never uploaded one, or the download failed). This is the
difference between "the plan drifted" and "there is no reviewed plan to apply" — without it, a `deploy`
would silently apply an unverified fresh plan, so a green run would not actually prove verification ran.
...
components:
  terraform:
    planfiles:
      ...
      on_missing: fail    # behavior when no stored plan was found (default: tracks `verify`)
```

<dl>
  <dt>`fail`</dt>
  <dd>Fail the deploy: a reviewed stored plan was expected but none was found.</dd>

  <dt>`warn`</dt>
  <dd>Log a warning and apply a freshly generated plan without verification.</dd>

  <dt>`off`</dt>
  <dd>Silently apply a freshly generated plan (the pre-existing behavior).</dd>
</dl>

When `on_missing` is unset it **tracks the resolved `verify` mode**, so a fail-by-default CI deploy also
fails loudly on a missing plan rather than applying an unverified one. Local deploys (no CI, no storage)
resolve to `off` and are unaffected. Precedence mirrors `verify`: **CLI flag > config > tracks `verify`**.
