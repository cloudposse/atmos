...
Per-run overrides: `--verify-plan` forces `fail`, `--no-verify-plan` forces `off`. Precedence is
...
if `verify` resolves to `off` (including `--no-verify-plan`), nothing is downloaded, verified, or required.
