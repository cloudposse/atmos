...
- **Policy Validation**: Run security scanners (Checkov, tfsec, OPA) against planfiles.
...
Planfiles enable integration with security scanning and policy tools:
...
3. **Consider JSON planfiles for sensitive environments** - Use `terraform show -json` on planfiles for reviewable, scannable output
...
| Security scanning required | JSON planfile | `atmos terraform plan vpc -s prod` then `terraform show -json` |
...
| Compliance audit required | JSON planfile + version control | `atmos terraform plan` with Git |
