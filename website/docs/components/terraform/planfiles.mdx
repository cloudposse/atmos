...
the review guarantee while using the apply-time identity and state, avoiding the common failure where
the role that created the plan differs from the role applying it. (To replay the stored planfile
directly, use [`--from-plan` / `--planfile`](/cli/commands/terraform/deploy).)
