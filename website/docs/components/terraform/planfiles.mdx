...
- **Artifact Storage**: Store planfiles in external storage (e.g., S3, GitHub Actions artifacts). Atmos provides a [planfile storage GitHub Action](https://github.com/cloudposse/github-action-atmos-terraform-planfile) for this purpose. The native `github/artifacts` store talks to the GitHub Artifacts API directly, so inside GitHub Actions you must surface the runner's runtime credentials with the [`github-runtime`](https://github.com/cloudposse/atmos/tree/main/actions/github-runtime) action — see [Planfile Storage](/ci/planfile-storage#using-github-artifacts-in-github-actions)
...
```text
<component-working-dir>/<stack>-<component>.planfile
```

## Drift Verification

When [planfile storage](/ci/planfile-storage) is configured and `atmos terraform deploy` runs in CI
mode, Atmos verifies the stored plan against current state before applying: it downloads the planfile
that `plan --ci` uploaded, generates a **fresh** plan, and compares them with the JSON plan-diff. On a
match it applies the *fresh* plan — reconciled against, not replayed from, the reviewed one. This keeps
the review guarantee while using the apply-time state and credentials, avoiding the brittleness of a
saved plan: it goes stale when state moves, and its base credentials come from the apply environment,
not the plan. (To replay the stored planfile directly, use
[`--from-plan` / `--planfile`](/cli/commands/terraform/deploy).)

Control the behavior with `components.terraform.planfiles.verify`:
...
components:
  terraform:
    planfiles:
      ...
      # Drift verification behavior on `deploy` (default: fail under CI).
      verify: fail   # fail | warn | off
```

<dl>
  <dt>`fail` <em>(default under CI)</em></dt>
  <dd>Fail the deploy when the stored plan drifts from a fresh plan; apply the fresh plan on a match.</dd>

  <dt>`warn`</dt>
  <dd>Log a warning on drift but proceed with the deploy (applies the freshly generated plan).</dd>

  <dt>`off`</dt>
  <dd>Disable verification entirely; the stored plan is not downloaded and `deploy` performs a normal plan-and-apply.</dd>
</dl>

Per-run overrides: `--verify-plan` forces `fail`, `--verify-plan=false` forces `off`. Precedence is
**CLI flag > config > CI default**. Verification runs only on `deploy` (not `apply`), and only under CI
when planfile storage is configured. `--verify-plan` requires planfile storage: when
`components.terraform.planfiles` is not configured the deploy errors, since there is no stored plan
to verify against.

### Missing stored plan

`verify` governs the comparison when a stored plan **exists**. A separate boolean,
`components.terraform.planfiles.required`, governs whether a stored plan **must exist** to verify against
(for example, when the `plan` job never uploaded one). This is the difference between "the plan drifted"
and "there is no reviewed plan to apply" — without it, a `deploy` would silently apply an unverified fresh
plan, so a green run would not actually prove verification ran.

```yaml
components:
  terraform:
    planfiles:
      verify: fail        # drift behavior when a stored plan exists
      required: true      # must a reviewed stored plan exist? (default: tracks `verify`)
```

<dl>
  <dt>`true`</dt>
  <dd>Fail the deploy when no stored plan is found: a reviewed plan was expected.</dd>

  <dt>`false`</dt>
  <dd>Apply a freshly generated plan when no stored plan is found (you opted out of requiring one); a warning is logged.</dd>
</dl>

When `required` is unset it **tracks `verify` strictness** — a stored plan is required only when
verification resolves to `fail` (for example, under CI with planfile storage configured). So a
fail-by-default CI deploy fails loudly on a missing plan rather than applying an unverified one, while
local deploys (no CI, no storage) never require one. `required` only applies when verification is active:
if `verify` resolves to `off` (including `--verify-plan=false`), nothing is downloaded, verified, or required.

:::note GitHub Actions runtime token
With the `github/artifacts` store, the automatic download uses the GitHub Artifacts runtime API, so the
workflow must surface `ACTIONS_RUNTIME_TOKEN` / `ACTIONS_RESULTS_URL` with the
[`github-runtime`](https://github.com/cloudposse/atmos/tree/main/actions/github-runtime) action
(`mode: env`) before `deploy` — the same requirement as the upload on `plan`. See
[Planfile Storage → Using GitHub Artifacts in GitHub Actions](/ci/planfile-storage#using-github-artifacts-in-github-actions).
:::
...
- [Terraform Configuration](/cli/configuration/components/terraform#configuration) - Configure Terraform behavior
