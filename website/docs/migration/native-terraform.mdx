...
<EmbedFile filePath="examples/native-terraform/atmos.yaml" />
...
<Tabs>
  <TabItem value="include" label="Keep Using tfvars" default>
    ...
    <EmbedFile filePath="examples/native-terraform/stacks/dev.yaml" />

    The `!include` function resolves paths relative to the stack file and automatically parses `.tfvars` files (HCL format). This is the fastest migration path—your existing variable files keep working. You still get stack inheritance, imports, and all other Atmos features.
  </TabItem>
  <TabItem value="convert" label="Convert to YAML">
    As you grow, convert your `.tfvars` to native YAML for deep merge benefits:

    <EmbedFile filePath="examples/native-terraform/stacks/prod.yaml" />
...
Instead of copying `.tfvars` files, use YAML [imports](/stacks/imports) and [inheritance](/howto/inheritance):
...
## Real Example: VPC Migration

Let's walk through a complete migration. The full example is in [`examples/native-terraform/`](https://github.com/cloudposse/atmos/tree/main/examples/native-terraform).
...
Your existing `.tfvars` files look like this:

<EmbedFile filePath="examples/native-terraform/legacy/dev.tfvars" />
...
Your Terraform code stays the same — you just move it and add stack configs:
...
<Tabs>
  <TabItem value="structure" label="Structure" default>
    ...
    legacy/
      dev.tfvars         # Your existing tfvars, still used!
      prod.tfvars
    stacks/
      _defaults.yaml     # Shared config
  ...
  <TabItem value="defaults" label="stacks/_defaults.yaml">
    <EmbedFile filePath="examples/native-terraform/stacks/_defaults.yaml" />
  </TabItem>
  <TabItem value="dev" label="stacks/dev.yaml (keep tfvars)">
    <EmbedFile filePath="examples/native-terraform/stacks/dev.yaml" />
  </TabItem>
  <TabItem value="prod" label="stacks/prod.yaml (converted)">
    <EmbedFile filePath="examples/native-terraform/stacks/prod.yaml" />
  </TabItem>
  <TabItem value="main" label="components/terraform/vpc/main.tf">
    <EmbedFile filePath="examples/native-terraform/components/terraform/vpc/main.tf" />
  </TabItem>
  <TabItem value="variables" label="components/terraform/vpc/variables.tf">
    <EmbedFile filePath="examples/native-terraform/components/terraform/vpc/variables.tf" />
  </TabItem>
</Tabs>
...
# Prod uses explicit name "production" (not the filename "prod")
