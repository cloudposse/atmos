...
| **Dependencies** | `dependency` blocks | Remote state + `!terraform.output` |
...
| **[Vendoring](/cli/commands/vendor)** | Pull and version external modules locally with `atmos vendor pull`. Customize vendored code while tracking upstream. |
...
| **[Component Validation](/cli/commands/validate)** | JSON Schema and OPA policy validation for stack configurations before deployment. |
