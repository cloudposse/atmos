...
<Tabs>
  ...
  <TabItem value="atmos-source" label="Atmos Source Provisioning">
    ...
    Workdir provisioning is strongly recommended with source provisioning. It stages each component instance under `.workdir/`, so Terraform runs with isolated `.terraform/` directories, lockfiles, generated varfiles, and backend files.
  </TabItem>
