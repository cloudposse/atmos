...
A step cannot call another custom command through `type: atmos`. That step type is only for
native Atmos verbs, such as `terraform plan` or `terraform apply`. To reuse a command from
another command or workflow step, call it directly with `type: shell` and
`command: atmos build`.
...
`terraform plan`. To call another custom command, use a `type: shell` step with
`command: atmos <other-command-name>`.
</FAQItem>
