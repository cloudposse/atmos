...
| `internal: true` task                             | Command `hidden: true`                                            |
...
Set `hidden: true` on the custom command. The command still runs (`atmos <name> ...`, or as a
...
- [ ] Mark `internal: true` tasks as `hidden: true` custom commands
</TaskList>
...
Set `hidden: true` on the custom command. It stays fully runnable, including as a `default:`
