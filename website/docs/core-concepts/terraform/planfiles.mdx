...
- **State Version Tracking**: Planfiles record the state version at plan time, helping detect state changes between plan and apply (actual concurrency protection depends on backend state locking mechanisms)
...
- **Policy Validation**: Run security scanners (Checkov, OPA) against planfiles before applying

### 3. **CI/CD Pipeline Safety**
- **Atomic Operations**: Apply exactly what passed through your CI/CD tests
- **Artifact Storage**: Store planfiles alongside other deployment artifacts
- **Rollback Understanding**: Saved planfiles document exactly what changes were made
...
These backends manage the entire plan lifecycle remotely and cannot save planfiles locally:

| Backend | Type | Why Planfiles Aren't Supported | Recommended Approach |
|---------|------|--------------------------------|---------------------|
| **Remote Backend** | Enhanced | Plans execute and are stored in Terraform Cloud/Enterprise | Use remote workflow or migrate to standard backend |
| **Terraform Cloud** | Enhanced | All operations happen in cloud workspaces | Configure `skip_planfile: true` in Atmos |
| **Terraform Enterprise** | Enhanced | Enterprise manages complete workflow | Configure `skip_planfile: true` in Atmos |

#### Special Cases

| Backend/Platform | Issue | Details | Solution |
|-----------------|--------|---------|----------|
| **HTTP Backend** | Credential Storage | Credentials are embedded in planfiles, causing issues with job-specific tokens in CI/CD | Use consistent credentials or environment variables |
| **GitLab-managed State** | Platform Integration | GitLab CI/CD manages its own plan/apply workflow | Use GitLab's native terraform integration, not planfiles |
...
**For GitLab-managed State:**
- Use GitLab's merge request pipelines
- Let GitLab handle the plan/apply workflow
- Configure Atmos to skip planfiles

#### Option 3: Migrate to a Supporting Backend

If planfiles are critical to your workflow, consider migrating to a supporting backend:

<Terminal>
```bash
# Example: Migrate from HTTP to S3 backend
...
For GitLab-managed Terraform state (no planfiles):
...
# GitLab manages the plan internally
plan:
  extends: .terraform:plan
  script:
    - atmos terraform plan vpc -s prod --skip-planfile
...
```bash
# Production workflow
atmos terraform plan app -s prod -out=prod-$(date +%Y%m%d-%H%M%S).planfile
# Get approval...
atmos terraform apply app -s prod --planfile prod-20240115-143022.planfile
```
