...
| **HTTP Backend** | Standard | While technically supports `-out`, credentials are embedded in planfiles which breaks CI/CD with rotating tokens | Use `atmos terraform generate planfile` for JSON/YAML output instead |
...
## Alternative: JSON/YAML Planfiles

For backends that don't support binary planfiles or when you need human-readable plan output, use:

### `atmos terraform generate planfile`

This command generates a planfile in JSON or YAML format instead of Terraform's binary format:
...
- Works with any backend (including HTTP, Remote)
- Can be stored in version control
- Suitable for compliance and audit requirements
- Can be compared using standard diff tools

### `atmos terraform plan-diff`

Compare two planfiles to understand changes between them:
