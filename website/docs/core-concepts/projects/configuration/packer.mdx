...
### Define Atmos `prod` and `nonprod` stacks
...
==> amazon-ebs.al2023: Inferring subnet from the selected VPC "vpc-e4da9c8d"
==> amazon-ebs.al2023: Set subnet as "subnet-f078eb8b"
...
==> amazon-ebs.al2023: changing public IP address config to true for instance on subnet "subnet-f078eb8b"
...
# Use a YQ expression to get a specific section or attribute from the Packer manifest,
# in this case, the AMI (second part after the `:`) from the `artifact_id`
