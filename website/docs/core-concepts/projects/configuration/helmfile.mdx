...
<dl>
  ...
  <dd>A flag indicating whether the component is configured to use Amazon EKS (Elastic Kubernetes Service). When set to `true`, the component will interact with EKS for provisioning and managing Kubernetes clusters. Also, it means `cluster_name_template` must be defined.</dd>
  ...
  <dt>`cluster_name_template` (required when `use_eks=true`)</dt>
