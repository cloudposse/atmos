...
the `{terraform_workspace}` token in a stack manifest in `_defaults.yaml`. The token `{terraform_workspace}` will be
automatically replaced by Atmos with the Terraform workspace for each component.
