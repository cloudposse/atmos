...
Backends define where Terraform stores its [state](https://developer.hashicorp.com/terraform/language/state) data files.
...
If the `key` is not specified (either in the global or component manifests, refer to [Terraform Backend Inheritance](#terraform-backend-inheritance)),
Atmos will use the component name to auto-generate the `key` parameter in the format `<component-name>.terraform.tfstate`
replacing `<component-name>` with the Atmos component name.
All occurrences of `/` (slash) will be replaced with `-` (dash) in `<component-name>`.

If `auto_generate_backend_file` is set to `true` in the `atmos.yaml` CLI config file in the `components.terraform` section,
Atmos will deep-merge the backend configurations from the `_defaults.yaml` manifests and from the component itself, and
will generate a backend config JSON file `backend.tf.json` in the component's folder, similar to the following example:
