...
### Advanced: Using Go Templates for Workspace Names
...
Terraform workspaces work together with backend configuration to determine where state files are stored. For remote backends like S3, state files are organized in a two-level hierarchy:
...
These are two separate, independent settings that work together to create the full state path:

- **Workspace NAME** (`{workspace}` in path): The Terraform workspace name, controlled by:
...
- **Workspace KEY PREFIX** (`{workspace_key_prefix}` in path): The backend storage grouping, controlled by:
...
Each serves a different purpose in organizing your Terraform state.
:::
