...
<Tabs>
  <TabItem value="subdir" label="With Subdirectory" default>
    ...
    **Result:** Clones the repository and extracts only the `modules/vpc` directory.
  </TabItem>
  ...
  <TabItem value="no-delimiter" label="Without Delimiter">
    ```yaml
    source: "github.com/cloudposse/atmos.git?ref=v1.0"
    ```
...
<Tabs>
  <TabItem value="tag" label="Git Tag" default>
    ```yaml
    source: "github.com/cloudposse/terraform-aws-components.git//modules/vpc?ref=v1.398.0"
    ```
  </TabItem>
  ...
  <TabItem value="commit" label="Git Commit SHA">
    ```yaml
    source: "github.com/owner/repo.git//path?ref=a1b2c3d4"
    ```
  </TabItem>

  <TabItem value="depth" label="Custom Clone Depth">
    ```yaml
    source: "github.com/owner/repo.git?ref=main&depth=10"
    ```
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem value="github-simple" label="Simple" default>
    ```yaml
    # Implicit HTTPS, root directory
    source: "github.com/cloudposse/terraform-null-label.git?ref=0.25.0"
    ```
  </TabItem>

  <TabItem value="github-subdir" label="With Subdirectory">
    ```yaml
  ...
  <TabItem value="github-explicit" label="Explicit git::">
    ```yaml
  ...
  <TabItem value="github-ssh" label="SSH">
    ```yaml
  ...
  <TabItem value="github-creds" label="With Credentials">
    ```yaml
...
<Tabs>
  <TabItem value="gitlab-simple" label="Simple" default>
    ```yaml
    # Implicit HTTPS
  ...
  <TabItem value="gitlab-subdir" label="With Subdirectory">
    ```yaml
...
<Tabs>
  <TabItem value="bitbucket-simple" label="Simple" default>
    ```yaml
    # Implicit HTTPS
    source: "bitbucket.org/owner/repo.git?ref=master"
    ```
  </TabItem>

  <TabItem value="bitbucket-subdir" label="With Subdirectory">
    ```yaml
    # Extract specific directory
...
<Tabs>
  <TabItem value="ghcr" label="GitHub Container Registry" default>
    ```yaml
    source: "oci://ghcr.io/cloudposse/terraform-aws-components:v1.0.0"
    ```
  </TabItem>

  <TabItem value="ecr" label="AWS ECR Public">
    ```yaml
...
**Result:** Downloads from `modules/vpc` at version `1.398.0` to `components/terraform/vpc`.
...
<Tabs>
  ...
  <TabItem value="bad" label="❌ Avoid">
    ...
    source: "github.com/cloudposse/terraform-aws-components.git//modules/vpc?ref=main"
    ```
  </TabItem>
</Tabs>

### 2. Use Subdirectories for Monorepos

```yaml
# ✅ Extract only what you need
source: "github.com/cloudposse/terraform-aws-components.git//modules/vpc?ref=v1.398.0"

# ❌ Clone entire monorepo
source: "github.com/cloudposse/terraform-aws-components.git?ref=v1.398.0"
```
