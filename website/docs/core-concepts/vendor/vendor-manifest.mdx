...
When vendoring multiple versions of the same component, use the `{{ .Version }}` template parameter in the `targets` attribute to ensure the components are vendored into different folders. Then update the stack configuration to point to the correct version of the component.

### Understanding Workspace Key Prefix with Versioned Components

When vendoring multiple versions of the same component, you need to ensure the Terraform state path remains stable across version upgrades. Atmos provides two approaches:

#### How Atmos Calculates `workspace_key_prefix`

Atmos automatically generates the `workspace_key_prefix` (S3), `prefix` (GCS), or `key` (Azure) for your backend using this priority order:
...
#### Recommended: Use `metadata.name` (NEW)
  ...
  terraform:
    vpc:
      metadata:
        ...
        component: vpc/1.2.3   # Physical version path
      # Result: effective workspace_key_prefix will be "vpc"
      # When you upgrade to vpc/2.0.0, workspace_key_prefix STAYS "vpc"
      # No Terraform state migration needed!
```

:::tip Why This Matters
**Problem:** If `workspace_key_prefix` includes the version (e.g., `vpc-1-2-3`), upgrading to a new version creates a NEW state file, and you lose your existing infrastructure state.

**Solution:** `metadata.name` separates the stable logical identity (`vpc`) from the physical version path (`vpc/1.2.3`), so state paths remain stable across upgrades.
:::
...
#### Legacy: Manual Workaround (OLD)

Before `metadata.name` was introduced, you had to manually set `workspace_key_prefix` to avoid version changes:

```yaml
# ❌ OLD WAY - Required manual configuration
components:
  terraform:
    vpc:
      metadata:
      ...
      backend:
        s3:
          workspace_key_prefix: vpc  # Had to set this manually!
```

**With `metadata.name`, this manual step is no longer needed.**
