...
:::tip
View the full Vendor configuration schema in the [Vendor Schema](/schemas/vendor) reference section.
:::
...
<dl>
  ...
  The `source` and `targets` attributes support [Go templates](https://pkg.go.dev/text/template) and [Sprig Functions](http://masterminds.github.io/sprig/). This can be used to templatise the `source` and `targets` paths with the component name specified in the `component` attribute and artifact versions specified in the `version` attribute.
    ...
    spec:
      ...
      - component: "vpc-flow-logs-bucket"
        ...
        included_paths:
          - "**/**"
          # Note: doublestar `**` matches recursively across subdirectories; explicit patterns are optional
          - "**/modules/**"
    ```
    </File>
    ...
    Atmos uses doublestar-style globs where `**` matches across all nested directories. Use explicit sub-patterns only to narrow the match.
 ...
 This imports configurations from `vendor/backend.yaml` and `vendor/frontend.yaml`, along with any vendoring configurations in `vendor/apps/`, recursively merging them into the main `vendor.yaml` manifest:
