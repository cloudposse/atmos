...
<dl>
  ...
  <dd>
    ...
    **Token injection settings in `atmos.yaml`:**
    ...
    For custom Git hosts or advanced scenarios not covered by automatic token injection, you can manually inject credentials using Go templates:
    ...
    **Option 1: Single-quoted YAML string (recommended)**
    ```yaml
    source: 'git::https://{{getenv "TOKEN"}}@custom-host.com/org/repo.git?ref={{.Version}}'
    ```

    **Option 2: YAML folded scalar**
    ```yaml
    source: >-
      git::https://{{getenv "TOKEN"}}@custom-host.com/org/repo.git?ref={{.Version}}
    ```
    ...
    **Why manual injection is rarely needed:**
    - Automatic injection works for GitHub, GitLab, and Bitbucket
    - Manual injection requires careful YAML quoting
    - Credentials in URLs are harder to audit and rotate
    - Environment variables are more secure and easier to manage
