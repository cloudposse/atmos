...
errors[message] {
  ...
  not input.tf_cli_vars.config.encryption_enabled
...
# Ensure plan file is specified for apply operations
errors[message] {
  "apply" in input.cli_args
  not contains_plan_flag(input.env_tf_cli_args)
  message = "Apply operations must specify a plan file via TF_CLI_ARGS (e.g., -out=planfile)"
}

# Helper function to check for plan-related flags
contains_plan_flag(args) {
  some i
  startswith(args[i], "-out=")
}
...
errors[message] {
  ...
  parallelism_str := substring(input.env_tf_cli_args[i], 13, -1)
  parallelism := to_number(parallelism_str)
  parallelism > 20
...
get_total_instances := total {
  instance_counts := [
    input.vars.instance_count,
    input.tf_cli_vars.instance_count,
    input.env_tf_cli_vars.instance_count
  ]

  valid_counts := [count | count := instance_counts[_]; is_number(count)]
