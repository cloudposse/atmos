...
Atmos allows enforcing custom governance rules based on metadata about Atmos command executions and provides a powerful
...
- `cli_subcommand`: The first-level subcommand, such as `apply`, `plan`, or `generate`
- `cli_subcommand2`: The second-level subcommand when applicable, e.g., `varfile`, `planfile`
- `vars`: A map of variables passed to the command, either via CLI flags or stack config files
- Other contextual attributes: Such as the stack name and component name

#### Atmos Policy Metadata Example

Below is an OPA policy rule to enforce infrastructure governance during command execution.
Specifically, this rule blocks the execution of `atmos terraform apply` if the Terraform variable `foo` is set to the string `"foo"`.
...
The policy receives a structured input map containing metadata about the Atmos command being executed, such as:

 - `cli_command`: The main Atmos command (`terraform`)
 - `cli_subcommand`: The subcommand (`apply`)
 - `vars`: A map of variable names and their values passed to the command

Rule Logic:

The rule checks if:
 - The main command is `terraform`
 - The subcommand is `apply`
...
Atmos interprets the presence of any messages in `errors` as a policy violation and blocks the operation.
