...
On the other hand, when passing the `name` variable on the command line using the `-var name=api` flag, this command will succeed:
...
errors["for the 'my-component' component, the variable 'name' can't be overridden on the command line using the '-var' flag"] {
    input.cli_vars.name
}
```

When executing the following command, Atmos will validate the component using the OP policy, which will fail and prevent
the component from being provisioned:
...
for the 'my-component' component, the variable 'name' can't be overridden on the command line using the '-var' flag
```
