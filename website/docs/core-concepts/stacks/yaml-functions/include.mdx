...
injecting the contents of the referenced files directly into the current location. This applies to all Atmos configurations,
including CLI and stack configs. Included data (JSON, YAML, HCL, or text) is parsed and converted into the appropriate
...
This pattern allows you to plug Atmos into your existing Terraform root modules with minimal changes — no need to
