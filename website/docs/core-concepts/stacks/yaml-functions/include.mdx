...
- GitHub URLs, such as `github.com/cloudposse/atmos`, are automatically changed to Git protocol over HTTP
- GitLab URLs are automatically changed to Git protocol over HTTP
- BitBucket URLs are automatically changed to a Git or Mercurial protocol using the BitBucket API
...
Atmos uses [`go-getter`](https://github.com/hashicorp/go-getter)
(used by [Terraform](https://www.terraform.io/) for downloading modules) to download remote files from different sources.

### Downloading from OCI Registries

The following example shows how to include a file from a public AWS ECR registry:

  ```yaml
    components:
      terraform:
        my-component:
          vars: !include oci://public.ecr.aws/cloudposse/components/terraform/stable/aws/my-component.tfvars
  ```

### Downloading from SSH
...
:::tip
In automated systems like GitHub Actions, we recommend sticking with the `https://` scheme. Atmos will automatically inject the `GITHUB_TOKEN`.
:::
...
### Git over HTTPS Downloading

Atmos supports remote files downloading using **Git over HTTPS**.
