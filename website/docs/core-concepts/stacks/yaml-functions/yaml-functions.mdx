...
components:
  terraform:
    component2:
      vars:
        # Correctly handle the output of type list form the `atmos.Component` template function
        test_1: '!template {{ toJson (atmos.Component "component1" "plat-ue2-dev").outputs.test_list }}'

        # Correctly handle the output of type map form the `atmos.Component` template function
