...
As support for new backend types is added (e.g. `azurerm`), this document will be updated accordingly.

If you are using backends other than the supported ones, consider the [__`!store`__](/core-concepts/stacks/yaml-functions/store)
