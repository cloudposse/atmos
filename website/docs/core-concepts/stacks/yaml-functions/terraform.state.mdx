...
or [__`!terraform.output`__](/core-concepts/stacks/yaml-functions/terraform.output) YAML functions to read remote state
...
configuration layers (globals, environment and component-level settings).
...
- **Terraform State Backend Lookup**
Based on the resolved context, Atmos identifies the corresponding Terraform state backend configuration, including workspace and key prefix derived from backend settings.

- **Terraform Output Extraction**
Atmos locates the Terraform state file for the referenced component and stack—typically from an S3 bucket (for remote state backends)—and reads it directly without executing terraform output.

- **Output Parsing and Interpolation**
The relevant output variable is extracted from the JSON-formatted state file. Atmos parses and interpolates the value into the final configuration structure, replacing the !terraform.output directive.
