...
The `!template` function can be called with either two or three parameters:
...
:::note
Using the `.stack` or `.atmos_stack` template identifiers to specify the stack is the same as calling the `!template`
...
When the function is executed, Atmos detects that the `test` component has the `static` remote state configured,
