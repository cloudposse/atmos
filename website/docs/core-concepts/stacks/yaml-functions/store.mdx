...
The `!store` function can be called with either two, three, four or five parameters, and optionally a default value:

```yaml
  !store <store_name> <stack> <component> <key> <yq-expression> | default <default_value>
```
...
```yaml
  ...
  !store <store_name> <component> <key> <yq-expression>
  ...
  # Get the `key` from the store of a `component` in a different stack,
  # and retrieve an individual value from the result using the YQ expression
  !store <store_name> <stack> <component> <key> <yq-expression>

  # Get the `key` from the store of a `component` in a different stack, with a default value
  !store <store_name> <stack> <component> <key> | default <default_value>
```
