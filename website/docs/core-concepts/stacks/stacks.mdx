...
Stacks define the complete configuration of an environment. Think of a Stack like an architectural "Blueprint" composed
of one or more [Components](/core-concepts/components) configurations and defined using a [standardized YAML configuration](#schema).
