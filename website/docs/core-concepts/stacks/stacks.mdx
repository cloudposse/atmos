...
These are computed from `name_template` (new way) or the `name_pattern` (old way). These are configured in the `atmos.yaml` configuration file. The `name_template` is a Go template that can reference any section in the stack configuration of a component.
...
<details>
<summary>Old way using `name_pattern` (deprecated)</summary>

The `name_pattern` is the *original* way Atmos would determine the stack name.

It only supports the following optionated interpolations (`{name}`, `{stage}`, `{stage}`, `{namespace}`, `{environment}`) and therefore is not recommended. Use `name_template` instead.
...
:::tip
Find the complete schema for the Stack configuration in the [Stack Schema](/schemas/stack) reference section.
:::
