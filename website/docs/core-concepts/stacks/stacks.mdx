...
<dt>`stacks.name_template` (deprecated)</dt>
<dd>

  The `name_template` is the *legacy* method for determining stack names, retained for backward compatibility.
  ...
  The `name_template` is deprecated and will be removed in a future release. Please use `name_template` instead.
  :::
...
This computes the stack name from the `tenant`, `environment`, and `stage` variables in the stack configuration.

For example, with `tenant=acme`, `environment=ue2`, and `stage=dev`, the slug is `acme-ue2-dev`. Apply the `vpc` component in that stack:

```bash
atmos terraform apply vpc -s acme-ue2-dev
```
