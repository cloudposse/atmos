...
### Configure the Redis store in `atmos.yaml`
...
### Configure Atmos stack and component
...
at any position inside a string:
...
This is one of the key reasons to prefer using the `atmos.Store` template function over the
[`!store` YAML function](/core-concepts/stacks/yaml-functions/store).

The `!store` YAML function is generally recommended for its simpler, more readable syntax, and it doesn't involve `Go`
templates — which come with their own advantages and trade-offs.
