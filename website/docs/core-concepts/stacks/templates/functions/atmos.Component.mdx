...
### Using `toRawJson` or `toJson` Functions
...
### Using `!template` Atmos YAML Function

Instead of adjusting the template delimiters in `atmos.yaml`, you can use the [`!template`](/core-concepts/stacks/yaml-functions/template) Atmos YAML function.
Atmos executes the __`!template`__ YAML function, which correctly handles the JSON returned from the
...
components:
  terraform:
    component2:
      vars:
        # Handle the output of type list form the `atmos.Component` template function
        test_1: '!template {{ toJson (atmos.Component "component1" "plat-ue2-dev").outputs.test_list }}'

        # Handle the output of type map form the `atmos.Component` template function
