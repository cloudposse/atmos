...
### Skipping Components with `metadata.enabled`

The `metadata.enabled` parameter allows for conditional inclusion of components. By default, components are enabled. When `metadata.enabled` is set to `false`, the component is skipped during deployment, which is useful for disabling configurations without deletion.
...
This enables flexible deployment configurations, ensuring that only the intended components are active in each environment.
