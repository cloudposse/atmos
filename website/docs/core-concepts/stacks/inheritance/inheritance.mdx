...
| `metadata` | Yes | All fields except `metadata.inherits` |

:::info Metadata Inheritance
The `metadata` section is inherited from base components, with one exception: `metadata.inherits` itself is not inherited (since it defines the inheritance relationship). This enables patterns like defining `metadata.name` once in a base component for stable [workspace key prefixes](/core-concepts/components/terraform/backends).
