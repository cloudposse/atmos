...
* `max_attempts`: The maximum number of times the command will be retried. The default is `3`.
...
* `random_jitter`: Whether to add a random jitter to the delay between retries. The default is `true`.
    ...
    steps: 
      - command: terraform apply vpc -auto-approve
        retry:
          ...
          random_jitter: false # the random jitter to be added while retrying.
