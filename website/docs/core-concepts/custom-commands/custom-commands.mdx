...
Atmos supports trailing arguments. This aregument is the one that is added by the user after `--`. 
...
- name: echo
  ...
  steps:
    - "echo Before Trailing Dash: {{ .Arguments.name }}"
    - "echo After Trailing Dash: {{ .TrailingArgs }}"
```
Output:

```bash
$ atmos echo Sam -- trailing dash
Before Trailing Dash: Sam
After Trailing Dash: trailing dash
```
