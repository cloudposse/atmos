...
<Intro>
Atmos ships 20 agent skills that give AI coding assistants deep, accurate knowledge of Atmos conventions, stack
...
<KeyPoints>
- 20 domain-specific skills covering every Atmos subsystem
...
Atmos includes 20 skills organized into five categories:
...
Skills are organized into five plugin categories, each distributable as a Claude Code plugin:

```text
agent-skills/                                  # Plugin-compatible hierarchy
  ...
  configuration/                               # Plugin: atmos-configuration
    ...
    skills/
      ...
      atmos-components/                        # + atmos-vendoring, atmos-schemas, atmos-introspection
        SKILL.md
        references/
  orchestration/                               # Plugin: atmos-orchestration
    .claude-plugin/plugin.json
    skills/
      atmos-terraform/                         # + atmos-helmfile, atmos-packer, atmos-ansible,
        SKILL.md                               #   atmos-workflows, atmos-custom-commands
        references/
  platform/                                    # Plugin: atmos-platform
    .claude-plugin/plugin.json
    skills/                                    # atmos-auth, atmos-stores, atmos-toolchain
  integrations/                                # Plugin: atmos-integrations
    .claude-plugin/plugin.json
    skills/                                    # atmos-gitops, atmos-validation, atmos-templates, atmos-devcontainer
  guidance/                                    # Plugin: atmos-guidance
    .claude-plugin/plugin.json
    skills/                                    # atmos-design-patterns

.claude/skills/                                # Flat view for auto-discovery
...
The two directories serve different purposes. `agent-skills/` uses a category-based hierarchy required by the
Claude Code plugin system and other AI tools. `.claude/skills/` provides a flat view via symlinks so Claude Code
can auto-discover skills at `.claude/skills/<skill-name>/SKILL.md` when working directly in the Atmos repo.
Both paths resolve to the same 20 `SKILL.md` files.
...
Skills are available immediately after installation and persist across sessions.
...
install a plugin, Claude Code downloads the plugin directory and caches it locally. No central registry or
...
| Plugin | Skills |
|--------|--------|
| `atmos-configuration` | atmos-config, atmos-stacks, atmos-components, atmos-vendoring, atmos-schemas, atmos-introspection |
| `atmos-orchestration` | atmos-terraform, atmos-helmfile, atmos-packer, atmos-ansible, atmos-workflows, atmos-custom-commands |
| `atmos-platform` | atmos-auth, atmos-stores, atmos-toolchain |
| `atmos-integrations` | atmos-gitops, atmos-validation, atmos-templates, atmos-devcontainer |
| `atmos-guidance` | atmos-design-patterns |
...
```bash
/plugin install atmos-configuration@claude-plugin-directory
```
...
`.claude/skills` symlink -- no plugin installation needed.
...
project's `.claude/settings.json` (see [Team Auto-Discovery](#using-skills-with-ai-tools)).

### Other AI Tools

Gemini CLI, OpenAI Codex, Cursor, Windsurf, and GitHub Copilot do not have third-party marketplace support
for Atmos skills (Cursor has a curated marketplace but requires separate submission). For these tools, copy
the skills from the Atmos GitHub repository into your project:

```bash
# Download just the agent-skills directory (no need to clone the full repo)
...
:::warning Keep the directory structure intact
Do not flatten the downloaded `agent-skills/` directory. The `AGENTS.md` router references skills using
relative paths like `agent-skills/configuration/skills/atmos-config/SKILL.md`. Restructuring or flattening the
directory will break these references.
:::

Then configure your AI tool to reference the skills (see the tool-specific tabs above).
...
1. Follow the existing folder structure: `agent-skills/<category>/skills/<skill-name>/SKILL.md`
