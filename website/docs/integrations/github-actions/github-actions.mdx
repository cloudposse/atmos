...
:::tip Native CI Integration Available
Atmos now includes **native CI integration** that provides job summaries, status checks, and CI output variables without requiring separate GitHub Actions. Just run `atmos terraform plan` in your workflow and get rich CI artifacts automatically.

**[Learn more about Native CI Integration](/cli/configuration/ci)**

The GitHub Actions listed below remain fully supported for teams that prefer the action-based approach or need features like DynamoDB-backed planfile storage.
:::
...
For example, [vendor in](/vendor/) the [`s3-component`](https://docs.cloudposse.com/components/library/aws/s3-bucket/), then using an [Atmos stack configuration](/learn/stacks/), define a bucket using the [`s3-bucket` component](https://github.com/cloudposse-terraform-components/aws-s3-bucket) with this catalog configuration:
