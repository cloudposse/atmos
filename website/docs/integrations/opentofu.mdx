...
<img width="250" align="right" src={useBaseUrl('/img/opentofu-icon.svg')} /> Atmos natively supports [OpenTofu](https://opentofu.org), similar to the way it supports [Terraform](/integrations/terraform).
It's compatible with every version of `opentofu` and designed to work with multiple different versions of it concurrently,
and can even work along side with [HashiCorp Terraform](/integrations/terraform). 
...
only on what's different in this document, in order to utilize OpenTofu.
...
All of the default configuration settings to support OpenTofu are defined in the [Atmos CLI Configuration](/cli/configuration),
...
Just add the following configuration somewhere in the `atmos.yaml` configuration.
...
specific `command` set in either the `atmos.yaml` or the Stack configurations.
:::
...
For example, to provision a Terraform component using OpenTofu with the `atmos` CLI, run the following commands in the container shell:
