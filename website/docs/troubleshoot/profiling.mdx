...
<dl>
    ...
    <dt>`Total`</dt>
    <dd>Total wall-clock time spent in all calls, including time spent in child function calls (where to focus optimization)</dd>

    <dt>`Avg`</dt>
    <dd>Average self-time per call - actual work done in the function, excluding time spent in child function calls (SelfTime ÷ Count). This metric accurately represents the function's own execution time.</dd>

    <dt>`Max`</dt>
    <dd>Maximum wall-clock execution time for a single call</dd>

    <dt>`P95`</dt>
    <dd>95th percentile of self-time latency (consistent performance indicator for the function's own work)</dd>
</dl>

:::tip Understanding Total vs Self-Time

**Total Time (Wall-Clock)**: Includes the entire execution time from function entry to exit, including all time spent in child function calls. This is useful for identifying functions that contribute most to overall execution time.

**Self-Time (Avg & P95)**: Excludes time spent in child functions, showing only the actual work done in the function itself. This is more accurate for understanding where a function's own performance can be optimized.

**Example**: If `ProcessConfig` takes 100ms total but 80ms is spent calling child functions, the Total would be 100ms but the Avg self-time would reflect only the 20ms of actual work in `ProcessConfig` itself.
