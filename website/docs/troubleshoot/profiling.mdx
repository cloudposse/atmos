...
- Sortable columns (in interactive mode)
...
<dl>
    ...
    <dt>`Total`</dt>
    <dd>Total time spent in all calls (where to focus optimization)</dd>
    ...
    <dt>`P95`</dt>
    <dd>95th percentile latency (consistent performance indicator)</dd>
</dl>
...
<dl>
    <dt>`--heatmap`</dt>
    <dd>Enable performance heatmap visualization</dd>

    <dt>`--heatmap-mode`</dt>
    <dd>Visualization mode: `bar`, `sparkline`, or `table` (default: `bar`)</dd>
</dl>
...
<dl>
    ...
    <dt>`--profile-type`</dt>
    <dd>Type of profile to collect: `cpu`, `heap`, `allocs`, `goroutine`, `block`, `mutex`, `threadcreate`, `trace` (default: `cpu`)</dd>
