...
<Intro>
    Atmos includes a built-in performance profiling that provides real-time heatmap visualization of function execution times and call statistics.
    This lightweight profiling feature helps you identify where performance bottlenecks might be causing Atmos to run slow.
</Intro>
...
This will launch an interactive TUI (Terminal User Interface) where you can switch between different visualization modes by pressing 1-3.
...
<dl>
    ...
    <dt>`--heatmap-mode`</dt>
    <dd>Visualization mode: `bar`, `sparkline`, or `table` (default: `bar`). In interactive mode, press 1-3 to switch modes.</dd>
</dl>
...
In interactive mode (when a TTY is available), you can switch between modes by pressing 1-3:

1. **Bar Chart** - Horizontal bars showing relative execution times with color gradient
2. **Sparklines** - Compact sparkline charts for quick overview
3. **Table** - Detailed tabular view with all metrics
...
<Terminal title="Sparkline visualization">
    ...
    Press 1-3 to switch modes | q to quit
    ```
</Terminal>
...
- **bar**: Best for identifying relative performance differences with color-coded gradient
