...
Enable performance tracking and display the heatmap after command execution:

```shell
# Set environment variable to enable performance tracking
export ATMOS_PROF=1
...
Performance tracking is controlled by two environment variables:

<dl>
    <dt>`ATMOS_PROF`</dt>
    <dd>Enable performance tracking (set to `1` to enable)</dd>

    <dt>`ATMOS_PROF_HDR`</dt>
    <dd>Enable HDR histogram for percentile calculations like P95 (set to `1` to enable, requires `ATMOS_PROF=1`)</dd>
</dl>

**Basic tracking (count, total, avg, max):**
```bash
export ATMOS_PROF=1
atmos terraform plan vpc -s dev --heatmap
```

**Advanced tracking with percentiles (P95):**
```bash
export ATMOS_PROF=1
export ATMOS_PROF_HDR=1
atmos terraform plan vpc -s dev --heatmap
```
...
<dl>
    <dt>`--heatmap`</dt>
    <dd>Display performance heatmap visualization after command execution (requires `ATMOS_PROF=1`)</dd>
...
<dl>
    ...
    <dt>`P95`</dt>
    <dd>95th percentile latency (only shown when `ATMOS_PROF_HDR=1`)</dd>
</dl>
...
export ATMOS_PROF=1
atmos describe stacks --heatmap > baseline.txt

# After optimization
atmos describe stacks --heatmap > optimized.txt
...
# Run commands and capture metrics
...
```bash
# Enable in your shell profile for persistent tracking
echo 'export ATMOS_PROF=1' >> ~/.bashrc
echo 'export ATMOS_PROF_HDR=1' >> ~/.bashrc

# Run commands with heatmap during development
atmos validate stacks --heatmap
atmos describe component vpc -s dev --heatmap
```
...
**Solution**: Ensure `ATMOS_PROF=1` is set:
```bash
export ATMOS_PROF=1
...
**Solution**: Enable HDR histogram tracking:
```bash
export ATMOS_PROF=1
export ATMOS_PROF_HDR=1  # Required for P95 calculations
atmos describe stacks --heatmap
```
...
# Analyze heatmap during command
# Then analyze pprof data after completion
...
Only enable performance tracking when needed:

```bash
# Development: Enable in current shell
export ATMOS_PROF=1

# Production: Enable for specific commands only
ATMOS_PROF=1 atmos terraform plan vpc -s prod --heatmap

# CI/CD: Enable conditionally
