...
<Intro>
    ...
    When you design cloud architectures with Atmos, you will first break them apart into pieces called components. Then, you will implement Terraform "root modules" for each of those components. Finally, compose your components in any way you like using stacks, without need to write any code or messy templates.
</Intro>
...
For example, let’s take a typical architecture of a modern website. Its components might include a network, a cluster, some applications running on it, a database to store data, a cache, and maybe some object storage.
...
When place all the infrastructure into a single "root module" we end up with a [Terralith](/terms/terralith), which is roughly [Stage-2 of the Terraform maturity path](/introduction/why-atmos/stage-2). All your eggs are in one basket.
...
<Step>
    ...
    <ActionCard>
        Projects are organized into a well-structured folder layout, embracing best practices and conventions for a consistentl organized project.
...
<Step>
    ...
    For each of the components in your architecture, write a Terraform root module.

    Use your existing Terraform root modules or create new ones. As your component library grows, you’ll need to write fewer new components, enabling faster development cycles through reuse of infrastructure components across projects.

    Try to reuse 80% of your components across projects, and only write new ones when you have to.
...
<Step>
    ...
    Use Terraform's native ability to read remote state from any other component, for a "loosely coupled" architecture.
    Atmos provides [methods to make working with remote state easier](/core-concepts/components/terraform/remote-state).
    ...
    <ActionCard>
        ...
        <PrimaryCTA to="/core-concepts/components">Learn Remote State</PrimaryCTA>
    </ActionCard>
</Step>
...
<Step>
    ...
    Configure your environments—development, staging, production—each tailored to different stages of the lifecycle, ensuring smooth transitions and robust deployment strategies. Use imports to keep your configurations DRY and enforce consistency across your stacks.
...
<Step>
    ...
    Execute deployments with precision using Terraform's `plan` and `apply` commands, fully integrated with [native GitOps workflows](/integrations) through [GitHub Actions](/integrations/github-actions) for seamless deployment automation.
    ...
    <ActionCard>
        ...
        <PrimaryCTA to="/core-concepts/stacks">Learn Deployment</PrimaryCTA>
    </ActionCard>
</Step>
