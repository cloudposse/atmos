...
Then we can deploy both stations in the `dev` stack with the same bulk command you used in the main Quick Start:

```shell
atmos terraform deploy --all -s dev
```

<Note>Use a single-component command, such as `atmos terraform plan station/1 -s dev`, when you want to preview or operate on one specific instance.</Note>
