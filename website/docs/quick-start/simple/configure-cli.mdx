...
Therefore, this file should exist in the project repository alongside your Terraform components and Atmos stacks. It's also where you can [configure integrations](/cli/configuration/integrations), like with [native CI for GitHub Actions](/ci).
...
<CastPlayer title="atmos help" src="/casts/screengrabs/atmos-version.cast" static chrome />
...
<dl>
  ...
  <dd>
    Atmos uses “slugs” to refer to stacks, which are defined by the `name_template` setting.<br/>
    ...
    Atmos matches the slug `dev` by evaluating the template `{{ .vars.stage }}` against stack configuration
