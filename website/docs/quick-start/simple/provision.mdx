...
- Atmos uses the `stacks.name_template` defined in the [CLI config](/quick-start/advanced/configure-cli). In this example, we have defined a simple name based on `stacks.name_template: "{stage}"`. This means that the stack name is just the `stage` part of the stack name.
