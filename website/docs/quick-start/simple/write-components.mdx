...
Design them to serve a single purpose, making them the smallest possible unit of infrastructure in a typical software development lifecycle (SDLC). Group pieces usually change together and separate those that change independently.

We discuss some of the [best practices for components](/best-practices/components) in our documentation.

Since our goal is to retrieve the current weather, let's create a component that will do just that.
