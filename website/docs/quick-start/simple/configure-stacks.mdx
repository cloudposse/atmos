...
For example, when executing the `atmos terraform deploy --all -s dev` command, the Atmos stack `dev` is specified by the `-s` flag. Atmos evaluates `name_template: "{{ .vars.stage }}"` (see [Configure CLI](/quick-start/advanced/configure-project)) for each stack and finds the stack whose `stage` renders to `dev`.
...
components:
  terraform:
    station:
      metadata:
        component: weather
        description: "Weather station component"
```
