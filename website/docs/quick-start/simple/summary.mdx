...
- Stacks define the configuration of your "root modules", and can have arbitrary names and be located anywhere in the `stacks/` directory; Atmos finds stacks using a "slug" which is based on context variables like `namespace`, `tenant`, `environment`, and `stage`.
