...
### Configure Tenant, Region and Stage Mixins
...
Add `stacks/mixins/stage/prod.yaml` and `stacks/mixins/stage/staging.yaml` the same way, setting `stage: prod` and `stage: staging` respectively.

As we can see, the tenant, region and stage mixins define the global context variables `tenant`, `environment` and `stage`, which Atmos uses when
searching for a component in a stack. These mixins then get imported into the parent Atmos stacks, so we don't repeat the context variables in every
top-level stack — keeping the configuration DRY. This is the [multi-region](/design-patterns/stack-organization/multi-region-configuration) modeling
pattern.
...
In `stacks/orgs/acme/plat/_defaults.yaml`, configure the `plat` OU (tenant). This is also where the whole backend catalog is brought in, by importing
`catalog/backend`:
...
In `stacks/orgs/acme/plat/dev/_defaults.yaml`, configure the `dev` account:
...
Add the `prod` and `staging` account defaults the same way, importing `mixins/stage/prod` and `mixins/stage/staging` respectively.
...
The catalog defaults can be overridden in the derived Atmos components per stage or region using
[Component Inheritance](/design-patterns/inheritance-patterns/component-inheritance). For example, you could import `catalog/s3-bucket/defaults` again
in a `prod` stack and override `versioning_enabled` — which is exactly the kind of difference the [OPA policy](/quick-start/advanced/configure-validation)
enforces for `prod`.
:::
