...
- To be able to use [Component Remote State](/connections/remote-state), we put the `atmos.yaml` CLI config file
...
<dl>
  ...
  <dt>`schemas`</dt>
  <dd>See [JSON Schema](https://json-schema.org/) and [OPA Policy](https://www.openpolicyagent.org/) configurations for: [Atmos Schema Validation](/cli/schemas), [Atmos Custom Validation](/validation/validating).</dd>
</dl>
