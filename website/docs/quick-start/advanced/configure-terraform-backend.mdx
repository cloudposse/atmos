...
sidebar_position: 12
sidebar_label: "Terraform State Backend"
---
import File from '@site/src/components/File'

:::tip No real AWS account needed
This example stores Terraform state in an **S3 backend** — the same backend you'd use in production —
but runs it entirely against the local **AWS emulator**. Atmos **provisions the state bucket
automatically inside the emulator** (`provision.backend.enabled: true`) and **generates a
`backend.tf.json` per component** (`auto_generate_backend_file: true` in
[`atmos.yaml`](/quick-start/advanced/configure-project)). The `local-aws` identity
(`kind: aws/emulator`) injects the emulator endpoint and credentials into OpenTofu, so the **exact
same configuration** works unchanged against real S3 when you graduate to live AWS accounts.
:::

The whole point of this advanced example is to exercise the **Atmos-managed S3 state backend**, not a
local one. You get production-grade remote state — remote storage, native S3 locking, versioning —
with zero cloud credentials, because Atmos provisions and targets the bucket in the running emulator.
This page explains how that backend is configured and what changes (only bucket names and IAM roles)
when you point it at real AWS for shared staging and production environments.

For background, Terraform also ships a [local](https://developer.hashicorp.com/terraform/language/settings/backends/local)
backend that keeps state on the local filesystem. It's fine for a quick single-developer scratchpad,
but this example deliberately does **not** use it — the sections below cover why, and how the
[s3](https://developer.hashicorp.com/terraform/language/settings/backends/s3) backend is wired up
instead.
...
Before using a Terraform S3 backend, the backend S3 bucket needs to exist.

:::tip This example already does this for you
This tutorial uses **Atmos's built-in backend provisioning** (`provision.backend.enabled: true`) — Atmos creates the state bucket with secure defaults
(versioning, encryption, public-access block) **inside the local emulator**, using the component's own `local-aws` identity. No bootstrap module, no
extra component, no real AWS account. See [Backend Provisioning](/stacks/components/provision/backend). This is the recommended path for real AWS too.
:::

If you'd rather manage the backend bucket yourself (entirely optional, and **not** required by this example or by Atmos), any approach works — for
example the [tfstate-backend](https://github.com/cloudposse/terraform-aws-tfstate-backend) Terraform module, your own bucket Terraform, or a one-off
`aws s3 mb`. As one illustration, the [tfstate-backend](https://github.com/cloudposse/terraform-aws-components/tree/main/modules/tfstate-backend) component
...
- Manually create `backend.tf` file in each component's folder with the following content:
  ...
  terraform {
    backend "s3" {
      ...
      workspace_key_prefix = "<component name, e.g. `s3-bucket` or `app-config`>"
      use_lockfile         = true
    }
  }
  ```
...
- Set `auto_generate_backend_file` to `true` in the `atmos.yaml` CLI config file in the `components.terraform` section:
  ...
  Refer to [Quick Start: Configure CLI](/quick-start/advanced/configure-project) and [CLI Configuration](/cli/configuration) for more details.
...
- Configure the S3 backend in one of the `_defaults.yaml` manifests. You can configure it for the entire Organization, or per OU/tenant, or per
  ...
  :::note About `_defaults.yaml`
  The `_defaults.yaml` stack manifests hold the default settings for Organizations, Organizational Units, and accounts. They are **not** imported automatically — you import them explicitly with [imports](/stacks/imports).
  :::
...
- (This step is optional) For each component, you can add `workspace_key_prefix` similar to the following:
  <File title="stacks/catalog/s3-bucket/defaults.yaml" type="stack">
  ```yaml
  components:
    terraform:
      # `s3-bucket` is the Atmos component name
      s3-bucket:
  ...
  automatically (which, in this example, is `s3-bucket`). `/` (slash) in the Atmos component name will be replaced with `-` (dash).
...
Once all the above is configured, when you run the commands `atmos terraform plan s3-bucket -s <stack>`
or `atmos terraform apply s3-bucket -s <stack>`, before executing the Terraform commands, Atmos will deep-merge the backend configurations from
...
When you provision the `s3-bucket` component into the `dev` account (by executing the command `atmos terraform apply s3-bucket -s plat-ue2-dev`), Atmos will
...
add `workspace_key_prefix` for the component, generating the following final deep-merged backend config for the `s3-bucket` component in the `dev` account:
...
For example, the following config shows how to define two Atmos components, `s3-bucket/1` and `s3-bucket/2`, which both point to
the same Terraform component `s3-bucket`:
  ...
  terraform:
    ...
    s3-bucket/1:
      metadata:
        ...
        inherits:
          - s3-bucket/defaults
      # Define variables specific to this `s3-bucket/1` component
      vars:
        name: assets-1
      # Optional backend configuration for the component
      # If not specified, the Atmos component name `s3-bucket/1` will be used (`/` will be replaced with `-`)
      backend:
        s3:
          workspace_key_prefix: s3-bucket-1
    ...
    s3-bucket/2:
      metadata:
        ...
        inherits:
          - s3-bucket/defaults
      # Define variables specific to this `s3-bucket/2` component
      vars:
        name: assets-2
      # Optional backend configuration for the component
      # If not specified, the Atmos component name `s3-bucket/2` will be used (`/` will be replaced with `-`)
      backend:
        s3:
          workspace_key_prefix: s3-bucket-2
```
</File>

If we manually create a `backend.tf` file for the `s3-bucket` Terraform component in the `components/terraform/s3-bucket` folder
using `workspace_key_prefix: "s3-bucket"`, then both `s3-bucket/1` and `s3-bucket/2` Atmos components will use the same `workspace_key_prefix`, and they will
...
For example, when the command `atmos terraform apply s3-bucket/1 -s plat-ue2-dev` is executed, the following `backend.tf.json` file is generated in the
`components/terraform/s3-bucket` folder:
...
Similarly, when the command `atmos terraform apply s3-bucket/2 -s plat-ue2-dev` is executed, the following `backend.tf.json` file is generated in the
`components/terraform/s3-bucket` folder:
