...
<Tabs>
  ...
  <TabItem value="deb" label="Debian/Ubuntu">
    ...
    # Install atmos
    sudo apt-get install atmos@="${ATMOS_VERSION}-*"
    ```
  </TabItem>
...
<Tabs>
  <TabItem value="native" label="Native Version Manager" default>
    ...
    **Precedence order:** `--use-version` flag / `ATMOS_VERSION_USE` > `ATMOS_VERSION` > `version.use` in config file
    ...
    __NOTE:__ You need an initial installation of Atmos to bootstrap the version manager. Use any installation method above to get started.
  </TabItem>
...
<ActionCard
  title="Choose a Quick Start"
  primaryCtaText="Simple Tutorial"
  primaryCtaLink="/quick-start/simple"
  secondaryCtaText="Advanced Tutorial"
  secondaryCtaLink="/quick-start/advanced">
