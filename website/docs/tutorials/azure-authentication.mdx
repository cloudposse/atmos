...
description: Complete guide for authenticating to Azure using Atmos Auth with interactive browser flow, device code flow, OIDC, and service principals.
---
...
- **Interactive Browser Flow** (recommended for users; the same flow as `az login`)
- **Device Code Flow** (fallback for environments where a browser can't be opened)
...
### Basic Azure Interactive Browser Authentication

The simplest Azure authentication uses the interactive browser flow for user login — the same flow `az login` uses:
...
This will:
1. Open your default browser to Microsoft's sign-in page (authorization code + PKCE)
2. Prompt you to sign in (with MFA if configured)
3. Cache credentials for all Azure providers (azurerm, azuread, azapi)

## Authentication Methods

### Interactive Browser Flow (Recommended for Users)

Best for: **Developer workstations — the default choice for human logins**

The interactive browser flow performs the exact authentication `az login` does: an authorization code + PKCE sign-in in your default browser, with a localhost redirect. Because the browser session carries full Conditional Access context, it works in tenants where the device code flow is blocked:
...
**How it works:**
1. Atmos first tries silent acquisition from the persisted MSAL cache — repeat logins are silent (no browser)
2. If no cached session exists, your default browser opens for sign-in (with MFA if configured)
3. Atmos acquires tokens for Azure Resource Manager, Microsoft Graph, and Azure Key Vault
4. Atmos writes the Azure CLI-compatible cache files, so the `az` CLI works without ever running `az login`

**Usage:**
```bash
# Authenticate interactively (one command — no az login needed)
...
:::note
The interactive browser flow requires an interactive terminal and a browser. For headless environments and CI/CD, use [OIDC](#oidc-workload-identity-for-cicd); when a browser can't be opened (e.g., SSH sessions), use the [device code flow](#device-code-flow-fallback).
:::

### Device Code Flow (Fallback)

Best for: **Environments where a browser can't be opened on the machine running Atmos (e.g., SSH sessions)**

:::warning Conditional Access may block device code
Microsoft-managed Conditional Access policies now block the device code flow in many tenants (error `AADSTS530035`). If sign-in fails with this error, use the [interactive browser flow](#interactive-browser-flow-recommended-for-users) instead.
:::
...
1. **Provider Authentication**: Atmos authenticates using configured method (interactive browser/device code/OIDC/service principal)
...
Use interactive browser login for developers, OIDC for pipelines:
...
The `cloud_environment` setting applies to all Azure provider kinds: `azure/interactive`, `azure/device-code`, `azure/oidc`, and `azure/cli`.
...
- Configure [authentication for workflow steps](/workflows/steps/identity)
- Set up [authentication for custom commands](/cli/configuration/commands/identity#using-authentication-with-custom-commands)
