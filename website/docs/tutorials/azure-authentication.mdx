...
Azure sovereign clouds (US Government / GCC High, China / Mooncake) use different authentication endpoints, API scopes, and storage URLs than Azure Commercial. Atmos supports all Azure clouds through the `cloud_environment` provider setting.
...
Atmos uses this `environment` field to resolve the correct blob storage endpoint when reading Terraform state files.
