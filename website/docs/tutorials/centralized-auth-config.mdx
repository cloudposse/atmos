...
```bash
# Option A: isolated subshell, credentials auto-cleaned on exit
atmos auth shell
...
Install Atmos once; use it on every repo in the organization. No other setup required: clone a repo, run
...
Either way, updates are centrally managed: add an account or rotate a role once in the central repo, tag
a release, and every project that imports it picks it up — no re-broadcast, no per-developer drift, no
stale access lingering after revocation.
