...
this guide (the import, private-repo access, the `atmos auth` workflow) is identical no matter which tab
you're in:
...
This import is re-resolved (a fresh shallow clone) on every single `atmos` command in the project, not
just auth commands — there's no local cache for this form of import. That means every command now
depends on network reachability to the central repo; a transient outage there will affect this project
too.
...
### 3. Private repo access — one token, set once
...
**`gh auth login` by itself is not enough here.** That authenticates the `gh` CLI, but this import goes
through a plain `git clone`, which `gh auth login` doesn't wire up on its own. Two ways to make it work:

- Export `GITHUB_TOKEN` (or `ATMOS_GITHUB_TOKEN`) in your shell profile, pointing at a token with read
  access to the central repo — the most explicit option, and the one Atmos itself checks directly.
- Or run `gh auth setup-git` once. This configures Git's own credential helper to use your `gh` session,
  so a plain `git clone` (and therefore this import) picks it up — no Atmos-specific token needed, but
  it's `gh` wiring up Git, not Atmos reading `gh`'s token.

Either is a one-time setup step per developer, not per project.
...
setting up Git access once — a `GITHUB_TOKEN`/`ATMOS_GITHUB_TOKEN`, or `gh auth setup-git` (see
[step 3](#3-private-repo-access--one-token-set-once) above; plain `gh auth login` alone isn't enough).
From there: clone a repo, run `atmos auth login`, then `atmos auth shell` or `atmos auth exec` — no
profile block to copy, no account ID to transcribe.
...
**That debug line shows an authentication error** — confirm `ATMOS_GITHUB_TOKEN`/`GITHUB_TOKEN` is set,
or that `gh auth setup-git` has been run (`gh auth status` alone is not enough — see
[step 3](#3-private-repo-access--one-token-set-once)), and that the token's account has read access to
the central config repo.
