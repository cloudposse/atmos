...
**That debug line shows an authentication error** — confirm `ATMOS_PRO_GITHUB_TOKEN` is set if you use
Atmos Pro's GitHub integration (it's set automatically in that case), otherwise that
`ATMOS_GITHUB_TOKEN`/`GITHUB_TOKEN` is set, or that `gh auth setup-git` has been run (`gh auth status`
alone is not enough — see [step 3](#3-private-repo-access--one-token-set-once)), and that the token's
