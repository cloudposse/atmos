...
- **Isolated credentials**: Docker config stored separately from your default `~/.docker/config.json`
...
ECR credentials are stored in `~/.config/atmos/docker/config.json` (following XDG conventions). This location:

- Respects `XDG_CONFIG_HOME` and `ATMOS_XDG_CONFIG_HOME` environment variables
- Is isolated from your default `~/.docker/config.json`
- Has restricted permissions (0600)

### Docker Integration

Atmos sets the `DOCKER_CONFIG` environment variable to point Docker to the Atmos-managed config:

```bash
# After ECR login
echo $DOCKER_CONFIG
# /home/user/.config/atmos/docker
```
...
Verify `DOCKER_CONFIG` points to Atmos config:
