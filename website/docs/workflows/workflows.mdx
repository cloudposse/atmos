...
Atmos supports 20+ step types for building interactive CLI wizards. Use `input`, `confirm`, `choose`, and `filter` to collect user input, `markdown` and `success`/`info`/`warn`/`error` for rich output, and `spin` for progress spinners. See the [Step Types](#step-types) section below for complete documentation.

:::

## Step Types

Atmos workflows support 20+ step types across four categories: Command, Interactive, UI Messages, and Output. These step types enable building sophisticated CLI wizards and interactive deployment pipelines.
...
Joins array values into a string with a separator.
...
steps:
  - name: formatted
    type: join
    separator: ", "
    values: "{{ .steps.regions.values }}"
```

<dl>
  <dt>`separator`</dt>
  <dd>String to join values with</dd>

  <dt>`values`</dt>
  <dd>Array of values to join (use template to reference step output)</dd>
</dl>
