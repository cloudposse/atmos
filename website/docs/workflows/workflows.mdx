...
<dl>
  ...
  <dt>`conditions`</dt>
  <dd>List of regular expressions identifying which failures are retryable. For the [`webhook`](#webhook) step type, patterns are matched against `"<status-code> <response-body>"`, letting you retry responses that would otherwise fail fast (e.g., a specific `4xx`). Omit to use each step type's default retry classification.</dd>
</dl>
...
<dl>
  ...
  <dt>`type`</dt>
  <dd>The type of the step. Can be `atmos`, `shell`, `exec`, or any of the [interactive step types](#step-types) like `input`, `confirm`, `choose`, `filter`, `markdown`, `spin`, and more. Type `atmos` is implicit for command steps. Type `shell` is required for shell scripts. Type `exec` replaces the Atmos process entirely (shell `exec` semantics) and must be the final step — see [Replacing the Atmos Process](/cli/configuration/commands#replacing-the-atmos-process-type-exec). See the [Step Types](#step-types) section for all available types</dd>
