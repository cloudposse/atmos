...
import StepTypes from './_partials/_step-types.mdx'
...
Workflows can run shell steps inside a long-lived container-backed sandbox. This is similar to a GitHub Actions job container: Atmos creates one container for the workflow run, mounts the workflow working directory into it, executes eligible `shell` steps with `docker exec` or `podman exec`, and removes the container when the workflow exits.

```yaml title=stacks/workflows/containerized.yaml
workflows:
  test:
    description: Run commands inside a workflow container
    working_directory: .
    container:
      image: alpine:latest
      shell: /bin/sh
...
<dl>
  ...
  <dd>Container image used for the workflow sandbox. Required when `container` is configured.</dd>

  <dt>`container.provider`</dt>
  <dd>Container runtime provider: `docker` or `podman`. When omitted, Atmos auto-detects the runtime (Docker, then Podman). Defaults globally from the top-level `container.runtime.provider`.</dd>

  <dt>`container.runtime_auto_start`</dt>
  <dd>When `true`, allow Atmos to attempt runtime recovery, such as starting a Podman machine. Defaults globally from the top-level `container.runtime.auto_start` setting (or the `ATMOS_CONTAINER_RUNTIME_AUTO_START` env var).</dd>

  <dt>`container.pull`</dt>
  <dd>Image pull policy: `missing` (default), `always`, or `never`.</dd>

  <dt>`container.workspace`</dt>
  <dd>Path inside the container where the workflow working directory is mounted. Defaults to `/workspace`.</dd>

  <dt>`container.workspace_read_only`</dt>
  <dd>Mount the workflow workspace read-only when set to `true`.</dd>

  <dt>`container.cleanup`</dt>
  <dd>Sandbox cleanup policy: `always` (default), `on_success`, or `never`. Atmos uses unique container names and labels so reruns are not blocked by orphaned sandboxes from crashed or killed runs.</dd>

  <dt>`container.env`</dt>
  <dd>Environment defaults passed to commands in the sandbox. Workflow and step `env` values are also passed to container exec, and step `env` overrides workflow values for the same key.</dd>

  <dt>`container.mounts`</dt>
  <dd>Additional mounts using `type`, `source`, `target`, and optional `read_only` fields.</dd>

  <dt>`container.ports`</dt>
  <dd>Port mappings using `host`, `container`, and optional `protocol` fields.</dd>

  <dt>`container.shell`</dt>
  <dd>Shell used to execute commands inside the container. For example: `/bin/sh`, `/bin/bash`.</dd>

  <dt>`container.user`</dt>
  <dd>User context for container execution. Accepts a username or `UID:GID` format.</dd>

  <dt>`container.run_args`</dt>
  <dd>Additional arguments passed to the container runtime's run command.</dd>
</dl>

### Step Behavior in Workflow Containers

- `shell` steps run inside the workflow container by default.
- Set `container: false` on a step to run it on the host.
- Set a step-level `container:` block to run that shell step in its own one-shot container override.
- Existing `type: container` build, push, and run steps keep their explicit container-step behavior and do not inherit the workflow container.
- Interactive and TTY shell steps are supported with the same `interactive` and `tty` fields.

### Working Directory Mapping

`working_directory` remains a host-side Atmos path in workflow configuration. Atmos resolves it on the host, verifies it is inside the mounted workflow workspace, then maps it to the corresponding container path.

For example, with `container.workspace: /workspace`, a step `working_directory: services/api` runs inside the container at `/workspace/services/api`.

### Sandboxed Execution

Workflow containers provide container-backed sandboxed execution for developer workflows. Atmos relies on Docker or Podman isolation and does not add a stronger security boundary beyond the configured container runtime.
...
<dl>
  <dt>`max_attempts`</dt>
  <dd>Maximum number of retry attempts. Omit for unlimited retries. Must be greater than zero if specified.</dd>

  <dt>`max_elapsed_time`</dt>
  <dd>Total time budget for all attempts combined (e.g., `10m`, `1h`). This is the cumulative time across all retry attempts plus backoff delays, not a per-attempt timeout. Omit for no time limit. Must be greater than zero if specified.</dd>

  <dt>`backoff_strategy`</dt>
  <dd>The backoff strategy between retries: `constant`, `linear`, or `exponential`. Default: `constant`.</dd>

  <dt>`initial_delay`</dt>
  <dd>Delay before the first retry (e.g., `5s`, `100ms`). Omit for no delay. Cannot be negative.</dd>

  <dt>`max_delay`</dt>
  <dd>Maximum delay between retries, caps exponential/linear growth (e.g., `30s`). Omit for no cap. Must be greater than zero if specified.</dd>

  <dt>`multiplier`</dt>
  <dd>Multiplier for exponential or linear backoff (e.g., `2.0`). Omit to use the default multiplier (2.0). Must be greater than zero if specified.</dd>

  <dt>`random_jitter`</dt>
  <dd>Jitter factor between 0.0 and 1.0 to add randomness to delays. Omit for no jitter.</dd>
</dl>
...
<dl>
  ...
  <dt>`type`</dt>
  <dd>The type of the step. Can be `atmos`, `shell`, `container`, `exec`, or any of the [interactive step types](#step-types) like `input`, `confirm`, `choose`, `filter`, `markdown`, `spin`, and more. Type `atmos` is implicit for command steps. Type `shell` is required for shell scripts. Type `container` builds, pushes, or runs Docker/Podman containers. Type `exec` replaces the Atmos process entirely (shell `exec` semantics) and must be the final step — see [Replacing the Atmos Process](/cli/configuration/commands#replacing-the-atmos-process-type-exec). See the [Step Types](#step-types) section for all available types</dd>
  ...
  <dt>`interactive`</dt>
  <dd>Attach host stdin to a `shell` step and let the step handle Ctrl-C, like `docker -i` (optional). While the step runs, Atmos suspends its own interrupt handling so pressing Ctrl-C interrupts the step's process, not Atmos</dd>

  <dt>`tty`</dt>
  <dd>Allocate a pseudo-terminal for a `shell` step, like `docker -t` (optional). Use for commands that take over the terminal (SSM sessions, SSH, database consoles, `vim`). Secret masking is still applied; the step's `output` mode does not apply, and the step produces no capturable output. Combine with `interactive: true` for full terminal sessions. See [Interactive and TTY Steps](/cli/configuration/commands#interactive-and-tty-steps) for details</dd>
</dl>
...
<EmbedExample example="demo-workflows" />

<EmbedExample example="interactive-workflows" />
