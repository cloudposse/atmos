...
|----------|------------|-------------|
| **Command** | `atmos`, `shell`, `container` | Execute Atmos, shell, or container operations |
...
Builds, pushes, or runs containers with Docker or Podman. `action: run` mounts the resolved step `working_directory` at `/workspace` by default, runs the command from that directory, and removes the container after the step unless configured otherwise.
...
<dl>
  <dt>`action`</dt>
  <dd>Container action: `build`, `push`, or `run`. When omitted, flat container fields are treated as backward-compatible `run` shorthand.</dd>

  <dt>`build`</dt>
  <dd>Build action block. Supports `provider`, `runtime_auto_start`, `engine`, `context`, `dockerfile`, `tags`, `build_args`, `target`, `no_cache`, `pull`, and optional Docker Buildx Bake configuration under `bake`.</dd>

  <dt>`build.engine`</dt>
  <dd>Optional build engine. `buildx` enables Docker Buildx commands and requires `provider: docker` in V1.</dd>

  <dt>`build.bake`</dt>
  <dd>Docker Buildx Bake configuration. Supports `file`, `files`, `target`, `targets`, `set`, `vars`, `load`, `push`, and `print`. Bake requires Docker Buildx in V1; `provider: podman` is rejected for Bake builds.</dd>

  <dt>`push`</dt>
  <dd>Push action block. Supports `provider`, `runtime_auto_start`, `image`, and optional `tags` to tag before pushing.</dd>

  <dt>`run`</dt>
  <dd>Run action block. Supports `provider`, `runtime_auto_start`, `image`, `command`, `shell`, `pull`, `workspace`, `workspace_read_only`, `cleanup`, `user`, `run_args`, `mounts`, and `ports`.</dd>

  <dt>`outputs`</dt>
  <dd>Declared outputs evaluated after the step succeeds. Use `{{ .steps.NAME.outputs.KEY }}` in later steps.</dd>

  <dt>`provider`</dt>
  <dd>Container runtime provider: `docker` or `podman`. When omitted, Atmos auto-detects the runtime (Docker, then Podman). Defaults globally from `container.runtime.provider`.</dd>

  <dt>`runtime_auto_start`</dt>
  <dd>When `true`, Atmos auto-initializes/starts the Podman machine if no running runtime is found. Defaults globally from `container.runtime.auto_start` (or the `ATMOS_CONTAINER_RUNTIME_AUTO_START` env var).</dd>

  <dt>`pull`</dt>
  <dd>Image pull policy: `missing` (default), `always`, or `never`.</dd>

  <dt>`workspace`</dt>
  <dd>Container workspace path. Defaults to `/workspace`.</dd>

  <dt>`workspace_read_only`</dt>
  <dd>Mount the workspace read-only when set to `true`.</dd>

  <dt>`cleanup`</dt>
  <dd>Container cleanup policy: `always` (default), `on_success`, or `never`.</dd>
  ...
  <dt>`identity`</dt>
  <dd>Atmos [auth identity](/cli/configuration/auth) used to authenticate the step. Authenticating an identity auto-provisions its linked [integrations](/cli/configuration/auth) — for example an `aws/ecr` integration performs the Docker login needed to `push` to (or `build`/`run` from) a private ECR registry. Applies to the whole step and can also be supplied on the command line with `--identity`.</dd>
</dl>

##### Registry authentication (pushing to ECR and other private registries)

Container steps do not implement registry login themselves. Instead, set `identity:` on the step (or pass `--identity`) and let Atmos's [auth integrations](/cli/configuration/auth) materialize the credentials. When the step's identity is authenticated, every integration linked to it with `auto_provision: true` runs automatically; the `aws/ecr` integration writes a Docker login to the standard Docker config, which the step's `docker`/`podman` subprocess then uses.

```yaml
# atmos.yaml — authenticate an identity and link an aws/ecr integration to it.
auth:
  identities:
    dev-admin:
...
```yaml
# The push step references the identity; ECR login happens automatically.
steps:
  - name: build
    type: container
    action: build
...
:::tip
The identity-resolved environment (including the `DOCKER_CONFIG` exported by the `aws/ecr` integration and any `AWS_*` credentials) is forwarded to the container runtime subprocess, so private base-image pulls during `build` and pushes to private registries work even when Atmos uses an isolated Docker config directory.
:::

##### Build caching and registry mirrors

Build-layer caching is available through the Docker Buildx Bake path: set cache backends with `bake.set` (or define `cache-from`/`cache-to` natively in your `docker-bake.hcl`).
...
The non-bake `build` path supports `no_cache` and `pull` only. Registry **mirrors** are not a build flag — configure them at the daemon level (`daemon.json` `registry-mirrors`) or in the buildkit builder (`buildkitd.toml`); container steps then use whatever the runtime is configured to use.
