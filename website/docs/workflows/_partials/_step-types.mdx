...
|----------|------------|-------------|
| **Command** | `atmos`, `shell`, `webhook` | Execute Atmos/shell commands or call an HTTP endpoint |
...
hit a deployment webhook, or poll a health endpoint. The `webhook` step supports any HTTP
...
:::tip
The `webhook` step also works in [custom commands](/cli/configuration/commands) — any step that supports `type:` can use it.
:::
