...
<dl>
  ...
  <dt>`build`</dt>
  <dd>Build action block. Supports `runtime`, `runtime_auto_start`, `engine`, `context`, `dockerfile`, `tags`, `build_args`, `target`, `no_cache`, `pull`, and optional Docker Buildx Bake configuration under `bake`.</dd>

  <dt>`build.engine`</dt>
  <dd>Optional build engine. `buildx` enables Docker Buildx commands and requires `runtime: docker` in V1.</dd>

  <dt>`build.bake`</dt>
  <dd>Docker Buildx Bake configuration. Supports `file`, `files`, `target`, `targets`, `set`, `vars`, `load`, `push`, and `print`. Bake requires Docker Buildx in V1; `runtime: podman` is rejected for Bake builds.</dd>

  <dt>`push`</dt>
  <dd>Push action block. Supports `runtime`, `runtime_auto_start`, `image`, and optional `tags` to tag before pushing.</dd>

  <dt>`run`</dt>
  <dd>Run action block. Supports `runtime`, `runtime_auto_start`, `image`, `command`, `shell`, `pull`, `workspace`, `workspace_read_only`, `cleanup`, `user`, `run_args`, `mounts`, and `ports`.</dd>
