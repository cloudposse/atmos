...
| [`script`](/workflows/steps/type/script) | Run an inline script with an explicit interpreter. | `interpreter`, `script`, `working_directory` |
...
| [`emulator`](/workflows/steps/type/emulator) | Start, stop, or reset a local cloud emulator component. | `component`, `stack`, `action` |
| [`http`](/workflows/steps/type/http) | Call an HTTP endpoint (alias: `webhook`). | `url`, `method`, `body`/`form`, `expect`, `retry` |
| [`archive`](/workflows/steps/type/archive) | Pack (or update) a zip/tar/tgz archive. | `source`, `destination`, `action`, `format` |
| [`require`](/workflows/steps/type/require) | Assert that required tools are on PATH and files/dirs exist (alias: `assert`). | `tools`, `files`, `dirs`, `hint` |
| [`workdir`](/workflows/steps/type/workdir) | Provision a repeatable writable directory from a source. | `path`, `source`, `reset` |
| [`cast`](/workflows/steps/type/cast) | Record nested steps or a terminal session as an asciicast. | `mode`, `steps`, `output.cast` |
| [`store`](/workflows/steps/type/store) | Write a value to a configured store backend. | `store`, `key`, `value`, `stack`, `component` |

## Orchestration Types

| Type | Purpose | Primary fields |
| --- | --- | --- |
| [`parallel`](/workflows/steps/type/parallel) | Run child steps concurrently with dependencies and bounded concurrency. | `steps`, `max_concurrency`, `fail`, `output`, `needs` |
| [`test`](/workflows/steps/type/test) | Run checks with a test tree, progress, and failure-only logs. | `steps`, `title`, `fail`, `output` |
| [`matrix`](/workflows/steps/type/matrix) | Expand axes into generated child steps and run them concurrently. | `matrix`, `steps`, `max_concurrency`, `fail`, `output` |
| [`wait`](/workflows/steps/type/wait) | Block until named background container services are healthy. | `for` |
| [`wait-all`](/workflows/steps/type/wait#wait-all) | Block until every background container service is healthy. | none |
| [`cancel`](/workflows/steps/type/cancel) | Stop and remove a background container service. | `for` |
...
| [`hint`](/workflows/steps/type/hint) | Display a muted tip message. | `content` |
...
| [`junit`](/workflows/steps/type/junit) | Read JUnit XML and publish a CI summary and test annotations. | `files`, `action`, `title` |
| [`hint`](/workflows/steps/type/hint) | Display a muted terminal tip. | `content` |
