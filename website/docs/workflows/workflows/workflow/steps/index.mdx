...
<dl>
  ...
  <dd>Declare this step's freshness sources (`sources`) so Atmos can skip the step when nothing has changed since it last ran successfully. Pair with [`artifacts`](/workflows/steps/artifacts) (outputs) and [`precondition`](/workflows/steps/precondition) (required tools) — three separate sibling fields.</dd>

  <dt>[`artifacts`](/workflows/steps/artifacts)</dt>
  <dd>Declare this step's expected output files.</dd>

  <dt>[`precondition`](/workflows/steps/precondition)</dt>
...
- `checksum`, `timestamp`, `precondition`, `sources`, `artifacts` — freshness facts for a step's [`inputs`](/workflows/steps/inputs)/[`artifacts`](/workflows/steps/artifacts)/[`precondition`](/workflows/steps/precondition) (`checksum.changed`, `timestamp.changed`, `precondition.success`), computed lazily so a step only pays for the specific fact its `when` expression references. `sources`/`artifacts` resolve to structured per-file records (`path`, `mtime`, `checksum`), not bare strings — see [inputs: Advanced](/workflows/steps/inputs#advanced-structured-records).
