...
While in an active chat session, you can switch between sessions using the built-in session picker:
...
- Session names and creation dates
...
3. Press Ctrl+L to open session picker

4. Navigate to a different session using arrow keys

5. Press Enter to switch

6. Continue conversation in the new session context
```

The session picker provides a seamless way to manage multiple conversations without leaving the TUI.
