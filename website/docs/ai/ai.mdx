...
Use **API providers** with purchased tokens (Anthropic, OpenAI, Bedrock, Azure OpenAI, Gemini, GitHub Models, Grok, Ollama),
or use **CLI providers** that reuse your existing subscription (Claude Code, OpenAI Codex, GitHub Copilot, Gemini CLI) — no API keys needed.
</Intro>
...
Atmos supports the [Model Context Protocol](https://modelcontextprotocol.io) (MCP) in both directions. See the **[full MCP documentation](/mcp)** for complete setup, configuration, and command reference.

<Tabs>
<TabItem value="client" label="As a Client" default>

Connect to AWS, GCP, Azure, and custom MCP servers. Their tools become available
...
**MCP Pass-Through** — With CLI AI providers (`claude-code`, `codex-cli`, `copilot-cli`), all configured MCP servers are
...
- **Copilot CLI**: MCP servers merged into `~/.copilot/mcp-config.json` (backup/restore)
...
<ActionCard title="MCP as a Client">
    Full setup, install/export, and command reference for connecting Atmos to external MCP servers.
    <div>
        <PrimaryCTA to="/mcp#as-a-client">MCP Client Documentation</PrimaryCTA>
    </div>
</ActionCard>

</TabItem>
<TabItem value="server" label="As a Server">

Exposes Atmos AI tools to external clients (Claude Desktop, Claude Code, VS Code, Cursor, Gemini CLI).
Use Atmos tools inside your preferred AI assistant.

```bash
# Enable the Atmos MCP server in atmos.yaml
atmos config set --type=bool mcp.enabled true
```

<ActionCard title="MCP as a Server">
    Full setup and per-client instructions (Claude Desktop, Claude Code, Cursor, VS Code, and more).
    <div>
        <PrimaryCTA to="/mcp#as-a-server">MCP Server Documentation</PrimaryCTA>
    </div>
</ActionCard>
