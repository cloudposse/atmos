...
- **Provider Filtering**: Filter sessions by AI provider (Claude, GPT, Gemini, Grok, Ollama, Bedrock, Azure OpenAI)
...
- **IDE Integration**: Create specialized `atmos-expert` subagents for Claude Code
- **MCP-Powered**: Subagents use the Atmos MCP server for infrastructure access
- **Isolated Context**: Each subagent maintains independent conversation threads
...
:::tip IDE Integration
Want Atmos expertise in your IDE? See [Claude Code Subagents](/ai/subagents) to create specialized subagents that use the MCP server.
:::
...
For organizations with strict security, compliance, and data governance requirements, Atmos supports enterprise-grade AI providers.
...
- IAM-based access control (no API keys)
- AWS CloudTrail audit logging
- VPC isolation and PrivateLink support
...
Uses standard AWS SDK credentials (IAM roles, profiles, environment variables).
...
- Data must stay within specific regions
