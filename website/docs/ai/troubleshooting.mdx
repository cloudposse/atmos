  ...
  ai:
    skills:
      stack-analyzer:
        ...
        system_prompt: |
          You are a specialized Atmos Stack Analyzer.
...
- **Security Auditor**: Read-only (cannot modify files)
- **Config Validator**: Validation tools only (cannot execute commands)
- **Component Refactor**: Has `edit_file`, but it's in `restricted_tools` (requires confirmation)
...
settings:
  ai:
    default_skill: "stack-analyzer"    # ✅ Skill name (not display name)

    skills:
      stack-analyzer:                  # ✅ Must match exactly
        display_name: "Stack Analyzer"
        description: "..."
        system_prompt: "..."
```
...
**Common mistakes**:
- Using `display_name` instead of skill key: `default_skill: "Stack Analyzer"` ❌
- Typo in skill name: `default_skill: "stack_analyzer"` ❌ (should be `stack-analyzer`)
...
# Look for: "Switched to skill: Stack Analyzer" or similar
```
</Terminal>
