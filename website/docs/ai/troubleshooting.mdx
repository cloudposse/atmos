...
**Problem**: Too many API requests or "429 Too Many Requests" errors.

**Common Causes**:
- Long conversations sending full message history with each request
- Multi-turn tool execution making multiple API calls
- Exceeding provider's tokens-per-minute limits

**Solutions**:

1. **Limit conversation history** (recommended):
```yaml
settings:
  ai:
    max_history_messages: 20  # Keep only last 20 messages
```
This prevents sending excessive conversation history to the API, which is the most common cause of rate limits in long conversations.

2. **Wait and retry**: Rate limits typically reset after a short period (usually 1 minute)

3. **Check usage**: Review your API usage in the provider dashboard

4. **Upgrade plan**: Consider upgrading if you consistently hit limits even with history limiting
...
- **Solution**: Use a valid model name like `claude-sonnet-4-5-20250929`
...
providers:
  bedrock:
    model: "anthropic.claude-sonnet-4-5-20250929-v1:0"  # Full Bedrock model ID
```
...
**Problem**: Auto-compact is enabled but summaries are not being generated.

**Solutions**:

1. **Verify auto-compact is enabled with required settings**:
```yaml
settings:
  ai:
    enabled: true
    max_history_messages: 50  # Required for auto-compact to calculate thresholds
    sessions:
      enabled: true             # Sessions required for auto-compact
      auto_compact:
        enabled: true
        trigger_threshold: 0.75
```

2. **Check if threshold has been reached**:
<Terminal title="shell">
```bash
...
# With max_history_messages: 50 and trigger_threshold: 0.75,
# auto-compact triggers at 37 messages (50 × 0.75)
```
</Terminal>

3. **Verify summaries table exists**:
<Terminal title="shell">
```bash
# Check if message_summaries table exists
sqlite3 .atmos/sessions/sessions.db \
  ".schema message_summaries"

# Should show CREATE TABLE statement
```
</Terminal>

4. **Check for AI summarization errors**:
If `use_ai_summary: true`, ensure:
- AI provider configured correctly
- API key valid
- Network connectivity available
- `summary_provider` (if set) is valid

5. **Test with simple summary first**:
```yaml
sessions:
  auto_compact:
    enabled: true
    use_ai_summary: false  # Use simple concatenation first
```

### Auto-Compact Summaries Not Appearing

**Problem**: Auto-compact is working but summaries don't appear in conversation.

**Solutions**:

1. **Check if summaries were created**:
<Terminal title="shell">
```bash
...
2. **Verify archived messages**:
<Terminal title="shell">
```bash
# Count archived messages (should be compacted)
sqlite3 .atmos/sessions/sessions.db \
...
3. **Enable summary markers for debugging**:
```yaml
sessions:
  auto_compact:
    enabled: true
    show_summary_markers: true  # Shows [SUMMARY] markers in conversation
```

Then check if markers appear in conversation history.

### Auto-Compact Triggering Too Often

**Problem**: Auto-compact runs too frequently, disrupting conversation flow.
...
sessions:
  auto_compact:
    trigger_threshold: 0.85  # Trigger at 85% instead of 75%
```

2. **Increase max_history_messages**:
```yaml
settings:
  ai:
    max_history_messages: 100  # More headroom before compaction
```

3. **Reduce compact_ratio**:
```yaml
sessions:
  auto_compact:
    compact_ratio: 0.3  # Compact 30% instead of 40%
```

### Auto-Compact Summaries Too Long

**Problem**: AI-generated summaries are too verbose.

**Solutions**:

1. **Reduce summary_max_tokens**:
```yaml
sessions:
  auto_compact:
    summary_max_tokens: 1024  # Shorter summaries (default: 2048)
```

2. **Use faster/cheaper model** (they tend to be more concise):
```yaml
sessions:
  auto_compact:
    summary_model: "claude-3-haiku-20240307"  # Concise summaries
```

3. **Adjust compact_ratio** to compact fewer messages at once:
```yaml
sessions:
  auto_compact:
    compact_ratio: 0.3  # Smaller batches = shorter summaries
```

### Auto-Compact Using Too Many Tokens

**Problem**: Auto-compact feature causing high API costs.

**Solutions**:

1. **Use cheaper model for summarization**:
```yaml
sessions:
  auto_compact:
    summary_provider: "anthropic"
    summary_model: "claude-3-haiku-20240307"  # 10x cheaper than Sonnet
```

2. **Reduce summary_max_tokens**:
```yaml
sessions:
  auto_compact:
    summary_max_tokens: 1024  # Lower token limit = lower cost
```

3. **Increase preserve_recent** to compact fewer messages:
```yaml
sessions:
  auto_compact:
    preserve_recent: 15  # Keep more messages verbatim
    compact_ratio: 0.3   # Compact less at once
```

4. **Disable AI summaries** (use simple concatenation):
```yaml
sessions:
  auto_compact:
    use_ai_summary: false  # Free, no API calls
```

**Cost Comparison** (per compaction):
- Simple concatenation: $0 (no API calls)
- Claude Haiku: ~$0.01 (15 messages summarized)
- Claude Sonnet: ~$0.10 (15 messages summarized)

See [Auto-Compact](/ai/sessions#auto-compact-extended-conversations) for detailed configuration and best practices.
...
**Problem**: Pressing Ctrl+A shows skill selector, but a specific skill is missing.
...
<Terminal title="shell">
```bash
# Verify skill is defined in atmos.yaml
atmos describe config | grep -A 20 "skills:"
```
</Terminal>
  ...
  ai:
    skills:
      my-skill:              # Skill name
        ...
        # category, allowed_tools, restricted_tools are optional
```
</File>

**Common mistakes**:
- Missing required fields (`display_name`, `description`, `system_prompt`)
- Invalid YAML indentation
- Skill name contains spaces (use hyphens: `my-skill` not `my skill`)

### Skill Not Using Specialized Prompt

**Problem**: Switched to a specialized skill, but it's not behaving as expected.

**Symptoms**:
- Skill gives generic responses instead of specialized ones
- Skill doesn't follow its domain-specific instructions
- No confirmation message when switching skills

**Solutions**:

1. **Verify skill switch was successful**:

When you select a skill with Ctrl+A and press Enter, you should see:
```
System: Switched to skill: atmos-stacks (Stack configuration, imports, inheritance, deep merging)
```

If you don't see this message, the skill didn't activate.

2. **Check skill's system prompt**:
...
settings:
  ai:
    skills:
      atmos-stacks:
        display_name: "Atmos Stacks"
        description: "Analyzes stack configurations"
        system_prompt: |
          ...
          Always use tools to gather data first.
```
</File>

The `system_prompt` field must be a string with clear instructions. Use `|` for multi-line prompts.

3. **Test with a specific question**:

After switching skills, ask a domain-specific question:

<Terminal title="atmos ai chat">
```bash
# Switch to atmos-stacks (Ctrl+A)
You: "Analyze the inheritance chain for prod-use1-eks"

# The skill should follow its systematic approach
AI: Starting with high-level overview...
    1. Base configuration: catalog/eks/defaults.yaml
    ...
```
</Terminal>

If the skill gives a generic response, its system prompt may not be detailed enough.

### Custom Skill Not Working

**Problem**: Created a custom skill, but it's not working as expected.
...
settings:
  ai:
    skills:
      cost-optimizer:
        display_name: "Cost Optimizer"        # ✅ Required
        description: "Finds cost savings"     # ✅ Required
        system_prompt: |                      # ✅ Required (multi-line)
          You are a cost optimization expert.

          Always analyze costs before recommending changes.
        category: "analysis"                  # ⚠️ Optional
        allowed_tools: []                     # ⚠️ Optional (empty = all tools)
        restricted_tools: []                  # ⚠️ Optional
```
</File>

2. **System prompt is comprehensive**:

A good system prompt includes:
- Role definition ("You are a...")
- Focus areas (bulleted list)
- Approach/methodology (numbered steps)
- Tool usage instructions ("Always use tools to...")

<File title="Good System Prompt Example">
```yaml
system_prompt: |
...
```yaml
system_prompt: "Help with cost optimization"
```

3. **Tool names are correct**:
...
settings:
  ai:
    skills:
      my-skill:
        allowed_tools:
          - atmos_describe_component      # ✅ Correct
          - read_file                     # ✅ Correct
          - describe_component            # ❌ Wrong (missing prefix)
          - atmos-list-stacks             # ❌ Wrong (wrong separator)
```
</File>
...
See [AI Tools](/ai/tools) for complete list.

### Skill Switch Not Responding

**Problem**: Press Ctrl+A, but nothing happens.

**Solutions**:

1. **Verify you're in chat view**:

Ctrl+A only works in the main chat view, not in:
- Session picker (Ctrl+L)
- Provider selector (Ctrl+P)
- Session creation form (Ctrl+N)

Press Esc to return to chat view, then try Ctrl+A again.

2. **Check terminal input mode**:

If you're in the middle of typing a message, the textarea may capture Ctrl+A. Clear the textarea first:
- Press Esc to clear
- Then press Ctrl+A

3. **Verify skill registry loaded**:

If skills failed to load from configuration, the skill selector will be empty.

<Terminal title="shell">
```bash
# Test skill loading
atmos ai chat --session test-skills

# Try to open skill selector
# Press Ctrl+A

# If you see an empty selector, check configuration
atmos describe config | grep -A 50 "ai:"
```
</Terminal>

### Tool Access Denied for Skill

**Problem**: Skill says it cannot use a specific tool.

**Example**:
```
You: "Edit the VPC component"
AI: "I don't have access to the edit_file tool"
```

**Reason**: The skill's `allowed_tools` list doesn't include `edit_file`.

**Solutions**:

1. **Check skill's tool permissions**:
    ...
    skills:
      atmos-validation:
        ...
        allowed_tools:
          ...
          - read_stack_file
          - read_file
          # ❌ edit_file is NOT in this list
```
</File>

2. **Add the tool to allowed_tools**:
    ...
    skills:
      atmos-validation:
        allowed_tools:
          ...
          - read_stack_file
          - read_file
          - edit_file              # ✅ Now allowed
```
</File>

3. **Or use a skill with full tool access**:

Press Ctrl+A and switch to the **General** skill, which has access to all tools.

**Design consideration**: Some skills intentionally have restricted tool access for safety. For example:
- **atmos-validation**: Read-only (cannot modify files)
- **atmos-stacks**: Analysis tools only (cannot execute commands)
- **atmos-components**: Has `edit_file`, but it's in `restricted_tools` (requires confirmation)

### Default Skill Not Applying

**Problem**: Set `default_skill` in config, but chat still starts with "General" skill.
...
settings:
  ai:
    default_skill: "atmos-stacks"      # ✅ Skill name (not display name)

    skills:
      atmos-stacks:                    # ✅ Must match exactly
        display_name: "Atmos Stacks"
        description: "..."
        system_prompt: "..."
```
...
**Common mistakes**:
- Using `display_name` instead of skill key: `default_skill: "Atmos Stacks"` ❌
- Typo in skill name: `default_skill: "atmos_stacks"` ❌ (should be `atmos-stacks`)
- Skill not defined in `skills` section

**Verify**:

<Terminal title="shell">
```bash
# Start chat and check first message
atmos ai chat

# The initial greeting should indicate the active skill
# Look for: "Switched to skill: atmos-stacks" or similar
```
</Terminal>

### Skill Configuration Not Loading

**Problem**: Defined custom skills in `atmos.yaml`, but they're not appearing.
...
<File title="atmos.yaml (correct indentation)">
```yaml
settings:
  ai:                        # 2 spaces
    ...
    skills:                  # 4 spaces
      my-skill:              # 6 spaces
        display_name: "..."  # 8 spaces
        description: "..."   # 8 spaces
        system_prompt: "..." # 8 spaces
```
</File>

3. **Verify skills section location**:
...
Start chat and press Ctrl+A. If "Test" appears, your syntax is correct and you can add more complex skills.
...
See [Claude Code Integration](/ai/claude-code-integration) for complete setup guide.
...
1. **Verify instructions are enabled**:
<Terminal title="shell">
    ```bash
...
<Terminal title="shell">
    ...
    atmos ai chat  # Look for instructions-related warnings in logs
    ```
</Terminal>
...
settings:
  ai:
    instructions:
      ...
      # Or absolute path
      file: "/absolute/path/to/ATMOS.md"
```

### Instruction Sections Not Appearing
...
**Problem**: AI isn't adding learnings to instructions file.
...
**Solution**: Optimize instructions file size:
...
4. **Limit conversation history** (for long conversations):
```yaml
settings:
  ai:
    max_history_messages: 20  # Reduce tokens sent in each request
```
...
- [Claude Code Integration](/ai/claude-code-integration) - IDE integration guide
