...
- **API providers:** `anthropic`, `openai`, `gemini`, `github`, `grok`, `openrouter`, `deepseek`, `zai`, `ollama`, `bedrock`, `azureopenai`
- **CLI providers:** `claude-code`, `codex-cli`, `copilot-cli`, `opencode`, `gemini-cli`
...
<Tabs>
  ...
  <TabItem value="copilot-cli" label="GitHub Copilot">
    ```bash
    npm install -g @github/copilot
    copilot  # then /login (or set GH_TOKEN)
    ```
  </TabItem>
...
**Workaround:** Use `claude-code`, `codex-cli`, `copilot-cli`, or `opencode` for MCP workflows. Gemini CLI works for prompt-only queries.
...
All providers support conversation memory.
