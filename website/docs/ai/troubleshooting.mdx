...
### Agent Not Available in Selector

**Problem**: Pressing Ctrl+A shows agent selector, but a specific agent is missing.
...
<Terminal title="shell">
```bash
# Verify agent is defined in atmos.yaml
atmos describe config | grep -A 20 "agents:"
```
</Terminal>
...
- Agent name contains spaces (use hyphens: `my-agent` not `my agent`)

### Agent Not Using Specialized Prompt

**Problem**: Switched to a specialized agent, but it's not behaving as expected.

**Symptoms**:
- Agent gives generic responses instead of specialized ones
- Agent doesn't follow its domain-specific instructions
- No confirmation message when switching agents

**Solutions**:

1. **Verify agent switch was successful**:

When you select an agent with Ctrl+A and press Enter, you should see:
```
System: Switched to agent: Stack Analyzer (Analyze stack configurations and dependencies)
```

If you don't see this message, the agent didn't activate.
...
After switching agents, ask a domain-specific question:
...
# The agent should follow its systematic approach
...
If the agent gives a generic response, its system prompt may not be detailed enough.

### Custom Agent Not Working

**Problem**: Created a custom agent, but it's not working as expected.
...
3. **Verify agent registry loaded**:

If agents failed to load from configuration, the agent selector will be empty.

<Terminal title="shell">
```bash
# Test agent loading
atmos ai chat --session test-agents
...
### Tool Access Denied for Agent

**Problem**: Agent says it cannot use a specific tool.

**Example**:
```
You: "Edit the VPC component"
Agent: "I don't have access to the edit_file tool"
```

**Reason**: The agent's `allowed_tools` list doesn't include `edit_file`.

**Solutions**:

1. **Check agent's tool permissions**:
...
3. **Or use an agent with full tool access**:

Press Ctrl+A and switch to the **General** agent, which has access to all tools.

**Design consideration**: Some agents intentionally have restricted tool access for safety. For example:
...
### Default Agent Not Applying

**Problem**: Set `default_agent` in config, but chat still starts with "General" agent.
...
settings:
  ai:
    default_agent: "stack-analyzer"    # ✅ Agent name (not display name)
...
**Common mistakes**:
- Using `display_name` instead of agent key: `default_agent: "Stack Analyzer"` ❌
- Typo in agent name: `default_agent: "stack_analyzer"` ❌ (should be `stack-analyzer`)
- Agent not defined in `agents` section
...
# The initial greeting should indicate the active agent
# Look for: "Switched to agent: Stack Analyzer" or similar
```
</Terminal>

### Agent Configuration Not Loading

**Problem**: Defined custom agents in `atmos.yaml`, but they're not appearing.
...
3. **Verify agents section location**:

Agents must be under `settings.ai.agents`, not `settings.agents` or `ai.agents`.
...
Start chat and press Ctrl+A. If "Test" appears, your syntax is correct and you can add more complex agents.
