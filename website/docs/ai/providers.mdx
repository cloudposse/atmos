...
| **xAI (Grok)** | `grok-4` | API Key | OpenAI-compatible, real-time knowledge |
...
providers:
  grok:
    model: "grok-4"  # Recommended (default)
    # Current models:
    # model: "grok-4"              # Most intelligent, tool use, real-time search
    # model: "grok-4-fast"         # Faster variant, 2M token context
    # model: "grok-3"              # Flagship model, enterprise use cases
    # model: "grok-3-mini"         # Lightweight reasoning model
    # model: "grok-2-image"        # Text-to-image generation
    # Legacy:
    # model: "grok-beta"           # Previous beta version
...
| Model | Model ID | Context Window | Features | Pricing (per 1M tokens) | Best For |
|-------|----------|----------------|----------|-------------------------|----------|
| **Current Generation** | | | | | |
| Grok 4 | `grok-4` | 2M tokens | Tool use, real-time search | $3 input / $15 output | Most intelligent, enterprise tasks (recommended) |
| Grok 4 Fast | `grok-4-fast` | 2M tokens | Faster responses | $0.20-$0.40 input / $0.50-$1.00 output | High-speed queries, lighter workloads |
| Grok 3 | `grok-3` | - | Data extraction, coding | $3 input / $15 output | Enterprise use, coding, summarization |
| Grok 3 Mini | `grok-3-mini` | - | Native reasoning | Lower cost | Logic-based tasks, lightweight reasoning |
| Grok 2 Image | `grok-2-image` | N/A | Text-to-image | Per image pricing | Image generation |
...
**Knowledge Cutoff:** November 2024 (Grok 3, Grok 4)

**Model Selection Guide:**
- **For enterprise tasks**: Grok 4 - most intelligent with tool use and real-time search
- **For speed**: Grok 4 Fast - optimized for faster responses with 2M token context
- **For coding**: Grok 3 - strong data extraction, coding, and text summarization
- **For budget**: Grok 3 Mini - lightweight model for logic-based tasks
- **For images**: Grok 2 Image - text-to-image generation

**Additional Features:**
- **Live Search API**: Real-time web search integration ($25 per 1,000 sources or $0.025 per source)
- **Prompt Caching**: Reduce costs on repeated prompts by reusing stored data
