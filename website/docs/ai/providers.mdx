...
- **Enable token caching**: Save up to 90% on repeated content (see below)
:::

### Token Caching (Prompt Caching)

Token caching (also called "prompt caching") dramatically reduces costs by reusing frequently-sent content like system prompts and project instructions. Instead of paying full price for the same tokens every message, cached tokens cost 90% less (Anthropic), 50% less (OpenAI/Azure), or are free (Gemini).

**Cost Savings:**
- **No caching**: Pay full price for system prompts and ATMOS.md content every message
- **50% cache hit**: Save ~41% on total API costs
- **90% cache hit**: Save ~74% on total API costs (typical with caching enabled)

**Provider Support:**

| Provider | Support | Implementation | Discount | Cache Duration | Configuration Required |
|----------|---------|---------------|----------|----------------|----------------------|
| **Anthropic** | ✅ YES | Manual markers | 90% | 5 minutes | ✅ Yes (see below) |
| **OpenAI** | ✅ YES | **Automatic** | 50% | 5-10 minutes | ❌ **None** |
| **Gemini** | ✅ YES | **Automatic** | Free (included) | Varies | ❌ **None** |
| **Grok** | ✅ YES | **Automatic** (>90% hit rate) | 75% | Not specified | ❌ **None** |
| **Bedrock** | ✅ YES | Simplified automatic | Up to 90% | 5 minutes | ❌ **None** |
| **Azure OpenAI** | ✅ YES | **Automatic** | 50-100% | 5-10 minutes | ❌ **None** |
| **Ollama** | N/A | Local (no API costs) | N/A | N/A | N/A |

**6 of 7 providers support token caching!** Most providers implement caching automatically with no code changes required. Only Anthropic requires explicit cache configuration.

#### Anthropic Configuration

Anthropic requires explicit cache markers to indicate which content should be cached. By default, Atmos caches both the system prompt and your ATMOS.md project instructions:
  ...
  ai:
    providers:
      anthropic:
        ...
        # Token caching (enabled by default)
        cache:
...
**How It Works:**
1. First message: Creates cache entries for system prompt + ATMOS.md (normal cost)
2. Subsequent messages: Reuses cached content (90% discount on cached tokens)
3. Cache expires after 5 minutes of inactivity
4. Cache automatically refreshes when you send another message
...
- Output: 500 tokens × $15/1M = $0.0075
- **Total: $0.0108 per message (71% savings)**
...
To cache only system prompt (not project instructions):
    ...
    providers:
      anthropic:
        cache:
          ...
          cache_project_instructions: false  # Don't cache ATMOS.md
```
</File>

**When to Disable:**
- Very short conversations (1-2 messages) where cache overhead exceeds savings
- Rapidly changing system prompts or project instructions
- Cost monitoring during development

#### Other Providers

**OpenAI, Gemini, Grok, Azure OpenAI, Bedrock**: These providers implement automatic caching. Simply use them normally - caching happens transparently with no configuration required.

For OpenAI specifically, caching activates automatically when:
- Prompts exceed 1,024 tokens
- Content is reused within 5-10 minutes

**Ollama**: No caching configuration needed (local models have no API costs).

:::tip Maximize Cache Savings
For best results with Anthropic:
- Enable both system prompt and project instructions caching
- Keep ATMOS.md content stable (avoid frequent updates during conversations)
- Use named sessions for extended conversations on the same topic
- Cache duration is 5 minutes - stay active to maintain cache

For other providers, caching works automatically - just use the AI normally.
:::
...
providers:
  gemini:
    ...
    # model: "gemini-2.5-flash"    # Experimental flash model
...
| Gemini 2.0 Flash | `gemini-2.5-flash` | 1M tokens | 8K tokens | Experimental fast model | Nov 2024 |
| **Legacy Models (1.5)** | | | | | |
...
- **Claude (Anthropic)**: ~$3/1M input tokens, ~$15/1M output tokens
- **GPT-4o (OpenAI)**: ~$2.50/1M input tokens, ~$10/1M output tokens
- **Gemini (Google)**: Varies by model and region
- **Grok (xAI)**: Similar to OpenAI pricing
...
| Model | API Model ID | Notes |
|-------|--------------|-------|
| Claude Sonnet 4 | `claude-sonnet-4-5-20250929` | Previous version |
