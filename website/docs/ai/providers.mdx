...
Token caching (also called "prompt caching") dramatically reduces costs by reusing frequently-sent content like system prompts and project memory. Instead of paying full price for the same tokens every message, cached tokens cost 90% less (Anthropic), 50% less (OpenAI/Azure), or are free (Gemini).
...
Anthropic requires explicit cache markers to indicate which content should be cached. By default, Atmos caches both the system prompt and your ATMOS.md project memory:
...
To cache only system prompt (not project memory):
...
- Rapidly changing system prompts or project memory
...
- Enable both system prompt and project memory caching
