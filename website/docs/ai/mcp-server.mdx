...
The Atmos MCP Server is built on the [official MCP Go SDK](https://github.com/modelcontextprotocol/go-sdk), ensuring full compatibility with the MCP specification and supporting all MCP features and protocols. This standards-based implementation allows you to use Atmos infrastructure management capabilities from any AI assistant that supports the MCP protocol:
...
The Atmos MCP Server leverages the official [MCP Go SDK v1.0.0](https://github.com/modelcontextprotocol/go-sdk), providing:

- **Full Protocol Compliance** - Implements the complete [MCP specification (2025-03-26)](https://modelcontextprotocol.io/specification/2025-03-26)
- **Standard JSON-RPC 2.0** - Compatible with all MCP-compliant clients
- **Automatic Schema Generation** - Tools are automatically registered with JSON Schema for their parameters
- **Type-Safe Communication** - Leverages the SDK's type system for reliable message handling
- **Future-Proof Architecture** - Automatically benefits from SDK updates and new MCP features
- **Community-Tested** - Built on the same foundation as official MCP reference implementations

All MCP protocol features are supported, including:
- ✅ **Tools** - Execute infrastructure operations (describe, list, validate, plan, read, write)
- ✅ **Resources** - Access infrastructure configuration files and metadata
- ✅ **Prompts** - Pre-configured interaction patterns (planned for future releases)
...
1. **Check the status icon**: Look for the 🔌 (plug) icon in the bottom-right corner of the Claude Desktop window
2. **Click the icon**: You should see "atmos" listed with a green dot (● connected) or yellow dot (● initializing)
...
<Terminal title="atmos mcp start">
```bash
atmos mcp start
```
</Terminal>
...
3. **Check the MCP status icon** - Look for 🔌 in the bottom-right corner
...
- **CLI Tools**: Best for terminal-based workflows and automation
- **HTTP transport**: Best for team collaboration and CI/CD pipelines
:::

#### AI CLI Tools

Beyond desktop applications and IDE extensions, several AI CLI tools support MCP for terminal-based workflows.

##### Claude Code

**Prerequisites**: [Claude Code](https://docs.claude.com/en/docs/claude-code) installed

**Configuration Scopes**:
- **Local**: Project-specific (default)
- **Project**: Shared via `.mcp.json` in project root
- **User**: Cross-project (`~/.claude.json`)
...
claude mcp add --transport stdio atmos \
  --env ATMOS_BASE_PATH=/path/to/project \
  -- atmos mcp start

# With specific scope
claude mcp add --scope project --transport stdio atmos -- atmos mcp start
```
</Terminal>

**Management Commands**:

<Terminal title="claude mcp commands">
```bash
# List all MCP servers
claude mcp list

# Get server details
claude mcp get atmos

# Remove server
claude mcp remove atmos
```
</Terminal>

**Manual Configuration** (`.mcp.json`):

<File title=".mcp.json (project root)">
```json
{
  "mcpServers": {
    "atmos": {
...
# With environment variables
gemini mcp add atmos -e ATMOS_BASE_PATH=/path/to/project atmos mcp start
...
<Terminal title="gemini mcp commands">
```bash
# List all MCP servers
gemini mcp list

# Remove server
gemini mcp remove atmos

# Check server status within CLI
gemini
# Then type: /mcp
```
</Terminal>
...
```json
{
  "mcpServers": {
    "atmos": {
      ...
      "args": ["mcp", "start"],
      "env": {
        "ATMOS_BASE_PATH": "$HOME/infrastructure"
      },
      "timeout": 30000,
      "trust": false
    }
...
**Prerequisites**: [Codex CLI](https://developers.openai.com/codex/cli/) installed
...
Codex CLI uses `config.json` in the `~/.codex/` directory. Refer to the [official documentation](https://developers.openai.com/codex/cli/) for the latest configuration format.
:::

##### Grok CLI

**Prerequisites**: [Grok CLI](https://github.com/superagent-ai/grok-cli) installed
...
**Adding MCP Servers** (Interactive):

<Terminal title="grok interactive">
```bash
# Start Grok CLI
grok

# Within the CLI, add MCP server
/mcp add atmos "atmos" "mcp" "start"

# Add with environment variables
/mcp add atmos "atmos" "mcp" "start" "--env=ATMOS_BASE_PATH=/path/to/project"
...
**Alternative Grok CLI** (superagent-ai):

<Terminal title="grok mcp add">
```bash
# stdio transport
grok mcp add atmos --transport stdio --command "atmos" --args "mcp,start"

# HTTP transport
grok mcp add atmos --transport http --url "http://localhost:8080"

# With environment variables
grok mcp add atmos --transport stdio \
  --command "atmos" --args "mcp,start" \
  --env "ATMOS_BASE_PATH=/path/to/project"
```
</Terminal>

**Manual Configuration** (`.grok/settings.json`):
...
| CLI Tool | Provider | Installation | MCP Support | Best For |
|----------|----------|--------------|-------------|----------|
| **Claude Code** | Anthropic | Official installer | Excellent | Claude users, deep integration |
| **Gemini CLI** | Google | npm install | Excellent | Gemini users, FastMCP integration |
| **Codex CLI** | OpenAI | npm install | Good | OpenAI users, coding assistance |
| **Grok CLI** | xAI/Community | npm install | Good | xAI users, terminal workflows |

:::tip Which CLI Tool Should I Use?
- **Claude Code**: Best if you're already using Claude, most mature MCP support
- **Gemini CLI**: Best for Google Cloud users, excellent FastMCP integration
- **Codex CLI**: Best for OpenAI ecosystem users (GPT-4, GPT-5)
- **Grok CLI**: Best for xAI users, latest AI models from xAI

All support stdio and HTTP transports for Atmos MCP server!
:::
...
<Terminal title="atmos mcp start --transport http">
```bash
...
The Atmos MCP Server is implemented as a lightweight wrapper around the [official MCP Go SDK](https://github.com/modelcontextprotocol/go-sdk):
...
- **Standards Compliance**: Full adherence to MCP specification
- **Minimal Maintenance**: SDK handles protocol complexities
- **Automatic Updates**: Benefits from SDK improvements
- **Type Safety**: Compile-time validation of messages
...
```
┌─────────────────┐     ┌─────────────────┐
│  Cloud Desktop  │     │  Custom Client  │
│   Web Client    │     │  CI/CD Pipeline │
└────────┬────────┘     └────────┬────────┘
         │                       │
         │ HTTP/SSE              │ HTTP/SSE
         └───────────┬───────────┘
...
1. **Client Launches Server as Subprocess**
   - When Claude Desktop starts, it spawns `atmos mcp start` as a child process
...
HTTP servers:
- ✅ Run independently of any client
- ✅ Handle multiple concurrent connections
- ✅ Suitable for production deployments
- ✅ Can be daemonized with systemd, Docker, etc.
- ⚠️ Require additional security measures (authentication, TLS)
...
Running in the foreground is **not a limitation** for stdio servers—it's the designed behavior that ensures:
...
- Use HTTP transport instead: `atmos mcp start --transport http`
...
- ✅ Let the client manage the process lifecycle
- ✅ Use stderr for logging, stdout for protocol messages only
- ✅ Implement graceful shutdown on signal handlers
- ✅ Handle context cancellation properly
- ❌ Don't try to daemonize or detach the process
- ❌ Don't write non-protocol data to stdout
...
- ❌ Don't expose unauthenticated HTTP servers to the internet
...
**stdio Transport**
- ✅ Runs as local subprocess under your user account
- ✅ No network exposure
- ✅ Inherits file system permissions
- ⚠️ Any local process can connect if they can spawn the binary

**HTTP Transport**
- ⚠️ No authentication by default
- ⚠️ No encryption (HTTP only, not HTTPS)
- ⚠️ Accessible to anyone who can reach the port
- ⚠️ All clients share the same permission context
...
atmos mcp start --transport http --host localhost --port 8080
...
1. **Verify MCP server is connected:**
   - Click the 🔌 icon in bottom-right corner of Claude Desktop
   - "atmos" should show green dot (● connected)
   - If yellow (● initializing), wait 10 seconds
   - If red (● error), check logs for error messages
...
| Feature | `atmos ai chat` | `atmos mcp start` |
|---------|----------------|-------------------|
...
| **Session Management** | ✅ Built-in | ❌ Client-managed |
| **Project Instructions** | ✅ Built-in | ❌ Not available |
| **Portability** | ❌ Atmos CLI only | ✅ Universal (MCP standard) |
| **Setup** | Simple | Requires client configuration |

**When to use each:**

- **Use `atmos ai chat`** when working directly in the terminal
- **Use `atmos mcp start`** when integrating with Claude Desktop, VSCode, or custom applications
...
- [MCP Go SDK](https://github.com/modelcontextprotocol/go-sdk) - **Official Go SDK used by Atmos MCP Server** - Provides full protocol compliance and type-safe communication
- [Official MCP Servers](https://github.com/modelcontextprotocol/servers) - Reference implementations and examples in multiple languages
...
### AI CLI Tools
- [Claude Code Documentation](https://docs.claude.com/en/docs/claude-code) - Official Claude Code CLI documentation
- [Claude Code MCP Guide](https://docs.claude.com/en/docs/claude-code/mcp) - Guide for adding MCP servers to Claude Code
- [Gemini CLI GitHub](https://github.com/google-gemini/gemini-cli) - Official Gemini CLI repository
- [Gemini CLI MCP Documentation](https://google-gemini.github.io/gemini-cli/docs/tools/mcp-server.html) - MCP server integration guide for Gemini
- [Gemini CLI + FastMCP](https://developers.googleblog.com/en/gemini-cli-fastmcp-simplifying-mcp-server-development/) - Simplifying MCP server development with Gemini
- [OpenAI Codex CLI Documentation](https://developers.openai.com/codex/cli/) - Official OpenAI Codex CLI documentation
- [Grok CLI (webdevtoday)](https://www.npmjs.com/package/@webdevtoday/grok-cli) - Community-maintained Grok CLI with MCP support
- [Grok CLI (superagent-ai)](https://github.com/superagent-ai/grok-cli) - Alternative Grok CLI implementation
- [Adding MCP Servers to Claude Code](https://mcpcat.io/guides/adding-an-mcp-server-to-claude-code/) - Step-by-step guide for Claude Code MCP setup
