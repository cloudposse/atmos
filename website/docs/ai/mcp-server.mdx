...
Atmos MCP supports two transports. You can only use one at a time.

| | stdio (default) | HTTP |
|---|---|---|
| **How it works** | Client spawns Atmos as a subprocess; communication over stdin/stdout | Atmos listens on an HTTP port with SSE |
| **Best for** | Desktop apps, local development | Remote access, shared servers, CI/CD |
| **Multiple clients** | No (1:1 with client) | Yes |
| **Network exposure** | None | Requires security measures |
| **Lifecycle** | Tied to client process | Runs independently |

### Starting the Server

<Terminal title="atmos mcp start">
```bash
# stdio (default) -- used by desktop clients automatically
atmos mcp start

# HTTP on default port (localhost:8080)
atmos mcp start --transport http

# HTTP on a custom host/port
...
<dl>
  ...
  <dt>`--host`</dt>
  <dd>Host to bind (HTTP only). Default: `localhost`.</dd>

  <dt>`--port`</dt>
  <dd>Port to bind (HTTP only). Default: `8080`.</dd>
</dl>
...
and the permission system, see [AI Tools](/ai/tools).
...
| `atmos_read_component_file` | Read a component source file |
| `atmos_read_stack_file` | Read a stack configuration file |
| `atmos_write_component_file` | Modify a component file (requires permission) |
| `atmos_write_stack_file` | Modify a stack file (requires permission) |
...
|--------|----------------|----------|
| **Claude Desktop** | `~/.config/claude/claude_desktop_config.json` | General infrastructure queries |
| **VS Code Copilot** | `.vscode/mcp.json` | Coding with infrastructure context |
| **Cursor** | `~/.cursor/mcp.json` | AI-first development |
| **Cline** | GUI + `cline_mcp_settings.json` | Autonomous coding |
| **Continue.dev** | `.continue/mcpServers/atmos.yaml` | Multi-model workflows |
| **Windsurf** | `~/.windsurf/mcp.json` | AI-powered development |
| **Claude Code** | `.mcp.json` or CLI | Terminal-based workflows |
| **Gemini CLI** | `~/.gemini/settings.json` | Google Cloud users |
| **Codex CLI** | `~/.codex/config.json` | OpenAI ecosystem |
| **Grok CLI** | `.grok/settings.json` | xAI users |
...
### Advanced Claude Desktop Options

If `atmos` is not in your PATH, or you need environment variables:
...
| **Setup** | None | Requires client configuration |
...
- [AI Tools](/ai/tools) -- Tool details and permission system
