...
<Terminal>
```shell
# Automated: writes mcp.enabled + the client entry for you.
atmos mcp add self --install
```
</Terminal>

`atmos mcp add self --install` does steps 1 and 2 for supported clients (Claude Code, Cursor, VS
Code, Codex, Gemini) in one command — see [`atmos mcp add`](/cli/commands/mcp/add). Claude
Desktop isn't one of the auto-installable clients yet, so configure it manually below.
...
<dl>
  ...
  <dd>Validate atmos.yaml (and other configured YAML files) against their JSON Schemas.</dd>
...
<dl>
  ...
  <dd>Restrict tools in `atmos.yaml` using `ai.tools.allowed` and `ai.tools.blocked`. Disable write operations if not needed.</dd>
...
Tools register for MCP regardless of `ai.tools.enabled` (that setting only gates `atmos ai
chat`/`ask`/`exec`'s own tool-use loop) — `mcp.enabled: true` is the MCP server's own opt-in.
Make sure you have not restricted the set with `ai.tools.allowed`, and that the tool isn't in
`ai.tools.blocked`. If tools still do not appear, try asking Claude explicitly:
...
**"MCP server is not enabled"** -- Set `mcp.enabled: true` in `atmos.yaml`. This is the MCP
server's own opt-in; `ai.enabled`/`ai.tools.enabled` are unrelated to it.

**Tools not executing** -- Check `ai.tools.allowed` and `ai.tools.blocked` in your AI tool config. Remove overly
restrictive entries.
