...
Atmos AI includes specialized skills for different tasks. You can install skills from the marketplace or create custom ones.

### Installing Skills from the Marketplace

Install official Atmos AI skills from the [Skill Marketplace](/ai/skill-marketplace):
...
Switch between skills during a chat session by pressing **Ctrl+A**.

### Custom Skills

Create your own specialized skills:
...
settings:
  ai:
    ...
    default_skill: "atmos-stacks"       # Use atmos-stacks by default
...
:::tip AI Skills
For complete skill documentation including examples and best practices, see [AI Skills](/ai/skills).
:::
...
settings:
  ai:
    ...
    sessions:
      ...
      # Auto-compact settings (optional)
      auto_compact:
        enabled: true                  # Enable auto-compact (default: false)
        trigger_threshold: 0.75        # Trigger at 75% of max_history_messages (default: 0.75)
        compact_ratio: 0.4             # Compact 40% of messages (default: 0.4)
        preserve_recent: 10            # Always keep last 10 messages (default: 10)
        use_ai_summary: true           # Use AI for summaries (default: true)
        summary_provider: "anthropic"  # Provider for summarization (default: default_provider)
        summary_model: "claude-3-haiku-20240307"  # Faster/cheaper model for summaries
        summary_max_tokens: 2048       # Max tokens for summary (default: 2048)
        show_summary_markers: false    # Show [SUMMARY] markers (default: false)
        compact_on_resume: true        # Auto-compact when resuming (default: true)
```
</File>
...
<dl>
  ...
  <dt>`sessions.auto_compact`</dt>
  <dd>Auto-compact configuration for intelligent message history management. See [Auto-Compact](/ai/sessions#auto-compact-extended-conversations) for detailed documentation</dd>

  <dt>`sessions.auto_compact.enabled`</dt>
  <dd>Enable automatic conversation compaction using AI-powered summarization (default: `false`)</dd>

  <dt>`sessions.auto_compact.trigger_threshold`</dt>
  <dd>Trigger compaction at N% of max_history_messages (0.0-1.0, default: `0.75` = 75%)</dd>

  <dt>`sessions.auto_compact.compact_ratio`</dt>
  <dd>Percentage of old messages to compact when triggered (0.0-1.0, default: `0.4` = 40%)</dd>

  <dt>`sessions.auto_compact.preserve_recent`</dt>
  <dd>Number of most recent messages to always keep uncompacted (default: `10`)</dd>

  <dt>`sessions.auto_compact.use_ai_summary`</dt>
  <dd>Use AI to generate intelligent summaries instead of simple concatenation (default: `true`)</dd>

  <dt>`sessions.auto_compact.summary_provider`</dt>
  <dd>AI provider for summarization (default: uses `default_provider`)</dd>

  <dt>`sessions.auto_compact.summary_model`</dt>
  <dd>Model for summarization, typically a faster/cheaper model like `claude-3-haiku-20240307` (default: provider's default model)</dd>

  <dt>`sessions.auto_compact.summary_max_tokens`</dt>
  <dd>Maximum tokens for summary generation (default: `2048`)</dd>

  <dt>`sessions.auto_compact.show_summary_markers`</dt>
  <dd>Show `[SUMMARY]` markers around compacted sections in conversation history (default: `false`)</dd>

  <dt>`sessions.auto_compact.compact_on_resume`</dt>
  <dd>Automatically check and compact when resuming a session (default: `true`)</dd>
</dl>
...
### Three Approaches: Simple Truncation, Auto-Compact, or Both
...
Atmos offers three ways to manage conversation history:

#### Approach 1: Simple Truncation (Message/Token Limits)

Simple truncation drops old messages when limits are reached. Easy to configure but loses context.
...
#### Approach 2: Auto-Compact (Intelligent Summarization)

Auto-compact uses AI to intelligently summarize old messages instead of dropping them. **Recommended for long, context-heavy conversations** - preserves important information while managing history.
...
settings:
  ai:
    enabled: true
    default_provider: "anthropic"
    max_history_messages: 50           # Required: defines capacity for auto-compact
    ...
    sessions:
      ...
      auto_compact:
        ...
        trigger_threshold: 0.75        # Trigger at 75% capacity (37 messages)
...
**What happens:**
- When conversation reaches 37 messages (75% of 50), auto-compact triggers
- Oldest 15 messages (40% of 37) are summarized by AI
- Summary preserves key context (decisions, configurations, insights)
- Most recent 10 messages always preserved verbatim
- Summary replaces original messages in history

**Benefits over simple truncation:**
- **Preserves context**: Important decisions and context remain available
- **Seamless**: AI continues conversation naturally with summarized history
- **Configurable**: Control when to compact and what to preserve
- **Cost-effective**: Use cheaper models (Haiku) for summarization

:::info Complete Auto-Compact Documentation
See [Auto-Compact](/ai/sessions#auto-compact-extended-conversations) for detailed configuration, examples, and best practices.
:::

#### Approach 3: Combine Auto-Compact with Hard Limits

For maximum control, use auto-compact with hard truncation limits. Auto-compact handles normal growth, hard limits provide safety net.
...
settings:
  ai:
    enabled: true
    default_provider: "anthropic"
    max_history_messages: 100          # Hard limit (safety net)
    max_history_tokens: 15000          # Hard token limit
    ...
    sessions:
      ...
      auto_compact:
        enabled: true
        trigger_threshold: 0.75        # First line: compact at 75 messages
        compact_ratio: 0.5             # Compact 50% to stay below hard limits
        preserve_recent: 15            # Keep more recent messages
```
</File>

**How this works:**
1. **Normal operation**: Auto-compact maintains history at 75 messages by summarizing
2. **Safety net**: If summaries accumulate and hit 100 messages, hard truncation kicks in
3. **Cost control**: Token limit ensures you never exceed budget regardless of message count
...
| Use Case | Message-Based (`max_history_messages`) | Token-Based (`max_history_tokens`) | Auto-Compact | Best Approach |
|----------|----------------------------------------|-------------------------------------|--------------|---------------|
| Short questions | 0 (unlimited) | 0 (unlimited) | Disabled | No limit needed |
| Long conversations | 20-30 messages | 6000-8000 tokens | Enabled (50 msg, 75% threshold) | Auto-compact preserves context |
| Complex analysis | 40-50 messages | 10000-15000 tokens | Enabled (100 msg, 70% threshold) | Auto-compact + token limit |
| Cost optimization | 10-20 messages | 4000-6000 tokens | Enabled with cheaper model | Auto-compact with Haiku |
| Rate limit issues | 10-20 messages | 6000-8000 tokens | Enabled (30 msg, 70% threshold) | Auto-compact + token limit |
| Context preservation | 50+ messages | 12000+ tokens | Enabled (higher thresholds) | Auto-compact essential |
| Combined protection | 50 messages | 8000 tokens | Enabled (safety net) | Auto-compact + hard limits |
...
Atmos AI uses project-specific context, patterns, and conventions from a persistent instructions file (`ATMOS.md`). This provides consistent, context-aware responses without repeatedly explaining your project setup.
...
**Learn more:** [Project Instructions](/ai/instructions) - Complete instructions documentation including:
- Instructions file structure and sections
...
- **For long conversations (intelligent)**: Enable auto-compact to preserve context while managing history - see [Auto-Compact](/ai/sessions#auto-compact-extended-conversations)
...
- **For context preservation**: Enable auto-compact instead of simple truncation to keep important information from earlier in the conversation
