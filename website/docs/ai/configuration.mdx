...
<dl>
  ...
  <dd>Maximum conversation messages to keep in history (default: `0` = unlimited). Use this to prevent rate limiting and reduce token usage in long conversations. Only the most recent N messages are sent to the AI.</dd>
</dl>
...
Use `max_history_messages` to keep only the most recent N messages:
...
When `max_history_messages` is set:

1. **Provider filtering applied first**: Only messages from the current provider session are included
2. **Sliding window applied**: If history exceeds the limit, only the most recent N messages are kept
3. **System messages excluded**: UI notifications don't count toward the limit
4. **Current message added**: Your new message is always included (not counted in the limit)
...
|----------|------------------|---------|
| Short questions | 0 (unlimited) | No need to limit, conversations are brief |
| Long conversations | 20-30 | Balance between context and cost |
| Complex analysis | 40-50 | More context for intricate discussions |
| Cost optimization | 10-20 | Minimize token usage |
| Rate limit issues | 10-20 | Prevent hitting provider limits |
...
| `max_history_messages` | 0 (unlimited) | Maximum conversation messages to keep in history |
...
- **For long conversations**: Set `max_history_messages` to 20-50 to prevent rate limiting and reduce costs
