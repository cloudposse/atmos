...
To get an idea of what it looks like using `atmos` on the command line, just [try our quickstart]() and run [`atmos`](/cli/commands/atmos) command to start an interactive UI in the terminal. Use the arrow keys to select stacks and components to deploy.
