...
# Atmos is the Ultimate Terraform Composition Tool for DevOps
...
Concepts like [Stacks](/core-concepts/stacks) and [Components](/core-concepts/components), help you overcome most of [Terraform's limitations](/reference/terraform-limitations/) 
...
To get an idea of what it looks like using `atmos` on the command line, just [try our quickstart](/quick-start/) and run the [`atmos`](/cli/commands/atmos) command to start an interactive UI in the terminal. Use the arrow keys to select stacks and components to deploy.
...
perfectly into continuous integration (CI/CD) workflows with it's [GitHub Actions](/integrations/github-actions/) or support for other [TACOS](/integrations/). 
...
At a high-level, Atmos is responsible for managing the configuration that get's passed to Components when they are instantiated.
...
as generic and resuable as possible by parameterizing them. In doing so, we shift the complexity from the HCL code, to managing 
...
[Components](/core-concepts/components) (e.g. Terraform root modules) we need to bring up our environment. For example, in it's 
...
making it an indispensable asset for managing complex infrastructures precisely. Atmos revolutionizes technical workflows, ensuring
seamless, automated infrastructure management.

* **Consistent Invocation:** Standardizes how Terraform is invoked to avoid confusion and errors.
* **Seamless Initialization:** Prepares backends, modules, and other prerequisites automatically.
* **Separation of Configuration from Code:** Divides configuration into Stacks and Components for clarity and reusability.
* **Enables Efficient Configuration:** Utilizes imports, hierarchy, inheritance, and deep merging for streamlined configurations.
* **Ensures Structured Modularity:** Builds Terraform configurations using stacks and catalogs for modular design.
* **Promotes Reusable Building Blocks:** Establishes a library of reusable components (a.k.a Terraform root modules). Reduce the need for custom Terraform components.
* **Makes Composition Easier:** Integrates root modules into a stack manifest, akin to Docker Compose.
* **Stack Organization:** Utilizes a stack-based approach for organizing Terraform configurations.
* **Simplifies State Backends:** Dynamically generates backend configurations (as JSON) based on the stack context.
* **OPA Policy Controls:** Integrates Open Policy Agent for high-level policy enforcement.
* **Dependency Coordination:** Manages dependencies among Terraform components.
* **Standardized Workflows:** Offers workflows for standardizing command invocation.
* **Turnkey CI/CD:** Facilitates integration with CI/CD platforms without the overhead of commercial solutions
