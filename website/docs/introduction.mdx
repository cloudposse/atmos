...
Atmos is a workflow automation tool for developers and DevOps teams alike, that make it fun to operate very large infrastructures with DRY configuration. 
It's especially adept at [overcoming Terraform's limitations](/reference/terraform-limitations/) with [established conventions](/category/core-concepts) and [design patterns](/design-patterns/) that 
give cloud architect's the warm and fuzzies.
...
To get an idea of what it looks like using `atmos` on the command line, just [try our quickstart](/quick-start/) and run [`atmos`](/cli/commands/atmos) command to start an interactive UI in the terminal. Use the arrow keys to select stacks and components to deploy.
...
With [so many of these alternatives](/reference/alternatives/), why does the world need another "workflow" automation tool?

The answer is simple. **None of them pull off what you can do with Atmos.**
...
We help our customers own their infrastructure using our [reference architecture blueprints](https://cloudposse.com/reference-architecture/) and [implement it with their teams](https://cloudposse.com/services/) while showing them the ropes.
We needed to scratch our own itch. After trying everything, and running upgainst [Terraform's limitations](/reference/terraform-limitations/), we realized it would not be possible to achieve the outcome we wanted for our customers using a mashup of existing off-the-shelf tools.
...
Why not give Atmos a try? Check out our [quickstart](/quick-start).
...
- [Hierarchical configurations](/core-concepts/stacks/catalogs#filesystem-layout) to reduce complexity
- DRYness with [inheritance](/core-concepts/components/inheritance) and [imports](/core-concepts/stacks/imports), and native [vendoring](/core-concepts/vendoring/)
- Team collaboration with [reusable service catalogs](/core-concepts/stacks/catalogs), and [GitOps-style automation](/category/integrations).
- [Reusable workflows](/core-concepts/workflows/) that run anywhere and a beautiful UI to run them.
...
- Established conventions and [design patterns](/design-patterns/) to manage terraform @ scale
- Break terraliths into smaller pieces ([“components”](/components/))
- Enable team collaboration with [catalogs](/core-concepts/stacks/catalogs) and [libraries](/core-concepts/components/library)
- Enforce guardrails with [OPA and JSON Schema validation](/core-concepts/components/validation)
- Zero Vendor Lock-in

### Established Conventions

Part of the benefit of working with Atmos is we have many [established conventions](/category/core-concepts) that you can adopt.
...
- Atmos is not an alternative to CI/CD systems, but you will likely call Atmos from those systems.
