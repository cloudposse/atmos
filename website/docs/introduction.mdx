...
description: >-
  Atmos is a workflow automation tool to manage complex configurations with ease. 
...
Atmos is a workflow automation tool that makes it more managable to operate very large environments with DRY configuration.
...
We needed to scratch our own itch. After trying everything (`make`, `variant`, `mage`, `gotask`, `terragrunt`, etc), we realized it would not be possible to achieve the outcome we wanted for our customers using a mashup of existing off-the-shelf tools. 
...
- Large (even massive) Multi-Account Environments
...
:::info
By running a [single command](/cli/commands/terraform/generate-varfiles.md), you can eject from Atmos and never touch it again.
:::

<br/>

[Why not give Atmos a try?](/category/quick-start)
...
Using YAML, `atmos` is easily able to support features like [imports](/core-concepts/stacks/imports), [inheritance](/core-concepts/components/component-inheritance), deep merging, and [policies](/core-concepts/components/component-validation).
...
<ReactPlayer controls url='https://www.youtube.com/watch?v=0dFEixCK0Wk&t=1415s' />
<br/>
