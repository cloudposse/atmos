...
When you only need to set a single value deep in a nested structure, you can use **dot notation** in YAML mapping keys instead of creating the full hierarchy. This makes your configuration more concise and readable.

:::info Configuration Required
Dot notation requires enabling the `key_delimiter` setting in your `atmos.yaml`. Without it, dotted keys are treated as literal key names.
...
See [YAML Settings](/cli/configuration/settings#yaml-settings) for details.
:::

:::warning Existing Dotted Keys
After enabling `settings.yaml.key_delimiter: "."`, any unquoted mapping key containing `.` is expanded. If you need a literal dotted key, quote it:
...
Before enabling this in an existing stack, check for unquoted dotted keys that should remain literal.
:::
...
When dot notation and traditional nesting set the same nested key in the same file, the later entry wins:

<File title="stacks/prod.yaml">
```yaml
# Result: metadata.description is "from dot notation"
metadata:
  description: from nesting
metadata.description: from dot notation

# Result: settings.spacelift.workspace_enabled is false
settings.spacelift.workspace_enabled: true
settings:
  spacelift:
...
### Quoting Preserves Literal Keys

When `key_delimiter` is enabled, you can use YAML quoting (single or double quotes) to prevent mapping key expansion. Quoted keys are always treated as literal key names:
...
components:
  terraform:
    vpc:
      # Unquoted: expands to metadata: { component: vpc-base }
      metadata.component: vpc-base

      # Quoted: stays as literal key "output.json"
      "output.json": true
      'config.file': /etc/app.conf
```
</File>

This is useful when your key names genuinely contain the delimiter character (e.g., filenames like `output.json`).

### Custom Delimiters

You can use any string as the delimiter, not just `.`:
...
components:
  terraform:
    vpc:
      # With "::" delimiter, dots are literal
      metadata::component: vpc-base
      settings::spacelift::workspace_enabled: true

      # Dotted keys stay as-is (not expanded)
      output.json: true
```
</File>
...
Keys with leading, trailing, or consecutive delimiters are treated as literal (never expanded):

```yaml
# These are ALL treated as literal keys (not expanded)
.leading: value        # Leading delimiter
trailing.: value       # Trailing delimiter
double..dot: value     # Consecutive delimiters
```

:::warning External Tooling
Dot notation is an Atmos-specific feature. External YAML tools — IDE linters, `yamllint`, YAML language servers, and schema validators — will see the unexpanded keys and may report false errors. For example, a linter validating against a JSON Schema that expects `metadata.component` to be a nested object will see `metadata.component` as a single flat key and flag it as invalid.

If your workflow relies heavily on external YAML validation, consider using traditional nesting for validated sections and dot notation only where convenience outweighs tooling support.
:::
