...
To better understand why we wrote Atmos, it might help to understand how we got here by taking a closer look at a tool near and dear to our heart: Terraform (& OpenTofu).
...
- Terraform's HCL started strictly as a configuration language, not a markup or programming language, although has evolved considerably over the years.
- HCL is backward compatible with JSON, although it's not a strict superset of JSON. HCL is more human-friendly and readable, while JSON is often used for machine-generated configurations. This means you can write Terraform configurations in HCL or JSON, and Terraform will understand them. This feature is particularly useful for generating configurations programmatically or interoperation with systems that already use JSON.
...
- **Lack of DRY Configurations**: Terraform does not inherently support hierarchical configurations. There's no support for deep merging configurations, making manual `varfile` maintenance unscalable. This makes it more difficult to enforce organizational standards, security controls, tagging, and policies.
...
- **Dependency Management**: Standalone Terraform doesn't provide any mechanisms for orchestrating dependencies among root modules.
...
## What is the natural progression of a Terraform adoption?
...
2. The more root modules, composition is difficult and there's no automatic dependency resolution for multiple root modules in Terraform
3. Root modules get more complicated, and terraform command line to invoke is complicated (sometimes using `-target` as a hack to work around cycles)
...
2. The scripts evolved to solve specific problems encountered that made Terraform cumbersome as a tool.

**New Problems**

1. What happens in practice is every team using Terraform ends up building different scripts, often then combining those with Makefiles, into a hodgepodge of configurations with different levels of validation. The scripts grow in complexity and have to survive generations of developers.
...
1. Configuration starts drifting more often, as the team neglects to apply changes consistently, or "ClickOps" persists in the organization.
2. Developers are stepping on each other's toes. What worked when only a few developers could access Terraform no longer scales.
3. Poor controls and lack of consistency of Terraform code. Tons of duplication.

### Stage 9: DIY GitOps

With the greater adoption of Terraform and DevOps principles, Developers are now using Terraform daily. They decide to use the same patterns for deploying applications with Terraform. Only Terraform is extremely different from deploying containerized apps. There are no rollbacks. It's more akin to performing database migrations without transactions. Scary business. Controls are needed.

1. Developers stick their scripts in a CI/CD pipeline.
2. Post comments to every PR which the output of the Terraform Plan
3. Apply changes on merge to main (which changes, though? Did they store the plan file as an artifact that will get applied?)

**New Problems**

- Static cloud credentials are frequently used as simple environment variables (not to mention these are usually Administrative-level credentials)
- No visibility into the impact of the changes across multiple environments at the same time
- Limited security controls lack of plan file storage means incorrect implementation of plan/apply workflows.
- The entire organization is spammed by GitHub comments every time someone pushes a commit, and a plan is run on a PR

### Stage 10: Terraform Bankruptcy

Developers realize something is not right.

- Scripts all over the place to call terraform
- Inconsistent conventions on what is a module, root module and when to combine them
- Root modules need to be provisioned in specific orders to bring them up and down
- No way to define dependencies between root modules
- Passing state between root modules is inconsistent
- Automating the terraform in CI/CD is oversimplified
- Inconsistent conventions or a total lack of conventions
- Incomplete or out of date documentation

They begin to ask, why is this so hard? This should be a solved problem. *(Then they discover Cloud Posse and Atmos!)* 😉

### Stage 11: New Toolchain Adoption
...
3. **Supported Efficient Configuration:** I need a method similar to Kustomize for configurations. It should reduce redundancy and support features like imports, hierarchy, inheritance, and deep merging to help set organizational standards, security, and tags with ease.
...
5. **State Governance:** I need powerful tools to protect and manage remote state, especially considering the complexities of managing state at scale.
...
13. **Turnkey CI/CD:** I want to effortlessly integrate with GitHub Actions and run operations without being weighed down by the hefty fees of HashiCorp Terraform Enterprise and other TACOS.

*Oh, one last ask. Can it also be free and open source?*

## What's the Solution?

😎 Good news! Atmos supports all of this out of the box and exactly what you **cannot** do with "standalone" Terraform.

- **Consistent Invocation:** Atmos ensures Terraform is invoked the same way every time. This removes guesswork and confusion across teams with what parameters to pass, and what order of operations things need to be invoked.
- **Seamless Initialization:** Backend? Modules? Prep steps? Atmos makes sure everything is ready before you even think of "terraform apply."
...
- **Ensures Structured Modularity:** With Atmos, Terraform configurations are built modularly. This not only ensures code is recyclable but also that it's efficient.
- **Promotes Reusable Building Blocks:** Set up a robust library of proven and easily reusable components (terraform root modules). Once done, there's minimal need for extra HCL work. This reduces the number of root modules needed in an organization and facilitates integration testing.
- **Makes Composition Easier:** Think Docker Compose, but for Terraform. Atmos seamlessly integrates root modules.
...
- **Simplifies State Governance:** Atmos doesn't mess around. It ensures the remote state is safeguarded and managed, especially when scaling is a challenge.
...
- **Turnkey CI/CD:** Integrate with [GitHub Actions](https://atmos.tools/category/github-actions), [Spacelift](https://atmos.tools/integrations/spacelift), or [Atlantis](https://atmos.tools/integrations/atlantis) seamlessly. Avoid the steep costs associated with commercial solutions that charge based on the number of resources under management, self-hosted runners, or users.

Oh yes, and it's entirely free and true Open Source (licensed APACHE2) like [everything else Cloud Posse builds](https://github.com/cloudposse) (unlike HashiCorp and Terraform 🔥)

- It's proven its weight in gold for hundreds of organizations
- Supported by an active community and corporate sponsors
- There's also lots more in the works, coming to a happy DevOps team near you.
