...
The LSP Server and LSP Client validate **completely different things**:

<dl>
  <dt>**Atmos structure** (import arrays, component maps, stack inheritance)</dt>
  <dd>**Atmos LSP Server:** Yes. **External LSP Servers (via Client):** No.</dd>

  <dt>**Generic YAML syntax** (YAML schemas)</dt>
  <dd>**Atmos LSP Server:** No. **External LSP Servers (via Client):** Yes.</dd>

  <dt>**Terraform syntax** (HCL parsing)</dt>
  <dd>**Atmos LSP Server:** No. **External LSP Servers (via Client):** Yes.</dd>

  <dt>**Typos/unknown properties** (schema validation)</dt>
  <dd>**Atmos LSP Server:** No. **External LSP Servers (via Client):** Yes.</dd>
</dl>
...
<dl>
  ...
  <dt>**LSP Server**</dt>
  <dd>Validates Atmos structure. Used by IDEs (VS Code, Neovim). N/A (IDEs connect TO it).</dd>

  <dt>**Direct Validation**</dt>
  <dd>Validates Atmos structure. Used by Atmos AI. N/A (same process).</dd>
</dl>

**Why both exist:**
- **LSP Server** (this doc): IDEs need to validate Atmos-specific structure (imports, components, stack inheritance), so they connect to Atmos' LSP server for real-time validation and autocomplete
- **LSP Client**: Atmos doesn't have built-in Terraform/HCL parsers, so Atmos AI connects to external LSP servers (terraform-ls, yaml-language-server) to validate generic syntax
...
- **Basic autocomplete** - Suggestions for Atmos keywords and components based on current line context
...
atmos lsp start --transport tcp --address 127.0.0.1:7777
```
...
- **Base:** ~20-30MB per server instance
- **Per open file:** ~1-2MB
- **Maximum recommended:** 50 open files per instance

### Validation Latency

- **Small files** (&lt;100 lines): &lt;50ms (P95)
- **Medium files** (100-1000 lines): &lt;200ms (P95)
- **Large files** (1000+ lines): &lt;500ms (P95)

### Startup Time

- **Cold start:** &lt;1 second
- **Subsequent starts:** &lt;500ms

### File Size Limits

- **Recommended:** &lt;5000 lines per file
- **Maximum:** &lt;10000 lines per file
- **Note:** Larger files may experience degraded performance
...
- **Firewall configuration** - Restrict access to trusted IPs
- **Network segmentation** - Keep LSP server on internal network
- **TLS encryption** - Consider using reverse proxy with TLS for WebSocket
...
<dl>
  <dt>**Purpose**</dt>
  <dd>**LSP Server:** IDE integration for Atmos files. **LSP Client:** AI validation of infrastructure files.</dd>

  <dt>**Direction**</dt>
  <dd>**LSP Server:** IDEs connect TO Atmos. **LSP Client:** Atmos connects TO language servers.</dd>

  <dt>**Provides**</dt>
  <dd>**LSP Server:** Atmos-specific features. **LSP Client:** Generic YAML/TF/HCL validation.</dd>

  <dt>**Use Case**</dt>
  <dd>**LSP Server:** Editing Atmos stack files. **LSP Client:** Atmos AI file validation.</dd>

  <dt>**Autocomplete**</dt>
  <dd>**LSP Server:** Atmos keywords & components. **LSP Client:** No autocomplete (validation only).</dd>

  <dt>**Hover Docs**</dt>
  <dd>**LSP Server:** Atmos-specific documentation. **LSP Client:** No hover (validation only).</dd>

  <dt>**Diagnostics**</dt>
  <dd>**LSP Server:** Atmos structure validation. **LSP Client:** Generic syntax validation.</dd>

  <dt>**Implementation**</dt>
  <dd>**LSP Server:** `pkg/lsp/server/`. **LSP Client:** `pkg/lsp/client/`.</dd>

  <dt>**Configuration**</dt>
  <dd>**LSP Server:** Editor config. **LSP Client:** `atmos.yaml`.</dd>
</dl>
...
- **LSP Server** (this doc): For editing Atmos files in your IDE with Atmos-specific autocomplete and validation
- **LSP Client**: For Atmos AI to validate generic YAML/Terraform/HCL syntax using external language servers
- **Both together**: Use the LSP Server for IDE integration while editing, and the LSP Client for AI-powered generic syntax validation

**Important:** The LSP Client does **not** connect to the Atmos LSP Server. That would be redundant since Atmos AI can call Atmos validation functions directly. The LSP Client only connects to external LSP servers (yaml-language-server, terraform-ls) for generic syntax validation that Atmos doesn't provide natively.
...
- [Stacks](/stacks) - Learn about Atmos stack files
- [Components](/components) - Learn about Atmos components
