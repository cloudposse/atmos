...
- Deprecation warnings for fields the JSON Schema marks `deprecated: true`, surfaced as
  warning-severity diagnostics (never errors) with migration guidance when the schema
  provides an `x-atmos-replacement` hint
