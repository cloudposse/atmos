...
**Atmos LSP Client** (this document): Connects TO external LSP servers (yaml-ls, terraform-ls) for **Atmos AI validation**. This is implemented in `pkg/lsp/client`.

**Atmos LSP Server** (future): Will allow **IDEs to connect TO Atmos** for Atmos-specific features like stack syntax validation, component autocomplete, and intelligent suggestions. This will be implemented in `pkg/lsp/server` and is not yet available.
:::
...
- [IntelliJ LSP](https://github.com/redhat-developer/lsp4ij) - LSP plugin for IntelliJ IDEA
    ...
    If you want **your IDE to connect to Atmos** for Atmos-specific features like stack validation and component autocomplete, you're looking for an **Atmos LSP Server** (not the LSP client documented here).

    **Atmos LSP Server** will be implemented in the future in `pkg/lsp/server/` and will allow IDEs like VS Code, Neovim, and others to get intelligent Atmos-specific assistance. This is a separate feature from the LSP client.
