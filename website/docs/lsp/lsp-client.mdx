...
2. If `tools.allowed` is set, make sure it includes `validate_file_lsp` — otherwise it's never registered:
```yaml
ai:
  tools:
    enabled: true
    allowed: []   # empty = every tool registers, including validate_file_lsp
    blocked: []   # or, if using an allow-list: allowed: [validate_file_lsp, ...]
```
