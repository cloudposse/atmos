...
Vendored dependency management works well with [native CI in GitHub Actions](/ci). Use `atmos vendor update --check` to identify eligible updates, review them with `atmos vendor diff`, then commit the selected dependency pins for pull-request review.
:::
