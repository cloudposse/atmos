...
<dl>
  ...
  :::note
  When no retry configuration is provided, defaults apply. When a partial configuration is provided, only the specified fields are set—unspecified fields use zero values (e.g., `0` for attempts, `0s` for durations), not defaults.
  :::
  ...
  <dl>
    <dt>`max_attempts`</dt>
    <dd>Maximum number of retry attempts. Default: `1` (no retries)</dd>

    <dt>`initial_delay`</dt>
    <dd>Initial delay before the first retry. Default: `100ms`</dd>

    <dt>`max_delay`</dt>
    <dd>Maximum delay between retries. Default: `5s`</dd>
    ...
    <dt>`random_jitter`</dt>
    <dd>Random jitter factor (0.0-1.0) to add randomness to delays. Default: `0.0` (no jitter)</dd>

    <dt>`max_elapsed_time`</dt>
    <dd>Maximum total time for all retry attempts. Default: `30m`</dd>
  </dl>
  </dd>
</dl>
