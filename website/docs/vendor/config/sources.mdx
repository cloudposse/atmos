...
<dl>
  ...
  <dd>
    ...
    A relative local path is resolved against the directory containing the manifest that declares it. For a source declared in a manifest
    pulled in through [`spec.imports`](/vendor/config/imports), that's the imported manifest's own directory — not the directory of the
    manifest doing the importing. This keeps a manifest self-contained: it can be moved, or imported from anywhere, and its local sources
    still point at the same files.
  ...
  <dt>`targets`</dt>
  <dd>
  The `targets` in each source supports absolute paths and relative paths. Multiple targets can be specified.

  Relative targets are resolved against one of two roots, depending on how Atmos located the manifest:

  <dl>
    <dt>The Atmos `base_path`</dt>
    <dd>
      When `atmos.yaml` declares where the manifest lives via the [`vendor.base_path`](/cli/configuration/vendor) setting, relative targets
      resolve against the Atmos `base_path` — the same root that `components.terraform.base_path` and `stacks.base_path` are relative to.
      This lets you keep `atmos.yaml` and `vendor.yaml` together in a configuration directory while vendored artifacts still land in the
      project, rather than next to the manifest.
    </dd>

    <dt>The manifest's own directory</dt>
    <dd>
      When `vendor.base_path` is not configured and Atmos discovers `vendor.yaml` in the working directory, relative targets resolve against
      the directory containing the manifest.
    </dd>
  </dl>

  Absolute targets are always used as-is.
  </dd>
  ...
  <dl>
    <dt>`max_attempts`</dt>
    <dd>Maximum number of retry attempts. Set to `1` or higher to enable retries.</dd>

    <dt>`initial_delay`</dt>
    <dd>Initial delay before the first retry (e.g., `1s`, `500ms`).</dd>

    <dt>`max_delay`</dt>
    <dd>Maximum delay between retries (e.g., `30s`, `1m`).</dd>

    <dt>`backoff_strategy`</dt>
    <dd>Strategy for increasing delay between retries. Values: `exponential`, `linear`, `constant`.</dd>

    <dt>`multiplier`</dt>
    <dd>Multiplier for exponential/linear backoff (e.g., `2.0`).</dd>

    <dt>`random_jitter`</dt>
    <dd>Random jitter factor (0.0-1.0) to add randomness to delays.</dd>

    <dt>`max_elapsed_time`</dt>
    <dd>Maximum total time for all retry attempts (e.g., `5m`, `1h`).</dd>
  </dl>
  </dd>
</dl>
