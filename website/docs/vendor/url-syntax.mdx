...
<Tabs>
  <TabItem value="subdir" label="With Subdirectory" default>
    ```yaml
    source: "github.com/cloudposse-terraform-components/aws-vpc.git//src?ref=v1.539.0"
    ```

    **Result:** Clones the repository and extracts only the `src` subdirectory.
  </TabItem>
  ...
  <TabItem value="no-delimiter" label="Without Delimiter">
    ```yaml
    source: "github.com/cloudposse-terraform-components/aws-vpc.git?ref=v1.539.0"
    ```
...
sources:
  - component: "vpc"
    source: "github.com/cloudposse-terraform-components/aws-{{.Component}}.git//src?ref={{.Version}}"
    version: "v1.539.0"
    targets:
      - "components/terraform/{{.Component}}"
```

**Result:** Downloads from `aws-vpc` repository at version `v1.539.0` to `components/terraform/vpc`.
...
```yaml
# ✅ Extract src subdirectory from a cloudposse-terraform-components repo
source: "github.com/cloudposse-terraform-components/aws-vpc.git//src?ref=v1.539.0"

# ✅ Use root directory (no subdirectory needed for repos where TF files are at root)
source: "github.com/cloudposse-terraform-components/aws-vpc.git?ref=v1.539.0"
```
