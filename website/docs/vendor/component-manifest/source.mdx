...
<dl>
  ...
  <dt>`constraints`</dt>
  <dd>
    Optional. Controls which upstream versions <a href="/cli/commands/vendor/vendor-update">`atmos vendor update`</a> may select for this component, mirroring the `constraints` key supported on `vendor.yaml` sources. See <a href="/cli/commands/vendor/vendor-update#version-constraints">Version constraints</a> for the full `version` / `excluded_versions` / `no_prereleases` schema.
