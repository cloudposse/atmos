...
The multiline `<<EOF` delimiter form handles any setting value safely, including ones with embedded newlines — a plain `value=$(...)` assignment can silently split into multiple `$GITHUB_OUTPUT` records if the query result isn't a single-line scalar.
