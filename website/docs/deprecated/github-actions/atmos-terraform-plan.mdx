...
This isn't a drop-in swap of one action call. You still own and manage the storage either way — the same S3 bucket works with both — but the dedicated DynamoDB table, storage IAM role, and separate storage action go away in favor of config the `atmos` CLI talks to directly. Here's what actually changes:

**Removed**
- The `github-action-atmos-terraform-plan` step
- The S3 bucket + DynamoDB table (or Azure Blob + CosmosDB) provisioned for [`github-action-terraform-plan-storage`](/deprecated/github-actions/terraform-plan-storage), and the IAM role used only to write to it
