...
- `logs.level` - Log level. Supported log levels are `Off`, `Trace`, `Info`. If the log level is set to `Off`, Atmos will not log any messages (note
  that this does not prevent other tools like Terraform from logging). The ENV variable `ATMOS_LOGS_LEVEL` can also be used
...
| ATMOS_LOGS_LEVEL                                      | logs.level                                      | Log level. Supported log levels are `Off`, `Trace`, `Info`. If the log level is set to `Off`, Atmos will not log any messages (note that this does not prevent other tools like Terraform from logging) |
