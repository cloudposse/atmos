...
<ActionCard title="Configure Schema Validation">
    Learn how to configure JSON Schema and OPA policy paths for validating Atmos stack manifests.
    <div>
...
<CastPlayer title="atmos validate stacks --help" src="/casts/screengrabs/atmos-validate-stacks--help.cast" static chrome />
...
Stack manifest fields the JSON Schema marks `deprecated: true` — for example the legacy
component-level `depends_on` in favor of `dependencies.components` — produce warnings, not
validation errors. A deprecated field is still valid input; it never causes
`atmos validate stacks` (or its `atmos stack validate` alias) to exit non-zero:

<Terminal title="atmos validate stacks">
```console
stacks/deploy/dev.yaml:14:7: warning: `application.depends_on` is deprecated; use `dependencies.components`
```
</Terminal>

Where the schema provides migration guidance, the warning names the field to use instead.
The scanner only resolves `deprecated` annotations reached through local `$ref`s within the
configured `--schemas-atmos-manifest` document — a remote schema that declares deprecated
fields only in a separately fetched, externally referenced document validates without a
warning for those fields.
...
- Configure the following section in the `atmos.yaml` [CLI config file](/cli/configuration)
  ...
  schemas:
    ...
    atmos:
      ...
      # Or pin to the schema shipped with a specific Atmos release instead of the latest:
      # manifest: "https://atmos.tools/schemas/atmos/atmos-manifest/1.219.0/atmos-manifest.json"
  ```

  See [Floating vs. Pinned Schema URLs](/cli/configuration/schemas#floating-vs-pinned-schema-urls) for when to use a pinned URL.
