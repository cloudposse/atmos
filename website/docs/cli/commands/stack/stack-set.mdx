...
<dl>
  ...
  <dd>How to interpret `<value>`: `auto` (infer from the type of the value already at the path, falling back to string with a warning), `string`, `int`, `bool`, `float`, `null`, or `yaml` (raw literal).</dd>
  <dt>`--file` (string)</dt>
