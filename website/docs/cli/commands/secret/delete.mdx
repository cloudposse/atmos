...
<Intro>
Remove a declared secret's value from its configured backend. Atmos prompts for confirmation before deleting unless you pass `--force`. The secret declaration in your stack configuration is not modified — only the stored value is removed. This command is also available under the `rm` alias.
</Intro>
