...
description: Use this command to perform sequential execution of `atmos`, `shell`, and `script` commands defined as workflow steps.
---
...
<ActionCard title="Step Types Reference">
    Explore 20+ step types for building interactive workflows including prompts, spinners, tables, and more.
    <div>
      <PrimaryCTA to="/workflows/steps/type">Step Types Reference</PrimaryCTA>
    </div>
</ActionCard>

## Usage

<CastPlayer title="atmos workflow --help" src="/casts/screengrabs/atmos-workflow--help.cast" static chrome />
...
[Atmos Custom Commands](/cli/configuration/commands)), shell commands, inline scripts, and have access to the stack configurations.
...
### Filter Atmos Steps by Tags and Labels

Pass `--tags` and `--labels` to apply the existing component selectors to every `type: atmos` workflow step:
...
```shell
atmos workflow deploy --tags networking --labels deployment:dev --labels cost-center:platform
```

For example, a step with `command: terraform apply --include-dependencies` receives both selectors and lets the Terraform command select the matching components.

### Inline Script Step

Use `type: script` when the step body should be interpreted by a non-shell runtime:
    ...
    steps:
      - name: validate
        ...
        script: |
          ...
          text = Path("../../website/static/casts/examples/sops-secrets/secret-lifecycle.cast").read_text()
          for needle in ["describe (masking on)", "encrypted at rest", "All proofs passed"]:
...
<dl>
  ...
  <dd>Component tag selector. Tags match any provided value and are forwarded to every `type: atmos` workflow step.</dd>

  <dt>`--labels` <em>(optional)</em></dt>
  <dd>Component label selector. Labels match all provided key-value pairs (comma-separated within one occurrence, and/or repeated for more) and are forwarded to every `type: atmos` workflow step.</dd>
