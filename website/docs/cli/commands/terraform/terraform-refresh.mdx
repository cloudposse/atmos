...
<CastPlayer title="atmos terraform refresh --help" src="/casts/screengrabs/atmos-terraform-refresh--help.cast" static chrome />
...
<dl>
    ...
    <dd>
        Enable streaming UI mode for real-time resource refresh progress. Shows which resources are being refreshed with status indicators.

        The UI automatically disables when output is piped, in CI environments, or when running unsupported commands.

        ```shell
        atmos terraform refresh vpc -s dev --ui
        ```
