...
<dl>
    ...
    <dd>
        Enable CI mode for automated pipelines. When set, Atmos fires the `before.terraform.generate.planfile`
        and `after.terraform.generate.planfile` hook events around the command, allowing user-defined hooks
        in stack configuration to run for `generate planfile` (parallel to how `--ci` works for
        `atmos terraform plan`).

        The flag is auto-enabled when `ATMOS_CI=true` or the standard `CI=true` env var is present, so most
        CI platforms (GitHub Actions, GitLab CI, etc.) activate it without explicit configuration.
        ...
        :::note
        Built-in CI plugin integration (job summaries, planfile upload, check runs, PR comments) for
        `generate planfile` is tracked separately in [issue #2497](https://github.com/cloudposse/atmos/issues/2497).
        This flag wires the hook lifecycle; the plugin-side integration follows in a subsequent PR.
        :::
    </dd>
</dl>
