...
<dl>
    ...
    <dd>
        ...
        When set to `false` (or `0`, `no`, `off`), Atmos skips identity authentication entirely and uses standard AWS credential resolution (environment variables, `~/.aws/credentials`, instance metadata, OIDC). This is useful in CI/CD environments using GitHub Actions OIDC or other external authentication mechanisms.
