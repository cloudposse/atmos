...
HCL-based domain-specific language and its interpreter. Atmos works with [OpenTofu](/projects/configuration/opentofu).
:::
...
- `atmos terraform apply` and `atmos terraform deploy` commands support `--from-plan` flag. If the flag is specified, the commands will use
  ...
  is saved to the component's folder. See [Terraform Planfiles](/core-concepts/terraform/planfiles) for comprehensive planfile documentation
...
- `atmos terraform apply` and `atmos terraform deploy` commands support `--planfile` flag to specify the path to a planfile.
  ...
  planfile. Learn more in the [terraform apply](/cli/commands/terraform/apply) documentation
...
- `atmos terraform plan` command accepts a `--skip-planfile` flag to skip writing the plan to a file. If the flag is set to `true`
  ...
  Terraform Cloud automatically stores plans in its backend and can't store it in a local file. See [terraform plan](/cli/commands/terraform/plan) for details
...
<dl>
    ...
    <dd>
        ...
        in dependency order (if [component dependencies](/stacks/dependencies/) are configured).
    ...
    <dd>
        For each directly affected component, detect the dependent components and process them in dependency order, recursively.
        Dependents are components that are indirectly affected, meaning that nothing in the current branch modifies their code
        or configs, but they are configured as [dependencies](/stacks/dependencies/) of the components that are modified.
    ...
    <dd>
        ...
        See [Disabling Identity Authentication](/cli/commands/auth/usage#disabling-identity-authentication) for more details.
...
with [dependencies between the components](/stacks/dependencies/):
