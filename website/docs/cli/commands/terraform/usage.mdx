...
- `atmos terraform plan` command accepts a `--plan-skip-planfile` flag to skip writing the plan to a file. If the flag is set to `true`
  (e.g., `atmos terraform plan <component> -s <stack> --plan-skip-planfile=true`), Atmos will not pass the `-out` flag
...
<dl>
    ...
    <dt>`--plan-skip-planfile` <em>(optional)</em></dt>
    <dd>
        ...
        ```shell
        atmos terraform plan <component> -s <stack> --plan-skip-planfile=true
        ```
    </dd>
