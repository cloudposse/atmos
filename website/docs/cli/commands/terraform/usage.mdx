...
<dl>
    ...
    <dd>
        Filter by labels (comma-separated `key=value` pairs, matches all): `--labels=cost-center=platform,compliance=sox`. Components are labeled via an optional `metadata.labels: {...}` map in stack manifests. Composes with `--all`/`--affected`/`--components`/`--query`/`--tags`. Cannot be combined with a single `component` argument.
