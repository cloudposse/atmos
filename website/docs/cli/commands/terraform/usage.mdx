...
<CastPlayer title="atmos terraform --help" src="/casts/screengrabs/atmos-terraform--help.cast" static chrome />
...
# Execute on affected components that are also tagged "production"
atmos terraform <command> --affected --tags production

# Execute on components labeled cost-center=platform
atmos terraform <command> --all --labels cost-center=platform
```
</Terminal>
...
- **Automatic initialization**: Atmos runs `terraform init` before executing other commands, but only when needed — it's skipped automatically once nothing that affects init has changed since the last run. See [Automatic Initialization](/cli/commands/terraform/init#automatic-initialization). Use `--skip-init` to disable it for one invocation.
...
<dl>
    ...
    <dd>
        Atmos Terraform/OpenTofu component name or filesystem path.<br/><br/>
        Supports both:
        <ul>
...
<dl>
    ...
    <dt>`--skip-hooks` <em>(optional)</em></dt>
    <dd>
        Bypass [lifecycle hooks](/stacks/hooks) for the current invocation
        without editing stack configuration. Hooks only run on `terraform`
        `plan`, `apply`, and `deploy`, so this flag lives on the `terraform`
        command and its subcommands.

        - **No value** (`--skip-hooks`): skip every hook.
        - **Comma-separated list** (`--skip-hooks=cost,security`): skip only the named hooks; others still run.
        - **Boolean** (`--skip-hooks=true|false`): explicit on/off.
        - Also available as the `ATMOS_SKIP_HOOKS` environment variable (same value syntax).
        - **Per-invocation only** — does not propagate to nested commands or workflows.

        ```shell
        # Skip all hooks for one run (emergency rollback, debugging, etc.)
        atmos terraform apply <component> -s <stack> --skip-hooks

        # Skip just the cost-estimator and security scanner
        atmos terraform plan <component> -s <stack> --skip-hooks=cost,security

        # Via environment variable
        ATMOS_SKIP_HOOKS=true atmos terraform apply <component> -s <stack>
        ```
    </dd>
    ...
    <dd>
        ...
        __NOTE__: All Atmos sections are available in the expression, e.g. `vars`, `locals`, `settings`, `env`, `metadata`, `backend`, etc.
    ...
    <dd>
        ...
        in dependency order (if [component dependencies](/stacks/dependencies/components) are configured).
    ...
    <dd>
        Filter by tags (comma-separated, matches any): `--tags=production,tier-1`. Components are tagged via an optional `metadata.tags: [...]` list in stack manifests. Composes with `--all`/`--affected`/`--components`/`--query` to narrow the selected set further, rather than being mutually exclusive with them. Cannot be combined with a single `component` argument.

        ```shell
        atmos terraform apply --tags production,tier-1
    ...
    <dd>
        Filter by labels (comma-separated `key=value` or `key:value` pairs within one occurrence, and/or repeated for more, matches all): `--labels=cost-center=platform,compliance=sox` or `--labels cost-center=platform --labels compliance=sox`. Components are labeled via an optional `metadata.labels: {...}` map in stack manifests. Composes with `--all`/`--affected`/`--components`/`--query`/`--tags`. Cannot be combined with a single `component` argument.

        ```shell
        atmos terraform apply --labels cost-center=platform
        atmos terraform apply --all --labels cost-center=platform,compliance=sox
        atmos terraform apply --labels cost-center=platform --labels compliance=sox
        ```
    </dd>

    <dt>`--include-dependents` <em>(optional; requires a multi-component selection)</em></dt>
    <dd>
        For each selected component, detect the components that depend on it and process them in dependency order, recursively.
        Dependents are components that are not selected themselves, but are configured as [dependencies](/stacks/dependencies/components) of the components that are.
        Works with any multi-component selection (`--all`, `--components`, `--query`, `--stack`, `--tags`, `--labels`, `--affected`), and dependents run even when they do not match the selector flags.

        Accepts an optional depth: the bare flag expands the full dependent chain, while `--include-dependents=2` bounds the expansion to two levels.
        Pass the depth with `=`; a space-separated value is not bound to the flag. Values `true` and `false` are accepted for backward compatibility (unlimited and off).
        ...
        atmos terraform apply --labels=env=dev --include-dependents=2 --dry-run
        ```

        **Environment variable:** `ATMOS_INCLUDE_DEPENDENTS`
    </dd>

    <dt>`--include-dependencies` <em>(optional; requires a multi-component selection)</em></dt>
    <dd>
        For each selected component, also process everything it depends on (its prerequisites), in dependency order, recursively.
        Prerequisites run even when they do not match the selector flags, and they can live in other stacks — for example, `--include-dependencies` with `-s dev` legitimately processes prerequisite components outside the `dev` stack.

        Accepts an optional depth: the bare flag expands the full dependency chain, while `--include-dependencies=1` bounds the expansion to direct dependencies.
        Pass the depth with `=`; a space-separated value is not bound to the flag. Values `true` and `false` are accepted for backward compatibility (unlimited and off).

        With `terraform destroy`, this flag also destroys shared prerequisites of the selection, so Atmos prints a warning; prefer `--include-dependents` for safe dependents-first teardown.

        ```shell
        atmos terraform plan --labels=env=dev --include-dependencies
        atmos terraform apply --components=eks/apps --stack=prod --include-dependencies=1 --dry-run
        ```
...
with [dependencies between the components](/stacks/dependencies/components):
