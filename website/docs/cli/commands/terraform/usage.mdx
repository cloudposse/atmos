...
- `atmos terraform apply` and `atmos terraform deploy` commands support `--planfile` flag to specify the path to a planfile.
  ...
  planfile. Learn more in the [terraform apply](/cli/commands/terraform/terraform-apply) documentation
...
- `atmos terraform plan` command accepts a `--skip-planfile` flag to skip writing the plan to a file. If the flag is set to `true`
  ...
  Terraform Cloud automatically stores plans in its backend and can't store it in a local file. See [terraform plan](/cli/commands/terraform/terraform-plan) for details
