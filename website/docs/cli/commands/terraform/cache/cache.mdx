...
<Screengrab title="atmos terraform cache --help" slug="atmos-terraform-cache--help" />
...
The proxy caches lazily — only the platform Terraform requests at run time. To pre-seed the cache for **multiple** platforms (mixed CI/developer fleets, air-gapped bundles), declare `mirror.platforms` and run [`atmos terraform cache mirror`](/cli/commands/terraform/cache/mirror).
