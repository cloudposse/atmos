...
<Intro>
Two different Terraform plan files can represent identical infrastructure changes. The `atmos terraform plan-diff` command reveals the **semantic differences** between plans—what actually changes in your infrastructure—not just file-level noise like timestamps or resource ordering.
</Intro>

<CastPlayer title="atmos terraform plan-diff --help" src="/casts/screengrabs/atmos-terraform-plan-diff--help.cast" static chrome />
