...
<CastPlayer title="atmos terraform import --help" src="/casts/screengrabs/atmos-terraform-import--help.cast" static chrome />
...
Instead of retyping `-lock-timeout` on every invocation, declare a default in
[`components.terraform.flags`](/cli/configuration/components/terraform#flags) — globally
in `atmos.yaml`, per stack, or per component. `import` supports only `lock_timeout` and
`lock` (it does not support `-parallelism`, `-refresh`, or `-compact-warnings`). A native
flag typed directly on the command line always wins over a declared default.
