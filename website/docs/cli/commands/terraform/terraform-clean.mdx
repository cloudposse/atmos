...
<Intro>
Clean up your local Terraform workspace by removing `.terraform`, `.terraform.lock.hcl`, and other transient files. Useful for troubleshooting provider issues, clearing cached modules, or preparing for a fresh init. Only affects local filesystem - remote workspaces remain untouched.
</Intro>
