...
The `--format=github` option is specifically designed for GitHub Actions workflows. It automatically writes to `$GITHUB_OUTPUT` (unless `--output-file` is specified) and handles multiline values using heredoc syntax:
...
<dl>
    ...
    <dd>
        ...
        | `github` | `key=value` or heredoc | GitHub Actions step outputs (`$GITHUB_OUTPUT`) |
