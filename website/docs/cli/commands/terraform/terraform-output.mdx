...
id: output
description: Read Terraform output values for an Atmos component from the state file
---
...
<CastPlayer title="atmos terraform output --help" src="/casts/screengrabs/atmos-terraform-output--help.cast" static chrome />
...
:::info Atmos Behavior
Atmos provides standard setup for this command including automatic `terraform init` (skipped when nothing relevant has changed since the last init — see [Automatic Initialization](/cli/commands/terraform/init#automatic-initialization)), workspace selection, and variable file generation. The output retrieval itself is handled by native Terraform.
:::
...
<dl>
    ...
    <dd>
        ...
        ```shell
        atmos terraform output vpc -s dev --init-mode=always
        ```
    </dd>
    ...
    <dd>
        ...
        ```shell
        atmos terraform output vpc -s dev --init-reconfigure=always
        ```
    </dd>
    ...
    <dd>
        ...
        ```shell
        atmos terraform output vpc -s dev --init-upgrade=always
        ```
    </dd>
