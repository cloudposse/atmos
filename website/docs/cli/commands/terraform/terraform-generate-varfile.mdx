...
| `--skip`              | Skip processing a specific Atmos YAML function<br/>in Atmos stacks manifests when executing the command.<br/>To specify more than one function,<br/>use multiple `--skip` flags, or separate the functions with a comma:<br/>`atmos terraform generate varfile <component> -s <stack> --skip=eval --skip=include`<br/>`atmos terraform generate varfile <component> -s <stack> --skip=terraform.output,include` |       | no       |                                                 |       | no       |
