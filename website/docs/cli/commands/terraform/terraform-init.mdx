...
- `init.upgrade: auto` (default) adds `-upgrade` only when Terraform/OpenTofu reports that an upgrade is
  required — for example, after a provider version constraint was raised beyond the locked version.

Set `init.mode`, `init.reconfigure`, or `init.upgrade` to `always` to force Atmos to add the corresponding
behavior to every automatic init (the previous default), or to `never` to disable it entirely. See
