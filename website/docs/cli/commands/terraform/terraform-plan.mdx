...
<dl>
    ...
    <dd>
        Plan only affected components based on changes in the repository.

        ```shell
        atmos terraform plan vpc -s dev --affected
        ```
    </dd>
...
The `atmos terraform plan` command supports all native `terraform plan` flags. Some commonly used flags include:
...
- [`atmos terraform apply`](/cli/commands/terraform/terraform-apply) - Apply the changes from a plan
- [`atmos terraform deploy`](/cli/commands/terraform/terraform-deploy) - Deploy with auto-approval
- [`atmos terraform plan-diff`](/cli/commands/terraform/terraform-plan-diff) - Compare two plan files
- [`atmos terraform generate planfile`](/cli/commands/terraform/terraform-generate-planfile) - Generate a planfile in JSON/YAML format
