...
<Screengrab title="atmos terraform backend --help" slug="atmos-terraform-backend--help" />
...
<dl>
  ...
  <dt>`--identity` / `-i`</dt>
  <dd>Identity to use for authentication. Overrides component's default identity. Use without value to select interactively</dd>
...
See the [Automatic Backend Provisioning](/core-concepts/components/terraform/backend-provisioning) docs for more migration patterns.
