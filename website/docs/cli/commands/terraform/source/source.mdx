...
<Intro>
Use these commands to manage Terraform component sources with just-in-time (JIT) vendoring. This enables components to declare their source location inline using the top-level `source` field without requiring a separate `component.yaml` file.
</Intro>
...
For more details on configuring authentication identities, see the [Authentication Guide](/core-concepts/projects/configuration/auth).
...
*Mixins allow combining multiple source configurations into a single component. See [Vendoring](/core-concepts/vendor/vendor-manifest) for details.*
