...
|----------|---------------------|---------------|-------|
| Anthropic (Claude) | `ANTHROPIC_API_KEY` | `claude-sonnet-4-5-20250929` | Industry-leading reasoning |
| OpenAI (GPT) | `OPENAI_API_KEY` | `gpt-4o` | Most popular, widely adopted |
| Google (Gemini) | `GEMINI_API_KEY` | `gemini-2.5-flash` | Strong multimodal capabilities |
| xAI (Grok) | `XAI_API_KEY` | `grok-4` | Real-time data access |
| Ollama | `OLLAMA_API_KEY` | `llama3.3:70b` | Local models, no API key for local |
| AWS Bedrock | AWS credentials | `anthropic.claude-sonnet-4-5-20250929-v1:0` | Enterprise-grade, AWS security |
...
All configured providers are available concurrently. Press `Ctrl+P` during a chat session to switch between them. You can also specify a provider at startup:

```shell
atmos ai chat --provider openai
```
