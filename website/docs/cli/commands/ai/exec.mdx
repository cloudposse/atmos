...
id: exec
description: Run Atmos and shell commands via AI prompts non-interactively for automation and CI/CD integration
---
...
<CastPlayer title="atmos ai exec --help" src="/casts/screengrabs/atmos-ai-exec--help.cast" static chrome />
...
<dl>
  ...
  <dt>`--provider, -p`</dt>
  <dd>Override AI provider. API: `anthropic`, `openai`, `gemini`, `github`, `grok`, `openrouter`, `deepseek`, `zai`, `ollama`, `bedrock`, `azureopenai`. CLI: `claude-code`, `codex-cli`, `copilot-cli`, `opencode`, `gemini-cli`.</dd>
...
<dl>
  ...
  <dt>`1`</dt>
  <dd>AI error (API failure, invalid response, configuration error).</dd>

  <dt>`2`</dt>
  <dd>Tool execution error.</dd>
</dl>
...
<dl>
  <dt>`atmos ai ask`</dt>
  <dd>Non-interactive. Human-readable text output. Best for quick terminal questions.</dd>

  <dt>`atmos ai chat`</dt>
  <dd>Interactive. Rich TUI with markdown. Best for extended conversations.</dd>

  <dt>`atmos ai exec`</dt>
  <dd>Non-interactive. JSON/Text/Markdown output. Best for automation, scripting, CI/CD.</dd>
</dl>
...
<DocCardList items={[
  ...
  {type: 'link', href: '/cli/commands/ai/chat', label: 'atmos ai chat', description: 'Interactive chat sessions'},
