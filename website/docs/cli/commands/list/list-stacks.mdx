...
atmos list stacks --labels cost-center=platform --labels compliance=sox
...
<dl>
  ...
  <dt>`--error-mode`</dt>
	<dd>How to handle a recoverable YAML-function error, such as a Terraform backend that has not been provisioned yet (default `warn`). `strict` fails the command on the first such error. `warn` substitutes `(computed)` for the unresolved value, prints a warning naming the stack/component/function, and continues processing the rest of the stacks. `silent` continues without the summary. Errors unrelated to backend provisioning (auth failures, malformed YAML, etc.) still fail the command in either mode.<br/>Environment variable: `ATMOS_LIST_ERROR_MODE`</dd>
</dl>
