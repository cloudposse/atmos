...
The command supports multiple output formats to accommodate different use cases:
...
:::tip
When scripting or automating `atmos list stacks`, consider using the `--format json` or `--format csv` flags for more predictable and parseable output.
:::
