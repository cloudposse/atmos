...
<dl>
  ...
  <dt>`--format` / `-f`</dt>
  <dd>Output format: `table`, `json`, `yaml`, `csv`, `tsv`, `tree`. Overrides `stacks.list.format` configuration in atmos.yaml (default: `tree`).<br/>The default changed from `table` to `tree` in the 2026-07-16 [edition](/cli/configuration/edition); an explicit `--format` always wins, so scripts that parsed the plain piped output should pass `--format=csv` or `--format=json`.<br/>Environment variable: `ATMOS_LIST_FORMAT`</dd>
...
The default output format is `tree` (since the 2026-07-16 [edition](/cli/configuration/edition); it was previously a flat table). To restore the flat table by default, set `stacks.list.format` in your `atmos.yaml`:

```yaml
# atmos.yaml
stacks:
  list:
    format: table  # Supported formats: table, json, yaml, csv, tsv, tree
```

**Precedence**: CLI `--format` flag > Environment variable `ATMOS_LIST_FORMAT` > Config file > Default (`tree`)
