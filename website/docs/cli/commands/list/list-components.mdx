...
<dl>
  ...
  <dd>Authenticate with a specific identity before listing components.<br/>`atmos list components --identity my-aws-identity`<br/><br/>Note that listing components never evaluates credential-backed YAML functions (`!terraform.state`, `!terraform.output`, `!store`, `!secret`, `!aws.*`). Listing walks every stack in the repository, so in a multi-account setup no single identity could read them all; those functions are always skipped and their values are left unresolved. Use [`atmos describe stacks`](/cli/commands/describe/stacks) or [`atmos describe component`](/cli/commands/describe/component) when you need resolved values.<br/><br/>Environment variable: `ATMOS_IDENTITY`</dd>
