...
<dl>
  ...
  <dt>`--identity` / `-i`</dt>
  <dd>Authenticate with a specific identity (inherited from parent `list` command).<br/>Can also be set via `ATMOS_IDENTITY` environment variable.<br/>Note: This flag is typically not needed for `list themes` as it doesn't access cloud resources.</dd>
</dl>
