...
<dl>
  ...
  <dt>`--selector string` (`-l`)</dt>
  <dd>Filter results based on label selectors. Supports equality (`key=value`), inequality (`key!=value`), set inclusion (`key in (a,b)`), set exclusion (`key notin (a,b)`), existence (`key`), and non-existence (`!key`) operations</dd>
</dl>
