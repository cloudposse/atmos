...
<dl>
  ...
  <dt>`--identity` / `-i`</dt>
  <dd>Authenticate with a specific identity before listing values.<br/>Required when stack configurations use YAML template functions that require authentication.<br/>Can also be set via `ATMOS_IDENTITY` environment variable.</dd>
</dl>
