...
<DemoVideo id="vendor-pull" title="atmos vendor" showCaption={false} />

<CastPlayer title="atmos list vendor --help" src="/casts/screengrabs/atmos-list-vendor--help.cast" static chrome />
...
<dl>
  ...
  <dt>`--tags`</dt>
  <dd>Filter by the manifest's own declared tags (comma-separated, matches any): `--tags=networking,storage`. These are the `tags` declared on vendor manifest sources (`vendor.yaml`) — not the `metadata.tags` of components in stack manifests, which other `list` commands filter on. Vendor and component manifests have no labels concept, so `--labels` is not available on this command.<br/>Environment variable: `ATMOS_COMPONENT_TAGS`</dd>
