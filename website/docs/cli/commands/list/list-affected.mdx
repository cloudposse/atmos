...
<dl>
  ...
  <dt>`--format` / `-f`</dt>
  <dd>Output format: `table`, `json`, `yaml`, `csv`, `tsv`, `tree` (default: `table`).<br/>Environment variable: `ATMOS_LIST_FORMAT`</dd>
...
| **Formats supported** | table, json, yaml, csv, tsv, tree | json, yaml |
