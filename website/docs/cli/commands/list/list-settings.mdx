...
<dl>
  ...
  <dt>`--error-mode`</dt>
  <dd>How to handle unavailable requested values, including missing Terraform state and unavailable implicit authentication (default `warn`). `warn` substitutes `(computed)` and prints one summary; `silent` omits the summary; `strict` fails. Default identities authenticate only when an evaluated value needs credentials. Explicit `--identity` or `ATMOS_IDENTITY` failures, malformed configuration, invalid expressions, and dependency cycles remain fatal.<br/>Environment variable: `ATMOS_LIST_ERROR_MODE`</dd>
</dl>
