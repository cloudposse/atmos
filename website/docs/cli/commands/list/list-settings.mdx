...
- Cells contain the settings values for each key in each stack
...
<dl>
  <dt>`--query string`</dt>
  <dd>JMESPath query to filter settings (default: `.settings`)</dd>
  ...
  <dt>`--stack string`</dt>
  <dd>Filter by stack by wildcard pattern (e.g., '*-dev-*', 'prod-*')</dd>
</dl>
...
List specific settings using JMESPath queries:
```shell
