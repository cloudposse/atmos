...
<dl>
  ...
  <dd>Enable/disable YAML functions processing in Atmos stack manifests (default `true`). YAML functions include `!terraform.state`, `!terraform.output`, `!store`, `!aws.*`, etc. This is distinct from Go template functions like `atmos.Component(...)`, which are controlled by `--process-templates`.<br/><br/>Credential-backed functions (`!terraform.state`, `!terraform.output`, `!store`, `!secret`, `!aws.*`) are resolved on demand: with the default columns and no `--query`, `--filter`, or `--upload`, they are skipped, because the default output renders only identity fields those functions cannot produce. Listing walks every stack in the repository, so in a multi-account setup no single identity could read them all. Passing any of those flags asks for the values, so Atmos resolves them again.<br/>Environment variable: `ATMOS_PROCESS_FUNCTIONS`</dd>
