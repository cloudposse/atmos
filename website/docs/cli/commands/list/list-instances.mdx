...
<dl>
  <dt>`--format` / `-f`</dt>
  <dd>Output format: `table`, `json`, `yaml`, `csv`, `tsv`, `tree`, `matrix`. Overrides `list.instances.format` configuration in atmos.yaml (default: `tree`).<br/>The default changed from `table` to `tree` in the 2026-07-16 [edition](/cli/configuration/edition); an explicit `--format` always wins, so scripts that parsed the plain piped output should pass `--format=csv` or `--format=json`.</dd>
...
List all instances (rendered as a tree by default):
```shell
atmos list instances
```

List all instances as a flat table:
```shell
atmos list instances --format=table
```
