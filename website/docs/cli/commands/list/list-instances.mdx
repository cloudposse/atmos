...
The upload only includes instances where `settings.pro.enabled` is the boolean `true`. For example:
...
components:
  terraform:
    vpc:
      settings:
        pro:
          enabled: true  # Must be a boolean, not the string "true"
```

If no instances match this criterion, the command prints `No Atmos Pro-enabled instances found; nothing to upload.` and exits successfully.
