...
<dl>
  ...
  <dd>Authenticate with a specific identity before listing dependencies.<br/><br/>Note that listing dependencies does not evaluate credential-backed YAML functions (`!terraform.state`, `!terraform.output`, `!store`, `!secret`, `!aws.*`). This command renders a fixed dependency graph of component and stack names, so its output can never contain one of those values. Use [`atmos describe stacks`](/cli/commands/describe/stacks) or [`atmos describe component`](/cli/commands/describe/component) when you need resolved values.<br/><br/>Environment variable: `ATMOS_IDENTITY`</dd>
