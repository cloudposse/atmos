...
components:
  container:
    api:
      component: api   # resolves to <base_path>/api, independent of the `atmos` invocation's CWD
      build:
        context: app        # <base_path>/api/app
        dockerfile: docker/Dockerfile.prod  # <base_path>/api/app/docker/Dockerfile.prod (relative to build.context, not <base_path>/api)
```
...
<dl>
  ...
  <dt>`--labels <labels>`</dt>
  <dd>Operate on container components matching all of the given labels (comma-separated `key=value` pairs within one occurrence, and/or repeated for more): `--labels=cost-center=platform,compliance=sox` or `--labels cost-center=platform --labels compliance=sox`. Components are labeled via an optional `metadata.labels: {...}` map in stack manifests. Composes with `--all`/`--tags`; cannot be combined with a single component argument.</dd>
...
<dl>
  ...
  <dd>Filter by labels (comma-separated `key=value` or `key:value` pairs within one occurrence, and/or repeated for more, matches all): `--labels=cost-center=platform,compliance=sox` or `--labels cost-center=platform --labels compliance=sox`. Composes with `--all`/`--tags`; cannot be combined with a `<component>` argument.</dd>
...
atmos container up --all --labels cost-center=platform --labels compliance=sox
