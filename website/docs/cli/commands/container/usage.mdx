...
`restart`/`stop`/`rm`/`down`), discovered by labels derived from the canonical component instance
...
# Lifecycle
atmos container run <component> -s <stack>           # one-shot foreground process (vars.run)
...
atmos container exec <component> -s <stack> -- sh    # command after `--`
```
</Terminal>

:::tip Build before start
`up` and `run` build the image automatically when the component declares `vars.build` and the image
is not present locally. Components that reference an existing `vars.image` are pulled on demand.
:::
...
| Labels | `com.cloudposse.atmos.stack=dev`, `com.cloudposse.atmos.component_type=container`, `com.cloudposse.atmos.component=api`, `com.cloudposse.atmos.instance=dev/container/api` |

`ps`, `logs`, `exec`, `restart`, `stop`, `rm`, and `down` discover the container by these labels — there
...
<dl>
  ...
  <dt>`--` <em>(exec)</em></dt>
  <dd>Everything after `--` is the command run inside the container, e.g. `atmos container exec api -s dev -- sh -c 'env'`.</dd>
</dl>
