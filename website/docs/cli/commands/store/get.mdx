...
<dl>
  ...
  <dd>
    Print the raw value with **no trailing newline** (text only). Ideal for piping into another command or redirecting to a file (for example, `--raw > image_tag.txt`, or on macOS `--raw | pbcopy` for the clipboard). Cannot be combined with `--format=json` or `--format=env`.
  </dd>
</dl>
