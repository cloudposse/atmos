...
- **Per-devcontainer**: Configure runtime in `atmos.yaml` under `components.devcontainer.<name>.settings.runtime`

## Configuration

Devcontainers are configured in `atmos.yaml` under the `components.devcontainer` section:
...
- [Atmos Devcontainer Configuration](/core-concepts/components/devcontainer)
