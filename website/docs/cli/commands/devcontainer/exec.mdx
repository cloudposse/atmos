...
By default, `exec` runs in **non-interactive mode** where output masking works reliably. Sensitive data like AWS keys, GitHub tokens, and other secrets are automatically redacted according to your mask configuration.

When you use the `--interactive` (or `-i`) flag, the command runs in TTY mode for full terminal features (tab completion, colors, cursor control), but output masking is not available due to TTY data flowing at the kernel level.

**Example**: Running commands that display environment variables:

```shell
# Non-interactive mode - output is automatically masked
atmos devcontainer exec geodesic -- env | grep AWS
# AWS_ACCESS_KEY_ID=***MASKED***
# AWS_SECRET_ACCESS_KEY=***MASKED***

# Interactive mode - full TTY support, but no masking
atmos devcontainer exec geodesic --interactive -- bash
# Inside bash: echo $AWS_SECRET_ACCESS_KEY shows actual value

# Disable masking in non-interactive mode if needed
atmos devcontainer exec geodesic --mask=false -- env | grep AWS
```

:::tip
- Use **non-interactive mode** (default) when you need automatic masking of sensitive data
- Use **interactive mode** (`-i`) when you need full TTY features like tab completion or interactive editors
- For interactive shells, use [`atmos devcontainer shell`](/cli/commands/devcontainer/shell) instead
:::
