...
<CastPlayer title="atmos devcontainer shell --help" src="/casts/screengrabs/atmos-devcontainer-shell--help.cast" static chrome />
...
<dl>
  ...
  <dt>`--replace`</dt>
  <dd>Destroy and recreate the instance specified by `--instance` flag. This rebuilds the container from scratch: if using a `build` configuration, it rebuilds the image from the Dockerfile; if using an `image`, it pulls the latest version. The container is then recreated with current configuration.</dd>
