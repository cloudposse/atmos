...
<dl>
  ...
  <dt>`--pty`</dt>
  <dd>Experimental: Use PTY mode with masking support (not available on Windows). Enables masking of sensitive values in terminal output. See [Secret Masking Configuration](/cli/configuration/mask) for pattern configuration.</dd>
</dl>
