...
<CastPlayer title="atmos kubernetes deploy --help" src="/casts/screengrabs/atmos-kubernetes-deploy--help.cast" static chrome />
...
<dl>
  <dt>`split: true`</dt>
  <dd>`path` is a directory; every rendered object is written to its own generated filename inside it.</dd>

  <dt>`split: false`</dt>
  <dd>`path` is the exact output file; every rendered object is merged into one multi-document YAML file written to that path.</dd>
...
<dl>
  <dt>
    `--stack`, `-s` <em>(optional)</em>
  </dt>
  <dd>Atmos stack. Required when deploying a named component; not required with `--affected`.</dd>
