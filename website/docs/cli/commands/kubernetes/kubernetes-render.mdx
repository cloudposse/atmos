...
atmos kubernetes render --affected --labels cost-center=platform --labels compliance=sox
```
...
<dl>
  ...
  <dd>Atmos stack. Required when rendering a named component; not required with `--all`/`--affected`/`--tags`/`--labels` bulk selection.</dd>
