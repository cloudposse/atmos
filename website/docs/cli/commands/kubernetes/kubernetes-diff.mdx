...
<CastPlayer title="atmos kubernetes diff --help" src="/casts/screengrabs/atmos-kubernetes-diff--help.cast" static chrome />
...
atmos kubernetes diff --affected --labels cost-center=platform --labels compliance=sox
```
...
<dl>
  ...
  <dd>Atmos stack. Required when diffing a named component; not required with `--all`/`--affected`/`--tags`/`--labels` bulk selection.</dd>
