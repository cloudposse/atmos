...
cluster's *currently persisted* state. If a manifest set creates its own
...
<dl>
  ...
  <dd>Atmos stack. Required when validating a named component; not required with `--all`/`--affected`/`--tags`/`--labels` bulk selection.</dd>
