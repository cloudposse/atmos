...
<dl>
    ...
    <dd>
        Seed the new repository's content from another repository URI (a template, or a repository being migrated).
        Environment variable: `ATMOS_GIT_FROM`
    </dd>

    <dt>`--keep-history` <em>(optional)</em></dt>
    <dd>
        Keep the `--from` repository's full history and keep the source reachable as the `upstream` remote so future updates can be pulled. Requires `--from`.
