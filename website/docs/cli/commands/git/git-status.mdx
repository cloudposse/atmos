...
<Screengrab title="atmos git status --help" slug="atmos-git-status--help" />
...
<dl>
    <dt>`name-or-path` <em>(required unless `--all`)</em></dt>
    <dd>
        A repository name configured under `git.repositories`, or a filesystem path to an existing Git working tree. Use an explicit path prefix (`./deployments`) to force path interpretation when an argument collides with a repository name.
    </dd>
</dl>
