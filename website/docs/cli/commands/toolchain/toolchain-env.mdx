...
When [toolchain proxies](/cli/configuration/toolchain/proxies) are configured, this command also creates their command links and exports the private context needed to resolve them after changing directories.

<CastPlayer title="atmos toolchain env --help" src="/casts/screengrabs/atmos-toolchain-env--help.cast" static chrome />
...
This also activates configured proxies, so a proxy such as `ls` can be used directly in the shell.
