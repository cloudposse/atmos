...
id: get
description: Display version information for tools configured in your .tool-versions file.
---
...
<CastPlayer title="atmos toolchain get --help" src="/casts/screengrabs/atmos-toolchain-get--help.cast" static chrome />
...
<dl>
    ...
    <dd>
        ...
        Use `plain` to print just the bare version string, ideal for shell substitution in scripts and CI.
        Use `json` for structured output that includes installation status.
...
Returns an error, since `--all` lists multiple versions and there's no single version to print in
plain format. Use `--format=json` instead.
