...
<CastPlayer title="atmos toolchain info --help" src="/casts/screengrabs/atmos-toolchain-info--help.cast" static chrome />
...
| **Type** | Package type (`github_release`, `http`, `github_archive`, `github_content`) |
...
`atmos toolchain info` always includes an "Available Versions" section in its output, so there's no
flag needed to see what versions exist:

```shell
# View available versions alongside the rest of the tool info
atmos toolchain info terraform
