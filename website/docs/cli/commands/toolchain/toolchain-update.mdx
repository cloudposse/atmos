...
- **Pinned to `pr:NNNN`, `sha:xxxxxxx`, or `ref:<name>`** — skipped. These are
  immutable-by-design pins; updating them automatically would defeat their
  purpose. Use [`atmos toolchain add`](/cli/commands/toolchain/add) to change
  one explicitly.
...
[`dependencies.tools`](/cli/commands/toolchain/usage) in a stack manifest, or
