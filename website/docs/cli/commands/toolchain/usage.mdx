...
:::note Purpose
Manage CLI tool versions natively in Atmos with seamless Aqua registry integration—no external tool managers required.
:::
...
While Atmos uses the Aqua registry by default, you can configure custom registry URLs:

```yaml
toolchain:
  registry:
    # Use custom Aqua registry fork
    base_url: "https://github.com/your-org/custom-registry"
```
