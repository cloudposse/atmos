...
<CastPlayer title="atmos toolchain uninstall --help" src="/casts/screengrabs/atmos-toolchain-uninstall--help.cast" static chrome />
...
```shell
atmos toolchain uninstall --all
```

Uninstalls every tool found in the toolchain install directory, including tools that were
installed automatically via component, workflow, or command `dependencies.tools` and are not
listed in `.tool-versions`.
...
<dl>
    ...
    <dd>
        Uninstall all installed tools, not just the ones listed in `.tool-versions`. This also
        removes tools that were installed via component, workflow, or command `dependencies.tools`.

        Environment variable: `ATMOS_TOOLCHAIN_UNINSTALL_ALL`
    </dd>
