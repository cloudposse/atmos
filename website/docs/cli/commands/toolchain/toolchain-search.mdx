...
The `atmos toolchain search` command is a convenient alias to [`atmos toolchain registry search`](/cli/commands/toolchain/registry/registry-search). It provides the exact same functionality with a shorter command for faster tool discovery workflows.
