...
<CastPlayer title="atmos toolchain search --help" src="/casts/screengrabs/atmos-toolchain-search--help.cast" static chrome />
...
<dl>
  <dt>`--limit` (default `20`)</dt>
  <dd>Maximum results to display.</dd>

  <dt>`--registry`</dt>
  <dd>Search only in the specified registry.</dd>

  <dt>`--format`</dt>
  <dd>Output format: `table`, `json`, or `yaml`.</dd>

  <dt>`--installed-only`</dt>
  <dd>Show only installed tools.</dd>

  <dt>`--available-only`</dt>
  <dd>Show only available (not installed) tools.</dd>
</dl>
