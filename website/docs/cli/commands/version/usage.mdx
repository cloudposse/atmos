...
The `fips` field reports whether this binary was built with Go's FIPS 140-3 validated cryptographic module linked in and enforced by default at runtime — every official release binary is. See [this announcement](/changelog/fips-140-mode) for what that does and doesn't cover.
