...
<ActionCard title="Configure Version Checking">
    Learn how to configure automatic version checking and update notifications in your atmos.yaml.
    <div>
      <PrimaryCTA to="/cli/configuration/version">Configuration Reference</PrimaryCTA>
    </div>
</ActionCard>

## Usage

<CastPlayer title="atmos version" src="/casts/screengrabs/atmos-version.cast" static chrome />
...
The `fips` field reports whether this process is currently running in FIPS 140-3 mode: the binary was built with Go's FIPS 140-3 crypto module linked in (`GOFIPS140=latest`) and that build-time default hasn't been overridden off via `GODEBUG=fips140=off` — every official release binary defaults to `true`. This describes FIPS 140-3 build/runtime *mode*, not module validation: `GOFIPS140=latest` links Go's current, unvalidated FIPS module source, not a CMVP-certified one (Go reserves `GOFIPS140=certified` for that). See [this announcement](/changelog/fips-140-mode) for what that does and doesn't cover.
