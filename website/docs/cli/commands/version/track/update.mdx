...
Unlike [`lock`](/cli/commands/version/track/lock), `update` starts from the locked state and respects strategy caps, cooldown windows, and allow/ignore rules. Newer candidates held back by policy are reported with a reason.
