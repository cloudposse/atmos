...
```shell
atmos describe locals [options]
```

This command reads all stack manifest files and extracts the `locals` sections, showing both raw and resolved values organized by scope (global, terraform, helmfile, packer).
...
<dl>
  ...
  <dd>Filter by a specific stack. Supports both stack manifest names (including subfolder paths like `deploy/dev`) and Atmos stack names derived from context vars.</dd>
