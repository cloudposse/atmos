...
When a component is specified with `--stack`, it shows the merged locals that would be **available to** that component. This includes:
...
When a component is specified with `--stack`, the output shows the locals that would be **available to** that component. The output includes the component name, stack, component type, and the merged locals:
...
The `component_type` field shows how Atmos determined which section-specific locals to include. Since `vpc` is a Terraform component, Atmos merged:
1. Global locals from the stack manifest
2. `terraform:` section locals from the stack manifest
3. Component-level locals from the `vpc` component definition (including any inherited from base components)
