...
| `--upload`                         | Upload the affected components and stacks to a specified endpoint for<br/>further processing in a CI/CD pipeline.<br/><br/>`atmos describe affected --upload=true`<br/><br/>Atmos will perform an HTTP POST request to the endpoint<br/>`${ATMOS_PRO_BASE_URL}/api/affected-stacks`<br/>where the base URL is defined using the `ATMOS_PRO_BASE_URL` environment variable.<br/>The `token` for the `Authorization: Bearer <token>` header can be specified<br/>using the `ATMOS_PRO_TOKEN` environment variable | no       |
