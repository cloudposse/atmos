...
- `dependents` - list of components that depend on the current affected component. It will be included only if the
  ...
  that depend on the current component in the stack.
  Refer to [`atmos describe dependents`](/cli/commands/describe/dependents) for more details. The `dependents` list is
  hierarchical - each component in the list will also contain a `dependents` property if that component also has dependent
  components
