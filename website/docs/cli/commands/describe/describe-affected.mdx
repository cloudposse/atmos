...
The output of the `atmos describe affected` command can include dependencies for all the affected components.
...
In the above configuration, `component-3` depends on `component-2`, and `component-2` depends on `component-1`.
