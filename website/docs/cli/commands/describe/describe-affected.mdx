...
`vars`, `env`, `settings`, `metadata`, `providers`, `required_providers`, `required_version`, `hooks`, `generate`,
...
<dl>
  <dt>`locals`</dt>
  <dd>Used only to render templates, so any effective change already shows up in the rendered `vars`/`env`/etc.</dd>

  <dt>`overrides`</dt>
  <dd>Folded into the merged `vars`/`env`/`settings`/etc., so it would double-report.</dd>

  <dt>`inheritance`</dt>
  <dd>A derived (computed) inheritance chain; any real effect already appears in the other sections.</dd>

  <dt>`retry`</dt>
  <dd>Operational/execution-time behavior (how a run is retried), not provisioned infrastructure configuration.</dd>
</dl>
...
<dl>
    ...
    <dd>
        ...
        <dl>
            ...
            <dt>`stack.hooks`</dt>
            <dd>
                The `hooks` component section in the stack config has been modified.
            </dd>

            <dt>`stack.generate`</dt>
            <dd>
