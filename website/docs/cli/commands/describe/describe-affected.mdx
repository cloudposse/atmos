...
atmos describe affected --ref refs/heads/my-new-branch --verbose
...
atmos describe affected --clone-target-ref
atmos describe affected --include-settings
atmos describe affected --include-dependents
atmos describe affected --stack <stack>
atmos describe affected --stack <stack> --include-dependents --dependents-stack <stack>
...
<Terminal title="atmos describe affected --verbose">
```shell
> atmos describe affected --verbose
...
<dl>
    ...
    <dt>`--dependents-stack` <em>(optional)</em></dt>
    <dd>
        Filter the dependent components by a specific stack.
        Only used when `--include-dependents` is set to `true`.

        `atmos describe affected --include-dependents --dependents-stack <stack>`

        `atmos describe affected -s <stack> --include-dependents --dependents-stack <stack>`
    </dd>
    ...
    <dd>
        ...
        <dl>
            ...
            <dd>
                ...
                - The `atmos describe affected` is executed with the `--include-spacelift-admin-stacks` flag
...
<Terminal title="atmos describe affected --include-spacelift-admin-stacks">
```json
[
  {
    "component": "infrastructure-tenant1",
...
the `atmos describe affected --include-dependents` command will produce the following result:

<Terminal title="atmos describe affected --include-dependents">
 ```json
  [
    {
      "component": "component-1",
...
<Terminal title="Payload of 'atmos describe affected --upload' command">
  ```json
    {
