...
<DemoVideo id="describe-affected" title="atmos describe affected" showCaption={false} />
    ...
    <dd>
        ...
        <dl>
            ...
            <dd>
                ...
                Dependencies on external files (not in the component's folder) are declared in the [`dependencies.components`](/stacks/dependencies/components) list using `kind: file` and a `path`.
                  ...
                  terraform:
                    top-level-component3:
                      ...
                      dependencies:
                        components:
                          - kind: file
                            path: "tests/fixtures/scenarios/complete/components/terraform/mixins/introspection.mixin.tf"
                ```
                ...
                :::note Legacy format
                The deprecated [`settings.depends_on`](/stacks/settings/depends_on) map (with numeric keys and a `file:` attribute) is still recognized for backward compatibility, but new configurations should use `dependencies.components`.
                :::
            ...
            <dd>
                ...
                Dependencies on external folders are declared in the [`dependencies.components`](/stacks/dependencies/components) list using `kind: folder` and a `path`. You can mix `file` and `folder` entries in the same list.
                      ...
                      dependencies:
                        components:
                          - kind: file
                            path: "tests/fixtures/scenarios/complete/components/terraform/mixins/introspection.mixin.tf"
                          - kind: folder
                            path: "tests/fixtures/scenarios/complete/components/helmfile/infra/infra-server"
                ```
                ...
                :::note Legacy format
                The deprecated [`settings.depends_on`](/stacks/settings/depends_on) map (with numeric keys and a `folder:` attribute) is still recognized for backward compatibility, but new configurations should use `dependencies.components`.
                :::
