...
- **Pull requests**: Prefers the PR base commit SHA from the GitHub event payload, then falls back to local Git refs only if needed
...
|-------|----------------|
| `pull_request` / `pull_request_target` | PR base commit SHA from event payload (`pull_request.base.sha`), then `merge-base(HEAD, origin/<base>)`, then fallback refs |
| `pull_request` (closed/merged, no payload SHA) | Parent of current commit (`HEAD~1`), then fallback refs |
...
For GitHub Actions pull request events, CI auto-detection uses this order:

1. `pull_request.base.sha` from `GITHUB_EVENT_PATH`
2. `git merge-base HEAD origin/<base branch>` when a usable local remote-tracking ref exists
3. `HEAD~1` for closed/merged PRs when the payload does not provide a usable base SHA
4. `refs/remotes/origin/<base branch>` from `GITHUB_BASE_REF`
5. `refs/remotes/origin/HEAD`
