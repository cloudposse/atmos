...
| `--clone-target-ref`               | Clone the target reference with which to compare the current branch.<br/>`atmos describe affected --clone-target-ref=true`<br/><br/>If set to `false` (default), the target reference will be checked out instead<br/>(this means that the target reference must be already cloned by Git<br/>in the `.git` directory) | no       |
