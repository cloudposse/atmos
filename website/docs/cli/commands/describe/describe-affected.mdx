...
- **Pull requests**: Compares against the PR target branch (`GITHUB_BASE_REF`)
...
Run `atmos describe affected --format=matrix --output-file=$GITHUB_OUTPUT` directly in your workflow to fan out across affected components. See the [matrix workflow example](/ci#advanced-multi-component-matrix) for the full pattern.
:::
...
|-------|----------------|
| `pull_request` (opened/synchronize) | Target branch (`GITHUB_BASE_REF`) |
| `pull_request` (closed/merged) | PR base commit SHA from event payload |
