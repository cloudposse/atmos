...
`terraform apply` on all the affected components in the stacks and all dependencies).
...
:::note
If the `--upload=true` command-line flag is passed, the `--include-dependencies` flag is automatically set to `true`,
so the affected components will always be uploaded with their dependencies (if they are configured in Atmos stack manifests).
:::
