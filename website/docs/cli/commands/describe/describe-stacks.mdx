...
Without `--stack`, the command evaluates these functions for **every** component in **every** stack. In a multi-account repository where each stage keeps its Terraform state in its own account, no single identity can read them all. Values Atmos cannot read are degraded to the `(computed)` placeholder and the command continues, reporting a summary of what was degraded — run with `--logs-level=Debug` for the underlying cause of each.
