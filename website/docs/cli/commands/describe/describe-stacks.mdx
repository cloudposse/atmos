...
Default identity authentication occurs only when an evaluated YAML function or Go template needs credentials. Unused sections selected out by `--sections` do not trigger authentication. With `--error-mode=warn` (the default), unavailable implicit authentication, including a stopped emulator, produces `(computed)` for affected values and one summary; `silent` omits the summary and `strict` fails. Resolvable sibling values remain available.
