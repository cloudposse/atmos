...
Use `--error-mode=strict` if you would rather a failed read abort the command. To avoid the reads altogether, scope the run with `--stack <stack>`, skip the credential-backed functions with `--skip=terraform.state --skip=terraform.output`, or turn processing off with `--process-functions=false`.
