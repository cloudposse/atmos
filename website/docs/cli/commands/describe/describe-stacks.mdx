...
<CastPlayer title="atmos describe stacks --help" src="/casts/screengrabs/atmos-describe-stacks--help.cast" static chrome />
...
Without `--stack`, the command evaluates these functions for **every** component in **every** stack. In a multi-account repository where each stage keeps its Terraform state in its own account, no single identity can read them all. A `!terraform.state` value Atmos cannot read is degraded to the `(computed)` placeholder and the command continues, reporting a summary of what was degraded — run with `--logs-level=Debug` for the underlying cause of each. `!terraform.output` is not degraded this way: it runs `terraform output` as a subprocess, whose failures are too broad to distinguish from real defects, so those still abort the command.

Degradation applies only to failures caused by the environment — an unreachable backend, an unreadable one, or state that has not been provisioned yet. A failure caused by the configuration itself still aborts the command in **every** error mode, because no error mode can make it succeed and a `(computed)` placeholder would hide a typo behind a plausible-looking value. That covers a `backend_type` Atmos does not support, a state file that is not valid Terraform state, a `static` remote-state backend that does not declare the output being read, and a YQ expression that fails against state Atmos retrieved successfully.

Use `--error-mode=strict` if you would rather a failed read abort the command. To avoid the reads altogether, skip the credential-backed functions with `--skip=terraform.state --skip=terraform.output`, or turn processing off with `--process-functions=false`. Narrowing the run with `--stack <stack>` reduces how many components are evaluated, but does not by itself avoid cross-account reads — a `!terraform.state` inside the selected stack can reference a component in another stack and account.
