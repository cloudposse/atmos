...
<dl>
  ...
  <dd>Enable provenance tracking to show where configuration values originated.<br/>Displays file path, line number, and column for each value with inline YAML comments.<br/>Output includes inheritance depth indicators:<br/>&nbsp;&nbsp;● `[1]` - Defined in parent stack<br/>&nbsp;&nbsp;○ `[N]` - Inherited/imported (N=2+ levels deep)<br/>&nbsp;&nbsp;∴ - Computed/templated<br/>Depth indicators use color coding: cyan for parent stack, green for depth 2, orange for depth 3, red for depth 4+.<br/><br/>`atmos describe component <c> -s <stack> --provenance`</dd>
