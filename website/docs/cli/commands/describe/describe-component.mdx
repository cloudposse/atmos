...
<dl>
  ...
  <dd>Enable provenance tracking to show where configuration values originated.<br/>Displays file path, line number, and column for each value with inline YAML comments.<br/>Output includes inheritance depth indicators:<br/>&nbsp;&nbsp;● `[0]` - Defined in parent stack<br/>&nbsp;&nbsp;○ `[N]` - Inherited/imported (N levels deep)<br/>&nbsp;&nbsp;∴ - Computed/templated<br/><br/>`atmos describe component <c> -s <stack> --provenance`</dd>
