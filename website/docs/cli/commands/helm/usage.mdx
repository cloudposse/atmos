...
<ActionCard title="Configure Helm Components">
  ...
  <div>
    <PrimaryCTA to="/stacks/components/helm">Stack Configuration</PrimaryCTA>
  </div>
</ActionCard>
...
`template`/`render`, `diff`/`plan`, and `apply`/`deploy` accept Helm-compatible
value overrides for a single invocation. They never modify component or stack
configuration, so use the same arguments with `diff` and `apply` to preview the
exact release values that will be deployed.
...
<dl>
  ...
  <dt>[`values`](/cli/commands/helm/values)</dt>
  <dd>Print final component values as formatted, masked YAML.</dd>
