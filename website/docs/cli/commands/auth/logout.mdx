...
2. **Environment variable** (`ATMOS_AWS_FILES_BASE_PATH`)
3. **Default** (`~/.aws/atmos`)

#### Example with Environment Variable

```shell
export ATMOS_AWS_FILES_BASE_PATH=/opt/atmos/credentials
atmos auth logout dev-admin
```

This removes files from `/opt/atmos/credentials/aws-sso/` instead of `~/.aws/atmos/aws-sso/`.
