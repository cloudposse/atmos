...
Use this command to securely remove locally cached credentials from the system keyring and credential files. This makes credential cleanup explicit, comprehensive, and easy when switching identities, ending work sessions, or troubleshooting authentication issues.
</Intro>
...
4. Removes AWS credential files in `~/.aws/atmos/aws-sso/`
...
The default base path is `~/.aws/atmos`, but this can be customized (see [Custom File Paths](#custom-file-paths)).
...
```
✗ Files: ~/.aws/atmos/aws-sso/ (permission denied)
```

**Solution**: Ensure you have write permissions to the `~/.aws/atmos/` directory. Check file ownership and permissions:
