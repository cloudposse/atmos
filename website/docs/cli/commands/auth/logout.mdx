...
The default base path follows XDG Base Directory Specification (`~/.config/atmos/aws/` on Linux, `~/Library/Application Support/atmos/aws/` on macOS), but this can be customized (see [Custom File Paths](#custom-file-paths)).
...
2. **Default** (XDG-compliant: `~/.config/atmos/aws/` on Linux, `~/Library/Application Support/atmos/aws/` on macOS)
