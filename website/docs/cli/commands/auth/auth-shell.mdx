...
**Use `shell` for**: Interactive sessions where you'll run multiple commands (debugging, exploration, manual operations).
**Use [`exec`](/cli/commands/auth/auth-exec) for**: Single commands or scripts (automation, CI/CD, one-off operations).
</Intro>
