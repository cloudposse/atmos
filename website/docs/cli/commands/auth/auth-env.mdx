...
**Important:** This command **outputs environment variables** but does **not perform authentication**. It generates export statements pointing to credential files that are populated during login events. Since commands cannot modify their parent shell's environment, you must use `eval` to apply these variables to your current shell.
...
- Working with modern terminals like [Warp](https://warp.dev) where environment variables persist across panes
...
See the [`atmos auth shell`](/cli/commands/auth/auth-shell) documentation for more details on session isolation.
...
[Warp](https://warp.dev) is a modern terminal that works well with `atmos auth env`. Adding `eval $(atmos auth env)` to your shell profile (`.zshrc`, `.bashrc`) ensures that new terminal windows and panes automatically have the appropriate environment variables set for your default identity.
...
- [`atmos auth shell`](/cli/commands/auth/auth-shell) - Launch an isolated shell session with automatic credential cleanup
- [`atmos auth login`](/cli/commands/auth/auth-login) - Authenticate to a cloud identity
- [`atmos auth whoami`](/cli/commands/auth/auth-whoami) - Display information about the current authenticated identity
