...
```shell
atmos auth env [--identity <name>] [--format bash|json|dotenv|github] [--output <file>] [--login]
```
...
atmos auth env --identity prod-admin --format github --output /path/to/env/file
...
<dl>
  ...
  <dd>
    ...
    If `--format=github` is used without `--output`, the command automatically writes to the file specified by the `$GITHUB_ENV` environment variable (set automatically in GitHub Actions).
  </dd>
...
<dl>
  ...
  <dt>`ATMOS_AUTH_ENV_OUTPUT`</dt>
  <dd>Default output file path when `--output` is not provided. For `--format=github`, this defaults to the value of `$GITHUB_ENV` if set.</dd>
</dl>
