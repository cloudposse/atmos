...
[Warp](https://warp.dev) is a modern terminal that works well with `atmos auth env`. Adding `eval $(atmos auth env)` to your shell profile (`.zshrc`, `.bashrc`) is the preferred way to use Atmos authentication with Warp, as it ensures that new terminal windows and panes automatically have the appropriate environment variables set for your default identity.
:::
