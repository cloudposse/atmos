...
- **`Browser`** (recommended): Opens a browser for authentication. Browser drivers are managed automatically:
  ...
  **Recommended: Automatic Download**
  ...
  providers:
    my-saml:
      kind: aws/saml
      driver: Browser
      download_browser_driver: true  # Auto-downloads Chromium (~140 MB) on first use
  ```

  With this configuration, Atmos automatically downloads Chromium on your first authentication attempt and stores it in your user cache directory:
  ...
  Manual browser installation is **not recommended** because:
  ...
  **Note**: Browser drivers do not need to be in your system PATH. Atmos uses Chromium by default for maximum compatibility with SAML identity providers.
