...
2. **AWS SDK uses default credential resolution** - Falls back to standard AWS credential chain:
   - Environment variables (`AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, `AWS_SESSION_TOKEN`)
   - Shared credentials file (`~/.aws/credentials`)
   - EC2 instance metadata (IMDS)
   - Web identity token (for OIDC/GitHub Actions)
...
```bash
# Use your personal AWS profile temporarily
