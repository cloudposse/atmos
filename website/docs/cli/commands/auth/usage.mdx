...
1. User authenticates with `atmos auth login --provider {provider}`
...
3. Re-authenticate: `atmos auth login --provider {provider}`
4. Enable debug logging: `atmos auth login --provider {provider} --log-level debug`
