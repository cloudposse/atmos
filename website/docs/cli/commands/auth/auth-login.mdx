...
Integration failures are non-blocking - your identity authentication succeeds even if an integration fails. You can retry integrations separately using [`atmos auth ecr-login`](/cli/commands/auth/ecr-login) or [`atmos aws eks update-kubeconfig --integration`](/cli/commands/aws/eks-update-kubeconfig).
