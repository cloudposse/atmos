...
<dl>
  ...
  <dd>
    ...
    Session data is stored under the XDG data directory (`~/.local/share/atmos/console/sessions/`) and is keyed by realm and identity name, so reopening the same identity reuses its browser session.
