...
<CastPlayer title="atmos auth console --help" src="/casts/screengrabs/atmos-auth-console--help.cast" static chrome />
...
1. **Authentication**: Atmos authenticates using your configured identity (AWS IAM Identity Center, SAML, or an IAM user) to obtain temporary credentials with a session token.
...
| AWS (IAM user) | ✅ Supported | Atmos obtains temporary STS session credentials before console federation |
...
**Solution**: AWS console access requires temporary credentials with a session token. Atmos obtains these automatically for `aws/user` identities; other identities should use AWS IAM Identity Center, SAML, or assumed roles.
...
<dl>
  <dt>`auth.console.isolated_sessions`</dt>
  <dd>
