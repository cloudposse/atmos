...
4. Writes credentials to Atmos Docker config
5. Sets `DOCKER_CONFIG` environment variable
...
ECR credentials are written to `~/.config/atmos/docker/config.json` (following XDG conventions), **not** your default `~/.docker/config.json`. This isolation:

- Keeps Atmos-managed credentials separate from manually configured registries
- Prevents conflicts with other Docker credential helpers
- Allows easy cleanup by removing the Atmos config directory

The `DOCKER_CONFIG` environment variable is set to point Docker to this config file.
