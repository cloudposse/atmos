...
# instead filters vendor.yaml's own declared source tags -- the two aren't combinable.
...
<dl>
  ...
  <dt>`--tags` (string)</dt>
  <dd>Update only components whose `vendor.yaml` source declares any of these comma-separated tags (a manifest concept -- see `--labels` for the component-level equivalent). Cannot be combined with `--stack`/`--labels`.</dd>
  <dt>`--stack` / `-s` (string)</dt>
  <dd>Update only components belonging to the specified stack. Composes with `--labels` to narrow further. Cannot be combined with an explicit `--component` or with `--tags`.</dd>
  <dt>`--labels` (string)</dt>
  <dd>Update only components whose stack `metadata.labels` match ALL of these comma-separated `key=value` or `key:value` pairs (resolved the same way as `--stack`). Cannot be combined with an explicit `--component` or with `--tags`.</dd>
  <dt>`--group` (string)</dt>
