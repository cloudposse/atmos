...
When no explicit scheme is provided, for example:

```shell
git::git@github.com:cloudposse/terraform-null-label.git?ref={{.Version}}
```

Atmos’s rewrites this URL to:

```shell
git::ssh://git@github.com/cloudposse/terraform-null-label.git?depth=1&ref={{.Version}}
```

In this case, Atmos injects a default username ("git").

### Explicit SSH

When the SSH scheme is specified explicitly, for example:
...
Atmos supports vendoring components using HTTPS links. When a source repository is specified without an explicit protocol, it is resolved using HTTPS.
...
```
The **custom detector is invoked** in this case to ensure proper URL transformation.
...
When resolving HTTPS-based sources, Atmos follows these rules:

1. If a **full HTTPS URL** is provided (`git::https://github.com/...`), it is used as-is.
2. If a **repository name** is provided without a scheme (`github.com/org/repo.git`), it defaults to `https://`.
3. If a **username and repo name** are provided in SCP format (`git@github.com:org/repo.git`), it is rewritten to an SSH URL and handled separately.


:::note
For more details on configuration, refer to [Atmos Configuration](../configuration/configuration.mdx).
