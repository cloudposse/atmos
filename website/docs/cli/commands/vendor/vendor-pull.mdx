...
- If `--stack` is specified, Atmos vendors every component declared in that stack that has its own `component.yaml`, regardless of whether a
  `vendor.yaml` also exists. `--stack` cannot be combined with `--component`.
...
<dl>
  ...
  <dd>Only vendor components belonging to the specified stack. Vendors every component in the stack that has its own `component.yaml`, bypassing `vendor.yaml`. Cannot be combined with `--component`.</dd>
