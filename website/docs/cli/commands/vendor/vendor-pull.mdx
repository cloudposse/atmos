...
Atmos supports SSH for accessing non-public Git repositories, which is convenient for local development.
...
- A URL like:
  ...
  ```
  is **not valid** because go-getter misinterprets **github.com:** as a schema and will error out. To fix this, add the `git::` prefix to the source.

- A URL without any scheme, such as:
  ```shell
  github.com/cloudposse/terraform-null-label.git?ref={{ .Version }}
  ```
  defaults to HTTPS and triggers Atmos to inject credentials if available and enabled.
...
<dl>
  ...
  <dd>
    Bearer token for GitHub API requests, enabling authentication for private repositories and higher rate limits. The `ATMOS_GITHUB_TOKEN` takes precedence over the `GITHUB_TOKEN`.
    
    <dt>Default Username</dt>
    <dd>`x-access-token`</dd>
  </dd>
  <dt>`GITHUB_TOKEN`</dt>
