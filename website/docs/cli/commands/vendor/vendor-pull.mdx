...
Atmos supports vendoring components using HTTPS links. 
...
1. If a **full HTTPS URL** is provided (`git::https://github.com/...`), it is used as-is. No token data is added to the link even if it is set in respective environment variables. 
2. If a **repository name** is provided without a scheme (`github.com/org/repo.git`), it defaults to `https://`, in this case if set the token is injected into the URL.  
3. If a **username and repo name** are provided in SCP format (`git@github.com:org/repo.git`), it is rewritten to an SSH URL.
