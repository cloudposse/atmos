...
<Intro>
This command implements [Atmos Vendoring](/vendoring/vendor). Use this command to download sources from local and remote
...
- The `source` attribute supports all protocols (local files, Git, Mercurial, HTTP, HTTPS, Amazon S3, Google GCP), and all URL and
  ...
  [OCI registries](https://opencontainers.org). See [Vendor URL Syntax](/vendoring/url-syntax) for complete documentation on supported URL formats and authentication.
...
:::tip
Refer to [`Atmos Vendoring`](/vendoring/vendor) for more details
:::
...
:::tip
Refer to [`Atmos Component Vendoring`](/vendoring/component-manifest) for more details
:::
