...
- The `targets` in the `sources` support absolute paths and relative paths. Multiple targets can be specified. Relative targets are resolved
  against the directory containing the `vendor.yaml` file, unless `atmos.yaml` declares where the manifest lives via the
  [`vendor.base_path`](/cli/configuration/vendor) setting — in that case they're resolved against the Atmos `base_path`, so vendored artifacts
  land in the same tree as the components and stacks instead of next to the manifest.
