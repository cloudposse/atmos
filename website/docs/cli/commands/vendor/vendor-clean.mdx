...
```shell
atmos vendor clean [--component <name>] [--tags <a,b>] [--force] [--dry-run] [--prune-lock]
atmos vendor clean [--stack <stack>] [--labels <k>=<v>] [--tags <a,b>] [--force] [--dry-run] [--prune-lock]
```
...
To **permanently** remove a component — not just its files — pass `--prune-lock` to also forget the
cleaned components' lock entries. Use it when you've removed (or are about to remove) the source
from `vendor.yaml`: without it, the orphaned lock entry keeps its recorded files, which
`vendor verify` then reports as missing. `--prune-lock` only forgets the entries the same selectors
matched, so it never touches entries you didn't target.
...
<dl>
  ...
  <dd>Component type (`terraform`, `helmfile`, or `packer`). Pass this flag explicitly to scope a `--stack`/`--labels` selection to one component type. When you omit it, every component type is considered.</dd>
  <dt>`--file` (string, default `./vendor.yaml`)</dt>
  <dd>The vendor manifest to read declared source tags from when resolving `--tags`.</dd>
  <dt>`--force` (boolean)</dt>
  <dd>Delete modified lock-owned files instead of refusing to run.</dd>
  <dt>`--dry-run` (boolean)</dt>
  <dd>Show which files would be removed without deleting anything. Combined with `--prune-lock`, also shows which lock entries would be forgotten.</dd>
  <dt>`--prune-lock` (boolean)</dt>
  <dd>Also remove the cleaned components' entries from `vendor.lock.yaml`, so a source removed from `vendor.yaml` no longer leaves an orphan entry that `vendor verify` reports as missing. Only the entries the selectors matched are forgotten. Default: entries are preserved.</dd>
</dl>
