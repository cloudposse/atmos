...
<dl>
  ...
  <dd>Clean only components whose stack `metadata.labels` match ALL of these `key=value` or `key:value` pairs — comma-separated within one occurrence, and/or repeated for more (`--labels tier=1 --labels cost-center=platform`). Composes with `--stack` and `--tags`. Mutually exclusive with `--component`.</dd>
