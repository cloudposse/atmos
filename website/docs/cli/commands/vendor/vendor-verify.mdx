...
<dl>
  ...
  <dt>`--labels` (string)</dt>
  <dd>Verify only components whose stack `metadata.labels` match ALL of these comma-separated `key=value` or `key:value` pairs. Composes with `--stack` and `--tags`. Mutually exclusive with `--component`.</dd>
