...
A `type: computed` field is never prompted for and can't be set with `--set` — it's always
derived from other fields' answers through a `value:` Go-template expression, using the same
`answers.*` binding `options:`'s dynamic form uses below:
...
`.Config` is: file content, `target:`, `matrix:` axes, and other fields' `options:` expressions.
...
**earlier-declared** computed field's own result. Because that evaluation happens after the
