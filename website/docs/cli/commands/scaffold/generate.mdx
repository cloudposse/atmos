...
Sprig/Gomplate function when the scaffold's own `atmos.yaml` has
`templates.settings.sprig.enabled`/`templates.settings.gomplate.enabled` set (both default to
`true`). `collectKeys(m)` returns `m`'s top-level keys, sorted; `collectKeys(m, "nestedKey")`
collects `nestedKey`'s own keys from every value in `m`, flattened and deduplicated:
