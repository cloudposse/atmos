...
`type: atmos` step is exempt from this default: it always keeps running in the directory `atmos`
was launched from, since the nested `atmos` invocation must resolve its own config there.
