...
`collectKeys`, a template function available (alongside every Sprig/Gomplate function) to axis
expressions: `collectKeys(m)` returns `m`'s top-level keys, sorted; `collectKeys(m, "nestedKey")`
