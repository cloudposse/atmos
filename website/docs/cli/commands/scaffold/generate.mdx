...
`target:` is required whenever `matrix:` is set. The resolved combination is available as
`.matrix.<axis>` in `target:`, in `when:`, and in the file's own content — not just the output
path. Two files (matrixed or not) rendering to the same output path is a hard error, never a
silent overwrite.

When an axis's values aren't already list-shaped anywhere in `answers` — e.g. an answer is itself
a map of structured values rather than a flat `multiselect` — compute the list with `keys`, a
template function available (alongside every Sprig/Gomplate function) to axis expressions:
`keys(m)` returns `m`'s top-level keys, sorted; `keys(m, "nestedKey")` collects `nestedKey`'s own
keys from every value in `m`, flattened and deduplicated:
