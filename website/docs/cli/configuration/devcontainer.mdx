...
devcontainer:
  <name>:           # Named configuration (e.g., "geodesic", "terraform", "python")
    settings:       # Atmos-specific settings (optional)
      runtime: auto # "auto" (default), "docker", "podman", or omit
...
<dl>
  ...
  <dd>
    Specify the container runtime to use: `"auto"`, `"docker"`, `"podman"`, or omit for automatic detection.
...
<EmbedExample example="devcontainer" title="Simple Devcontainer Example" />

<EmbedExample example="devcontainer-build" title="Devcontainer with Dockerfile Build" />
