...
Individual steps can specify a `retry` block to automatically re-run a step that fails with a transient error (for example, a flaky network call or a registry returning an intermittent `5xx`). This uses the same retry engine as [workflow steps](/workflows#retry-configuration).

To attach a `retry` block, write the step in its structured form (a mapping with `command` and, optionally, `name` and `type`) instead of the shorthand string form:
  ...
  - name: helm
    ...
    commands:
      - name: plugins
        ...
        commands:
          - name: install
            description: Install the helm-diff plugin (with retry on transient registry errors)
            steps:
...
The `retry` block accepts the following parameters. **All parameters are optional** &mdash; omit any parameter to disable that feature or use unlimited behavior. Omitting the entire `retry` block runs the step exactly once (the default).

<dl>
  <dt>`max_attempts`</dt>
  <dd>Maximum number of attempts. Omit for unlimited retries. Must be greater than zero if specified.</dd>
...
:::note Any failure triggers a retry
For custom command steps, a step is retried on **any** non-zero exit, governed by `max_attempts`, the backoff settings, and `max_elapsed_time`. Output-pattern matching (`conditions`) is not consulted for `shell` and `atmos` steps &mdash; if you need to retry only on specific error output, gate it inside the step's own command logic.
:::
