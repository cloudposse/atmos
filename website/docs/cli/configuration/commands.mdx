...
Custom commands can define their own component types beyond the built-in `terraform`, `helmfile`, `packer`, and `ansible` types. This enables you to use Atmos's stack configuration system for any tool—AWS CDK, CloudFormation, Pulumi, Bicep, database migrations, and more.
