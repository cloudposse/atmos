...
- **New features are unaffected.** Only *changes* to previously shipped behavior are journaled. A new key that replaces implicit behavior, such as `vendor.max_concurrency` replacing sequential vendoring, is a default change. A feature introduced after your pinned date still works, with its initial defaults.
...
The journal records default changes, reaching back to February 2025:
...
| `2026-09-15` | `vendor.max_concurrency` | `1` | `4` | Download preparation and upstream checks use four workers; earlier pins preserve sequential execution |
