...
Use [`!include`](/functions/yaml/include) with YAML merge keys to load values from dotenv files into the CLI `env` section. The `<<` key is YAML's merge-key syntax, the same YAML mechanism commonly used with anchors and aliases:
