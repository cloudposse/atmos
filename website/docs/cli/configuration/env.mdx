...
- Supports template functions like `{{ env "VAR" }}` and `{{ exec "command" }}`
...
```yaml
env:
  # Set environment variables for all Atmos operations
...
<dl>
  <dt>`env`</dt>
  <dd>
    A map of environment variable names to values. Values can be literal strings or use template functions.
...
The `env` section supports Go template functions, allowing dynamic values:

### Reading from Other Environment Variables

Use `{{ env "VAR" }}` to read values from the current environment:
...
env:
  ...
  # Set AWS profile from a custom variable
  AWS_PROFILE: '{{ env "MY_AWS_PROFILE" | default "default" }}'
```

### Executing Commands

Use `{{ exec "command" }}` to set values from command output:
...
```yaml
env:
  TOFU_CLI_CONFIG_FILE: "${HOME}/.tofurc"
```
</File>
...
env:
  ...
  GITHUB_TOKEN: '{{ env "GITHUB_TOKEN" | default (env "GH_TOKEN") }}'
...
| Feature | CLI `env` (atmos.yaml) | Stack `env` (stack manifests) |
|---------|------------------------|------------------------------|
...
- [Templates](/cli/configuration/templates) - Template function reference
