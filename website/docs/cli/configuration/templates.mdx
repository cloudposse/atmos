...
against older Atmos releases may need updating. Each removed function has a direct replacement:
...
<dl>
  ...
  <dd>Use [`net.ParseAddr`](https://docs.gomplate.ca/functions/net/#netparseaddr), [`net.ParsePrefix`](https://docs.gomplate.ca/functions/net/#netparseprefix), and [`net.ParseRange`](https://docs.gomplate.ca/functions/net/#netparserange).</dd>
  ...
  <dd>
    ...
    of the same name (note that Sprig's argument order differs from Gomplate's in some cases). The bare `slice` name is a
    Go `text/template` built-in and continues to work. There is no Sprig fallback for `splitN`.
  </dd>
</dl>

Two further upstream changes affect existing templates:

- Conversion functions such as `conv.ToInt64`, `conv.ToFloat64`, and `conv.Atoi` (and the functions built on them) now return an error on input they cannot convert, instead of silently returning `0`.
