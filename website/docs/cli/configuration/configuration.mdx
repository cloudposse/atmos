...
| ATMOS_COMPONENTS_TERRAFORM_PLAN_SKIP_PLANFILE         | components.terraform.plan.skip_planfile         | Skip passing the `-out=FILENAME` flag when executing the `terraform plan` command. Set it to `false` when using Terraform Cloud since the `-out` flag is not supported. Terraform Cloud automatically stores plans in its backend |
