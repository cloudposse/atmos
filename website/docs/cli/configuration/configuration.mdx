...
- Environment variable `ATMOS_CLI_CONFIG_PATH` (the ENV var should point to a folder without specifying the file name . if is set the CLI will only look for the config file in the specified folder)
...
- atmos.d directory form base path (if `import` section is not set in the config) example: `examples/demo-atmos.d`
- Custom import form base path (if `import`  section is set in the config)  example: `examples/demo-atmos-cli-imports`
