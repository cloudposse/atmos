...
1. **`!env`** – Used to retrieve environment variables.  
   See the [**`!env` documentation**](../../core-concepts/stacks/yaml-functions/env) for more details.

2. **`!exec`** – Used to execute shell scripts and assign their output.  
   See the [**`!exec` documentation**](../../core-concepts/stacks/yaml-functions/exec) for more details.

3. **`!include`** – Used to include other YAML files into the current configuration.  
   See the [**`!include` documentation**](../../core-concepts/stacks/yaml-functions/include) for more details.

4. **`!repo-root`** – Used to retrieve the root directory of the Atmos repository. If the Git root is not found, it will return a default value if specified; otherwise, it returns an error.  
