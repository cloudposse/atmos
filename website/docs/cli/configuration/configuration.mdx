...
4. **Git repository root** (`repo-root/atmos.yaml`) - if in a git repository
5. **Parent directory search** - walks up from CWD looking for `atmos.yaml`
6. **Home directory** (`~/.atmos/atmos.yaml`)
7. **System directory** (`/usr/local/etc/atmos/atmos.yaml` on Linux, `%LOCALAPPDATA%/atmos/atmos.yaml` on Windows)
