...
| ATMOS_BITBUCKET_TOKEN                                 | N/A                                             | Bitbucket App Password for API requests; used to avoid rate limits. Unauthenticated requests are limited to 60 requests per hour. When both `ATMOS_BITBUCKET_TOKEN` and `BITBUCKET_TOKEN` are defined, the former prevails.       |
