...
YAML treats backslashes (`\`) as escape characters. This means Windows paths with single backslashes will not work as expected.
:::

#### Correct Ways to Specify Windows Paths

<Tabs>
<TabItem value="forward" label="Forward Slashes (Recommended)" default>
```yaml
