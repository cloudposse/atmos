...
Each setting (`components.terraform.base_path`, `components.helmfile.base_path`, `stacks.base_path`, and `workflows.base_path`) is treated independently and can accept either an absolute or a relative path.
## Configuration File (`atmos.yaml`)

The Atmos (CLI) configuration is loaded from the following locations, in order of precedence:
- The `--config` parameter is the relative or absolute path to a valid configuration file or directory containing strictly atmos configuration file(s).
- The Environment variable `ATMOS_CLI_CONFIG_PATH` (the ENV var should point to a folder without specifying the file name. If it is set, Atmos will only look for the config file in the specified folder). Note, that the `.yaml` and `.yml` extensions are interchangeable and loaded in lexicographical order. Atmos will look for the following files, in order, relative to the `ATMOS_CLI_CONFIG_PATH` environment variable.
  - `atmos.yaml`
  - `.atmos.yaml`
  - `.github/atmos.yaml`
  - `atmos.d/**/*`
...
- `$XDG_CONFIG_HOME/atmos/atmos.yaml` (if `$XDG_CONFIG_HOME` is set)
