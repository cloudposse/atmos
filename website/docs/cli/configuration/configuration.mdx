...
When the `base_path` is explicitly provided, if it is not an absolute path, then it is resolved to the absolute path relative to the current working directory.
