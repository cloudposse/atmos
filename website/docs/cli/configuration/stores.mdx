...
- **Is resolved only with [`!secret`](/functions/yaml/secret)** and the [`atmos secret`](/cli/commands/secret/usage) CLI — using `!store` against a `secret: true` store is an error, which keeps the declarative secret registry mandatory by construction.
