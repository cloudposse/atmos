...
## Using Stores from the CLI or a Workflow Step

For raw read/write access to any configured store -- not tied to a Terraform output -- use the
[`atmos store`](/cli/commands/store/usage) CLI command group or a
[`type: store`](/workflows/steps/type/store) workflow step. Not every operation is available on
every backend -- for example, `atmos store delete` returns an error for Redis and Artifactory,
which don't support deletion:
