...
By default, a remote `import:` source is re-cloned on every `atmos` invocation, so mutable refs (e.g.
`?ref=main`) always stay current. To reuse the clone **across invocations** — for example, with a warm CI
cache — set the same `imports.ttl` default used by [remote stack imports](/stacks/imports#caching-remote-imports):
