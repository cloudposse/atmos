...
This applies to `git::` imports that use a subdirectory (e.g. `git::https://github.com/org/repo.git//path?ref=main`).
By default the source repo is re-cloned on every `atmos` invocation, so mutable refs (e.g. `?ref=main`)
always stay current. To reuse the clone **across invocations** — for example, with a warm CI cache — set
the same `imports.ttl` default used by [remote stack imports](/stacks/imports#caching-remote-imports):
...
`ttl` for maximum reuse; keep `ttl` short for mutable refs like `main` so the config stays fresh.

Plain remote URLs (no `git::` subdirectory) use a separate, non-expiring cache instead — `ttl` has no
effect on them.
