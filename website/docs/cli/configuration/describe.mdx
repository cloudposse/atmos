...
<dl>
  ...
  <dd>
    ...
    **Default sections:** `vars`, `env`, `providers`, `required_providers`, `required_version`, `hooks`, `generate`,
    `backend`, `backend_type`, `remote_state_backend`, `remote_state_backend_type`, `auth`, `command`, `dependencies`,
    `source`, `provision`.

    When set, this list **fully replaces** those defaults — it is not additive. To add a custom section, list every
    default you still want plus your custom section name; to stop evaluating a default, omit it from the list. A custom
    section is reported as `stack.<name>` when it changes.
