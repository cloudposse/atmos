...
- For per-component configuration, see the [component `provision:` block](/stacks/components/provision)
</KeyPoints>
...
<Note>
There is no global `settings.provision.backend` today. Backend provisioning is opted in per component (or per toolchain) because it has stronger side effects (creating cloud resources). See the [component `provision:` block](/stacks/components/provision) for backend provisioning details.
</Note>

## See Also

- [Component `provision:` block](/stacks/components/provision) — Per-component configuration and full conceptual treatment
