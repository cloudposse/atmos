...
<dl>
  ...
  <dd>
    Global default for whether components run inside an isolated workdir under `.workdir/<componentType>/<stack>-<componentName>-<hash>/`. When enabled, every component instance gets its own working directory, allowing concurrent runs without conflicts on `.terraform/`, lockfiles, or generated varfiles.
