...
<Intro>
Atmos can print a one-line resource-usage summary — elapsed time, CPU time, and peak memory — after `terraform plan`/`apply`/`deploy` complete, plus a final aggregate summary covering every subprocess spawned during the whole invocation. The `settings.metrics` section controls whether that local display is shown. The same numbers also appear in the GitHub Actions job summary when [Native CI](/cli/configuration/ci) is active, independent of this setting.
</Intro>
