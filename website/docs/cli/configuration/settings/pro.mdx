...
`atmos pro` is a top-level CLI command group, so its connection config lives at the top-level
`pro:` key in `atmos.yaml` — a sibling of `auth:`, `docs:`, and `ci:`.

<Note>
`settings.pro` is a **deprecated alias** for the same config. Atmos still reads it, resolving
each field independently: a field set at the top level always wins; a field only set under
`settings.pro` still falls back to that value. New configs should use the top-level `pro:` form.
</Note>
...
<dl>
  ...
  <dt>`pro.github_oidc.request_url`</dt>
  <dd>
  ...
  <dt>`pro.github_oidc.request_token`</dt>
  <dd>
  ...
  <dt>`pro.git_sts.git_config_mode`</dt>
  <dd>
    Global default for how the [`github/sts`](/cli/configuration/auth#github-sts-atmos-pro) auth integration
    delivers minted tokens to child `git` processes: `env` (default) injects inline
    `GIT_CONFIG_KEY_n`/`GIT_CONFIG_VALUE_n` rewrites; `file` writes a `0600` gitconfig and emits an
    additive `include.path` so token values stay off the environment. Per-integration
    `git_config_mode` in a `github/sts` integration `spec` overrides this default.

    - **Type:** `string`
    - **Default:** `env`
  </dd>

  <dt>`pro.git_sts.revoke_on_exit`</dt>
  <dd>
    Global default for whether minted `github/sts` tokens are auto-revoked at command-end in CI.
    Set `false` to keep credentials alive for a separate CI step. Per-integration
    `revoke_on_exit` in a `github/sts` integration `spec` overrides this default.
...
<dl>
  ...
  <dd>Override the Atmos Pro API base URL. Maps to `pro.base_url`.</dd>

  <dt>`ATMOS_PRO_ENDPOINT`</dt>
  <dd>Override the API endpoint path. Maps to `pro.endpoint`.</dd>

  <dt>`ATMOS_PRO_WORKSPACE_ID`</dt>
  <dd>Workspace identifier for authentication. Not a secret. Maps to `pro.workspace_id`.</dd>

  <dt>`ATMOS_PRO_TOKEN`</dt>
  <dd>Bearer token obtained via OIDC token exchange (advanced). Maps to `pro.token`.</dd>
...
workflows on a per-stack, per-event basis. The contract is defined under the
top-level `pro:` section on each stack (or component) — a sibling of `vars:`,
`metadata:`, and `settings:` — for every event activity you want Atmos Pro to
handle, declare which workflow file to dispatch and which inputs to pass.

<Note>
`settings.pro` is a **deprecated alias** for this same per-component/stack config. An explicit
`pro:` block takes whole-block precedence over `settings.pro:` when both are set on the same
component (they are not merged together field-by-field). New stack configs should use the
top-level `pro:` form.
</Note>
...
<dl>
  <dt>`pro.pull_request.opened`</dt>
  <dd>Activity fired when a pull request is opened.</dd>

  <dt>`pro.pull_request.synchronize`</dt>
  ...
  <dt>`pro.pull_request.reopened`</dt>
  <dd>Activity fired when a closed pull request is reopened.</dd>

  <dt>`pro.pull_request.merged`</dt>
...
<dl>
  <dt>`pro.drift_detection.enabled`</dt>
  <dd>
    ...
    `pro.enabled: false` disables drift detection regardless of this value — Atmos
...
result badges, reflecting this same effective enabled state:

- **Green (`PRO-ENABLED`)** — the component is effectively Pro-enabled (`metadata.enabled` and
  `pro.enabled` both true or defaulted). Links to your [Atmos Pro dashboard](https://atmos-pro.com/dashboard).
- **Silver (`PRO-DISABLED`)** — the component is not Pro-enabled. Links to
...
<dl>
  <dt>`pro.merge_group.checks_requested`</dt>
  <dd>
...
1. `pro.merge_group.checks_requested.workflows` if defined — **this
   is the recommended path.**
2. Otherwise, `pro.pull_request.synchronize.workflows` is used as a
...
Atmos Pro, the right migration is to add a `pro.merge_group` block
(or `settings.pro.merge_group` if the repository is still on the deprecated form)
to every stack mixin that already has `pull_request`. In most
