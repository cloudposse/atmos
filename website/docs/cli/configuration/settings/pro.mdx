...
time and maximum resident memory for the **Atmos process itself**.

These measurements exclude Terraform/OpenTofu and provider subprocesses. CPU
time is not peak CPU utilization, and the reported memory is not the peak memory
of the whole job. Use measurements that include the complete workload when choosing
runner capacity; these execution records alone cannot establish Terraform runner
requirements. See [execution metadata](/changelog/pro-exec-metadata-upload) for
upload behavior and configuration.
