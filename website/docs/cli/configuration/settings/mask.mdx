...
:::warning Sensitive Terraform/OpenTofu outputs may be written to disk
...
<dl>
  ...
  <dd>
    ...
    Setting this to `false` has the same security implications as `--mask=false`: automatically registered sensitive Terraform/OpenTofu outputs may be written to generated `.tfvars.json` files. See [Disable Masking for Debugging](#disable-masking-for-debugging).
...
<EmbedExample example="secrets-masking" />
