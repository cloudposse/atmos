...
Set the default skill and optionally define custom skills in `atmos.yaml`. Marketplace skills (installed via `atmos ai skill install`) are available automatically -- browse the full set in the [Agent Skills Directory](/ai/skills).
...
Atmos AI skills are distributed via the [Agent Skills marketplace](https://agentskills.io). The official Atmos skills package contains 25 specialized skills covering all aspects of infrastructure orchestration.
...
Atmos includes 25 marketplace skills covering all aspects of infrastructure orchestration:
...
<dl>
  ...
  <dt>`atmos-schemas`</dt>
  <dd>JSON Schema for stack manifests, IDE auto-completion, and validation.</dd>
  ...
  <dd>Project-specific CLI commands in atmos.yaml with arguments, flags, env maps, and shared steps.</dd>

  <dt>`atmos-steps`</dt>
  <dd>Shared step DSL for workflows, custom commands, hooks, and cast recordings.</dd>

  <dt>`atmos-hooks`</dt>
  <dd>Lifecycle hooks for Terraform operations, store publishing, scanners, git, commands, and steps.</dd>

  <dt>`atmos-gitops`</dt>
  <dd>CI/CD: GitHub Actions, Spacelift, Atlantis, `atmos describe affected` for change detection.</dd>

  <dt>`atmos-helmfile`</dt>
  <dd>Helmfile sync/apply/destroy/diff, Kubernetes deployments, and EKS integration.</dd>

  <dt>`atmos-packer`</dt>
  <dd>Packer init/build/validate, machine image building, and template management.</dd>

  <dt>`atmos-stores`</dt>
  <dd>Store backends and cross-component data sharing.</dd>

  <dt>`atmos-toolchain`</dt>
  <dd>CLI tool version management via Aqua registries, .tool-versions, install, and exec.</dd>
  ...
  <dd>Devcontainer management: start/stop/shell/exec, Docker/Podman, and identity integration.</dd>
...
- [`atmos ai skill`](/cli/commands/ai/skill) - Install and share community skills
