...
When Pro is enabled, GitHub Actions invocations can also report exceptions to
Atmos Pro automatically. This works independently of `errors.sentry.enabled`.
See [automatic Pro exception reporting](/cli/configuration/settings/pro#automatic-exception-reporting)
for authentication, opt-out settings, and unprefixed metadata tag enrichment.
