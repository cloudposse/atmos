...
Aliases also provide the tool target for [toolchain proxies](/cli/configuration/toolchain/proxies), such as mapping `coreutils` to `uutils/coreutils` before exposing an `ls` command.
