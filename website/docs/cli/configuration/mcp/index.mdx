...
<dl>
  ...
  <dt>`read_only`</dt>
  <dd>Mark the server as read-only safe (default: `false`). Read-only servers are included in non-interactive commands like `atmos ai ask`. Servers that only retrieve data (documentation, pricing) should set this to `true`.</dd>
...
Auto-install prerequisites like `uv` (which provides `uvx` for AWS MCP servers) via the [Atmos Toolchain](/cli/configuration/toolchain):
...
mcp:
  servers:
    # No credentials needed — safe for atmos ai ask
    aws-docs:
      command: uvx
      args: ["awslabs.aws-documentation-mcp-server@latest"]
      env:
    ...
    # Requires AWS credentials — safe for atmos ai ask
    aws-pricing:
      command: uvx
