...
The MCP server exposes tools configured in `ai.tools` — filtered/permissioned by `ai.tools.allowed`,
`restricted`, and `blocked`, same as `atmos ai chat`. `ai.tools.enabled` does **not** gate the MCP
server, though: `mcp.enabled: true` is this command's own, sufficient opt-in, so tools are always
registered for MCP regardless of `ai.tools.enabled`. See [AI Tools Configuration](/cli/configuration/ai/tools)
...
<dl>
  ...
  <dt>`identity`</dt>
  <dd>Name of an Atmos Auth identity (from the [`auth`](/cli/configuration/auth) section) to use for credential injection. When set, Atmos authenticates through the identity chain and injects `AWS_SHARED_CREDENTIALS_FILE`, `AWS_CONFIG_FILE`, `AWS_PROFILE`, and `AWS_REGION` into the subprocess environment.</dd>
...
When using a CLI provider (`claude-code`, `codex-cli`, `copilot-cli`, `opencode`, `gemini-cli`), MCP servers are passed directly to the CLI tool instead of being managed by Atmos. Smart routing is skipped — all configured servers are passed and the AI model decides which to use.
...
| Copilot CLI | `~/.copilot/mcp-config.json` (backup/restore) | Honors `COPILOT_HOME` |
| opencode | Temp config referenced by `OPENCODE_CONFIG` | Deep-merged by opencode; your `opencode.json` is never modified; the temp file is removed after each run |
...
<dl>
  <dt>`routing.enabled`</dt>
  <dd>Enable automatic server routing (default: `true`). When enabled, Atmos sends the question and server descriptions to the AI provider, which returns which servers are relevant. Only those servers are started.</dd>
</dl>
...
<DocCardList items={[
  ...
  {type: 'link', href: '/cli/commands/mcp/export', label: 'atmos mcp export', description: 'Export .mcp.json for IDE integration'},
]} />
