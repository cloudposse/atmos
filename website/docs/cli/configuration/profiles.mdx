...
via [`--config` or `--config-path`](/cli/configuration) — so
...
You can pin a profile for a whole project by setting `ATMOS_PROFILE` in a committed dotenv file and including it in the base `atmos.yaml` [`env`](/cli/configuration/env) section:
...
trigger [interactive selection](#interactive-selection), so a profile with that name can never be
...
discovers (see [Profile Discovery](#profile-discovery)). Toggle the profiles you want with
space, then confirm — they're activated left-to-right in the order shown, same as if you'd typed
them explicitly.
...
is always kept in the final selection, even if you don't re-pick it in the form.
