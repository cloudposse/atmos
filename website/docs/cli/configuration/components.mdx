...
<dl>
    ...
    <dd>
        If set to `true`, Atmos automatically passes the generated varfile to the `init` command using the `--var-file` flag.
        ...
        **Important:** This feature is designed for OpenTofu. HashiCorp Terraform does not support `--var-file` for the `init` command and will fail if this option is enabled.
        Only enable this if you are using OpenTofu, or ensure your `terraform` command actually points to an OpenTofu binary.
    </dd>
</dl>
