...
<dl>
  ...
  <dd>
    ...
    When this setting is configured, relative [`targets`](/vendor/config/sources) in the manifest are resolved against the Atmos `base_path`
    as well — not against the directory containing the manifest. This means the manifest can live anywhere (for example, next to `atmos.yaml`
    in a configuration directory outside the project root) while vendored artifacts still land in the same tree as the components and stacks.
    ...
    **Default:** `vendor.yaml`
    ...
    - `config/vendor.yaml` - Manifest in a configuration directory
  </dd>
  ...
  <dd>
    ...
    through `vendor.yaml` or `component.yaml`. The <a href="/cli/commands/vendor/vendor-verify">`atmos vendor verify`</a> command checks
...
The `vendor.update` and `vendor.ci` sections configure <a href="/cli/commands/vendor/vendor-update">`atmos vendor update --pull-request`</a> — see
...
<dl>
  ...
  <dd>The only supported value is `scope`: one branch and one PR for the whole update run. Per-component batching (one PR per updated component) isn't implemented yet.</dd>
  ...
  <dd>Atmos applies these additively on every reconciliation. **Default label:** `[component-update]`.</dd>
...
<a href="/cli/commands/vendor/vendor-update">`atmos vendor update`</a>'s token precedence notes for details.
:::
