...
Configure session durations for both CLI and console access at the provider level.
...
auth:
  providers:
    company-sso:
      ...
      session:
        duration: 4h              # CLI credential lifetime

      console:
        session_duration: 12h     # Web console session (max 12h for AWS)
```
</File>
...
Use [Atmos profiles](/cli/configuration/profiles) to define different provider configurations for various use cases:
...
profiles:
  ...
  # Profile for platform engineers with extended sessions
  platform:
    auth:
      providers:
        company-sso:
