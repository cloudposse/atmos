...
:::info Credentials are never cached
`azure/oidc` re-reads its federated token from ambient state on every authentication, and
Atmos never writes the resulting tokens (or credentials derived from them) to the keyring.
A rotated OIDC token is always picked up on the next run.
:::
...
:::info Credentials are never cached
Like `gcp/adc`, `gcp/workload-identity-federation` re-reads its OIDC token from the environment
on every authentication, and Atmos never writes the resulting tokens (or credentials derived
from them) to the keyring. A rotated OIDC token is always picked up on the next run.
:::
