...
id: providers
description: Configure authentication providers for AWS SSO, SAML, GitHub OIDC, and Okta in your `atmos.yaml`.
---
...
<Intro>
Providers are the upstream systems that Atmos uses to obtain initial credentials. Configure providers in the `auth.providers` section of your `atmos.yaml`. Atmos supports AWS SSO, SAML, GitHub OIDC, and Okta authentication.
</Intro>
...
For organizations using Okta as their central identity platform. Uses the OAuth 2.0 Device Authorization Grant (RFC 8628) for CLI authentication.
  ...
  providers:
    okta-corp:
      ...
      spec:
        ...
        authorization_server: default    # Okta authorization server (default: "default")
        scopes: openid profile offline_access  # OAuth scopes (space-separated)
```
</File>

<dl>
  <dt>`kind`</dt>
  <dd>**Required.** Must be `okta/device-code`.</dd>

  <dt>`spec.org_url`</dt>
  <dd>**Required.** Your Okta organization URL (e.g., `https://company.okta.com`).</dd>

  <dt>`spec.client_id`</dt>
  <dd>**Required.** OAuth 2.0 client ID from your Okta application.</dd>

  <dt>`spec.authorization_server`</dt>
  <dd>Optional. Okta authorization server ID. Default: `default`.</dd>

  <dt>`spec.scopes`</dt>
  <dd>Optional. Space-separated OAuth scopes. Default: `openid profile offline_access`.</dd>
</dl>

### Setting Up Okta

To use the Okta device code provider, you need to create a Native Application in your Okta organization:

1. In the Okta Admin Console, go to **Applications** > **Create App Integration**
2. Select **OIDC - OpenID Connect** and **Native Application**
3. Enable the **Device Authorization** grant type
4. Set the **Sign-in redirect URI** to `http://localhost` (not used for device code, but required)
5. Note the **Client ID** for your `atmos.yaml` configuration

:::tip
Include the `offline_access` scope to enable token refresh. Without it, users must re-authenticate when the access token expires.
:::

### AWS OIDC Federation

Okta tokens can federate to AWS using `AssumeRoleWithWebIdentity`. Chain an Okta identity with an AWS assume-role identity:
...
This requires an AWS IAM OIDC identity provider trust policy that trusts your Okta authorization server.
