...
When no static credentials or keyring entries are available, Atmos automatically initiates an OAuth2 PKCE flow via the AWS sign-in service. This provides the same convenient web-based authentication that SSO users already enjoy, without requiring static access keys.

**Authentication priority:**

1. Static credentials from configuration or environment variables
2. Credentials stored in the system keyring (via `atmos auth user configure`)
3. Browser-based OAuth2 PKCE flow (automatic fallback)
