...
id: identities
description: Configure authentication identities including AWS permission sets, assume roles, IAM users, and Okta API access in your `atmos.yaml`.
---
...
For accessing Okta APIs and federating Okta tokens to cloud providers. Uses credentials from an `okta/device-code` provider.
...
auth:
  identities:
    okta-api:
      ...
      via:
        provider: okta-corp           # Reference to an Okta provider
```
</File>

<dl>
  <dt>`kind`</dt>
  <dd>**Required.** Must be `okta/api-access`.</dd>

  <dt>`via.provider`</dt>
  <dd>**Required.** Name of an `okta/device-code` provider to obtain credentials from.</dd>
</dl>

The Okta API access identity sets environment variables for Okta-aware tools:
- `OKTA_ORG_URL` - Your Okta organization URL
- `OKTA_BASE_URL` - Base URL for Okta API calls
- `OKTA_OAUTH2_ACCESS_TOKEN` - OAuth 2.0 access token
- `OKTA_CONFIG_DIR` - Directory containing Okta token files

### Federating to AWS

Chain an Okta identity with an AWS assume-role identity to federate Okta tokens to AWS using `AssumeRoleWithWebIdentity`:
...
:::note
AWS OIDC federation requires an IAM OIDC identity provider configured to trust your Okta authorization server's issuer URL.
:::
