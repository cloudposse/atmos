  ...
  integrations:
    github-sts:
      ...
      spec:
        ...
        token_env: GH_TOKEN          # optional: also export the raw token as $GH_TOKEN (for gh/checkout/API). Empty = off
```
...
<dl>
  ...
  <dt>`token_env`</dt>
  <dd>Optional env var name to export the **raw** minted token under, for consumers beyond git (`gh` CLI, `actions/checkout`, the REST API) — the [Octo-STS](https://github.com/octo-sts/action) use case. Empty (default) keeps the token off the environment; only the transparent `GIT_CONFIG_*` rewrites are emitted. A literal name (e.g. `GH_TOKEN`) requires exactly one minted token; for multi-org runs use a `{owner}` placeholder (e.g. `GH_TOKEN_{owner}`) to export one variable per owner (the owner is uppercased with non-alphanumerics replaced by `_`).</dd>
</dl>
