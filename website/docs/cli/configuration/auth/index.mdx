...
- `aws/rds` — Declare RDS/Aurora connection metadata for `atmos aws rds connect`, which mints a fresh IAM authentication token per connection. See [RDS Authentication Tutorial](/tutorials/rds-authentication).
...
- [RDS Authentication](/tutorials/rds-authentication) — Complete guide for connecting to AWS RDS and Aurora using Atmos identities and IAM authentication tokens
