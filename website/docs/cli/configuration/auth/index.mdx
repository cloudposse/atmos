...
auth:
  ...
  identities:
    <identity-name>:               # Named identity configurations
      kind: <identity-kind>
      spec: {}                     # Kind-specific SDK/client configuration
      # Identity-specific fields...

  integrations:
    <integration-name>:            # Named integration configurations
      kind: <integration-kind>     # aws/ecr, aws/eks, gcp/gke, github/sts
      via:
...
- `gcp/gke` — Generate GKE kubeconfig with dynamic Atmos exec credentials. See [GKE Kubeconfig Authentication](/cli/commands/gcp/gke).
...
:::note GitHub Enterprise Server (GHES)
`GITHUB_TOKEN` / `ATMOS_GITHUB_TOKEN` authenticate against whatever host `GITHUB_SERVER_URL` /
`GITHUB_API_URL` resolve to (public `github.com` by default, or a GHES instance) — see
[Environment Variables](/cli/environment-variables#github-enterprise-server-ghes) for the full
list. The `github/sts` (Atmos Pro) integration above brokers tokens for Atmos Pro's own GitHub
App, which is registered against public github.com; it is unaffected by `GITHUB_SERVER_URL`.
:::
...
<DocCardList items={[
  ...
  {type: 'link', href: '/cli/commands/gcp/gke/token', label: 'atmos gcp gke token', description: 'Generate GKE bearer tokens for kubectl'},
]} />
