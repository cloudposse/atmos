...
<dl>
  ...
  <dt>`name_pattern`</dt>
  <dd>
    <Deprecated feature="name_pattern" />

    Use `name_template` instead for more flexibility and reliability. `name_pattern` uses predefined tokens mapped to fixed variable names and may cause errors in certain contexts.
...
### Compact Join Pattern (Recommended)

For most use cases, the simplest and most reliable approach uses Sprig's `list`, `compact`, and `join` functions to skip empty values automatically:

```yaml title="atmos.yaml"
stacks:
  name_template: '{{ list .vars.namespace .vars.tenant .vars.environment .vars.stage | compact | join "-" }}'
```

This template:
- Automatically skips variables that are empty strings
- Produces clean hyphen-separated names from whatever variables are set
- Works reliably with both `atmos list stacks` and `atmos terraform plan`
- For variables `namespace: acme`, `tenant: plat`, `environment: ue2`, `stage: prod`, outputs `acme-plat-ue2-prod`
- For variables `tenant: plat`, `environment: ue2`, `stage: prod` (no namespace), outputs `plat-ue2-prod`

### Advanced Template with Validation

For cases where you need stricter validation, `name_template` supports complex logic with template expressions and functions.

:::caution Important: Template Whitespace Rules for Multi-line Templates
When using multi-line `name_template` with YAML block scalars (`|-`), follow these rules:

- **First template action**: Use `{{` (no leading dash). A leading `{{-` on the very first line can cause unexpected behavior with `atmos terraform` commands.
- **Last output action**: Use `}}` (no trailing dash). A trailing `-}}` on the final output line may cause the output to be stripped.
- **Intermediate actions**: Can use `{{- ... -}}` (both dashes) to suppress whitespace between lines — this is correct and expected.

In the example below: the first line uses `{{` (not `{{-`), intermediate assignment lines use `{{- ... -}}`, and the final output uses `{{- $stack_name }}` (leading dash to trim preceding whitespace, but no trailing dash).
:::
...
<DocCardList items={[
  {type: 'link', href: '/cli/configuration/stacks/inherit', label: 'Inheritance Behavior', description: 'Configure what sections are inherited'},
]} />
...
<DocCardList items={[
  ...
  {type: 'link', href: '/howto/inheritance', label: 'Inheritance Guide', description: 'Learn about component inheritance'},
]} />
