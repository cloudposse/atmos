...
Set default metadata for all components of a type using global `terraform.metadata`, `helmfile.metadata`, or `packer.metadata`:
...
components:
  terraform:
    ...
    eks:
      # Also inherits global metadata
      vars:
        cluster_name: "prod"
```
</File>

**Precedence:** Global metadata has the lowest priority:
1. Global metadata (lowest)
2. Base component metadata (from `metadata.inherits`)
3. Component metadata (highest)

This allows you to set stack-wide defaults while still allowing inheritance and component-specific overrides.
