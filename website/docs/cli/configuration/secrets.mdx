...
A store becomes a **secret backend** by setting `secret: true`. `!store`, `!store.get`, and `atmos.Store`
against a `secret: true` store are errors ("use `!secret`"), which makes the declarative registry
...
|---|---|---|
| Backends | AWS SSM, AWS Secrets Manager, HashiCorp Vault, Azure Key Vault, GCP Secret Manager, 1Password | SOPS (`age`/`aws-kms`/`gcp-kms`/`azure-kv`/`gpg`) |
...
`SOPS_AGE_KEY_FILE`/`SOPS_AGE_KEY` environment variables. The KMS-backed kinds (`sops/aws-kms`,
`sops/gcp-kms`, `sops/azure-kv`) authenticate to the cloud KMS using an Atmos
[`identity`](#sops-identity) — the same identity model as Track-1 stores and `terraform plan` — so you
do **not** need to wrap commands in `atmos auth exec` or export cloud credentials.
[`atmos secret keygen`](/cli/commands/secret/keygen) generates the age key and writes it to whichever
sink `spec.age_key` selects (KMS kinds bring externally-managed keys, so they are not generated).
...
secrets:
  providers:
    dev-sops:
      kind: sops/age          # or: sops/aws-kms, sops/gcp-kms, sops/azure-kv, sops/gpg
      # identity: aws/prod-admin        # KMS kinds only; auth identity for the cloud KMS (see below)
      spec:
...
#### Cloud KMS identity {#sops-identity}

The cloud-KMS kinds (`sops/aws-kms`, `sops/gcp-kms`, `sops/azure-kv`) wrap the SOPS data key with a
cloud KMS, so encrypt/decrypt require cloud credentials. Atmos obtains them from an
[auth identity](/cli/configuration/auth/identities) — the same way it does for `terraform plan` and
Track-1 stores — and injects them into the in-process KMS calls. No `atmos auth exec` wrapper or
ambient cloud credentials are needed. The cloud is inferred from the file's master keys at runtime; the
`identity` is the only knob (`sops/age` and `sops/gpg` use local keys and ignore it).

<dl>
  <dt>`identity`</dt>
  <dd>Optional auth identity (a provider-level field, sibling to `kind`) used to authenticate to the cloud KMS for KMS-backed kinds. Resolved with the following precedence (highest first):</dd>
</dl>

1. The provider's own `identity` (`secrets.providers.<name>.identity`).
2. The `--identity` flag / `ATMOS_IDENTITY` environment variable on the `atmos secret` command.
3. The stack/component **effective identity** (the same identity `terraform plan` would use for that component).

If none resolves, Atmos falls back to the ambient cloud credential chain (environment, shared config,
instance metadata), preserving prior behavior.
...
- [Custom components](/cli/configuration/commands/component#passing-secrets-to-custom-components) — read as `$VAR` in the command steps.
