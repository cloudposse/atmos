...
is required**. For `sops/age` you need an age key, supplied either declaratively via `spec.age_key_file`
or via the `SOPS_AGE_KEY_FILE`/`SOPS_AGE_KEY` environment variables; the KMS-backed kinds use your cloud
credentials.
  ...
  providers:
    dev-sops:
      ...
      spec:
        ...
        # age_key_file: secrets/keys.txt  # optional; age PRIVATE key for decryption (~ and $ENV expanded)
...
<dl>
  ...
  <dt>`age_key_file`</dt>
  <dd>Optional path to the age **private key** used to decrypt (`sops/age` only). Supports `~` and `$ENV`/`${ENV}` expansion. When set it takes precedence over `SOPS_AGE_KEY_FILE`/`SOPS_AGE_KEY`, so the key can be declared alongside the rest of the config instead of relying on the environment. KMS-backed kinds ignore this field (they decrypt with cloud credentials).</dd>
