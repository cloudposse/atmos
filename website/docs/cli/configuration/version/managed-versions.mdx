...
| `dependencies` | Base catalog of external dependencies tracked by Atmos. Track-level `dependencies` override the base catalog. |
...
The [`add`, `set`, `remove`, and `get`](/cli/commands/version/track#add) subcommands edit dependency entries in `atmos.yaml` without hand-editing YAML, preserving comments, anchors, and formatting. `add` infers the ecosystem from the package coordinate:
