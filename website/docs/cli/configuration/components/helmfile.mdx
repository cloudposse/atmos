...
<File title="atmos.yaml">
  ```yaml components: helmfile: base_path: components/helmfile use_eks: true
  kubeconfig_path: /dev/shm cluster_name: "production-eks-cluster" ```
</File>
...
<File title="Before (deprecated)">
  ```yaml components: helmfile: use_eks: true # was default
  helm_aws_profile_pattern: "{namespace}-{tenant}-gbl-{stage}-helm"
