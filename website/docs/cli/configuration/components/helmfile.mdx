...
The `use_eks` setting now defaults to `false` (previously `true`). If you use helmfile with EKS clusters, you must explicitly add `use_eks: true` to your configuration. See the [Helmfile EKS Modernization](/blog/helmfile-eks-modernization) blog post for migration details.
:::
...
- [Helmfile EKS Modernization (Breaking Change)](/blog/helmfile-eks-modernization)
