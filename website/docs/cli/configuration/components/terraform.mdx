...
<dl>
  ...
  <dt>`ui.enabled`</dt>
  <dd>
    When `true`, Atmos displays a streaming terminal UI during `terraform plan`, `apply`, `init`, and `refresh` commands. The UI shows real-time resource status with progress indicators, similar to Docker build output.

    The streaming UI automatically disables when:
    - Output is piped or redirected (no TTY)
    - Running in CI environments (`CI=true`)
    - Using commands that don't support JSON streaming
