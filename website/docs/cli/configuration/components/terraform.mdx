...
<dl>
  ...
  <dd>
    ...
    - **`init.upgrade`** — `auto` adds `-upgrade` only when Terraform or OpenTofu reports that an upgrade is required (for example, a provider version constraint was raised beyond the locked version). `always` adds it to every init. `never` never adds it.
