...
`components.terraform.flags` < stack-level `terraform.flags` < a component's own
`flags:` block < an explicit CLI-typed flag (e.g.
...
<dl>
  ...
  <dd>
    Default value for `-parallelism`. Applies to `plan`, `apply`, and `destroy`. Not
    supported by `terraform import`.
