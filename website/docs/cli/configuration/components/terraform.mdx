...
<dl>
  ...
  <dt><code>mirror.platforms</code></dt>
  <dd>Target platforms (<code>&lt;os&gt;_&lt;arch&gt;</code>, e.g. <code>linux_amd64</code>, <code>darwin_arm64</code>) for eager multi-platform pre-seeding via <a href="/cli/commands/terraform/cache/mirror"><code>atmos terraform cache mirror</code></a>. Empty defaults to the host platform.</dd>
</dl>
...
The lazy proxy only caches the platform Terraform requests at run time (your host's). To pre-seed the cache for **multiple** platforms — for mixed CI/developer fleets or air-gapped bundles — declare `mirror.platforms` and run [`atmos terraform cache mirror`](/cli/commands/terraform/cache/mirror), which wraps `terraform providers mirror` into the same cache directory.
