...
A command's `env:` field accepts [`!include`](/functions/yaml/include) to load values directly
from a `.env` file, instead of listing each variable inline:
...
commands:
  - name: deploy
    ...
    env: !include .env
    steps:
      - atmos terraform apply app -s {{ .Arguments.stack }}
```

Use a YAML merge key to combine dotenv values with inline overrides:
...
See [Environment Variables](/cli/configuration/env#including-dotenv-files) for the full dotenv
`!include` behavior, including merging multiple files and precedence rules.
