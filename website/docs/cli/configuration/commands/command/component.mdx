...
<Intro>
The `component` field lets a custom command define its own component type beyond the built-in `terraform`, `helmfile`, and `packer` types. This brings Atmos's stack configuration system to any tool — Ansible playbooks, Kubernetes manifests, shell scripts, CDK apps, and more.
</Intro>
