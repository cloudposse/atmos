...
<dl>
  ...
  <dt>`always`</dt>
  <dd>Run after success or failure. Use this for teardown and cleanup steps.</dd>
  ...
  <dt>`failure`</dt>
  <dd>Run only after an earlier step has failed.</dd>
</dl>

CEL expressions can read `ci`, `status`, `stack`, `component`, `workflow`,
`step`, and `env`. For workflow and custom command steps, `status` is the
current command status: `success` before any step fails and `failure` after an
earlier step failed. Also available: `os`/`arch`/`platform` (for example
`when: "os == 'darwin'"`) and the freshness facts `checksum`, `timestamp`,
`precondition`, `sources`, `artifacts` — see [`inputs`](/workflows/steps/inputs),
[`artifacts`](/workflows/steps/artifacts), and
[`precondition`](/workflows/steps/precondition).
...
GitHub Actions' `continue-on-error`. See [`continue`](/workflows/steps/continue)
...
[`inputs`](/workflows/steps/inputs) and [`artifacts`](/workflows/steps/artifacts)
for the full field reference.
...
Declare `precondition.tools` instead when a step's freshness only depends on
whether a tool is already on `PATH` — without an explicit `when`, declaring
`precondition` alone implicitly means `when: "!precondition.success"` (run only
when the tool is *not* already there). See
[`precondition`](/workflows/steps/precondition) for the full field reference.
...
### Running Interactive Steps in CI (non-TTY)

Interactive step types (`choose`, `input`, `confirm`, `filter`, `file`, `write`) need a terminal to
prompt. When a custom command runs without a TTY — most commonly in CI — each interactive step uses
its `default` value instead of prompting. A step **without** a `default` fails with
`interactive terminal required for step`, so an unattended run never proceeds with an unintended
value.

Source the default from the environment with the `!env` function (`!env VAR fallback`) so the same
command is interactive locally and non-interactive in CI:

```yaml
commands:
  - name: deploy
    description: Deploy to an account
    steps:
      - name: account
        type: choose
        prompt: "Account"
...
Locally this prompts; in CI it reads `STACK_ACCOUNT` (falling back to `dev`) and passes the value to
the shell step. The same behavior applies to
[workflow steps](/workflows/steps#interactive-steps-in-ci-non-tty).
