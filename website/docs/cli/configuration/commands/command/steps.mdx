...
`artifacts` — see [`inputs`](/workflows/steps/inputs),
[`artifacts`](/workflows/steps/artifacts), and
[`preconditions`](/workflows/steps/preconditions).
...
[`preconditions`](/workflows/steps/preconditions) for the full field reference.
...
Use `output: viewport` on a shell step to follow build logs in a small live window:
...
The four-line window contains up to three recent stdout/stderr lines and a status
row. It follows new output automatically, collapses on success, and expands to
show the full captured output on failure. CI and redirected output stream normally.
See [live viewport](/workflows/output#live-viewport) for sizing and terminal behavior.
