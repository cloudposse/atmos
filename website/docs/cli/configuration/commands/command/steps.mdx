...
without failing the command. Atmos evaluates built-in predicate keywords first;
any other scalar string is evaluated as a CEL expression. Use `!cel` when you
...
`step`, and `env`. For workflow and custom command steps, `status` is
`success`.
