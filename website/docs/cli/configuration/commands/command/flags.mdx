...
Set `values` to a fixed list of allowed strings so Atmos rejects anything else immediately, with the same validation and error format built-in commands' `valid_values:` already use:
...
commands:
  - name: deploy
    ...
    steps:
      - type: atmos
        command: terraform apply infra -s {{ .Flags.environment }}
```

```shell
$ atmos deploy --environment=typo
Error: invalid value for flag: invalid value "typo" for flag --environment (valid values: dev, staging, prod)
```

If the flag is also `required` and left unset in an interactive terminal, Atmos prompts with a picker built from `values` instead of erroring — the same behavior built-in commands already use for a missing required flag:
...
In a non-interactive environment (CI, no TTY, or `--interactive` disabled), a missing required flag still falls through to the normal "required flag not set" error — the picker never blocks an unattended run.
