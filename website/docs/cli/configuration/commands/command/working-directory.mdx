...
- **Relative paths** are resolved against the Atmos `base_path` — this is true of every relative
  value here, whether or not it starts with `./`, since a custom command isn't necessarily scoped
  to a single component
- The `!repo-root` YAML function can be used to reference the git repository root

This is unlike a [`kind: step` hook](/stacks/hooks#kind-step-run-a-step-type), where a plain
relative value (no `./` prefix) resolves against the component's own working directory instead.
