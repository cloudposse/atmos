...
<dl>
  ...
  <dd>
    Specifies the path to the folder where **all** Atmos stack config files (stack manifests) are defined.
  ...
  <dd>
    Tells Atmos where to search for the top-level stack manifests
  ...
  <dd>
    Tells Atmos which paths from `stacks.included_paths` to exclude. For example, we will exclude the config files that don't
  ...
  <dd>
    Configures the name pattern for the top-level Atmos stacks using the context variables `namespace`, `tenant`, `environment`
  ...
  <dd>
    Serves the same purpose as `stacks.name_pattern` (defines the naming convention for the top-level Atmos stacks), but
