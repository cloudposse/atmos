...
stacks:
  ...
  # To define Atmos stack naming convention, use either `name_template` or `name_template`.
  # `name_template` has higher priority (if `name_template` is specified, `name_template` will be ignored).
  # `name_template` uses the predefined context tokens {namespace}, {tenant}, {environment}, {stage}.
...
- `stacks.name_template` configures the name pattern for the top-level Atmos stacks using the context variables `namespace`, `tenant`, `environment`
  and `stage` as the tokens. Depending on the structure of your organization, OUs, accounts and regions, set `stacks.name_template` to the
  following:

  - `name_template: {stage}` - if you use just one region and a few accounts (stages) in just one organization and one OU. In this case, the
  ...
  - `name_template: {environment}-{stage}` - if you have multiple regions and accounts (stages) in just one organization and one OU. In this case, the
    ...
    and `atmos terraform apply <component> --stack ue1-prod`. Note that the `name_template` can also be defined
    as `{stage}-{environment}`, in which case the Atmos commands will look like `atmos terraform apply <component> --stack dev-ue2`

  - `name_template: {tenant}-{environment}-{stage}` - if you have multiple regions, OUs (tenants) and accounts (stages) in just one organization. In
  ...
  - `name_template: {namespace}-{tenant}-{environment}-{stage}` - if you have a multi-org, multi-tenant, multi-account and multi-region architecture.
...
- `stacks.name_template` serves the same purpose as `stacks.name_template` (defines the naming convention for the top-level Atmos stacks), but
  ...
  - `name_template: "{{.vars.tenant}}-{{.vars.environment}}-{{.vars.stage}}"` defines the same name pattern for the top-level
    Atmos stacks as `name_template: "{tenant}-{environment}-{stage}"` does
...
:::note
Use either `stacks.name_template` or `stacks.name_template` to define the naming convention for the top-level Atmos stacks.

`stacks.name_template` has higher priority.

If `stacks.name_template` is specified, `stacks.name_template` will be ignored.
:::
