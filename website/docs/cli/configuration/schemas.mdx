...
description: Configure paths for JSON Schema and OPA policy files to validate Atmos stack manifests, root atmos.yaml, and components.
id: schemas
...
<Intro>
Configure the paths where Atmos finds OPA and JSON Schema files to validate stack manifests, the root
`atmos.yaml` CLI config, and components. Schemas enable you to enforce consistency and catch configuration
errors early.
</Intro>
...
schemas:
  ...
  # JSON Schema to validate Atmos stack manifests
  atmos:
  ...
  # JSON Schema to validate root atmos.yaml (enabled by default)
  config:
...
<dl>
  ...
  <dt>`schemas.config.schema`</dt>
  <dd>
    Path or `atmos://` URI to the JSON Schema used to validate the root `atmos.yaml` file. Defaults to the embedded schema `atmos://schema/config/global/1.0`.
  </dd>

  <dt>`schemas.config.matches`</dt>
  <dd>
    Glob patterns for files validated by `schemas.config`. Defaults to `atmos.yaml` and `atmos.yml`.
  </dd>
</dl>
...
## Atmos Config JSON Schema

The [Atmos Config JSON Schema](pathname:///schemas/atmos/atmos-config/1.0/atmos-config.json) validates the root
`atmos.yaml` CLI configuration and provides auto-completion in supported editors.
...
For IDE integration, add this directive at the top of `atmos.yaml`:
