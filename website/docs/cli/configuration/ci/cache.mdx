...
There are two ways to use the cache in GitHub Actions, and they answer a single question: **who stores the cache?**

- **Native `actions/cache`** (Options 1 and 4) — Atmos only computes *what* to cache (the key and paths from `ci.cache`); `actions/cache` does the storage. This exposes **no** runtime token to your job.
- **Atmos-managed** (Options 2 and 3) — Atmos's own backend (`atmos ci cache restore`/`save`) does the storage via the GitHub Actions cache service. That requires the runtime credentials, which GitHub withholds from `run:` steps, so the [`github-runtime`](https://github.com/cloudposse/atmos/tree/main/actions/github-runtime) action exposes them.

:::tip Recommended (most secure)
Prefer **Option 4** (the `actions/cache` composite) or **Option 1** — both expose **no** runtime token. Security ranking: **Option 1 ≈ 4 (no token) > 2 (scoped token) > 3 (ambient token)**. If you need Atmos to own restore/save, prefer `mode: output` (scoped) over `mode: env` (ambient).
:::

<Tabs>
  <TabItem value="actions-cache" label="4. Composite (recommended)" default>
...
**Cons:** one more Atmos action in the chain (it pins `actions/cache` internally).
  ...
  <TabItem value="paths" label="1. paths + actions/cache">
...
**Cons:** three steps to wire by hand.
  ...
  <TabItem value="output" label="2. Atmos backend (scoped token)">
...
**Cons:** the token still enters those steps; more YAML.
  ...
  <TabItem value="env" label="3. Atmos backend (ambient token)">
