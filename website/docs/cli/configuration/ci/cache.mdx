...
of `ci.cache.paths`, with no opt-out. This is defensive hardening: it makes it structurally
impossible for these directories to end up in a cache archive, rather than relying on every
`ci.cache.paths` configuration to avoid them by convention.
...
<dl>
  ...
  <dd>
    ...
    cached, **except** the default-excluded auth-cache subdirectories, which are never cached.
