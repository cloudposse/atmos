...
| **[Component Configuration](#component-configuration)** | `components/` | Infrastructure code (Terraform, Helmfile, Packer, Ansible, Rain) |
...
- Terraform, Helmfile, Packer, Ansible, and Rain command configuration
...
Ansible playbooks in `components/ansible/`. Used for configuration management and automation.
...
[Ansible Configuration →](/components/ansible)

### Rain Components

Rain CloudFormation templates in `components/rain/`. Used for managing AWS CloudFormation stacks.
...
[Rain Configuration →](/components/rain)
