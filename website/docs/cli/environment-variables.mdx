...
:::note
The `owner/repo` and `github.com/owner/repo` shorthand forms (imports, vendoring, `atmos git
clone`, AI skill sources, etc.) always resolve to public `github.com`. On GHES, write the full
URL (e.g. `https://github.example.com/owner/repo`) instead of the shorthand — a bare hostname
can't be distinguished from a relative path, so shorthand detection can't safely guess a GHES
host.
:::
