...
<dl>
  ...
  <dd>Set to `true` to force planfile drift verification (fail on drift) on `atmos terraform deploy`, equivalent to `--verify-plan`. See [Planfile drift verification](/components/terraform/planfiles#drift-verification).</dd>

  <dt>`ATMOS_TERRAFORM_NO_VERIFY_PLAN`</dt>
  <dd>Set to `true` to disable planfile drift verification on `atmos terraform deploy`, equivalent to `--no-verify-plan`. Overrides config and the CI default.</dd>
