...
<dl>
  ...
  <dt>`ATMOS_SKILL`</dt>
  <dd>
    Specify an AI skill for domain-specific analysis context. Equivalent to the `--skill` flag.
    Requires `ATMOS_AI=true` or `--ai` flag. The skill's system prompt is prepended to the AI analysis
    for deeper, domain-aware insights.
...
<dl>
  ...
  <dd>
    Atmos Pro authentication token.
  </dd>

  <dt>`ATMOS_PRO_WORKSPACE_ID`</dt>
  <dd>
    Atmos Pro workspace identifier.
  </dd>
