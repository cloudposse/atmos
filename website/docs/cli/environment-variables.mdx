...
- **The endpoint is `https`.** `GITHUB_SERVER_URL` / `GITHUB_API_URL` and the `ATMOS_TOOLCHAIN_*`
  variables above may resolve to a plain `http://` endpoint (this is only expected in local
  testing); Atmos never attaches the token to a request against a non-`https` endpoint, since
  doing so would send it over the network in cleartext.
- **The toolchain endpoint belongs to the same host as your repository.** Your token is resolved
  without regard to host, so it is treated as scoped to `GITHUB_SERVER_URL`'s host. It is only
  forwarded to the toolchain endpoints (`ATMOS_TOOLCHAIN_GITHUB_URL` /
  `ATMOS_TOOLCHAIN_GITHUB_API_URL`) when they resolve to that *same* host. By default the
  toolchain endpoints point at public `github.com`, so on a GHES instance the toolchain
  fetches proceed **unauthenticated** unless you explicitly point `ATMOS_TOOLCHAIN_GITHUB_URL` /
  `ATMOS_TOOLCHAIN_GITHUB_API_URL` at your own GHES host too.

Both rules apply independently everywhere Atmos builds an authenticated GitHub request or
client (the CI provider, imports/vendoring, the `pkg/github` API client, the toolchain
installer, and the aqua-registry client).
:::
