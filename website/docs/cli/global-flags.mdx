...
<dl>
    ...
    <dd>
        ...
        - Options: `bar`, `ascii`, `table`, `sparkline`
        - Default: `bar`
    ...
    <dt>`--heatmap-hdr`</dt>
    <dd>
        Enable HDR histogram for P95 latency calculations.
        - Default: `false`
        - Uses High Dynamic Range histograms for accurate percentile metrics
        - Adds P95 column to performance output when enabled
        - Useful for identifying performance outliers and latency spikes
        - Minimal performance overhead (recommended for production profiling)
        - Only used with `--heatmap` flag
    </dd>
</dl>
