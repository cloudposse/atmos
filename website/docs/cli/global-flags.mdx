...
<dl>
    ...
    <dd>
        ...
        - Executes **before** all other operations, including configuration loading and `--base-path` resolution
        - Supports both absolute and relative paths
        - Relative paths are resolved from the current working directory
        - Can also use `ATMOS_CHDIR` environment variable
        - Useful for operating on infrastructure in different directories without changing your shell's working directory

        **Use Cases:**
        - **Development workflow**: Point development builds at other infrastructure repos
        - **CI/CD**: Run atmos from a build directory while operating on infrastructure elsewhere
        - **Multi-repo operations**: Execute commands across multiple repos without manual directory changes
        - **Scripting**: Simplify scripts that need to run atmos against different directories

        **Examples:**
        - `atmos -C /path/to/infra terraform plan vpc -s prod`
        - `atmos --chdir=../other-repo describe stacks`
    ...
    <dd>
        ...
        - Supports both absolute and relative paths
        - Resolved **after** `--chdir` is processed (if specified)
    </dd>
