...
<dl>
    <dt>`--chdir` / `-C`</dt>
    <dd>
        Change the process working directory before Atmos does anything. Equivalent to running `cd <directory>` before `atmos`.

        **What it actually does:**
        - Calls `os.Chdir()` to change the process's current working directory
        - Happens **before** any configuration loading or path resolution
        - ALL subsequent file operations use the new directory as the base
        - Changes where relative paths are resolved from

        **How it differs from `--base-path`:**
        - `--chdir`: Changes the **process working directory** (like running `cd` first)
        - `--base-path`: Provides a **path prefix** for Atmos configuration paths, without changing the working directory
        - When both are used: `--chdir` changes the directory first, then `--base-path` paths are resolved from that new directory
        ...
        **Example to show the difference:**
        ...
        # With --chdir (changes working directory to /infra)
        ...
        # ALL paths resolved relative to /infra

        # With --base-path (working directory stays /home/user)
        atmos --base-path=/infra terraform plan
        # Looks for atmos.yaml using: /infra (joined with config paths)
        # Resolves components from: /infra/components/terraform/
        # But relative paths in config still resolve from /home/user

        # With both (changes directory THEN applies prefix)
        atmos --chdir=/home/user --base-path=/infra terraform plan
        # Working directory: /home/user
        # Base path prefix: /infra
        # Paths joined: /home/user working dir + /infra base path
        ```

        **Path resolution:**
        - Supports both absolute and relative paths
        - Relative paths are resolved from your **current shell directory** before changing
        - Can also use `ATMOS_CHDIR` environment variable
    ...
    <dt>`--base-path`</dt>
    <dd>
        Specify a path prefix for Atmos configuration lookups without changing the working directory.

        **What it actually does:**
        - Sets `atmosConfig.BasePath` which is used as a prefix when joining paths
        - Does **not** call `os.Chdir()` or change the working directory
        - Used when constructing paths to: stacks, terraform components, helmfile components, packer components
        - The value is joined with component paths: `filepath.Join(basePath, componentPath)`

        **How it differs from `--chdir`:**
        - `--base-path`: A **configuration value** used as a path prefix (doesn't change working directory)
        - `--chdir`: Actually **changes the working directory** of the process
        - See examples above under `--chdir` for detailed comparison

        **When to use it:**
        - Your `atmos.yaml` and components are in a subdirectory, but you want to run Atmos from a parent directory
        - You're using Atmos programmatically (like the Terraform provider) and need to override the base path
        - You have a non-standard project layout where the Atmos project root isn't the current directory

        **Path resolution:**
        - Supports both absolute and relative paths
        - Relative paths are resolved using `filepath.Abs()` from the current working directory
        ...
        **Examples:**
        ...
        # Combine with --chdir: first change to /infra, then use ./custom as base path
        atmos --chdir=/infra --base-path=./custom describe stacks
        # This changes working dir to /infra, then uses /infra/custom as the base path
        ```
    </dd>
    ...
    <dd>
        ...
        - For richer output and additional flags, use the [`atmos version`](/cli/commands/version) command
