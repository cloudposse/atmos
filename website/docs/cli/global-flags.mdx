...
<dl>
    <dt>`--chdir` / `-C`</dt>
    <dd>
        Change the shell's working directory before Atmos processes configuration. Atmos runs as if it was started from this directory.

        **What it does:**
        - Changes the working directory **before** configuration processing begins
        - Affects all file operations: configuration loading, path resolution, and component execution
        - Makes Atmos behave as if you had run `cd <directory> && atmos <command>`
        - Processes **before** `--base-path` and all other flags

        **How it differs from `--base-path`:**
        - `--chdir`: Changes the **working directory** (like `cd`)
        - `--base-path`: Overrides the **Atmos project root** within the working directory
        - `--chdir` is applied first, then `--base-path` is resolved relative to the new working directory
        ...
        **Examples:**
        ```bash
        # Run as if you were in /path/to/infra
        atmos -C /path/to/infra terraform plan vpc -s prod

        # Combine with --base-path to override project root
        atmos --chdir=/infra --base-path=./custom terraform apply
    ...
    <dt>`--base-path`</dt>
    <dd>
        Override the Atmos project root directory. This tells Atmos where to find `atmos.yaml`, stacks, and components.

        **What it does:**
        - Overrides the project root (where `atmos.yaml` lives)
        - Does **not** change the working directory
        - Affects where Atmos looks for configuration, stacks, and components

        **How it differs from `--chdir`:**
        - `--base-path`: Overrides the **Atmos project root** (where `atmos.yaml` is located)
        - `--chdir`: Changes the **working directory** before any processing
        - `--base-path` is resolved **after** `--chdir` if both are specified

        **Path resolution:**
        - Supports both absolute and relative paths
        - Relative paths are resolved from the current working directory (or from `--chdir` if specified)
        - Can also use `ATMOS_BASE_PATH` environment variable

        **Examples:**
        ```bash
        # Look for atmos.yaml in /custom/path instead of current directory
        atmos --base-path=/custom/path terraform plan vpc -s prod

        # Combine: change to /infra, then use ./custom as project root
