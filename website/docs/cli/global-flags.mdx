...
<dl>
    ...
    <dt>`--cast`</dt>
    <dd>
        Record an Atmos command as an asciicast. Use bare `--cast` to write a generated `.cast` file, or pass an output file. The file extension determines the output type:
        - `.cast` writes the asciicast recording directly
        - `.gif` records to a temporary cast and renders a GIF (Atmos installs the managed `agg` renderer automatically on first use)
        - `.mp4` records to a temporary cast and renders an MP4 (Atmos installs managed `agg` and FFmpeg renderers automatically on first use; MP4 is GIF-derived and can retain GIF palette constraints)
        - `.html` renders a static HTML fragment of the final terminal content (native, no external tools)
        - `.ascii` renders the final terminal content as plain text with no ANSI codes (native)
        - `.png` / `.jpg` render a static image of the final terminal content (native)

        Help output is recorded too when `--cast` is passed explicitly, which makes documentation screenshots one command:

        **Basic Usage:**
        ```bash
        atmos about --cast
        atmos about --cast=artifacts/about.cast
        atmos about --cast=artifacts/about.gif
        atmos about --cast=artifacts/about.mp4
        atmos about --cast=artifacts/about.png
        atmos about --help --cast=artifacts/about-help.html
        ```
    </dd>
    ...
    <dd>
        ...
        **Modes:**
        - **With profile name(s)** (`--profile developer` or `--profile=ci,security`): Activate the specified profile(s)
        - **Without value** (`--profile`): Show an interactive multi-select to choose which discovered profiles to activate
        - **Omitted**: Use `profiles.default` from `atmos.yaml`, if set
        ...
        **Basic Usage:**
        ...
        # Interactive selection: bare --profile (no value) opens a multi-select
        # picker listing every discovered profile, in an interactive terminal
        ...
        **Available via:**
        ...
        Place a bare `--profile` (no value) **after** the command, not before it — the same placement rule as bare `--identity`:

        ```bash
        # Recommended: bare --profile after the command
        atmos auth login --profile

        # Avoid: bare --profile before the command consumes the command name as its value
        atmos --profile auth login
        # treated as: --profile=auth login  (fails with "profile not found" — "auth" isn't a profile)
        ```

        Using the equals syntax (`--profile=developer`) with an explicit value works in any position and is unambiguous either way.
        :::
    </dd>

    <dt>`--edition`</dt>
    <dd>
        Pin defaults to a date-anchored [edition](/cli/configuration/edition) for this invocation.

        - Accepts `YYYY`, `YYYY-MM`, or `YYYY-MM-DD`; partial dates round to the end of the period they name
        - Defaults that changed after the anchor keep their pre-change values; explicit configuration always wins
        - Takes precedence over the `ATMOS_EDITION` environment variable and the top-level `edition` setting in `atmos.yaml`

        ```bash
        # Preview the effect of a pin without changing configuration
        atmos --edition=2026-01 describe edition

        # Run a command with defaults as they stood at the end of September 2025
        atmos --edition=2025-09 helmfile apply demo --stack=dev
        ```

        See the [edition configuration reference](/cli/configuration/edition) for details.
    </dd>
    ...
    <dt>`--settings-list-merge-strategy`</dt>
    <dd>
        Override [`settings.list_merge_strategy`](/cli/configuration/settings#list-merge-strategy)
        for the current invocation, controlling how lists are merged when Atmos
        deep-merges stack manifests.

        - Options: `replace` (default), `append`, `merge`
        - Takes precedence over `settings.list_merge_strategy` in `atmos.yaml`
        - Per-invocation only — does not propagate to nested commands or workflows
        - Can also use `ATMOS_SETTINGS_LIST_MERGE_STRATEGY` environment variable

        **Strategies:**
        - `replace`: Lists from higher-priority manifests replace lists from lower-priority ones.
        - `append`: Lists are concatenated, with higher-priority items appended after lower-priority ones.
        - `merge`: Lists are merged element-by-element by index.

        **Examples:**
        ```bash
        # Append imported lists instead of replacing them for one run
        atmos --settings-list-merge-strategy=append describe component vpc -s prod

        # Flag also works after the subcommand
        atmos describe config --settings-list-merge-strategy=merge

        # Use environment variable
        ATMOS_SETTINGS_LIST_MERGE_STRATEGY=append atmos terraform plan vpc -s prod
        ```
    </dd>
</dl>
...
<dl>
    ...
    <dt>`ATMOS_EDITION`</dt>
    <dd>
        Pin defaults to a date-anchored [edition](/cli/configuration/edition).
        - Equivalent to the `--edition` flag
        - CLI flag takes precedence over this environment variable
        - Example: `ATMOS_EDITION=2026-01 atmos describe edition`
    </dd>
    ...
    <dd>
        Override [`settings.list_merge_strategy`](/cli/configuration/settings#list-merge-strategy) for the current invocation.
        - Equivalent to the `--settings-list-merge-strategy` flag
        - Options: `replace` (default), `append`, `merge`
        - CLI flag takes precedence over this environment variable
        - Example: `ATMOS_SETTINGS_LIST_MERGE_STRATEGY=append atmos terraform plan vpc -s prod`
    </dd>
