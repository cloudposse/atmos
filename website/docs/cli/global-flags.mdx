...
<dl>
    ...
    <dd>
        ...
        **Examples:**
        ...
        atmos --ai --logs-level=Debug terraform apply vpc -s prod
        ```
    </dd>

    <dt>`--skill`</dt>
    <dd>
        Specify an AI skill for domain-specific analysis context (requires `--ai`).
        - Default: empty (no skill)
        - Loads the skill's system prompt and prepends it to the AI analysis prompt
        - The AI gains deep knowledge of the specified Atmos subsystem for more accurate analysis
        - Skills are loaded from marketplace installations (`~/.atmos/skills/`) and custom skills in `atmos.yaml`
        - Can also use `ATMOS_SKILL` environment variable
        ...
        **Examples:**
        ...
        # Use stacks expertise for stack description
        atmos --ai --skill atmos-stacks describe stacks

        # Use validation expertise for stack validation
...
<dl>
    ...
    <dt>`ATMOS_SKILL`</dt>
    <dd>
        Specify an AI skill for domain-specific analysis context.
        - Equivalent to the `--skill` flag
        - Requires `ATMOS_AI=true` or `--ai` flag
        - CLI flag takes precedence over this environment variable
        - Example: `ATMOS_AI=true ATMOS_SKILL=atmos-terraform atmos terraform plan vpc -s prod`
    </dd>
</dl>
