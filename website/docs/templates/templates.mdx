...
<DemoVideo id="config-template-functions" title="Template Functions" showCaption={false} />
...
<dl>
  ...
  <dd>A boolean flag to enable/disable the processing of `Go` templates in Atmos stack manifests. Defaults to `true`. If set to `false`, Atmos will not process `Go` templates in stack manifests.</dd>
</dl>
...
<dl>
  ...
  <dd>A boolean flag to enable/disable the [Sprig Functions](https://masterminds.github.io/sprig/) in Atmos stack manifests. Defaults to `true`.</dd>

  <dt>`templates.settings.gomplate.enabled`</dt>
  <dd>A boolean flag to enable/disable the [Gomplate Functions](https://docs.gomplate.ca/functions/) and [Gomplate Datasources](https://docs.gomplate.ca/datasources) in Atmos stack manifests. Defaults to `true`.</dd>
...
### Practical AWS example: loading environment configuration with Gomplate datasources

A common use case is to keep repeated environment-specific values outside of stack manifests and load them with a Gomplate datasource.

For example, an Atmos project may need to reuse values such as the AWS account ID, region, Terraform backend bucket, DynamoDB lock table, VPC ID, subnet IDs, security group IDs, ECS cluster ARN, ALB target group name, and common tags across many components.

One way to do this is to store non-sensitive environment configuration in a YAML file and load it as a datasource.
...
templates:
  settings:
    ...
    # Use more than one evaluation when datasource URLs themselves contain
    # template expressions such as {{ .vars.environment }}.
    evaluations: 2
...
<File title="config/dev/service.yaml">
```yaml
aws:
...
terraform_state:
  ...
  region: us-east-2
  dynamodb_table: example-terraform-locks-dev
...
ecs:
  cluster_arn: arn:aws:ecs:us-east-2:111111111111:cluster/example-cluster

alb:
  target_group_name: example-service-tg
...
<File title="stacks/acme/dev/default/service.yaml">
```yaml
vars:
  tenant: acme
  environment: dev
  ...
  templates:
    settings:
      gomplate:
        datasources:
          environment:
            url: "./config/{{ .vars.environment }}/service.yaml"

backend:
  s3:
    bucket: '{{ (datasource "environment").terraform_state.bucket }}'
    region: '{{ (datasource "environment").terraform_state.region }}'
    dynamodb_table: '{{ (datasource "environment").terraform_state.dynamodb_table }}'
    encrypt: true
...
components:
  terraform:
    aws/example-service:
      vars:
        region: '{{ (datasource "environment").aws.region }}'
        account_id: '{{ (datasource "environment").aws.account_id }}'
        cluster_arn: '{{ (datasource "environment").ecs.cluster_arn }}'
        alb_target_group_name: '{{ (datasource "environment").alb.target_group_name }}'
        subnet_ids:
{{ (datasource "environment").network.subnet_ids | toYAML | indent 10 }}
        security_group_ids:
{{ (datasource "environment").network.security_group_ids | toYAML | indent 10 }}
        tags:
{{ (datasource "environment").tags | toYAML | indent 10 }}
```
</File>

You can verify the rendered component configuration with:

<Terminal title="Verify rendered component configuration">
```shell
atmos describe component aws/example-service -s acme-dev-default
```
</Terminal>

:::tip
Use datasources for non-sensitive platform configuration, shared metadata, and values that are safe to render into generated stack configuration.
:::

:::warning
Avoid rendering sensitive secret values directly into stack manifests or Terraform state unless that exposure is intentional and understood. For AWS workloads, prefer passing secrets to runtime systems using native mechanisms such as AWS Secrets Manager references in ECS task definitions.
:::
