...
3. **Use Terraform Remote State Directly**: Instead of relying on template functions to retrieve remote state, [use Terraform's native ability to retrieve the remote state](/connections/#using-terraform-remote-state) of other components.
...
- In [Atmos Custom Commands](/custom-commands/custom-commands)
...
- `Go` templates in [Atmos Custom Commands](/custom-commands/custom-commands) are processed when the custom commands are
