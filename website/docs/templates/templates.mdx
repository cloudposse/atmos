...
:::note
The `component_info` section is **not** processed as a `Go` template. It is generated by Atmos from the
Terraform/OpenTofu source code (variable and output names, types and descriptions parsed from the `.tf` files),
not from your stack manifests, and that source code can legitimately contain `{{` and `}}`. For example, a GCP
resource name format in an output description:
...
Atmos templating applies to the abstraction *above* Terraform modules, so these delimiters are left untouched
instead of failing the command with `function "project" not defined`. You can still **read** `component_info`
from templates in other sections, e.g. `{{ .component_info.component_path }}`.
:::
