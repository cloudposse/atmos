...
3. **`base_path`-relative paths** — all other relative paths are first checked relative to the manifest directory,
   then relative to the [`base_path`](/cli/configuration#base-path) setting in `atmos.yaml` (first match wins):
   `!include.raw stacks/catalog/template.tf`
