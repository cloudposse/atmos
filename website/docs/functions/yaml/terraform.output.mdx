...
:::tip Clean Syntax
Wrap the entire expression in YAML single quotes. No inner escaping is needed - just write normal JSON inside:

```yaml
value: !terraform.output 'component .output // {"key": "fallback"}'
```
:::
...
### Using YAML Block Scalars for Complex Expressions

For very complex expressions, you can use YAML folded block scalars (`>-`) to improve readability.
The folded block scalar folds newlines into spaces:

```yaml
config_map: !terraform.output >-
  config
  .config_map // {"api_endpoint": "localhost:3000", "user": "test", "timeout": 30}
```

This is equivalent to the single-line version but easier to read for long expressions.
...
:::tip
Wrap the entire expression in YAML single quotes. This allows you to use normal double quotes inside the expression
without any escaping.
:::

For complex string transformations, you can use YAML block scalars for better readability:

```yaml
postgres_url: !terraform.output >-
  aurora-postgres
  .master_hostname | "jdbc:postgresql://" + . + ":5432/events"
```
