...
(e.g. SSM Parameter Store, AWS Secrets Manager, Artifactory, Redis, etc.) for an Atmos component in a stack into Atmos stack manifests.
</Intro>
...
- `!store` reads values in cleartext and cannot read stores marked `secret: true`. For secrets, mark the store [`secret: true`](/cli/configuration/secrets) and read it with [`!secret`](/functions/yaml/secret), which masks values and gates access through the declarative secret registry.
