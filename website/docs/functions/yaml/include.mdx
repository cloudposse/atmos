...
When a YQ expression needs double quotes inside bracket notation (for example, `["github-dependabot"]`), wrap the entire `!include` call in single quotes so the parser treats it as one argument. The detailed guidance lives in the [!terraform.output documentation](/functions/yaml/terraform.output#handling-yq-expressions-with-bracket-notation-and-quotes), which applies here as well.
...
For paths that already require double quotes, combine both patterns—`!include '"path with spaces.yaml" ".data[""key""]"'`—and see the linked documentation for more escaping techniques.
