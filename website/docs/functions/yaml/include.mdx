...
3. **Paths relative to `base_path`** — All other relative paths (those that do **not** start with `./` or `../`)
   are resolved with **first-match-wins**: Atmos first checks relative to the manifest directory, and if not found,
   falls back to the [`base_path`](/cli/configuration#base-path) in `atmos.yaml`. Use bare paths (no prefix) for
   `base_path`-relative resolution; use `./` or `../` to make manifest-relative resolution unambiguous.
   ```yaml
