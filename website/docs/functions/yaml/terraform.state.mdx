...
For other backends, use [__`!store`__](/functions/yaml/store) or [__`!terraform.output`__](/functions/yaml/terraform.output) to read remote state
...
```yaml
value: !terraform.state 'component .output // {"key": "fallback"}'
```
:::
