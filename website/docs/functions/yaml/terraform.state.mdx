...
As support for new backend types is added (e.g. `gcs`), this document will be updated accordingly.

Meanwhile, if you are using backends other than `local`, `s3`, and `azurerm`, consider the [__`!store`__](/functions/yaml/store)
