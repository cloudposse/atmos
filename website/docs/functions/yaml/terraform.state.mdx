...
<Intro>
The `!terraform.state` YAML function is the **fastest** way to read Terraform/OpenTofu outputs ([remote state](/connections/remote-state))
...
Currently supports `local` and `s3` backends. For other backends, use [__`!store`__](/functions/yaml/store) or [__`!terraform.output`__](/functions/yaml/terraform.output).
:::
...
As support for additional backend types is added (e.g. `azurerm`, `gcs`), this document will be updated accordingly.
