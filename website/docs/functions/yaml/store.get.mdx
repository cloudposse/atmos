...
- **Security**: Like `!store`, `!store.get` reads values in cleartext, so using it with secrets will expose them to `stdout` in describe commands. For secrets, mark the store [`secret: true`](/cli/configuration/secrets) and read it with [`!secret`](/functions/yaml/secret), which masks values and gates access
