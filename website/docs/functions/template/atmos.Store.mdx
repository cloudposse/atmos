...
- `atmos.Store` reads values in cleartext, so using it with secrets can expose sensitive data to standard output (stdout) in any commands that describe stacks or components. For secrets, mark the store [`secret: true`](/cli/configuration/secrets) and read it with [`!secret`](/functions/yaml/secret), which masks values and gates access through the declarative secret registry.
