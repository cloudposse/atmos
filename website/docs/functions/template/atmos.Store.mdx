...
<Intro>
The `atmos.Store` template function allows reading the values from a remote [store](/cli/configuration/stores)
(e.g. SSM Parameter Store, AWS Secrets Manager, Artifactory, Redis, etc.) into Atmos stack manifests.
</Intro>
...
- `atmos.Store` reads values in cleartext and cannot read stores marked `secret: true`. For secrets, mark the store [`secret: true`](/cli/configuration/secrets) and read it with [`!secret`](/functions/yaml/secret), which masks values and gates access through the declarative secret registry.
