...
<Intro>
Atmos provides three types of functions for dynamic configuration: **YAML Functions** (recommended), **HCL Functions** (for HCL stack files), and **Template Functions**. Understanding the differences is crucial for writing reliable, maintainable configurations.
</Intro>
...
HCL functions use **native HCL syntax** with the `atmos::` namespace for Atmos-specific functions, plus all standard Terraform/OpenTofu functions. They are available when using `.hcl` files for stack configurations.

**Advantages:**
- **Native HCL syntax** - Uses familiar function call syntax from Terraform
- **Terraform compatible** - Same functions you know from Terraform/OpenTofu
- **Type-safe** - HCL has strong typing
- **70+ stdlib functions** - String, collection, numeric, encoding, date, regex
