...
Atmos supports five primary version management patterns. The first four are **trunk-based** approaches, while the fifth adapts **Git Flow** for infrastructure.

### Default Recommendation: Universal Pinning with Automation

**The simplest and most recommended approach** is to pin all environments to the same component version and decouple release from deployment using automation rather than version control strategies. This approach:

- **Promotes convergence**: All environments run the same version, reducing drift
...
<File title="Example: All environments on same version">
```yaml
...
components:
  terraform:
    vpc:
      metadata:
        component: vpc  # All environments use the same component
      vars:
        # Environment-specific configuration
```
</File>

Use feature flags, environment-specific configuration, and deployment automation to control rollout rather than versioning strategies. Only adopt the patterns below when you have specific requirements that universal pinning cannot address.

### Alternative Pattern: Release Artifacts (Advanced)
...
This pattern adds complexity but provides the strongest guarantees for regulated environments.

### Trunk-Based Patterns

#### 1. [Strict Version Pinning](/design-patterns/strict-version-pinning)
**Trunk-based**: Pin component versions per environment using tags or commit SHAs from the main branch. Optimizes for reproducibility and rollback capability at the cost of high operational overhead and weak feedback loops. Requires lockstep promotions.

#### 2. [Release Tracks/Channels](/design-patterns/release-tracks-channels)
**Trunk-based**: Environments subscribe to moving tracks (e.g., `alpha`, `beta`, `prod`) defined in configuration on the main branch. You promote tracks, not individual environment pins. Provides strong convergence and feedback with medium overhead.

#### 3. [Folder-Based Versioning](/design-patterns/folder-based-versioning)
**Trunk-based**: Create new component folders for deliberate, long-lived divergence (e.g., `vpc-v2`) in the main branch. Makes versioning explicit in the repository structure and allows gradual convergence by switching folders.

#### 4. [Vendoring Components](/design-patterns/vendoring-components)
**Trunk-based**: Copy upstream code locally into the main branch with a manifest tracking origin and lineage. Update locally in bulk; diverge intentionally when needed. Provides strong local control with predictable update windows.

#### 5. [Component Pinning with Automation](/design-patterns/component-pinning-automation) (Recommended Default)
**Trunk-based**: All environments pin to the same component version on the main branch, with automated testing and deployment pipelines. Decouples release from deploy using automation rather than VCS strategies. Promotes strong convergence, embraces trunk-based development, and allows easy preview of change impact across all dependent environments. This is the recommended default for most teams.

#### 6. Immutable Release Artifacts
**Artifact-based**: Generate immutable release artifacts (e.g., tarballs, container images) for each environment separate from Git. These artifacts represent an exact, reproducible record of what was deployed, including all dependencies and configurations frozen at a point in time. Provides the strongest guarantee of reproducibility but requires sophisticated artifact management infrastructure.

### Git Flow Pattern

#### 7. [Git Flow: Branches as Channels](/design-patterns/git-flow-branches-as-channels)
**Branch-based**: Long-lived branches map to release channels. Environments track branches; promotions happen via merges between branches. This is the only pattern that diverges from trunk-based development, using Git Flow's branching model instead.
...
| Immutable Artifacts      | Artifact-based   | High        | Strong              | Strong        | High             | Lower Risk           |
