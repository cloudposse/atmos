...
components:
  terraform:
    vpc:
      metadata:
        component: vpc  # Points to vendored component at v1.12.3
      vars:
        name: "prod-vpc"
...
### 3. Implement Lockstep CI/CD

Build pipelines that enforce sequential promotion:

```yaml
# Example GitHub Actions workflow
name: Promote Component Version
on:
  workflow_dispatch:
    inputs:
      component:
...
jobs:
  validate-promotion:
    ...
    steps:
      - name: Check Version Sequence
        run: |
          # Ensure no versions are skipped
          # This script would verify that all intermediate
          # versions have been applied in sequence
          ./scripts/validate-promotion-sequence.sh \
            --component ${{ inputs.component }} \
            --from ${{ inputs.from_env }} \
            --to ${{ inputs.to_env }}
```

### 4. Regular Cross-Environment Planning

Periodically run plans across all environments to detect issues early:

```bash
# Script to plan across all environments
for env in dev staging prod; do
  echo "Planning $env..."
  atmos terraform plan vpc --stack $env-us-east-1
done
```
...
Here's a complete example showing strict version pinning across environments:
...
```yaml
# Base configuration without version pinning
components:
  terraform:
    vpc/defaults:
      metadata:
...
To use different versions per environment, update the vendor configuration:
...
spec:
  sources:
    # Different environments vendor different versions
    - component: vpc
      source: "github.com/acme/terraform-components.git//modules/vpc?ref=v1.13.0"
      targets:
        - "components/terraform/vpc-dev"  # Dev-specific vendor
    - component: vpc
      source: "github.com/acme/terraform-components.git//modules/vpc?ref=v1.12.4"
      targets:
        - "components/terraform/vpc-staging"  # Staging-specific vendor
    - component: vpc
      source: "github.com/acme/terraform-components.git//modules/vpc?ref=v1.12.3"
      targets:
        - "components/terraform/vpc-prod"  # Prod-specific vendor
```
</File>
...
components:
  terraform:
    vpc:
      metadata:
        component: vpc-dev  # Points to vpc vendored at v1.13.0
        inherits:
          - vpc/defaults
      vars:
        name: "dev-use1-vpc"
...
components:
  terraform:
    vpc:
      metadata:
        component: vpc-staging  # Points to vpc vendored at v1.12.4
        inherits:
          - vpc/defaults
      vars:
...
components:
  terraform:
    vpc:
      metadata:
        component: vpc-prod  # Points to vpc vendored at v1.12.3
        inherits:
          - vpc/defaults
      vars:
        name: "prod-use1-vpc"
...
With strict version pinning, rollback is straightforward:

### Update Vendor Configuration

```yaml
# vendor.yaml - Revert to previous version
spec:
  sources:
    - component: vpc
      # Was: v1.12.4 (problematic)
      # Reverting to: v1.12.3 (last known good)
      source: "github.com/acme/terraform-components.git//modules/vpc?ref=v1.12.3"
      targets:
...
:::tip
Strict pinning makes rollback predictable since you're reverting to an exact, previously-tested version.
:::
