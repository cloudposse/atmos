...
| `{{ .settings.key }}` | Settings from the same file (merged with global settings) |
| `{{ .vars.key }}` | Vars from the same file (merged with global vars) |
| `{{ .env.key }}` | Env values from the same file (merged with global env) |
