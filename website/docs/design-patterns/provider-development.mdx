...
```
my-project/
├── .terraformrc                    # Terraform CLI config with dev_overrides
├── .gitignore                      # Exclude .terraformrc (developer-specific)
...
└── infrastructure/
    ...
    ├── components/
    │   └── terraform/
    │       └── myapp/              # Component using your provider
    └── stacks/
        └── catalog/
            └── myapp/
                └── defaults.yaml
...
Create `.terraformrc` in your project root (or home directory):
...
components:
  terraform:
    myapp:
      # Point Terraform to your CLI config
      env:
...
# Move to the location specified in .terraformrc
mkdir -p ../../bin
mv terraform-provider-mycloud ../../bin/

echo "Provider built and installed to ../../bin/"
```
</File>
...
# 3. Test immediately with Atmos (no publishing needed!)
cd ../../infrastructure
...
The `.terraformrc` file contains paths specific to each developer's machine. Don't commit it:
...
<File title=".terraformrc.example">
```hcl
...
1. Copy `.terraformrc.example` to `.terraformrc`
...
provider_installation {
  dev_overrides {
    "registry.terraform.io/acme/cloud" = "/Users/developer/work/providers/bin"
  }
  direct {}
}
```
...
components:
  terraform:
    web-app:
      ...
      # Point to dev overrides config (only in dev mode)
      env:
        TF_CLI_CONFIG_FILE: >-
          {{ if .settings.provider_dev_mode }}${PWD}/.terraformrc{{ end }}
...
# Or test manually with different configurations
cd ../../infrastructure
atmos terraform plan web-app -s dev
