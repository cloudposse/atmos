...
stacks:
  ...
  excluded_paths:
    ...
    - "**/_defaults.yaml"
  # If you are using multiple organizations (namespaces), use the following `name_template`:
  name_template: "{namespace}-{tenant}-{environment}-{stage}"
  # If you are using a single organization (namespace), use the following `name_template`:
