...
This pattern works best with Atmos auto-generated [backend configuration](/core-concepts/components/terraform/backends). The `workspace_key_prefix` must remain stable across version changes to prevent state conflicts:
...
- **Multiple copies** of similar code across version folders
- **Larger clone operations** as repository grows with versions
- **Storage overhead** for maintaining parallel implementations

### Maintenance Complexity
- **Multiple locations** to apply security patches and bug fixes
- **Divergence risk** as versions evolve independently
- **Documentation burden** for each version's specifics

### Limited Version Discovery
- **Manual tracking** of which environments use which versions
- **No automatic convergence** - requires active management
- **Potential for version sprawl** without governance

### Migration Challenges
- **Testing overhead** for each version transition
- **Rollback complexity** after extended periods on new versions
- **State management** requires careful workspace key planning

### Development Workflow Impact
- **Parallel development** can lead to duplicated effort
- **Cherry-picking fixes** between versions adds complexity
- **Code review challenges** when changes span multiple versions
