...
<Intro>
The **Folder-Based Versioning** Design Pattern uses the repository's directory structure to manage component versions. By creating separate folders for different versions (e.g., `vpc`, `vpc-v2`), teams can safely iterate on new versions while maintaining stability for existing deployments.
</Intro>

Folder-Based Versioning makes version boundaries explicit and visible in the repository structure. This approach allows for long-lived divergence between versions, gradual migration paths, and clear separation of concerns. It's particularly useful for major version changes that require extended development and testing periods.
...
components/
  terraform/
    vpc/                    # Original/v1 implementation
      main.tf
      variables.tf
      outputs.tf
      README.md
    vpc-v2/                 # New v2 implementation
      main.tf
      variables.tf
      outputs.tf
      README.md
      MIGRATION.md          # Migration guide from v1
    vpc-v3-preview/         # Future v3 in development
      main.tf
      variables.tf
      outputs.tf
      README.md
    rds/                    # Current stable version
      main.tf
    rds-aurora/             # Alternative implementation
      main.tf
    rds-v2-deprecated/      # Deprecated, pending removal
      main.tf
      DEPRECATED.md
```
</File>
...
for component in $COMPONENTS; do
  for version in $VERSIONS; do
    if [ -d "components/terraform/${component}-${version}" ]; then
      echo "Testing ${component}-${version}..."
      cd "components/terraform/${component}-${version}"
      terraform init
      terraform validate
      tflint
      cd -
...
<File title="scripts/migrate-component.sh">
```bash
#!/bin/bash
...
echo "Migrating $COMPONENT from $FROM_VERSION to $TO_VERSION in $STACK"

# Backup current state
atmos terraform state pull $COMPONENT --stack $STACK > "backup-$STACK-$COMPONENT-$(date +%s).tfstate"

# Update configuration
yq eval -i ".components.terraform.$COMPONENT.source.uri = \"./components/terraform/$COMPONENT-$TO_VERSION\"" "stacks/$STACK.yaml"
