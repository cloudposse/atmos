...
Regardless of organization, use a centralized configuration to manage track versions:
...
Release tracks make rollback simple by reverting the track version:

### Option 1: Revert Track in Vendor Configuration
...
### Option 2: Revert Track in Configuration Variables
...
vars:
  tracks:
    component_versions:
      vpc:
        prod: "v1.12.8"  # Was v1.13.2, rolling back
```

### Apply the Rollback

```bash
# Re-vendor if using vendor.yaml approach
...
:::tip
Track-based rollback affects all environments subscribed to that track, providing consistent rollback across similar environments.
:::
