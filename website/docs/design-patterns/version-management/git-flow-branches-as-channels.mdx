...
- **Use Feature Flags for Long-Running Changes:** Combine branches with feature flags to enable gradual rollout and environment-specific behavior, allowing features to be toggled based on deployment channel without code changes.
...
Use GitHub Environments to manage deployments with proper access controls:

<File title=".github/environments configuration">
```yaml
