...
##### 4. [Source-Based Version Pinning](/design-patterns/version-management/source-based-versioning)

   Native per-environment version pinning using the `source` field. Components declare their source directly in stack configuration, enabling just-in-time vendoring without managing separate vendor manifests. Ideal for strict version pinning with minimal operational overhead.
...
### Complementary Technique: Atmos-Managed Version Tracks

Atmos-managed version tracks are a complementary technique for external artifacts and runtime dependencies. They do not replace component source versioning patterns such as folder-based versioning, release channels, or strict component pinning. Instead, they give Atmos a native catalog and lock file for versions that appear throughout configuration and CI:

- Toolchain dependencies such as OpenTofu, Terraform, Helmfile, `kubectl`, and `jq`
- GitHub Actions refs such as `actions/checkout@v6`
- Docker, OCI, ECR, GHCR, Helm, GitHub, and GitLab package versions
- Image tags, Lambda layer tags, provider-side helper versions, and other external artifact refs

The pattern is:

1. Define policy and desired ranges in `atmos.yaml` under [`version.tracks`](/cli/configuration/version/managed-versions).
2. Commit deterministic resolved versions in `versions.lock.yaml`.
3. Use `!version name` for YAML values and `{{ .version.name }}` for embedded strings.
4. Use [`atmos version tracks`](/cli/commands/version/tracks) in CI to check status, update locks, verify locks, and render files that must contain literal refs.

This is congruent with the Atmos version-management philosophy: humans declare desired convergence and policy; Atmos owns deterministic resolution, locking, and CI-native rendering. It avoids scattering pins throughout stacks while still making the actual runtime versions explicit and reviewable in Git.
...
| **Source-Based Versioning** | `source: ...?ref=1.2.3` | Low | Low | Per-environment version pinning without vendor files |
...
**Atmos-Managed Version Tracks**: Works with any deployment strategy and folder approach. Manages external artifact versions through `atmos.yaml`, `versions.lock.yaml`, `!version`, `.version`, and CI-native `atmos version tracks` commands.
...
- [Component Inheritance](/design-patterns/inheritance-patterns/component-inheritance) - Base components that serve as blueprints
- [Component Catalog](/design-patterns/component-catalog) - Centralized component library
- [Multiple Component Instances](/design-patterns/inheritance-patterns/multiple-component-instances) - Deploying multiple instances of the same component
