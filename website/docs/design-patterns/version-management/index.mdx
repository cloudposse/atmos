...
4. Use [`atmos version track`](/cli/commands/version/track) in CI to check status, update locks, verify locks, and render files that must contain literal refs.
...
**Atmos-Managed Version Tracks**: Works with any deployment strategy and folder approach. Manages external artifact versions through `atmos.yaml`, `versions.lock.yaml`, `!version`, `.version`, and CI-native `atmos version track` commands.
