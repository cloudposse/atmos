...
A core principle of this pattern is separating deployment (putting code in production) from release (making features available to users). The industry recognizes the value of deploying frequently while controlling when features become available through gates and configuration.

**Atmos's Key Value**: **Preview changes across all environments before committing to any**. When you merge to main, you immediately see the plan output for every environment—dev, staging, and production—without updating any version pins. This visibility enables informed decisions about progressive rollout.

This decoupling is achieved through two separation mechanisms in Atmos:
...
The **primary mechanism** controlling when deployments are released to each environment is **CI/CD approval gates** (GitHub Actions, GitLab CI, etc.). These gates control the progression of deployments across environments through automated testing, manual approvals, and time-based holds.
...
Experimentation in Atmos is achieved through folder-based component isolation, not feature percentage rollouts. When you need to test a new approach or validate an experimental change, create a dedicated component folder for your experiment and pin specific environments to it.
