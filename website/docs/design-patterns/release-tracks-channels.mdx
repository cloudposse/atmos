...
<Intro>
The **Release Tracks/Channels** Design Pattern allows environments to subscribe to moving version tracks (e.g., `alpha`, `beta`, `prod`) instead of pinning individual versions. This approach dramatically reduces operational overhead while maintaining strong convergence and feedback loops across environments.
</Intro>

Release Tracks provide a middle ground between strict version pinning and bleeding-edge updates. By grouping environments into tracks and promoting versions through tracks rather than individual environments, teams can maintain control while reducing the burden of version management. This pattern scales well and provides early warning of issues through strong cross-environment feedback.
...
components:
  terraform:
    vpc:
      ...
      # Subscribe to production track
      source:
        uri: "./components/terraform/vpc/prod"
      vars:
        name: "prod-use1-vpc"
  ...
  terraform:
    vpc:
      ...
      settings:
        workspace_key_prefix: "prod/vpc"
      source:
        # Reference track via variable
        uri: "git::https://github.com/acme/terraform-components.git//modules/vpc?ref={{ .vars.component_tracks.vpc.prod }}"
      vars:
        name: "prod-use1-vpc"
        cidr_block: "10.0.0.0/16"
```
...
components:
  terraform:
    vpc:
      metadata:
        component: vpc
        # Specify which track this environment follows
        track: prod
      settings:
        workspace_key_prefix: "prod/vpc"
      source:
  ...
  terraform:
    vpc:
      ...
      settings:
        workspace_key_prefix: "prod/vpc"
      source:
        # Reference the prod track branch
        uri: "git::https://github.com/acme/terraform-components.git//modules/vpc?ref=track/prod"
```
</File>
...
components:
  terraform:
    vpc:
      ...
      source:
        # Use alpha track for dev
        uri: "git::https://github.com/acme/terraform-components.git//modules/vpc?ref={{ .vars.tracks.component_versions.vpc.alpha }}"
      vars:
        name: "dev-use1-vpc"
        cidr_block: "10.0.0.0/16"
        region: "{{ .vars.region }}"
...
components:
  terraform:
    vpc:
      ...
      source:
        # Use beta track for staging
        uri: "git::https://github.com/acme/terraform-components.git//modules/vpc?ref={{ .vars.tracks.component_versions.vpc.beta }}"
      vars:
  ...
  terraform:
    vpc:
      ...
      settings:
        workspace_key_prefix: "prod/vpc"
      source:
        # Use prod track for production
        uri: "git::https://github.com/acme/terraform-components.git//modules/vpc?ref={{ .vars.tracks.component_versions.vpc.prod }}"
      vars:
        name: "prod-use1-vpc"
...
<File title="scripts/promote-track.sh">
```bash
#!/bin/bash
# Script to promote versions between tracks
...
if [ -z "$COMPONENT" ] || [ -z "$FROM_TRACK" ] || [ -z "$TO_TRACK" ]; then
  echo "Usage: $0 <component> <from-track> <to-track>"
  echo "Example: $0 vpc alpha beta"
  exit 1
fi

# Get current version from source track
CURRENT_VERSION=$(yq eval ".vars.tracks.component_versions.$COMPONENT.$FROM_TRACK" stacks/catalog/tracks.yaml)

echo "Promoting $COMPONENT from $FROM_TRACK ($CURRENT_VERSION) to $TO_TRACK"

# Update target track
yq eval -i ".vars.tracks.component_versions.$COMPONENT.$TO_TRACK = \"$CURRENT_VERSION\"" stacks/catalog/tracks.yaml

# Create PR for the promotion
git add stacks/catalog/tracks.yaml
git commit -m "Promote $COMPONENT from $FROM_TRACK to $TO_TRACK ($CURRENT_VERSION)"
git push origin "promote-$COMPONENT-$TO_TRACK-$(date +%Y%m%d)"

echo "Promotion complete. Please review and merge the PR."
```
</File>
