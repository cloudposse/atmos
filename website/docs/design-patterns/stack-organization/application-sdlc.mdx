...
The key difference from other patterns: `name_pattern: "{stage}"` produces stack names that are simply `dev`, `staging`, `prod`, and `preview`. No region, no tenant, no namespace — just the SDLC environment.
