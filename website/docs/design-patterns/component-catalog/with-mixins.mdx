---
title: Configuration Catalog with Mixins
sidebar_position: 6
sidebar_label: With Mixins
description: Configuration Catalog with Mixins Atmos Design Pattern
---
...
<Intro>
The **Configuration Catalog with Mixins** Design Pattern is a variation of the [Configuration Catalog](/design-patterns/component-catalog) pattern, with the difference being that we first create parts of a component's configuration related to different environments (e.g. in `mixins` folder), then assemble environment-specific manifests from the parts, and then import the environment-specific manifests themselves into the top-level stacks.
</Intro>
...
Use the **Configuration Catalog with Mixins** pattern when:
...
The **Configuration Catalog with Mixins** pattern provides the following benefits:
