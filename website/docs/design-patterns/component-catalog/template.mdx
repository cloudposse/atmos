...
This is achieved by using [`Go` Templates in Imports](/stacks/imports#templated-imports) and
...
- In the component's catalog folder, create a [`Go` template](https://pkg.go.dev/text/template) manifest with all the configurations for the
  component (refer to [`Go` Templates in Imports](/stacks/imports#templated-imports) for more details)
