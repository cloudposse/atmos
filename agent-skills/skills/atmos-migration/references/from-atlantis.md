...
Native CI. For a project already on Atmos ([Shape A](#shape-a-the-project-uses-atmos-now)), the
steps change the CI system only. A project not yet on Atmos
([Shape B](#shape-b-the-project-uses-plain-terraform-and-atlantis)) also needs a separate move from
plain Terraform to Atmos first. The CI steps cover automatic plans, plan comments on pull requests,
approval before an apply, and locks on components.
...
|---|---|---|
| `projects[]` and `autoplan.when_modified` | `atmos describe affected --format=matrix` | Atmos does not need a fixed list of projects. For patterns that stay inside a component's own directory, Atmos finds affected components automatically. For `when_modified` patterns that point outside the component directory (a shared file or folder), map them to `dependencies.files` and `dependencies.folders` so a change there also marks the component as affected. Use `dependencies.components` for component-to-component relationships instead. See [atmos-ci](../../atmos-ci/SKILL.md#component-dependencies). |
| `workflow_templates.<name>.plan/apply.steps[].run` (custom init, plan, and apply commands) | [Hooks](../../atmos-hooks/SKILL.md) | A hook runs custom steps before or after almost any Atmos command. Examples: `before.terraform.plan`, `after.terraform.apply`. Atmos also has hook events for other component types, such as helmfile and packer. A hook gives the same result as an Atlantis workflow template. Both add custom steps at a fixed point in the command. `atmos terraform plan` runs init then plan. `atmos terraform apply` runs init then apply. `atmos terraform deploy` runs init, plan, then apply. Use a hook for most custom steps. Use a [custom command](../../atmos-custom-commands/SKILL.md) only when the step is not linked to a command lifecycle. |
...
| `apply_requirements: [approved]` | GitHub Environment protection rules (required reviewers, wait timers) | Required reviewers only gate a job that sets `environment: <name>` to the protected environment. Add that `environment:` field to the deploy job, not just the environment's protection rules. Do not build custom approval logic in Atmos config. See [atmos-ci Workflow Guidance](../../atmos-ci/SKILL.md#workflow-guidance). |
...
| The `$PLANFILE` passed from the plan step to the apply step | `components.terraform.planfiles` storage (S3 or GitHub Artifacts), with `--verify-plan` on `deploy` | The `deploy` command creates a new plan. The `deploy` command compares the new plan to the stored plan before it runs the apply step. The pull request plan workflow and the deploy workflow run on separate runners, so use S3 or GitHub Artifacts. A local path store only works when plan and apply run in the same job on the same runner. |
...
2. If the project uses the Atlantis `$PLANFILE` plan/apply consistency guarantee, configure
  `components.terraform.planfiles` storage in `atmos.yaml` with an S3 or GitHub Artifacts store,
  and enable `--verify-plan` on `deploy`. Use S3 or GitHub Artifacts, not a local path — the plan
  and deploy workflows run on separate runners. Skip this step for projects that do not rely on
  that guarantee.
