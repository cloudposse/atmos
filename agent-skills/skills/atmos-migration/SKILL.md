...
- **`workflows.base_path` needs to be set explicitly once the user has their own `atmos.yaml`.**
  Configure this when adopting workflow files. Workflows can organize general-purpose tasks as
  well as infrastructure operations; many target chains can stay custom commands.
  `atmos workflow <name>` fails with
  `'workflows.base_path' must be configured in 'atmos.yaml'` until you add it (for example,
  `workflows.base_path: "workflows"`). None of this skill's `atmos.yaml` snippets show it
