...
async function resolveTagSha(owner, repo, tag) {
  const res = await fetch(`https://api.github.com/repos/${owner}/${repo}/git/ref/tags/${tag}`, {
  ...
  if (ref.object.type === 'tag') {
    const tagRes = await fetch(`https://api.github.com/repos/${owner}/${repo}/git/tags/${ref.object.sha}`, {
      headers: { Authorization: `Bearer ${token}`, Accept: 'application/vnd.github+json' },
    });
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    while ((match = pattern.exec(lines[i])) !== null) {
      ...
      try {
        ...
        results.push({ label, line: i + 1, ok, pinnedSha, resolved });
      } catch (err) {
        results.push({ label, line: i + 1, ok: false, pinnedSha, error: err.message });
      }
    }
  }
  return results;
...
// Test 2: Deliberately wrong SHA (should fail)
