...
func TestProcessCustomYamlTagsCreatesContext(t *testing.T) {
	...
	_, err := ProcessCustomYamlTags(atmosConfig, input, "test-stack", nil, nil)
	require.NoError(t, err)
...
func TestProcessCustomTagsWithContextSkipFunctions(t *testing.T) {
	...
	result1, err := processCustomTagsWithContext(atmosConfig, input1, "test-stack", skip, ctx, nil)
	require.NoError(t, err)
	assert.Equal(t, input1, result1)

	input2 := "!terraform.output vpc dev output"
	result2, err := processCustomTagsWithContext(atmosConfig, input2, "test-stack", skip, ctx, nil)
	require.NoError(t, err)
	assert.Equal(t, input2, result2)

	// Non-skipped functions should still be processed.
	input3 := "!env HOME"
	result3, err := processCustomTagsWithContext(atmosConfig, input3, "test-stack", skip, ctx, nil)
	require.NoError(t, err)
...
func TestProcessCustomYamlTagsContextIsolation(t *testing.T) {
	...
	go func() {
		...
		_, err := ProcessCustomYamlTags(atmosConfig, input, "stack1", nil, nil)
		if err != nil {
			errChan <- err
			return
		}
	...
	go func() {
		...
		_, err := ProcessCustomYamlTags(atmosConfig, input, "stack2", nil, nil)
		if err != nil {
			errChan <- err
			return
		}
