...
func ExecutePackerOutput(
	...
	*info, err = ProcessStacks(&atmosConfig, *info, true, true, true, nil, nil)
	...
	if !u.FileExists(manifestPath) {
		return nil, fmt.Errorf("%w: `%s` - it's generated by Packer when executing `atmos packer build` and the manifest filename is specified in the `manifest_file_name` variable of the Atmos component",
