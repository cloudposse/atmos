...
func ExecuteAtmosVendorInternal(
	...
	for indexSource, s := range sources {
		...
		if s.Version != "" {
			uri, err = u.ProcessTmpl(cliConfig, schema.Settings{}, fmt.Sprintf("source-%d-%s", indexSource, s.Version), s.Source, s, false)
			if err != nil {
				return err
			}
		} else {
		...
		for indexTarget, tgt := range s.Targets {
			...
			if s.Version != "" {
				target, err = u.ProcessTmpl(cliConfig, schema.Settings{}, fmt.Sprintf("target-%d-%d-%s", indexSource, indexTarget, s.Version), tgt, s, false)
				if err != nil {
					return err
				}
			} else {
