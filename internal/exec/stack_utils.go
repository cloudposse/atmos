...
func BuildTerraformWorkspace(cliConfig schema.CliConfiguration, configAndStacksInfo schema.ConfigAndStacksInfo) (string, error) {
	...
	if cliConfig.Stacks.NameTemplate != "" {
		tmpl, err = u.ProcessTmpl(cliConfig, schema.Settings{}, "terraform-workspace-stacks-name-template", cliConfig.Stacks.NameTemplate, configAndStacksInfo.ComponentSection, false)
		if err != nil {
			return "", err
		}
		contextPrefix = tmpl
	...
	if terraformWorkspaceTemplate, terraformWorkspaceTemplateExist := componentMetadata["terraform_workspace_template"].(string); terraformWorkspaceTemplateExist {
		tmpl, err = u.ProcessTmpl(cliConfig, schema.Settings{}, "terraform-workspace-template", terraformWorkspaceTemplate, configAndStacksInfo.ComponentSection, false)
		if err != nil {
			return "", err
		}
		workspace = tmpl
