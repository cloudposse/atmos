...
func GetComponentAuthConfig(
	...
	// Load stacks to extract component-specific auth config.
	stacksMap, _, err := FindStacksMap(atmosConfig, false)
	if err != nil {
		return mergedAuthConfig, err
	}

	// Find component section in stacks.
	componentSection := findComponentSection(stacksMap, stack, component, componentType)
	if componentSection == nil {
		// Component not found - return global config.
		return mergedAuthConfig, nil
	}

	// Extract and merge component auth config.
	componentAuth, err := extractComponentAuthConfig(componentSection)
	if err != nil {
		return nil, err
	}

	mergeAuthConfigs(mergedAuthConfig, componentAuth)
...
// findComponentSection searches for component section in stacksMap.
// Tries direct lookup first (Directory-based stacks), then searches all stacks (File-based stacks).
func findComponentSection(
	stacksMap map[string]any,
	stack string,
	component string,
	componentType string,
) map[string]any {
	// Try direct lookup first (for Directory-based stacks).
	if section := findComponentInStack(stacksMap[stack], component, componentType); section != nil {
		return section
	}

	// Search through all stacks (for File-based stacks).
	for _, stackData := range stacksMap {
		if section := findComponentInStack(stackData, component, componentType); section != nil {
			return section
		}
	}
...
// findComponentInStack searches for component in a single stack.
func findComponentInStack(stackData any, component string, componentType string) map[string]any {
	...
	section, ok := componentTypeSection[component].(map[string]any)
	if !ok {
		return nil
	}
...
// extractComponentAuthConfig extracts and parses auth config from component section.
func extractComponentAuthConfig(componentSection map[string]any) (*schema.AuthConfig, error) {
	componentAuthSection, ok := componentSection[cfg.AuthSectionName].(map[string]any)
	if !ok || componentAuthSection == nil {
		return nil, nil
	}

	var componentAuth schema.AuthConfig
	if err := mapstructure.Decode(componentAuthSection, &componentAuth); err != nil {
		return nil, fmt.Errorf("failed to parse component auth config: %w", err)
	}

	return &componentAuth, nil
}

// mergeAuthConfigs merges component auth config into merged config.
// Component-specific identities override global identities with the same name.
func mergeAuthConfigs(mergedConfig *schema.AuthConfig, componentAuth *schema.AuthConfig) {
	if componentAuth == nil || componentAuth.Identities == nil {
		return
	}

	if mergedConfig.Identities == nil {
		mergedConfig.Identities = make(map[string]schema.Identity)
	}

	for identityName, identity := range componentAuth.Identities {
		mergedConfig.Identities[identityName] = identity
	}
}
