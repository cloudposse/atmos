...
func addAffectedSpaceliftAdminStack(
	...
	if cliConfig.Stacks.NameTemplate != "" {
		adminStackContextPrefix, err = u.ProcessTmpl(cliConfig, schema.Settings{}, "spacelift-admin-stack-name-template", cliConfig.Stacks.NameTemplate, configAndStacksInfo.ComponentSection, false)
		if err != nil {
			return nil, err
		}
	} else {
				...
				if terraformSection, ok := componentsSection["terraform"].(map[string]any); ok {
					for componentName, compSection := range terraformSection {
						if componentSection, ok := compSection.(map[string]any); ok {
							...
							if cliConfig.Stacks.NameTemplate != "" {
								contextPrefix, err = u.ProcessTmpl(cliConfig, schema.Settings{}, "spacelift-stack-name-template", cliConfig.Stacks.NameTemplate, configAndStacksInfo.ComponentSection, false)
								if err != nil {
									return nil, err
								}
							} else {
