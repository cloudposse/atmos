...
func ExecuteAwsEksUpdateKubeconfigCommand(cmd *cobra.Command, args []string) error {
	...
	integration, err := flags.GetString("integration")
	if err != nil {
		return err
	}
...
func executeEKSUpdateKubeconfigViaIntegration(integrationName string) error {
	defer perf.Track(nil, "exec.executeEKSUpdateKubeconfigViaIntegration")()
...
func executeEKSUpdateKubeconfigDirect(clusterName, region, kubeconfigPath, alias, identityName string) error {
	defer perf.Track(nil, "exec.executeEKSUpdateKubeconfigDirect")()
