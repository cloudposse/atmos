...
- JSON files: must still parse (`python3 -c "import json; json.load(open('<file>'))"`).
- YAML files: must still parse.
- Go files: `go build .` and `go test ./...` on affected packages.
- Confirm no conflict markers remain anywhere: `git grep -l '^<<<<<<<' -- .` should be empty.
...
"..."` produces a correct two-parent merge commit), verify `git log --show-signature -1`, then
plain `git push` (never a flag that could force).
