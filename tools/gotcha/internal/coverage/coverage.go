...
func parseCoverageLine(line string) (*types.CoverageLine, error) {
	// Parse line: "file:startline.col,endline.col numstmt count".
	parts := strings.Fields(line)
	if len(parts) < 3 {
		return nil, types.ErrInvalidCoverageLineFormat
	}

	// Extract filename from "file:line.col,line.col".
	colonIndex := strings.Index(parts[0], ":")
	if colonIndex == -1 {
		return nil, types.ErrInvalidFileFormat
	}
...
func calculateStatementCoverage(total, covered int) string {
	...
	coverage := float64(covered) / float64(total) * constants.PercentageMultiplier
	return fmt.Sprintf("%.1f%%", coverage)
}

// getFunctionCoverage gets function-level coverage information.
func getFunctionCoverage(profileFile string, excludeMocks bool) ([]types.CoverageFunction, error) {
...
func parseFunctionCoverageLine(line string) (types.CoverageFunction, error) {
	// Parse line: "file:line:	function	coverage%".
	// Use regex to extract parts more reliably.
	re := regexp.MustCompile(`^(.+?):\d+:\s+(.+?)\s+(\d+(?:\.\d+)?)%$`)
	matches := re.FindStringSubmatch(line)

	if len(matches) < constants.RegexMatchGroups-1 { // We expect 4 groups (including full match).
		return types.CoverageFunction{}, types.ErrInvalidFunctionCoverageFormat
	}

	coverage, err := strconv.ParseFloat(matches[3], constants.FloatBitSize)
	if err != nil {
		return types.CoverageFunction{}, err
	}

	return types.CoverageFunction{
		File:     matches[1],
		Function: matches[2],
		Coverage: coverage,
	}, nil
