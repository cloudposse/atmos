...
- [ ] T001 Create `pkg/proexec/` package with `doc.go` (package comment) and empty
...
- [ ] T002 [P] Create `pkg/metrics/process/` package with `doc.go`, empty `metrics.go`,
  `metrics_unix.go` (`//go:build unix`), `metrics_windows.go` (`//go:build windows`)
- [ ] T003 [P] Add sentinel errors to `errors/errors.go`: `ErrFailedToUploadExecMetadata`,
...
- [ ] T004 [P] Define `ExecUploadRequest` and `ExecUploadResponse` (embedding
  `dtos.AtmosApiResponse`) in `pkg/pro/dtos/exec.go`, per data-model.md's
  `ExecutionRecord`/`ResourceUsageMetrics` field tables
- [ ] T005 Add `UploadExecMetadata(dto *dtos.ExecUploadRequest) error` to
...
- [ ] T006 [P] Implement `ProcessMetrics` struct, `Snapshot` type, `Baseline()`, and
...
- [ ] T007 Implement `gateOpen(atmosConfig *schema.AtmosConfiguration) bool` in
...
- [ ] T008 Implement `buildRecord(cmd *cobra.Command, exitCode int, metrics
...
- [ ] T009 Implement payload-size truncation in `pkg/proexec/truncate.go` — compares the
...
- [ ] T010 [P] Add `ExecSettings{ SyncTimeoutSeconds int }` nested under
...
- [ ] T011 In `cmd/root.go`'s `Execute()`, capture `process.Baseline()` immediately before
...
- [ ] T012 [P] [US1] Unit tests for `gateOpen` (CI×Pro-configured 2×2 matrix) in
  `pkg/proexec/gate_test.go`
- [ ] T013 [P] [US1] Unit tests for `buildRecord` (field population, secret masking
  applied to `Args`/`Data`, truncation invoked when oversized) in
  `pkg/proexec/envelope_test.go`
- [ ] T014 [P] [US1] Unit tests for `CaptureAsync` — dispatches on gate-open, no-ops on
...
- [ ] T015 [P] [US1] Unit tests for `UploadExecMetadata` (success, 401-refresh-retry, 5xx
...
- [ ] T016 [US1] Implement `CaptureAsync(cmd *cobra.Command, err error)` in
...
- [ ] T017 [US1] Wire `proexec.CaptureAsync(cmd, err)` into `cmd/root.go` immediately
  after the existing `telemetry.CaptureCmd(cmd, err)` call (~line 1905) (depends on T016)
- [ ] T018 [US1] Ensure an async upload failure logs at debug level only
...
- [ ] T019 [P] [US2] Unit tests for `CaptureSync` timeout clamping — a configured value
  below 10s is clamped up to 10s; a configured value above 10s is honored — in
  `pkg/proexec/sync_test.go`
- [ ] T020 [P] [US2] Unit tests for `CaptureSync` warn-and-continue behavior on upload
  failure/timeout (returns without error to the caller, logs a warning) in
  `pkg/proexec/sync_test.go`
- [ ] T021 [P] [US2] Unit test confirming `ExecuteTerraform` invokes `proexec.CaptureSync`
  for `plan`/`apply` subcommands only (not `validate`, `output`, etc.) in
  `internal/exec/terraform_test.go`

### Implementation for User Story 2

- [ ] T022 [US2] Implement `CaptureSync(atmosConfig *schema.AtmosConfiguration, cmd
...
- [ ] T023 [US2] Call `proexec.CaptureSync(...)` from `internal/exec/terraform.go`'s
...
- [ ] T024 [US2] Call `proexec.CaptureSync(...)` with `data = nil` from
...
- [ ] T025 [US2] Document `settings.pro.exec.sync_timeout_seconds` in the Atmos Pro
...
- [ ] T026 [P] [US3] Unit test confirming `ExecuteTerraform` passes the already-computed
  ...
  `internal/exec/terraform_test.go`
- [ ] T027 [P] [US3] Unit test confirming `buildRecord` correctly omits/nils the `Data`
...
- [ ] T028 [US3] In `internal/exec/terraform.go`'s `ExecuteTerraform`, thread the
  ...
  argument to the T023 `proexec.CaptureSync(...)` call (depends on T023)
- [ ] T029 [US3] Confirm/adjust `buildRecord` (`pkg/proexec/envelope.go`) so a `nil` `data`
  ...
  contracts/interactions.md's "absent `data`" case (depends on T008)
...
- [ ] T030 [P] Add the 9th Pact interaction (`UploadExecMetadata`, `POST
...
- [ ] T031 Regenerate `pacts/atmos-AtmosPro.json` via
