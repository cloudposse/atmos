...
| Sync timeout | `CaptureSync` MUST bound its **total** wait — across both the `/exec/data` upload (if required) and the main `/exec` call combined — to `max(Settings.Pro.Exec.SyncTimeout, 10s)` (FR-011a) |
