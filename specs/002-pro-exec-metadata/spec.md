...
- What happens when the additional structured JSON data for a command is very large (e.g. a plan touching thousands of resources)? The existing large-payload handling used by other Atmos Pro uploads applies so a single execution record cannot exceed the platform's payload limits.
...
- **FR-008a**: System MUST bound how long a synchronous command waits for its execution-record upload to be confirmed, defaulting to approximately 10 seconds, and this wait duration MUST be configurable by the user to a longer value.
...
- **FR-011**: System MUST cap or chunk execution-record payload size consistent with the existing size limits already enforced for other Atmos Pro uploads, so oversized structured data does not cause delivery to fail outright.
...
- **Command Structured Data**: Represents optional, command-specific enrichment attached to an Execution Record. For `terraform plan`/`apply`, this is the set of created/updated/deleted/replaced resources, output values, and warnings/errors produced by that run.
