...
- **Centralized MCP server management** — one `atmos.yaml` defines every
  ...
  server is one block in `atmos.yaml` and one re-export.
- **Centralized auth and security** — [Atmos Auth](/cli/configuration/auth)
  is the only place AWS credentials live.
  Each MCP server is wrapped at spawn time with the right
  identity for the question it'll answer. One `atmos auth login`
...
- **Centralized toolchain management** — `uvx`, `npx`, and friends are
  ...
  coding assistant uses the same binaries.
