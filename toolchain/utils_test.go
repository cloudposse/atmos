...
func TestGetToolVersionsFilePath(t *testing.T) {
	// Save original values
	originalToolVersionsFile := toolVersionsFile
	originalToolsDir := toolsDir

	// Restore after test
	defer func() {
		toolVersionsFile = originalToolVersionsFile
		toolsDir = originalToolsDir
	}()

	// Reset the global variables to ensure we get the default behavior
	toolVersionsFile = ".tool-versions"
	toolsDir = ".tools"

	path := GetToolVersionsFilePath()
	assert.Equal(t, ".tool-versions", path)
}

func TestGetToolsDirPath(t *testing.T) {
	// Save original values
	originalToolVersionsFile := toolVersionsFile
	originalToolsDir := toolsDir
	...
	path := GetToolsDirPath()
	assert.Equal(t, ".tools", path)
}
