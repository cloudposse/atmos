...
Commit this file alongside any change that intentionally alters the request or response
...
- **Intentional change**: Run the tests to regenerate `pacts/atmos-AtmosPro.json`, review the diff, and commit both the code change and the updated pact file.
- **Accidental regression**: Read the pact failure output to find the mismatched field and fix the client code.
