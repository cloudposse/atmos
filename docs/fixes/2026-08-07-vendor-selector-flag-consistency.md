...
- `cd website && npm run build` — succeeds after the `.mdx` doc updates: links, images, and MDX
  rendering all resolved correctly, including the vendor-verify/vendor-update pages touched here.
  Same pre-existing, unrelated warnings as before (a few broken-anchor and MDX-normalize notices on
  unrelated pages, none touching vendor docs).
