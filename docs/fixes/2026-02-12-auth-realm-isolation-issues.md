...
| Issue | Severity | Fix                                                              |
|-------|----------|------------------------------------------------------------------|
| #2071 | Critical | Better error message when ATMOS_PROFILE matches an identity name |
| CI/CD | Critical | Made realm isolation opt-in (empty realm by default)             |
| #2072 | High     | Discard conflicting stack auth defaults                          |
