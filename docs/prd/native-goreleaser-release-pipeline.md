...
checkout (fetch-depth: 0, needed for changelog + tag history)
  ...
  → syft against the pushed multi-platform image digest (GoReleaser's own sboms: block cannot
    catalog Docker images — see note below — so this is a separate explicit step, not folded
    into the GoReleaser invocation)
...
sboms:
  # artifacts: archive/binary/package/installer/source/any -- GoReleaser's own SBOM tooling
  # cannot catalog Docker images at all ("Container images generated by GoReleaser are not
  # available to be cataloged by the SBOM tool", per GoReleaser's own docs), so the image SBOM
  # today's `build.yml` docker job produces (via cloudposse/github-action-docker-build-push's
  # sbom: true) needs its own explicit syft step against the pushed digest -- see the workflow
  # step list above -- not something this block can be extended to cover.
  - artifacts: archive
...
| Checksum signing/SBOM | `build.yml`'s downstream re-fetch job | Native `signs:`/`sboms:` inside the GoReleaser run |
| Docker image SBOM | `build.yml`'s `docker` job (`sbom: true` input) | Not GoReleaser-native (`sboms:` can't catalog images) — explicit syft step, see workflow list above |
...
2. **Docker.** Fold `build.yml`'s `docker` job into GoReleaser's `dockers_v2:`/`docker_signs:`; add an
    explicit syft step against the pushed digest to replace the image SBOM `build.yml`'s docker job
    produces today (GoReleaser's `sboms:` block can't catalog images natively — see above), then the
