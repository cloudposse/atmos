...
| 1.0.4 | 2026-08-01 | Review follow-ups. Reworded the guide and skill to say the style is *inspired by* ASD-STE100 rather than that it *is* ASD-STE100, which would claim conformance to a specification this repository neither reproduces nor verifies against. Widened `FutureTense` to absorb adverbs between "will" and the verb, so a match spans the whole construction instead of stopping at the adverb, and added `exceptions` for the noun sense of "will" ("free will are not..."), which RE2 cannot express in the pattern because it has no lookahead. Added committed fixtures under `.vale/testdata/`. Reworked the Rule 4 example in the `changelog` skill to show that replacing a Go package path with an architecture noun is only the middle rung, not the target. |
