...
1. **Zip bundle format**: All planfile artifacts use a zip archive containing:
   ...
   ```
   Metadata is **not** included in the zip. It is stored as a sidecar by each store backend (same as today).

2. **SHA256 on the zip**: The `metadata.SHA256` field reflects the checksum of the complete zip archive. It is set on the `*Metadata` struct passed to `store.Upload()` and stored in the sidecar, not inside the zip (avoids circular dependency).
...
- Return zip bytes. Metadata is **not** written into the zip — it is passed separately to `store.Upload()`.

**`ExtractBundle(data io.Reader) (*BundleFiles, error)`**:
- Read all data into memory.
- Check if first 4 bytes are `PK\x03\x04` (zip magic).
- If zip: open as `zip.Reader`, extract `plan.tfplan` (required), `.terraform.lock.hcl` (optional).
- If not zip: return raw data as `Plan`, nil `LockFile` (backward compat).
