...
8. Configurable + automatic: verification is auto-on under CI when planfile storage is configured, via `components.terraform.planfiles.verify` (`fail` | `warn` | `off`, default `fail`); `--verify-plan` / `--no-verify-plan` override (CLI > config > CI default); `warn` proceeds on drift, `off` skips the download. `planfile.ResolveVerifyMode()` resolves the mode at the download hook and the RunE gate — Done
